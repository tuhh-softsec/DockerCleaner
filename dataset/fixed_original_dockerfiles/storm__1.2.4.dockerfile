...
RUN set -eux ; apt-get update ; apt-get install --no-install-recommends bash=5.1-2+deb11u1 ca-certificates=20210119 dirmngr=2.2.27-2+deb11u2 gosu=1.12-1+b6 gnupg=2.2.27-2+deb11u2 python procps=2:3.3.17-5 wget=1.21-1+deb11u1 -y ; rm -rf /var/lib/apt/lists/* ; gosu nobody true
