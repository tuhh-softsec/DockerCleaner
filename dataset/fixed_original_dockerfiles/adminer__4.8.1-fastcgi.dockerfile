...
RUN set -x \
 && apk add postgresql-dev sqlite-dev=3.38.5-r0 unixodbc-dev=2.3.11-r0 freetds-dev=1.3.10-r0 --no-cache --virtual .build-deps \
 ...
 && apk add $runDeps --virtual .phpexts-rundeps \
 && apk del .build-deps
