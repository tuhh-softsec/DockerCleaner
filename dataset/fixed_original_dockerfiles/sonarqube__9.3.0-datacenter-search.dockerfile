...
RUN set -eux ; addgroup -S -g 1000 sonarqube ; adduser -S -D -u 1000 -G sonarqube sonarqube ; apk add gnupg=2.2.31-r1 unzip=6.0-r9 curl=8.0.1-r0 --no-cache --virtual build-dependencies ; apk add bash=5.1.16-r0 su-exec=0.2-r1 ttf-dejavu=2.37-r1 openjdk11-jre=11.0.14_p9-r0 --no-cache ; echo "networkaddress.cache.ttl=5" >> "${JAVA_HOME}/conf/security/java.security"; sed --in-place --expression="s?securerandom.source=file:/dev/random?securerandom.source=file:/dev/urandom?g" "${JAVA_HOME}/conf/security/java.security" ; for server in $( shuf -e ha.pool.sks-keyservers.net hkp://p80.pool.sks-keyservers.net:80 keyserver.ubuntu.com hkp://keyserver.ubuntu.com:80 pgp.mit.edu ;); do gpg --batch --keyserver "${server}" --recv-keys 679F1EE92B19609DE816FDE81DB198F93525EC1A \
