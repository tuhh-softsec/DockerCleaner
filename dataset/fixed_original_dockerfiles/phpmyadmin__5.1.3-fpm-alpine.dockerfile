...
RUN set -ex ; apk add bzip2-dev=1.0.8-r1 freetype-dev=2.12.1-r0 libjpeg-turbo-dev=2.1.3-r1 libpng-dev=1.6.37-r1 libwebp-dev=1.2.3-r0 libxpm-dev=3.5.15-r0 libzip-dev=1.8.0-r1 --no-cache --virtual .build-deps ; docker-php-ext-configure gd --with-freetype --with-jpeg --with-webp --with-xpm ; docker-php-ext-install -j "$( nproc ;)" bz2 gd mysqli opcache zip ; runDeps="$( scanelf --needed --nobanner --format '%n#p' --recursive /usr/local/lib/php/extensions | tr ',' '\n' | sort -u | awk 'system("[ -e /usr/local/lib/" $1 " ]") == 0 { next } { print "so:" $1 }' ;)" ; apk add $runDeps --virtual .phpmyadmin-phpexts-rundeps ; apk del --no-network .build-deps
