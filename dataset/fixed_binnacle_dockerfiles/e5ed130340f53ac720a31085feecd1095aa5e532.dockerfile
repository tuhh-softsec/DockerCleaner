...
RUN apt-key adv --recv-key --keyserver keyserver.ubuntu.com "A6616109451BBBF2" \
 ...
 && apt-get update \
 && apt-get install --no-install-recommends icewm=3.3.1-1 xterm=379-1ubuntu1 xfonts-base=1:1.0.5+nmu1 xauth=1:1.1.2-1 xinit=1.4.1-0ubuntu4 icewm-themes -y --allow-unauthenticated \
