...
RUN apt-get update -y \
 && apt-get install --no-install-recommends r-base libcurl4-openssl-dev libssl-dev libxml2-dev -y \
 && apt-get clean \
 && apt-get purge \
...
RUN wget https://storage.googleapis.com/qiaseq-dna/lib/ssw.tar.gz https://storage.googleapis.com/qiaseq-dna/lib/fgbio-0.1.4-SNAPSHOT.jar -P /srv/qgen/bin/ \
 && cd /srv/qgen/bin/ \
 && tar -xvf ssw.tar.gz \
 && rm ssw.tar.gz
...
RUN pip install edlib==1.3.9
#  # Download and install 3rd party libraries
RUN wget https://storage.googleapis.com/qiaseq-dna/lib/py-editdist-0.3.tar.gz https://storage.googleapis.com/qiaseq-dna/lib/sendgrid-v2.2.1.tar.gz https://github.com/weizhongli/cdhit/releases/download/V4.6.8/cd-hit-v4.6.8-2017-1208-source.tar.gz -P /srv/qgen/bin/downloads/ \
 ...
 && /opt/conda/bin/python setup.py install \
 && cd /srv/qgen/bin/downloads/ \
 && tar -xvf cd-hit-v4.6.8-2017-1208-source.tar.gz \
...
RUN wget https://storage.googleapis.com/qiaseq-dna/lib/TorrentSuite/tmap https://storage.googleapis.com/qiaseq-dna/lib/TorrentSuite/tvc -P /srv/qgen/bin/TorrentSuite/ \
...
RUN cd /srv/qgen/bin/downloads \
 ...
 && cd vcflib \
 && make \
 && mkdir -p /srv/qgen/bin/vcflib/bin/ \
 && cp bin/* /srv/qgen/bin/vcflib/bin/ \
 && cd / \
 && rm -rf /srv/qgen/downloads/vcflib/
