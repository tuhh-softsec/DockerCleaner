...
RUN apt-get update \
 && apt-get install --no-install-recommends ca-certificates=20230311 gnupg=2.2.40-1ubuntu2 build-essential=12.9ubuntu3 wget=1.21.3-1ubuntu1 subversion=1.14.2-4build2 unzip=6.0-27ubuntu1 automake=1:1.16.5-1.3 python cmake=3.25.1-1 ninja-build=1.11.1-1 curl=7.88.1-7ubuntu1 git=1:2.39.2-1ubuntu1 gcc-multilib=4:12.2.0-3ubuntu1 g++-multilib=4:12.2.0-3ubuntu1 libc6-dev=2.37-0ubuntu2 bison=2:3.8.2+dfsg-1build1 flex=2.6.4-8.1 libtool=2.4.7-5 autoconf=2.71-3 binutils-dev=2.40-2ubuntu3 binutils-gold software-properties-common=0.99.35 gnupg=2.2.40-1ubuntu2 apt-transport-https=2.6.0 systemd=252.5-2ubuntu3 sysvinit-utils=3.06-2ubuntu1 -y \
...
RUN apt-get update \
 && apt-get install --no-install-recommends bash-completion=1:2.11-6ubuntu1 buildbot-slave=3.7.0-2 -y \
 && rm -rf /var/lib/apt/lists/*
