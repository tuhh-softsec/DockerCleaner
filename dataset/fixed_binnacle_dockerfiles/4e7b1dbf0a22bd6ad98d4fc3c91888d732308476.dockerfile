...
RUN apt-get update -yqq \
 && apt-get install --no-install-recommends ca-certificates=20210119~16.04.1 expat=2.1.0-7ubuntu0.16.04.5 libgomp1=5.4.0-6ubuntu1~16.04.12 gcc=4:5.3.1-1ubuntu1 -yq \
...
RUN buildDeps="autoconf automake cmake curl bzip2 libexpat1-dev g++ gcc git gperf libtool make nasm perl pkg-config python libssl-dev yasm zlib1g-dev" \
 && apt-get update -yqq \
...
RUN DIR=/tmp/kvazaar \
 ...
 && make install \
 && rm -rf ${DIR}
RUN dir=/tmp/aom ; mkdir -p ${dir} ; cd ${dir} ; curl -sLO https://aomedia.googlesource.com/aom/+archive/${AOM_VERSION}.tar.gz ; tar -zx -f ${AOM_VERSION}.tar.gz ; rm -rf CMakeCache.txt CMakeFiles ; mkdir -p ./aom_build ; cd ./aom_build ; cmake -DCMAKE_INSTALL_PREFIX="${PREFIX}" -DBUILD_SHARED_LIBS=1 -DCONFIG_LOWBITDEPTH=1 .. ; make ; make install ; rm -rf ${dir}
#  # ffmpeg https://ffmpeg.org/
RUN DIR=$( mktemp -d ;) \
 && cd ${DIR} \
 && curl -sLO https://ffmpeg.org/releases/ffmpeg-${FFMPEG_VERSION}.tar.bz2 \
 && tar -jx --strip-components=1 -f ffmpeg-${FFMPEG_VERSION}.tar.bz2 \
 && ./configure --disable-debug --disable-doc --disable-ffplay --enable-shared --enable-avresample --enable-libopencore-amrnb --enable-libopencore-amrwb --enable-gpl --enable-libass --enable-libfreetype --enable-libvidstab --enable-libmp3lame --enable-libopenjpeg --enable-libopus --enable-libtheora --enable-libvorbis --enable-libvpx --enable-libx265 --enable-libxvid --enable-libx264 --enable-nonfree --enable-openssl --enable-libfdk_aac --enable-libkvazaar --enable-libaom --extra-libs=-lpthread --enable-postproc --enable-small --enable-version3 --extra-cflags="-I${PREFIX}/include" --extra-ldflags="-L${PREFIX}/lib" --extra-libs=-ldl --prefix="${PREFIX}" \
 && make \
 && make install \
 && make distclean \
 && hash -r \
