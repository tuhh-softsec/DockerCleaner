...
WORKDIR /root
RUN (apt-get update ;apt-get install --no-install-recommends automake bison flex gcc-4.8 g++-4.8 git libtool libssl-dev make pkg-config -y ) \
...
RUN git clone https://github.com/naver/pinpoint-c-agent.git \
 ...
 && git checkout master \
 && cd pinpoint_php \
 && bash Build.sh --always-make || bash Build.sh --always-make
