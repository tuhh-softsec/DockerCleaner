...
RUN set -eux \
 && yum --quiet makecache \
 && yum ${YUM_FLAGS_DEV} install autoconf automake gcc libcurl-devel libevent-devel libssh2-devel libxml2-devel make net-snmp-devel OpenIPMI-devel openldap-devel sqlite-devel git unixODBC-devel \
 && cd /tmp/ \
...
RUN set -eux \
 ...
 && yum ${YUM_FLAGS_PERSISTENT} install libcurl libevent libxml2 net-snmp-libs OpenIPMI-libs openldap openssl-libs pcre unixODBC \
