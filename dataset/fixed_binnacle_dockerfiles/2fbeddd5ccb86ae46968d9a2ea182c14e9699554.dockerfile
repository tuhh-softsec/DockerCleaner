...
RUN if [ "$CI_BUILD" ] ; then pip3 install awscli --user \
 && aws s3 cp s3://steemit-dev-ci/steemd-CTestCostData.txt /usr/local/src/steem/CTestCostData.txt ; fi
RUN if [ "$BUILD_STEP" = "1" ] || [ ! "$BUILD_STEP" ] ; then cd /usr/local/src/steem \
 && git submodule update --init --recursive \
 && mkdir build \
 && cd build \
 && cmake -DCMAKE_BUILD_TYPE=Release -DBUILD_STEEM_TESTNET=ON -DLOW_MEMORY_NODE=OFF -DCLEAR_VOTES=ON -DSKIP_BY_TX_ID=ON -DENABLE_MIRA=ON .. \
 && make -j$( nproc ;) chain_test mira_test test_fixed_string plugin_test \
 && if [ "$CI_BUILD" ] ; then mkdir -p build/tests/Testing/Temporary \
 && cp /usr/local/src/steem/CTestCostData.txt build/tests/Testing/Temporary ; fi \
 && cd tests \
...
RUN if [ "$BUILD_STEP" = "2" ] || [ ! "$BUILD_STEP" ] ; then cd /usr/local/src/steem \
 ...
 && mkdir build \
 && cd build \
 && cmake -DCMAKE_INSTALL_PREFIX=/usr/local/steemd-testnet -DCMAKE_BUILD_TYPE=Release -DBUILD_STEEM_TESTNET=ON -DLOW_MEMORY_NODE=OFF -DCLEAR_VOTES=ON -DSKIP_BY_TX_ID=ON -DENABLE_MIRA=ON -DENABLE_SMT_SUPPORT=ON -DSTEEM_STATIC_BUILD=${STEEM_STATIC_BUILD} .. \
 ...
 && PYTHONPATH=programs/build_helpers python3 -m steem_build_helpers.check_reflect \
 && programs/build_helpers/get_config_check.sh \
 && if [ "$CI_BUILD" ] ; then aws s3 cp s3://steemit-dev-ci/steemd-CTestCostData.txt s3://steemit-dev-ci/steemd-CTestCostData.txt.bk \
 && aws s3 cp build/tests/Testing/Temporary/CTestCostData.txt s3://steemit-dev-ci/steemd-CTestCostData.txt ; fi \
 && rm -rf /usr/local/src/steem/build ; fi
RUN if [ "$BUILD_STEP" = "1" ] || [ ! "$BUILD_STEP" ] ; then cd /usr/local/src/steem \
 && git submodule update --init --recursive \
 && mkdir build \
 && cd build \
 && cmake -DCMAKE_BUILD_TYPE=Debug -DENABLE_COVERAGE_TESTING=ON -DBUILD_STEEM_TESTNET=ON -DLOW_MEMORY_NODE=OFF -DCLEAR_VOTES=ON -DSKIP_BY_TX_ID=ON -DENABLE_MIRA=ON -DCHAINBASE_CHECK_LOCKING=OFF .. \
...
RUN if [ "$BUILD_STEP" = "2" ] || [ ! "$BUILD_STEP" ] ; then cd /usr/local/src/steem \
 ...
 && mkdir build \
 && cd build \
 && cmake -DCMAKE_INSTALL_PREFIX=/usr/local/steemd-default -DCMAKE_BUILD_TYPE=Release -DLOW_MEMORY_NODE=ON -DCLEAR_VOTES=ON -DSKIP_BY_TX_ID=OFF -DBUILD_STEEM_TESTNET=OFF -DENABLE_MIRA=ON -DSTEEM_STATIC_BUILD=${STEEM_STATIC_BUILD} .. \
 && make -j$( nproc ;) \
 && make install \
 && cd .. \
 ...
 && mkdir build \
 && cd build \
 && cmake -DCMAKE_INSTALL_PREFIX=/usr/local/steemd-full -DCMAKE_BUILD_TYPE=Release -DLOW_MEMORY_NODE=OFF -DCLEAR_VOTES=OFF -DSKIP_BY_TX_ID=ON -DBUILD_STEEM_TESTNET=OFF -DENABLE_MIRA=ON -DSTEEM_STATIC_BUILD=${STEEM_STATIC_BUILD} .. \
 && make -j$( nproc ;) \
 && make install \
 && rm -rf /usr/local/src/steem ; fi
RUN apt-get remove -y automake autotools-dev bsdmainutils build-essential cmake doxygen dpkg-dev libboost-all-dev libc6-dev libexpat1-dev libgcc-7-dev libhwloc-dev libibverbs-dev libicu-dev libltdl-dev libncurses5-dev libnuma-dev libopenmpi-dev libpython-dev libpython2.7-dev libreadline-dev libreadline6-dev libssl-dev libstdc++-7-dev libtinfo-dev libtool linux-libc-dev m4 make manpages manpages-dev mpi-default-dev python-dev python2.7-dev python3-dev \
