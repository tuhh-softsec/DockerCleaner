...
RUN set -ex \
 && apk update \
 && apk add openssl=1.1.1k-r0 git=2.20.4-r0 net-tools=1.60_git20140218-r2 tcpdump=4.9.3-r1 tcptraceroute=1.5b7-r1 vim=8.1.1365-r0 curl=7.64.0-r5 wget=1.20.3-r0 bash=4.4.19-r1 --no-cache \
 && cd /usr/bin \
 && wget http://www.vdberg.org/~richard/tcpping \
 && chmod 777 tcpping \
 && apk add zlib-dev=1.2.11-r1 cyrus-sasl-dev=2.1.27-r2 git=2.20.4-r0 autoconf=2.69-r2 g++=8.3.0-r0 libtool=2.4.6-r5 make=4.2.1-r2 pcre-dev=8.42-r2 tini=0.18.0-r0 libintl=0.19.8.1-r4 icu=62.1-r1 icu-dev=62.1-r1 libxml2-dev=2.9.9-r3 postgresql-dev=11.11-r0 freetype-dev=2.9.1-r3 libjpeg-turbo-dev=1.5.3-r6 libpng-dev=1.6.37-r0 gmp=6.1.2-r1 gmp-dev=6.1.2-r1 libmemcached-dev=1.0.18-r3 imagemagick-dev=7.0.8.68-r0 libssh2=1.9.0-r1 libssh2-dev=1.9.0-r1 libxslt-dev=1.1.33-r3 $PHPIZE_DEPS --no-cache --virtual .build-dependencies \
 ...
 && apk add openssh-server=7.9_p1-r6 --no-cache \
 && echo "$SSH_PASSWD" | chpasswd \
 && apk add openrc=0.39.2-r3 --no-cache \
 ...
 && apk add libaio=0.3.111-r0 libstdc++=8.3.0-r0 make=4.2.1-r2 pwgen=2.08-r0 sudo=1.8.25_p1-r3 tzdata=2020c-r1 $( test "${MARIADB_VER:0:4}" = "10.1" \
 && echo 'libressl-dev' || echo 'openssl-dev' ;) --update --no-cache -t .mariadb-run-deps \
 && apk add attr=2.4.47-r7 autoconf=2.69-r2 bison=3.0.5-r0 build-base=0.5-r1 cmake=3.13.0-r0 coreutils=8.30-r0 gnupg=2.2.19-r0 libaio-dev=0.3.111-r0 linux-headers=4.18.13-r1 ncurses-dev=6.1_p20190105-r0 patch=2.7.6-r6 readline-dev=7.0.003-r1 zlib-dev=1.2.11-r1 --update --no-cache -t .mariadb-build-deps \
 ...
 && apk update \
 && apk add logrotate=3.15.0-r0 supervisor=3.3.4-r1 \
