...
RUN NB_CORES=${BUILD_CORES-$( getconf _NPROCESSORS_CONF ;)} \
 && apk -U upgrade \
 && BUILD_DEPS=" linux-headers libtool build-base binutils-gold pcre-dev zlib-dev wget gnupg autoconf gcc g++ libc-dev make pkgconf curl-dev ca-certificates go cmake automake file jemalloc-dev patch re2c ${CUSTOM_BUILD_PKGS}" \
