...
RUN set -ex \
 && zypper -q update -y ; zypper --non-interactive install --no-recommends ca-certificates curl git-core gcc-c++ libffi-devel libopenssl-devel libpqxx-devel gcc-locale netcat-openbsd netcfg which python3 python3-setuptools python3-pip python3-devel python3-python-dateutil make \
