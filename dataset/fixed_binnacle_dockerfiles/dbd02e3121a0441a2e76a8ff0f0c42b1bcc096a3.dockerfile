...
RUN (apt-get update ;apt-get install --no-install-recommends python3-yaml=6.0-3build1 python3-pip=23.0.1+dfsg-1 -y )
...
RUN (apt-get update ;apt-get install --no-install-recommends libavbin-dev libavbin0 )
