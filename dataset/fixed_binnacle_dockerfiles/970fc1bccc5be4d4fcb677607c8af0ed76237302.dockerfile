...
RUN apt-get update \
 && apt-get install --no-install-recommends jessie-backports python3-git python3-pip gosu -y -t
...
RUN apt-get update \
 && apt-get install --no-install-recommends iptables-dev libatm1-dev libcap-dev libdb-dev libelf-dev libselinux1-dev -y
