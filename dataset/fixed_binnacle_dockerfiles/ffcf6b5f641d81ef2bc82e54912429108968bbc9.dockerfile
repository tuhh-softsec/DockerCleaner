...
RUN apt-get update \
 && apt-get install --no-install-recommends apt-transport-https build-essential curl git gnupg2 less make nano nginx patch php-apcu php-curl php-gd php-fpm php-ldap php-mbstring php-memcached php-mysql php-xml php-zip sudo supervisor wget cron -y \
 ...
 && cd /var/www/nc/apps2/calendar/js \
 && sudo -u www-data npm install bower \
 && cd .. \
