...
ENV PATH=".:$PATH"
RUN :
RUN (apt-get update ;apt-get install --no-install-recommends git wget unzip curl net-tools build-essential npm python python-setuptools python-dev python-numpy openssh-server sysstat -y )
#   libfontconfig is needed for grunt phantomjs...
RUN (apt-get update ;apt-get install --no-install-recommends libfontconfig -y )
...
#  ## JAVA ###
RUN curl -sL --retry 3 --insecure --header "Cookie: oraclelicense=accept-securebackup-cookie;" "http://download.oracle.com/otn-pub/java/jdk/8u31-b13/jdk-8u31-linux-x64.tar.gz" | gunzip | tar x -C /opt/
ENV JAVA_HOME="/opt/jdk1.8.0_31"
RUN ln -s $JAVA_HOME /opt/java
ENV PATH="$JAVA_HOME/bin:$PATH"
#  ## PYTHON ###
RUN (apt-get update ;apt-get install --no-install-recommends python-pip python-matplotlib python-pandas ipython python-nose -y )
...
#  ## SPARK ###
WORKDIR /opt
RUN curl -sL --retry 3 "http://archive.apache.org/dist/spark/spark-$SPARK_VERSION/spark-$SPARK_VERSION-bin-hadoop$HADOOP_PROFILE.tgz" | gunzip | tar x -C /opt/ \
