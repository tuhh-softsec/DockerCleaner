...
RUN apt-get update \
 && apt-get install --no-install-recommends curl=7.35.0-1ubuntu2.20 git=1:1.9.1-1ubuntu0.10 maven=3.0.5-1 oracle-java8-installer openssh-server=1:6.6p1-2ubuntu2.13 wget=1.15-1ubuntu1.14.04.5 -y
...
RUN if [ $CUSTOMIZE_TINKERPOP -eq "YES" ] ; then git clone https://github.com/apache/incubator-tinkerpop.git ;cd incubator-tinkerpop/ ; fi
RUN if [ $CUSTOMIZE_TINKERPOP -eq "YES" ] ; then cd incubator-tinkerpop ;git checkout tags/3.0.0-incubating ;TINKERPOP_VERSION=$( cat pom.xml | grep "^ <version>.*</version>$" | awk -F'[><]' '{print $3}' ;) ;echo "TINKERPOP_VERSION IS: $TINKERPOP_VERSION" ; fi
...
RUN if [ $CUSTOMIZE_TINKERPOP -eq "YES" ] ; then cd incubator-tinkerpop ;sed -i "s/^ <version>.*<\/version>/ <version>$TINKERPOP_VERSION<\/version>/g" pom.xml ;sed -i "s/^ <version>.*<\/version>/ <version>$TINKERPOP_VERSION<\/version>/g" $( find . -maxdepth 2 -mindepth 2 -type f -name 'pom.xml' ;) ;cat $( find . -maxdepth 2 -mindepth 2 -type f -name 'pom.xml' ;) | grep "^ <version>.*<\/version>" ;T_VERSION=$( cat pom.xml | grep "^ <version>.*</version>$" | awk -F'[><]' '{print $3}' ;) ;echo "TINKERPOP_VERSION IS: $T_VERSION" ; fi
RUN if [ $CUSTOMIZE_TINKERPOP -eq "YES" ] ; then cd incubator-tinkerpop ;mvn clean install -DskipTests -Denforcer.skip=true ; fi
ENV TITAN_VERSION="1.0.1-SNAPSHOT"
ENV TITAN_BRANCH="titan10"
RUN if [ ! -z $TITAN_BRANCH ] ; then echo "cloning the branch TITAN_BRANCH=$TITAN_BRANCH" ;git clone https://github.com/thinkaurelius/titan.git --branch $TITAN_BRANCH --single-branch \
 && cd titan/ ; else echo "cloning the master TITAN_BRANCH=$TITAN_BRANCH" ;git clone https://github.com/thinkaurelius/titan.git ;cd titan/ ;git checkout tags/$TITAN_VERSION ; fi
RUN if [ $CUSTOMIZE_TINKERPOP -eq "YES" ] ; then cd titan ;sed -i "s/tinkerpop.version.*/tinkerpop.version>$TINKERPOP_VERSION<\/tinkerpop.version>/g" pom.xml ;cat pom.xml | grep tinkerpop.version ; fi
RUN cd titan \
...
COPY run.sh /opt/$TITAN_DEPLOYMENT/
EXPOSE 8182/tcp
EXPOSE 8184/tcp
