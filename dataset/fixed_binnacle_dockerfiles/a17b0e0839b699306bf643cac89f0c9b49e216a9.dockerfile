...
RUN apt-get update \
 && apt-get install --no-install-recommends apt-utils --assume-yes \
 && apt-get install --no-install-recommends gettext postgresql-client --assume-yes \
 && apt-get clean \
...
RUN . /etc/os-release \
 && apt-get update \
 && apt-get install --no-install-recommends git --assume-yes \
 && apt-get clean \
 && rm --recursive --force /var/lib/apt/lists/*
#  ############################################################################################################
...
RUN apt-get update \
 && apt-get install --no-install-recommends apt-transport-https --assume-yes \
 && . /etc/os-release \
 && echo "deb https://deb.nodesource.com/node_10.x ${VERSION_CODENAME} main" > /etc/apt/sources.list.d/nodesource.list \
 && curl --silent https://deb.nodesource.com/gpgkey/nodesource.gpg.key | apt-key add - \
 && apt-get update \
 && apt-get install --no-install-recommends 'nodejs=10.*' --assume-yes \
 && apt-get clean \
 && rm --recursive --force /var/lib/apt/lists/*
#  ############################################################################################################
...
RUN npm install commander@10.0.1 angular-gettext-tools@2.5.3 --no-optional --global --unsafe-perm --no-package-lock \
...
RUN . /etc/os-release \
 && apt-get update \
 && apt-get install --no-install-recommends git make --assume-yes \
 && apt-get clean \
 && rm --recursive --force /var/lib/apt/lists/*
RUN python3 -m pip install --disable-pip-version-check --no-cache-dir --requirement=/tmp/requirements-dev.txt \
 && rm --recursive --force /tmp/* /var/tmp/* /root/.cache/*
#   For mypy
RUN touch /usr/local/lib/python3.7/dist-packages/zope/__init__.py \
...
RUN apt-get update \
 && apt-get install --no-install-recommends libxml2-utils --assume-yes \
 && apt-get clean \
 && rm --recursive --force /var/lib/apt/lists/*
COPY geoportal/package.json /app/c2cgeoportal/geoportal/
WORKDIR /app/c2cgeoportal/geoportal
RUN npm install ngeo@2.9.0-latest.20230404T071732Z.7c69eaeec --no-optional --global --unsafe-perm --no-package-lock \
 && npm cache clear --force \
 && rm -rf /tmp/*
COPY bin/npm-packages /usr/bin/
RUN npm-packages @camptocamp/cesium @type jasmine-core karma karma-chrome-launcher karma-coverage karma-coverage-istanbul-reporter karma-jasmine karma-sourcemap-loader karma-webpack typedoc typescript --src=/usr/lib/node_modules/ngeo/package.json --src=package.json --dst=npm-packages
...
RUN npm install $( cat /opt/npm-packages ;) --no-optional --global --unsafe-perm --no-package-lock \
 && npm cache clear --force \
 && rm -rf /tmp/*
RUN git clone --branch=v1.7.x --depth=1 --single-branch https://github.com/angular/angular.js.git /tmp/angular \
 && mv /tmp/angular/src/ngLocale/ /opt/angular-locale/ \
 && rm -rf /tmp/angular
RUN curl --output /opt/jasperreport.xsd http://jasperreports.sourceforge.net/xsd/jasperreport.xsd
#  ############################################################################################################
#   Build files for builder
...
COPY --from=build /app/c2cgeoportal/geoportal/c2cgeoportal_geoportal/locale/ /opt/c2cgeoportal_geoportal/c2cgeoportal_geoportal/locale/
#  ############################################################################################################
...
RUN ln -s /opt/c2cgeoportal_commons/c2cgeoportal_commons/alembic /opt \
 && python3 -m pip install --disable-pip-version-check --no-cache-dir --no-deps --editable=/opt/c2cgeoportal_commons --editable=/opt/c2cgeoportal_geoportal --editable=/opt/c2cgeoportal_admin
...
RUN python3 -m pip install --disable-pip-version-check --no-cache-dir --no-deps --editable=/opt/c2cgeoportal_commons --editable=/opt/c2cgeoportal_geoportal \
