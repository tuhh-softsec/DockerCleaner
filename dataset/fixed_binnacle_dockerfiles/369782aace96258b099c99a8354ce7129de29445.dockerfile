...
RUN apt-get update \
 && apt-get install --no-install-recommends python build-essential make gcc g++ xutils-dev pandoc python-dev python-pygments python-zmq python-pip -y
#   install python environment for crc scripts
RUN apt-get install --no-install-recommends python-numpy python-scipy python-gdal python-matplotlib libgdal-dev gdal-bin -y
#   package imported by auxil package (but not used)
RUN apt-get install --no-install-recommends python-tk -y
...
#   asf prerequisites
RUN apt-get install --no-install-recommends pkg-config bison flex gettext libgsl0-dev libgsl0ldbl zlib1g-dev libgdal-dev gdal-bin -y
...
RUN sed -i "/# Configuration file for jupyter-notebook./a c.NotebookApp.password = u'sha1:e008edc6061d:7960c7798fffe65531a2073e85e219e3fd61f3d0'" /root/.jupyter/jupyter_notebook_config.py
#    my auxil
