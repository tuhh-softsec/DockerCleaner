...
RUN go get -d go.opencensus.io/exporter/stackdriver/propagation `` \
 && (cd /go/src/go.opencensus.io \
 && (git cat-file -t $REV 2> /dev/null || git fetch -q origin $REV ) \
 && git reset --hard $REV )
