...
SHELL ["/bin/bash", "-c"]
RUN apt-get update \
 && apt-get install --no-install-recommends apt-transport-https=2.6.0 build-essential=12.9ubuntu3 bzip2=1.0.8-5build1 ca-certificates=20230311 curl=7.88.1-7ubuntu1 g++=4:12.2.0-3ubuntu1 git=1:2.39.2-1ubuntu1 gnupg=2.2.40-1ubuntu2 graphviz=2.42.2-7build3 locales=2.37-0ubuntu2 lsb-release=12.0-1ubuntu1 openssh-client=1:9.0p1-1ubuntu8 sudo=1.9.13p1-1ubuntu2 unzip=6.0-27ubuntu1 vim=2:9.0.1000-4ubuntu2 wget=1.21.3-1ubuntu1 zip=3.0-13 emacs=1:28.2+1-13ubuntu3 -yq \
 && apt-get clean \
