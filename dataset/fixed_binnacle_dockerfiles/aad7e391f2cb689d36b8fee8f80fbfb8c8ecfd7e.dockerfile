...
RUN apk add git=2.40.0-r0 make=4.4.1-r0 gcc=12.2.1_git20220924-r9 g++=12.2.1_git20220924-r9 openssl-dev=3.1.0-r2 --no-cache
WORKDIR /root  
RUN git clone https://github.com/proxytunnel/proxytunnel.git
WORKDIR /root/proxytunnel  
RUN make
FROM alpine:edge AS resource
RUN apk add bash=5.2.15-r2 curl=8.0.1-r1 git=2.40.0-r0 git-daemon=2.40.0-r0 gnupg=2.4.0-r1 gzip=1.12-r0 jq=1.6-r2 openssh=9.3_p1-r1 perl=5.36.0-r1 tar=1.34-r2 openssl=3.1.0-r2 libstdc++=12.2.1_git20220924-r9 --no-cache
