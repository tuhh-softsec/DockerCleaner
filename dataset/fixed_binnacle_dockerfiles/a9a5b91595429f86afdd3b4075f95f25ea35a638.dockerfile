...
#   Install modules
RUN apt-get update \
 && apt-get install --no-install-recommends wget build-essential libfreetype6-dev libicu-dev libjpeg62-turbo-dev libmagickwand-6.q16-dev libmcrypt-dev libssl-dev libyaml-dev libzip-dev imagemagick locales re2c unzip g++ zip ssmtp gettext openssh-client pdftk apt-transport-https mysql-client -y \
 && rm -rf /var/lib/apt/lists/*
RUN docker-php-ext-configure gd --with-freetype-dir=/usr/include/ --with-jpeg-dir=/usr/include/ \
 && docker-php-ext-install iconv gd pdo pdo_mysql mysqli mbstring soap sysvmsg sysvshm sysvsem intl opcache pcntl shmop zip gettext sockets exif
RUN echo 'en_US ISO-8859-1\n en_US.ISO-8859-15 ISO-8859-15\n en_US.UTF-8 UTF-8\n nl_NL ISO-8859-1\n nl_NL.UTF-8 UTF-8\n nl_NL@euro ISO-8859-15\n' >> /etc/locale.gen \
 && /usr/sbin/locale-gen
RUN ln -s /usr/lib/x86_64-linux-gnu/ImageMagick-6.8.9/bin-Q16/MagickWand-config /usr/bin/
RUN pecl install yaml imagick APCu
