...
RUN : \
 && DEBIAN_FRONTEND=noninteractive apt-get install -y x11-apps dbus-x11 gnome-terminal vim-gtk lyx libreoffice inkscape gimp chromium-browser texstudio evince mesa-utils xdotool xclip x11-xkb-utils
#   Microsoft's VS Code
...
#   RStudio
RUN (apt-get update ;apt-get install --no-install-recommends libjpeg62=1:6b2-3 -y ) \
