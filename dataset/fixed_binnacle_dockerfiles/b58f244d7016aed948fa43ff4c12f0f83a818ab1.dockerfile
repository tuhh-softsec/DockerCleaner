...
#   Databases
COPY circleci-provision-scripts/mysql.sh circleci-provision-scripts/mongo.sh circleci-provision-scripts/postgres.sh /opt/circleci-provision-scripts/
...
COPY targets/ubuntu-14.04-XXL-enterprise/Dockerfile /opt/circleci/Dockerfile
ARG IMAGE_TAG
RUN echo $IMAGE_TAG > /opt/circleci/image_version
COPY targets/ubuntu-14.04-XXL-enterprise/pkg-versions.sh /opt/circleci/bin/pkg-versions.sh
