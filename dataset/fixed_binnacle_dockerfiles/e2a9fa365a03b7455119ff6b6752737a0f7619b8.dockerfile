...
WORKDIR "/root" 
RUN mkdir lib
RUN which wget &> /dev/null || apt-get install --no-install-recommends wget=1.20.3 ; wget --no-verbose --output-document /root/lib/ https://github.com/textexploration/mtas/releases/download/v7.4.0.0/mtas-7.4.0.0.jar
RUN apt-get update \
