...
RUN : \
 ...
 && : \
 && DEBIAN_FRONTEND=noninteractive apt-get install -y libfreetype6-dev libicu-dev libssl-dev libjpeg62-turbo-dev libmcrypt-dev libedit-dev libedit2 libxslt1-dev apt-utils gnupg redis-tools mysql-client git vim wget curl lynx psmisc unzip tar cron bash-completion \
 && apt-get clean
#   Install Magento Dependencies
RUN docker-php-ext-configure gd --with-freetype-dir=/usr/include/ --with-jpeg-dir=/usr/include/ ; docker-php-ext-install opcache gd bcmath intl mbstring mcrypt pdo_mysql soap xsl zip
#   Install oAuth
RUN apt-get update \
 && (apt-get update ;apt-get install --no-install-recommends libpcre3=2:8.39-12 libpcre3-dev=2:8.39-12 -y ) \
 && pecl install oauth \
 && echo "extension=oauth.so" > /usr/local/etc/php/conf.d/docker-php-ext-oauth.ini
#   Install Node, NVM, NPM and Grunt
RUN curl -sL https://deb.nodesource.com/setup_6.x | bash - \
 && (apt-get update ;apt-get install --no-install-recommends nodejs=10.24.0~dfsg-1~deb10u3 build-essential=12.6 -y ) \
 && curl https://raw.githubusercontent.com/creationix/nvm/v0.16.1/install.sh | sh \
 && npm install grunt-cli@1.4.3 yarn@1.22.19 -g
#   Install Composer
...
RUN cd ~/.composer/vendor/magento/marketplace-eqp \
 && composer install
