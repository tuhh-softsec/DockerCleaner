...
#
RUN apt-get update \
 && apt-get install --no-install-recommends git apache2 haproxy nodejs npm build-essential gcc g++ python redis-server redis-tools python-redis imagemagick python-numpy libboost-thread-dev libjpeg-dev libcairo2-dev libpango1.0-dev libgif-dev telnet vim -y
...
RUN su meow -c " cd /home/meow \
 ...
 && chmod a+rwx stage \
 && cd /home/meow/weakpwh \
 && ./cmp.sh \
 && cd /home/meow/gbl2ngc/src \
 && ./cmp.sh \
 && cd /home/meow/bin \
 && ln -s /home/meow/gbl2ngc/src/gbl2ngc . \
 && ln -s /home/meow/weakpwh/weakpwh . \
 && ln -s /home/meow/gbl2ngc/scripts/drl2ngc.py ./drl2ngc \
 && cd /home/meow/www.meowcad.com/scripts \
 && ln -s ../cgi/meowaux.py . \
 && cd /home/meow/www.meowcad.com/aux \
 && ln -s ../cgi/meowaux.py . "
#   Setup certificate, pem file and setup listening port to point
#   to port 8080.
#
RUN cd /etc/ssl \
 && openssl req -new -newkey rsa:4096 -days 365 -nodes -x509 -subj "/C=US/ST=Denial/L=Springfield/O=Dis/CN=www.meowcad.com" -keyout snakeoil.key -out snakeoil.crt \
...
#
RUN npm install async@3.2.4 yargs@17.7.1 canvas@2.11.2 redis@4.6.5 -g \
