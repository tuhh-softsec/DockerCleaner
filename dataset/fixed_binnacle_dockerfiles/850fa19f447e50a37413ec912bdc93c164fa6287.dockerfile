...
EXPOSE 8000/tcp
VOLUME ["/config",  "/opt/observium/html",  "/opt/observium/logs",  "/opt/observium/rrd",  "/var/run/mysqld/mysqld.sock"]
...
RUN apt-get update -q \
 && apt-get install --no-install-recommends at fping git graphviz graphviz imagemagick ipmitool libapache2-mod-php5 libvirt-bin mariadb-client mtr-tiny nmap php5-cli php5-gd php5-json php5-ldap php5-mcrypt php5-mysql php5-snmp php-pear pwgen python-mysqldb rrdcached rrdtool snmp software-properties-common subversion unzip wget whois -y
RUN mkdir -p /config /opt/observium/html /opt/observium/logs /opt/observium/rrd RUN php5enmod mcrypt \
 && a2enmod rewrite
