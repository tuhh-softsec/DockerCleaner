...
RUN apk add libgcc=6.4.0-r9 libstdc++=6.4.0-r9 ca-certificates=20191127-r2 libcrypto1.0=1.0.2u-r0 libssl1.0=1.0.2u-r0 libgomp=6.4.0-r9 expat=2.2.8-r0 git=2.18.4-r0 --no-cache --update
FROM base AS build
WORKDIR /tmp/workdir
...
RUN buildDeps="autoconf automake bash binutils bzip2 cmake curl coreutils diffutils file g++ gcc gperf libtool make python openssl-dev tar yasm zlib-dev expat-dev" \
 && apk add ${buildDeps} --no-cache --update
