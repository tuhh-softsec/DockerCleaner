...
RUN set -xe ; apk add git sudo rsync curl nano bzip2 ruby php5-cli php5-mcrypt php5-gd php5-curl php5-json php5-phar php5-openssl php5-ctype php5-zip php5-zlib php5-pdo_mysql php5-dom php5-xml nodejs zsh mysql-client openssh-client postgresql-client --no-cache --virtual .persistent-deps \
...
#   Setup dev user
RUN set -xe ; adduser dev -s /bin/zsh -D \
 && su dev -c "git clone --quiet --depth=1 https://github.com/zsh-users/antigen.git /home/dev/antigen" \
 && echo 'dev ALL=(ALL) NOPASSWD:ALL' >> /etc/sudoers.d/dev \
 && chmod 0440 /etc/sudoers.d/dev
#   Fix bug https://github.com/npm/npm/issues/9863
RUN set -xe ; cd $( npm root -g ;)/npm \
 && npm install fs-extra@11.1.1 --no-color --no-progress \
 && sed -i -e s/graceful-fs/fs-extra/ -e s/fs.rename/fs.move/ ./lib/utils/rename.js \
 && npm install npm@9.6.4 yarn@1.22.19 gulp-cli@2.3.0 --production --no-color --no-progress -g \
 && rm -rf /root/.npm
RUN set -xe ; apk add make g++ --no-cache --virtual .build-deps ; cd /tmp ; export SASSC_VERSION="3.5.0" ; export LIBSASS_VERSION="3.5.4" ; curl -L https://github.com/sass/sassc/archive/${SASSC_VERSION}.tar.gz | tar -xz ; curl -L https://github.com/sass/libsass/archive/${LIBSASS_VERSION}.tar.gz | tar -xz ; export SASS_LIBSASS_PATH=`pwd `/libsass-${LIBSASS_VERSION} \
...
RUN set -xe ; chown dev: /home/dev/.zshrc \
 && su dev -c "source /home/dev/.zshrc" \
 && find /home/dev/ -type d -name .git -exec rm -rf -- {} +
COPY .gemrc /etc/gemrc
RUN set -xe ; export CONFIGURE_OPTS="--disable-install-doc" ; apk add gmp yaml ruby-json ruby-bigdecimal --no-cache \
 && apk add ruby-dev build-base openssl-dev --no-cache --virtual .build-deps \
 && gem update --clear-sources --quiet --system 2.7.9 \
 && gem update --clear-sources --quiet --force \
 && gem install compass --version 1.0.3 --clear-sources --quiet \
 && gem cleanup \
 && apk del --no-cache .build-deps \
 && rm -rf /tmp/* \
 && rm -rf /home/root/.gem/
