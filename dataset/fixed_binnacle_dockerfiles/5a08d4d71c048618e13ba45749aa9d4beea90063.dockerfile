...
RUN cd /tmp/kodi-source/build \
 && cmake ../. -DCMAKE_C_FLAGS="-march=armv7-a -mfpu=neon-vfpv4 -mfloat-abi=hard -mvectorize-with-neon-quad" -DCMAKE_CXX_FLAGS="-march=armv7-a -mfpu=neon-vfpv4 -mfloat-abi=hard -mvectorize-with-neon-quad" -DCMAKE_INSTALL_LIBDIR=/usr/lib -DCMAKE_INSTALL_PREFIX=/usr -DENABLE_AIRTUNES=OFF -DENABLE_ALSA=OFF -DENABLE_AVAHI=OFF -DENABLE_BLUETOOTH=OFF -DENABLE_BLURAY=ON -DENABLE_CAP=OFF -DENABLE_CEC=OFF -DENABLE_DBUS=OFF -DENABLE_DVDCSS=OFF -DENABLE_GLX=OFF -DENABLE_INTERNAL_FLATBUFFERS=ON -DENABLE_LIBUSB=OFF -DENABLE_NFS=ON -DENABLE_OPENGL=OFF -DENABLE_OPTICAL=OFF -DENABLE_PULSEAUDIO=OFF -DENABLE_SNDIO=OFF -DENABLE_UDEV=OFF -DENABLE_UPNP=ON -DENABLE_VAAPI=OFF -DENABLE_VDPAU=OFF \
 && make -j3 \
