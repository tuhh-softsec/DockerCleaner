...
RUN curl -L -O https://ftpmirror.gnu.org/gcc/gcc-5.5.0/gcc-5.5.0.tar.gz \
 ...
 && mkdir gccbuild \
 && cd gccbuild \
 && ../gcc-5.5.0/configure -v --build=x86_64-linux-gnu --host=x86_64-linux-gnu --target=x86_64-linux-gnu --prefix=/usr/local --enable-checking=release --enable-languages=c,c++,fortran --disable-multilib --with-pic \
 && make -j 4 \
 && make install \
 && cd .. \
...
RUN git clone https://github.com/xianyi/OpenBLAS.git \
 ...
 && cp libopenblas_core2p-r0.3.5.a /usr/local/lib/libopenblas.a \
 && cd .. \
 && rm -rf OpenBLAS
#   -----------------------
#   Freetype
...
RUN cd ${PETSC_DIR} \
 && ./configure CC=$CC CXX=$CXX FC=$FC CFLAGS=$CFLAGS CXXFLAGS=$CXXFLAGS FFLAGS=$FFLAGS --with-clanguage=cxx --with-debugging=0 --with-mpi=0 --with-mpiuni-fortran-binding=0 --download-mumps=yes --with-mumps-serial --with-shared-libraries=0 --with-x=0 --with-ssl=0 --with-scalar-type=complex --with-blaslapack-lib="/usr/local/lib/libopenblas.a /usr/local/lib64/libgfortran.a /usr/local/lib64/libquadmath.a -lpthread" \
 && make
