#   Google Tensorflow 2.0 (CUDA10) & Python 3.6
...
RUN apt-get update \
 && apt-get install --no-install-recommends nvinfer-runtime-trt-repo-ubuntu1804-$TENSORT_VERSION-ga-cuda10.0 \
 && apt-get update \
 && apt-get install --no-install-recommends openjdk-8-jdk-headless -y \
 && apt-get install --no-install-recommends curl gcc g++ make cmake git cuda-command-line-tools-10-0 cuda-cublas-dev-10-0 cuda-cudart-dev-10-0 cuda-cufft-dev-10-0 cuda-curand-dev-10-0 cuda-cusolver-dev-10-0 cuda-cusparse-dev-10-0 libssh-dev proj-bin libproj-dev libgeos-dev libgeos++-dev libcurl3-dev libfreetype6-dev libhdf5-serial-dev libpng-dev libzmq3-dev libffi-dev pkg-config rsync software-properties-common zip unzip zlib1g-dev wget imagemagick graphviz rsync sed swig git-core zip libcudnn7=$CUDNN_VERSION-1+cuda10.0 libcudnn7-dev=$CUDNN_VERSION-1+cuda10.0 libnvinfer5=$TENSORT_VERSION-1+cuda10.0 libnvinfer-dev=$TENSORT_VERSION-1+cuda10.0 libnccl2=$NCCL_VERSION-1+cuda10.0 libnccl-dev=$NCCL_VERSION-1+cuda10.0 -y \
 && apt-get clean \
