...
RUN if [ ${INSTALL_SOAP} = true ] ; then add-apt-repository -y ppa:ondrej/php \
 && apt-get update -yqq \
 && (apt-get update ;apt-get install --no-install-recommends libxml2-dev php7.1-soap -y ) ; fi
#  ####################################
#   PHP GMP
#  ####################################
...
RUN if [ ${INSTALL_XDEBUG} = true ] ; then apt-get update \
 && (apt-get update ;apt-get install --no-install-recommends php-xdebug -y --force-yes ) \
...
RUN if [ ${INSTALL_MONGO} = true ] ; then pecl install mongodb \
 && echo "extension=mongodb.so" >> /etc/php/7.1/cli/conf.d/30-mongodb.ini; fi
#  ####################################
#   Drush:
#  ####################################
USER root
...
RUN if [ ${INSTALL_V8JS} = true ] ; then add-apt-repository -y ppa:pinepain/libv8-5.4 \
 && apt-get update -yqq \
 && (apt-get update ;apt-get install --no-install-recommends php-xml php-dev php-pear libv8-5.4 -y ) \
 && pecl install v8js \
