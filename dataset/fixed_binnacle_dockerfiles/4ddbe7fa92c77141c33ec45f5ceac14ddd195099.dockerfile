...
ARG CUSTOM_BUILD_PKGS="  freetype-dev  openldap-dev  gmp-dev  icu-dev  postgresql-dev  libpng-dev  libwebp-dev  gd-dev  libjpeg-turbo-dev  libxpm-dev  libedit-dev  libxml2-dev  openssl-dev  libbz2  sqlite-dev  imap-dev"
ARG CUSTOM_PKGS="  freetype  openldap  gmp  bzip2-dev  icu  libpq  c-client"
COPY rootfs /
RUN NB_CORES=${BUILD_CORES-$( getconf _NPROCESSORS_CONF ;)} \
 && BUILD_DEPS=" linux-headers libtool build-base pcre-dev zlib-dev libzip-dev wget gnupg autoconf gcc g++ libc-dev make pkgconf curl-dev ca-certificates ${CUSTOM_BUILD_PKGS}" \
 && apk add s6=2.8.0.1-r0 su-exec=0.2-r0 curl=7.66.0-r4 libedit=20190324.3.1-r0 libxml2=2.9.9-r5 openssl=1.1.1k-r0 libwebp=1.0.2-r0 libzip=1.5.2-r0 gd=2.2.5-r3 pcre=8.43-r1 zlib=1.2.11-r1 ${BUILD_DEPS} ${CUSTOM_PKGS} -U \
