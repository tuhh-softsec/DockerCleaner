...
RUN apt-get update --quiet --yes \
 && apt-get install --no-install-recommends wget=1.21-1+deb11u1 tar=1.34+dfsg-1 unzip=6.0-26+deb11u1 lib32stdc++6=10.2.1-6 lib32z1=1:1.2.11.dfsg-2+deb11u2 git=1:2.30.2-1+deb11u2 file=1:5.39-3 build-essential=12.9 ca-certificates=20210119 openssh-server=1:8.4p1-5+deb11u1 --quiet --yes \
...
RUN ARCH= \
 ...
 && case "${dpkgArch##*-}" in (amd64) ARCH='x64' ;;(ppc64el) ARCH='ppc64le' ;;(s390x) ARCH='s390x' ;;(arm64) ARCH='arm64' ;;(armhf) ARCH='armv7l' ;;(*) echo "unsupported architecture" ; exit 1 ;; esac \
...
RUN npm install react-native-cli@2.0.1 exp@55.0.5 -g
