...
RUN apk add ca-certificates curl tar xz openssl --no-cache
...
RUN set -xe \
 && apk add argon2-dev coreutils curl-dev libedit-dev libsodium-dev libxml2-dev oniguruma-dev openssl-dev sqlite-dev $PHPIZE_DEPS --no-cache --virtual .build-deps \
