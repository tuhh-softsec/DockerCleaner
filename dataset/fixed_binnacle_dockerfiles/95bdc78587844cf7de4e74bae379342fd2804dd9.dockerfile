...
RUN apt-get update -y \
 && apt-get install --no-install-recommends supervisor ntp openssh-server -y \
 && apt-get install --no-install-recommends vim net-tools subversion -y \
 && apt-get install --no-install-recommends python-dev python-pip libmysqlclient-dev -y
#   3.7 安装pip依赖包
