FROM centos:centos6
COPY conf/setup.conf /tmp/setup.conf
COPY conf/cobbler_web.conf /etc/httpd/conf.d/cobbler_web.conf
COPY conf/ssl.conf /etc/httpd/conf.d/ssl.conf
COPY conf/tftpd.template /etc/cobbler/tftpd.template
COPY conf/modules.conf /etc/cobbler/modules.conf
COPY conf/distributions /tmp/distributions
COPY conf/dhcp.template /etc/cobbler/dhcp.template
...
RUN (echo -n "cobbler:Cobbler:" \
 && echo -n "cobbler:Cobbler:cobbler" | md5sum - | cut -d' ' -f1 ) > /etc/cobbler/users.digest
#   get adapters code
WORKDIR /root/
...
COPY conf/1404_distributions /tmp/1404_distributions
...
COPY conf/cobbler.settings /etc/cobbler/settings
