...
RUN GPG_KEYS=B0F4253373F8F6F510D42178520A9993A1C052F8 \
 ...
 && apk add gcc libc-dev make openssl-dev pcre-dev zlib-dev linux-headers curl gnupg libxslt-dev gd-dev geoip-dev perl-dev --no-cache --virtual .build-deps \
...
#  安装项目依赖
RUN apk add mariadb-client-libs --update --no-cache \
 && apk add mariadb-dev gcc musl-dev --no-cahe --virtual .build-deps \
