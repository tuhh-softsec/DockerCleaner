...
RUN apt-get update \
 && apt-get install --no-install-recommends ca-certificates curl librecode0 libsqlite3-0 libxml2 autoconf file g++ gcc libc-dev make pkg-config re2c apache2 apache2-mpm-prefork -y \
