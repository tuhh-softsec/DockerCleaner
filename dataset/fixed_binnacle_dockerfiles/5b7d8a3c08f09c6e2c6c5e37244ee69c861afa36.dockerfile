...
RUN :
RUN (apt-get update ;apt-get install --no-install-recommends wget ca-certificates openssh-client git build-essential gcc-4.9 g++-4.9 gcc-4.9-base software-properties-common libcudnn7=${CUDNN_VERSION} libnccl2=${NCCL_VERSION_OVERRIDE} libnccl-dev=${NCCL_VERSION_OVERRIDE} -y --allow-downgrades --allow-change-held-packages )
#   Install Python.
...
RUN if [[ ${MPI_KIND} == "OpenMPI" ]] ; then wget -O /tmp/openmpi-3.0.0-bin.tar.gz https://github.com/horovod/horovod/files/1596799/openmpi-3.0.0-bin.tar.gz \
 ...
 && ldconfig \
 && echo "mpirun -allow-run-as-root -np 2 -H localhost:2 -bind-to none -map-by slot -mca mpi_abort_print_stack 1" > /mpirun_command; else (apt-get update ;apt-get install --no-install-recommends mpich -y ) \
...
RUN ldconfig /usr/local/cuda/targets/x86_64-linux/lib/stubs \
 ...
 && ldconfig
#   Install PyTorch.
RUN if [[ ${PYTORCH_PACKAGE} == "torch-nightly" ]] ; then PYTORCH_CUDA=$( echo ${CUDA_DOCKER_VERSION} | awk -F- '{print $1}' | sed 's/\.//' ;) ;pip install torch_nightly==null -v -f https://download.pytorch.org/whl/nightly/cu${PYTORCH_CUDA}/torch_nightly.html ; else pip install ${PYTORCH_PACKAGE} ; fi
