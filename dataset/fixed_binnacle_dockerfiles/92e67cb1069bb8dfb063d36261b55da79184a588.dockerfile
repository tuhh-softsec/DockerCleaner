...
RUN conda install --quiet --yes altair=${ALTAIR_VERSION} bqplot=${BQPLOT_VERSION} earthengine-api=${EARTHENGINE_API_VERSION} google-api-python-client=${GOOGLE_API_PYTHON_VERSION} ipyleaflet=${IPYLEAFLET_VERSION} ipyvolume=${IPYVOLUME_VERSION} nbdime=${NBDIME_VERSION} palettable=${PALETTABLE_VERSION} pydrive=${PYDRIVE_VERSION} tensorflow=${TENSORFLOW_VERSION} vega_datasets=${VEGA_DATASETS_VERSION} \
 && conda clean -tipsy \
 && jupyter nbextension enable --py widgetsnbextension --sys-prefix \
 && pip install sidecar==0.5.2 \
 ...
 && pip install ipythonblocks==1.9.0 \
