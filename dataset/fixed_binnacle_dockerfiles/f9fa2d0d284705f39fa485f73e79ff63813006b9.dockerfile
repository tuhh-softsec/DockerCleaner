...
RUN apt-get update \
 ...
 && apt-get install --no-install-recommends cuda-command-line-tools-9-0 cuda-cublas-dev-9-0 cuda-cudart-dev-9-0 cuda-cufft-dev-9-0 cuda-curand-dev-9-0 cuda-cusolver-dev-9-0 cuda-cusparse-dev-9-0 curl git libcudnn7=7.2.1.38-1+cuda9.0 libcudnn7-dev=7.2.1.38-1+cuda9.0 libnccl2=2.2.13-1+cuda9.0 libnccl-dev=2.2.13-1+cuda9.0 libcurl3-dev libfreetype6-dev libhdf5-serial-dev libpng12-dev libzmq3-dev pkg-config rsync software-properties-common zip unzip zlib1g-dev wget imagemagick graphviz cmake gcc g++ curl wget pkg-config rsync sed swig git-core zip -y \
...
RUN apt-get update \
 ...
 && apt-get update \
 && apt-get install --no-install-recommends libnvinfer4=4.1.2-1+cuda9.0 \
 && apt-get install --no-install-recommends libnvinfer-dev=4.1.2-1+cuda9.0
