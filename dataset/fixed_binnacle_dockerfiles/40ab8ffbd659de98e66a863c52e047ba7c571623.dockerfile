...
RUN curl -fsSL "https://storage.googleapis.com/golang/go1.4.3.linux-amd64.tar.gz" | tar -xzC /root \
 && mv /root/go /root/go1.4 \
 && cd /usr/local \
 && curl -fsSL "https://storage.googleapis.com/golang/go$GO_VERSION.src.tar.gz" | tar -xzC /usr/local \
 && cd go \
