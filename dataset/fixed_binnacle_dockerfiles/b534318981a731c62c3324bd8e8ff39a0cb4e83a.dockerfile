...
RUN apt-get update \
 && apt-get install --no-install-recommends openssl=1.0.2g-1ubuntu4.20 wget=1.17.1-1ubuntu1.5 unzip=6.0-20ubuntu1.1 -y
ARG IMURL
ARG IBMID
ARG IBMID_PWD
ARG IFIXES=none
...
RUN mkdir /work \
 ...
 && /work/InstallationManagerKit/tools/imutilsc saveCredential -userName $IBMID -userPassword $IBMID_PWD -secureStorageFile /tmp/secureStore -masterPasswordFile /tmp/secureStorePwd -url $REPO -verbose \
 && /work/InstallationManagerKit/tools/imcl install $PRODUCTID com.ibm.java.jdk.v8 -acceptLicense -accessRights nonAdmin -showProgress -installationDirectory /opt/IBM/WebSphere/AppServer -repositories $REPO -installFixes $IFIXES -sRD /opt/IBM/WebSphere/AppServerIMShared -dataLocation /opt/IBM/WebSphere/AppServerIMData -secureStorageFile /tmp/secureStore -masterPasswordFile /tmp/secureStorePwd -preferences offering.service.repositories.areUsed=false,com.ibm.cic.common.core.preferences.searchForUpdates=true,com.ibm.cic.common.core.preferences.preserveDownloadedArtifacts=false,com.ibm.cic.common.core.preferences.keepFetchedFiles=false \
