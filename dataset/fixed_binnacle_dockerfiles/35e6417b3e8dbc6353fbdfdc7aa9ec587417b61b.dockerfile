...
RUN buildDeps="autoconf automake bash binutils bzip2 cmake curl coreutils diffutils expat-dev file g++ gcc gperf libtool make python openssl-dev tar yasm zlib-dev" \
 && apk add libgcc=6.4.0-r5 libstdc++=6.4.0-r5 ca-certificates=20190108-r0 libcrypto1.0=1.0.2t-r0 libssl1.0=1.0.2t-r0 ${buildDeps} --update
...
RUN DIR=/tmp/vpx \
 ...
 && ./configure --prefix="${PREFIX}" --enable-vp8 --enable-vp9 --enable-pic --enable-shared --disable-debug --disable-examples --disable-docs --disable-install-bins \
 && make \
 && make install \
 && rm -rf ${DIR}
...
RUN DIR=/tmp/kvazaar \
 ...
 && make install \
 && rm -rf ${DIR}
RUN dir=/tmp/aom ; mkdir -p ${dir} ; cd ${dir} ; curl -sLO https://aomedia.googlesource.com/aom/+archive/${AOM_VERSION}.tar.gz ; tar -zx -f ${AOM_VERSION}.tar.gz ; rm -rf CMakeCache.txt CMakeFiles ; mkdir -p ./aom_build ; cd ./aom_build ; cmake -DCMAKE_INSTALL_PREFIX="${PREFIX}" -DBUILD_SHARED_LIBS=1 .. ; make ; make install ; rm -rf ${dir}
