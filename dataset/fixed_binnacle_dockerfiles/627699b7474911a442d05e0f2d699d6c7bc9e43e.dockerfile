...
RUN apt-get update \
 && apt-get upgrade -y \
 && apt-get install --no-install-recommends curl=7.88.1-7ubuntu1 wget=1.21.3-1ubuntu1 -y
#  # install TM dependencies without cloning the repository. This allows for a more efficient use of the docker build cache
RUN /bin/bash -c "source <(curl https://raw.githubusercontent.com/mpicbg-scicomp/tissue_miner/master/installation/ubuntu/install_dependencies.sh | sed 's/sudo//g')"
#  # clone TM locally and compile the parser needed to convert TissueAnalyzer outputs into csv
RUN mkdir /tissue_miner \
 && git clone https://github.com/mpicbg-scicomp/tissue_miner /tissue_miner \
 && cd /tissue_miner/parser \
 && make
...
RUN /bin/bash -c "export TM_HOME='/tissue_miner'; su rstudio -c /tissue_miner/Setup.R \
 && /tissue_miner/misc/rstudio_TM_docker_image/RstudioSetup.R"
