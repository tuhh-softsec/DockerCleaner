...
RUN set -ex ; apt-get update ; if ! which gpg ; then apt-get install --no-install-recommends gnupg=2.2.4-1ubuntu1.6 -y ; fi ; if ! gpg --version | grep -q '^gpg (GnuPG) 1\.' ; then apt-get install --no-install-recommends dirmngr=2.2.4-1ubuntu1.6 -y ; fi ; rm -rf /var/lib/apt/lists/*
...
RUN apt-get update \
 && apt-get install --no-install-recommends apt-transport-https=1.6.14 ca-certificates=20211016ubuntu0.18.04.1 pwgen=2.08-1 tzdata=2022g-0ubuntu0.18.04 -y \
