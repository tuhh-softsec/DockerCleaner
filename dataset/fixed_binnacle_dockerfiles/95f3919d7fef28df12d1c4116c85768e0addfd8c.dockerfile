...
RUN cd /usr/local/src/viz \
 && git submodule deinit -f . \
 && git submodule update --init --recursive -f \
 && mkdir build \
 && cd build \
 && cmake -DCMAKE_BUILD_TYPE=Release -DBUILD_SHARED_LIBRARIES=FALSE -DLOW_MEMORY_NODE=FALSE -DCHAINBASE_CHECK_LOCKING=FALSE -DENABLE_MONGO_PLUGIN=TRUE .. \
 && make -j$( nproc ;) \
 && make install \
...
COPY share/vizd/snapshot5392323.json /var/lib/vizd
#   rpc service:
#   http
EXPOSE 8090/tcp
#   ws
