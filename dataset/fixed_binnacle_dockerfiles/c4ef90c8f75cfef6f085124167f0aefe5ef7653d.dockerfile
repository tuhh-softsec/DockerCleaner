...
RUN set -ex \
 ...
 && apk add zlib-dev cyrus-sasl-dev git autoconf build-base g++ libtool make pcre-dev tini libintl icu icu-dev libxml2-dev postgresql-dev freetype-dev libjpeg-turbo-dev libpng-dev gmp gmp-dev libmemcached-dev imagemagick-dev libssh2 libssh2-dev libxslt-dev ruby-dev gnupg $PHPIZE_DEPS --no-cache --virtual .build-dependencies \
 ...
 && apk add ruby ruby-irb ruby-etc ruby-webrick imagemagick --no-cache \
 && echo 'gem: --no-document' >> /etc/gemrc \
 && gem install oj:3.3.10 \
 && gem install fluent-plugin-azure-loganalytics --version 0.7.0 \
