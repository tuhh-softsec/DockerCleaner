#   Build target: lablup/common-tensorflow:1.10-py36
...
RUN cd /tmp/tensorflow-${TF_VERSION} \
 && CC_OPT_FLAGS="-march=x86-64 -mavx -mavx2 -mfma -mfpmath=both -msse4.1 -msse4.2" tensorflow/tools/ci_build/builds/configured CPU bazel build -c opt --copt=-mavx --copt=-mavx2 --config=mkl --config=monolithic --cxxopt="-D_GLIBCXX_USE_CXX11_ABI=0" //tensorflow/tools/pip_package:build_pip_package \
