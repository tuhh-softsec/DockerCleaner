...
RUN apt-get update \
 && apt-get install --no-install-recommends pkg-config=1.8.1-1ubuntu2 curl=7.88.1-7ubuntu1 libpcre3-dev=2:8.39-15 libtool=2.4.7-5 libssl-dev=3.0.8-1ubuntu1 zlib1g-dev=1:1.2.13.dfsg-1ubuntu4 libasound2-dev=1.2.8-1build1 build-essential=12.9ubuntu3 cmake=3.25.1-1 -y
#   nasm
RUN mkdir -p /dist \
 && cd /dist \
