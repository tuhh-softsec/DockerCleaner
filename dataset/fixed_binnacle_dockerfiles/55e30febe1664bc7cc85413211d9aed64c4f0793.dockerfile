...
RUN INSTALL_PKGS="OpenIPMI-libs curl fping iksemel java-1.8.0-openjdk-headless libcurl libevent libxml2 mariadb mariadb-server net-snmp-libs nginx openldap openssl-libs pcre php-bcmath php-fpm php-gd php-ldap php-mbstring python-setuptools php-mysql php-xml unixODBC" \
...
RUN REPOLIST="rhel-7-server-rpms,rhel-7-server-optional-rpms,zabbix-non-supported" \
 && INSTALL_PKGS="autoconf automake gcc gettext iksemel-devel java-1.8.0-openjdk-devel libcurl-devel libevent-devel libssh2-devel libxml2-devel make mariadb-devel net-snmp-devel OpenIPMI-devel openldap-devel git unixODBC-devel" \
 && yum -y install --disablerepo "*" --enablerepo "${REPOLIST}" --setopt=tsflags=nodocs ${INSTALL_PKGS} \
 && cd /tmp/ \
