...
RUN apk update \
 && apk upgrade \
 && buildDeps="autoconf automake bash binutils bzip2 cmake curl coreutils g++ gcc libtool make openssl-dev tar yasm zlib-dev expat-dev" \
 && apk add libgcc libstdc++ ca-certificates libcrypto1.0 libssl1.0 libgomp expat ${buildDeps} --no-cache \
 ...
 && rm -rf ${DIR} \
 && opencvDeps="clang clang-dev linux-headers" \
 && apk add ${opencvDeps} --no-cache \
 ...
 && cd build \
 && export MAKEFLAGS="-j$(($( grep -c ^processor /proc/cpuinfo ;) + 1))" \
 && cmake -D BUILD_opencv_python3=ON -D CMAKE_BUILD_TYPE=RELEASE -D INSTALL_C_EXAMPLES=OFF -D INSTALL_PYTHON_EXAMPLES=OFF -D CMAKE_INSTALL_PREFIX=/usr/local -D BUILD_EXAMPLES=OFF .. \
 && make \
 && make install \
