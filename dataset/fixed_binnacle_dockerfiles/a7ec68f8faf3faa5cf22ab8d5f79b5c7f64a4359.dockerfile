...
RUN apt-get update -qq \
 && apt-get install --no-install-recommends build-essential=11.6ubuntu6 wget=1.15-1ubuntu1.14.04.5 curl=7.35.0-1ubuntu2.20 unzip=6.0-9ubuntu1.5 make=3.81-8.2ubuntu3 -y -q
#
#   Install native prerequisites for the ruby gems we need (e.g.nokogiri)
#
RUN apt-get update -qq \
 && apt-get install --no-install-recommends zlib1g-dev=1:1.2.8.dfsg-1ubuntu1.1 libxml2-dev=2.9.1+dfsg1-3ubuntu4.13 libxml2=2.9.1+dfsg1-3ubuntu4.13 libxslt1-dev=1.1.28-2ubuntu0.2 -y -q
#   Prerequesites for capybara-webkit
RUN apt-get install --no-install-recommends qt5-default=5.2.1+dfsg-1ubuntu14.3 libqt5webkit5-dev=5.1.1-1ubuntu8 gstreamer1.0-plugins-base=1.2.4-1~ubuntu2.1 gstreamer1.0-tools=1.2.4-0ubuntu1.1 gstreamer1.0-x=1.2.4-1~ubuntu2.1 -y -q
#
...
#
RUN apt-get update -qq \
 && apt-get install --no-install-recommends xvfb=2:1.15.1-0ubuntu2.11 -y -q
#
#   Install firefox.
...
#
RUN apt-get update -qq \
 && apt-get install --no-install-recommends libxss1=1:1.2.2-1 libappindicator1=12.10.1+13.10.20130920-0ubuntu4.1 libindicator7=12.10.2+14.04.20141007.1-0ubuntu1 -y -q
...
#
RUN apt-get install --no-install-recommends software-properties-common=0.92.37.8 -y \
 && add-apt-repository ppa:openjdk-r/ppa \
 && apt-get update -qq \
 && apt-get install --no-install-recommends openjdk-7-jre-headless=7u211-2.6.17-0ubuntu0.1 -y -q
