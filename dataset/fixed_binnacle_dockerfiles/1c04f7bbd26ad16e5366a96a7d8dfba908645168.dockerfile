...
#   Build gcc 7.3
RUN wget --quiet -O - http://ftpmirror.gnu.org/gcc/gcc-7.3.0/gcc-7.3.0.tar.gz | tar zxf - \
 && cd gcc-7.3.0 \
...
#   Build libxml2
RUN wget --quiet -O - ftp://anonymous@ftp.xmlsoft.org/libxml2/libxml2-2.9.4.tar.gz | tar zxf - \
 && cd libxml2-2.9.4 \
 ...
 && rm -rf libxml2-2.9.4
#   Build expat
RUN wget --quiet -O - http://github.com/libexpat/libexpat/releases/download/R_2_2_6/expat-2.2.6.tar.bz2 | tar jxf - \
 && cd expat-2.2.6 \
 ...
 && rm -rf expat-2.2.6
#   Build APR
RUN wget --quiet -O - http://archive.apache.org/dist/apr/apr-1.7.0.tar.bz2 | tar jxf - \
 && cd apr-1.7.0 \
 ...
 && rm -rf apr-1.7.0
#   Build APR utilities
RUN wget --quiet -O - http://archive.apache.org/dist/apr/apr-util-1.6.1.tar.bz2 | tar jxf - \
 && cd apr-util-1.6.1 \
 && sed -i -e "s/for ac_lib in '' crypt ufc; do/for ac_lib in ''; do/" configure \
...
#   Build log4cxx
RUN wget --quiet -O - http://apache.cs.utah.edu/logging/log4cxx/0.10.0/apache-log4cxx-0.10.0.tar.gz | tar zxf - \
...
#   Build Boost
RUN wget --quiet -O - http://sourceforge.mirrorservice.org/b/bo/boost/boost/1.65.1/boost_1_65_1.tar.bz2 | tar jxf - \
 && cd boost_1_65_1 \
 ...
 && rm -rf boost_1_65_1
#   Build cppunit
RUN wget --quiet -O - http://dev-www.libreoffice.org/src/cppunit-1.13.2.tar.gz | tar zxf - \
 && cd cppunit-1.13.2 \
 && ./configure --prefix=/usr/local/gcc73 \
 && make -j`grep -c '^processor' /proc/cpuinfo ` \
 && make install \
...
#   Build patchelf
RUN wget --quiet -O - http://nixos.org/releases/patchelf/patchelf-0.9/patchelf-0.9.tar.gz | tar zxf - \
 && cd patchelf-0.9 \
 && ./configure --prefix=/usr/local/gcc73 \
 && make -j`grep -c '^processor' /proc/cpuinfo ` \
 && make install \
