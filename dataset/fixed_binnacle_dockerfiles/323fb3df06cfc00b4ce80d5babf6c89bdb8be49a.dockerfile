...
ENV V8_VERSION="${V8_VERSION}" \
    V8_DIR="${V8_DIR}"
RUN set -x \
 && apk add curl=8.0.1-r1 g++=12.2.1_git20220924-r9 gcc=12.2.1_git20220924-r9 glib-dev=2.76.1-r0 icu-dev=72.1-r2 linux-headers=6.2-r0 make=4.4.1-r0 ninja python tar=1.34-r2 xz=5.4.2-r0 --update --virtual .v8-build-dependencies \
 && : "---------- V8 ----------" \
 && mkdir -p /tmp/v8 \
