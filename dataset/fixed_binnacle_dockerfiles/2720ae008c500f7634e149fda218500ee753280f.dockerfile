...
RUN yum -y install python27-devel libffi-devel openssl-devel libssh2-devel autoconf automake libtool libxml2-devel libxslt-devel libjpeg-devel zlib-devel make cmake python27-setuptools gcc python-devel python-setuptools wget openssl
...
CMD [ "if [ -f /data/hubble_buildinfo ] ; then echo \"\" >> /hubble_build/hubblestack/__init__.py ; cat /data/hubble_buildinfo >> /hubble_build/hubblestack/__init__.py; fi \
 ...
 && fpm -s dir -t rpm -n hubblestack -v ${HUBBLE_VERSION} --iteration ${HUBBLE_ITERATION} --url ${HUBBLE_URL} --description \"${HUBBLE_DESCRIPTION}\" --rpm-summary \"${HUBBLE_SUMMARY}\" --after-install /hubble_build/conf/afterinstall.sh --after-upgrade /hubble_build/conf/afterupgrade.sh --before-remove /hubble_build/conf/beforeremove.sh etc opt usr /var/log/hubble_osquery/backuplogs \
 ...
 && openssl dgst -sha256 /data/hubblestack-${HUBBLE_VERSION}-${HUBBLE_ITERATION}.al1609.x86_64.rpm > /data/hubblestack-${HUBBLE_VERSION}-${HUBBLE_ITERATION}.al1609.x86_64.rpm.sha256" ]
