...
RUN yum install --setopt=keepcache=0 -y ed gcc gcc-c++ git glibc.i686 make man ncurses-devel numactl-devel openssh-clients openssl-devel python-devel redhat-lsb-core rpm-build ruby rubygems rubygem-rake tar unzip which \
...
RUN pip install pip==23.1 PyInstaller==3.1 --upgrade -Iv
#   * ccache (from source)
