...
RUN npm install bids-validator@1.2.3 -g
#   Installing and setting up ICA_AROMA
RUN mkdir -p /opt/ICA-AROMA \
 && curl -sSL "https://github.com/maartenmennes/ICA-AROMA/archive/v0.4.4-beta.tar.gz" | tar -xzC /opt/ICA-AROMA --strip-components 1 \
...
RUN conda install -y python=3.7.1 pip=19.1 mkl=2018.0.3 mkl-service numpy=1.15.4 scipy=1.1.0 scikit-learn=0.19.1 matplotlib=2.2.2 pandas=0.23.4 libxml2=2.9.8 libxslt=1.1.32 graphviz=2.40.1 traits=4.6.0 zlib ; sync \
...
#   Precaching atlases
RUN pip install "templateflow>=0.3.0,<0.4.0a0" --no-cache-dir \
 && python -c "from templateflow import api as tfapi; tfapi.get('MNI152NLin6Asym', atlas=None); tfapi.get('MNI152NLin2009cAsym', atlas=None); tfapi.get('OASIS30ANTs');" \
...
RUN echo "${VERSION}" > /src/fmriprep/fmriprep/VERSION \
 ...
 && pip install "/src/fmriprep[all]" --no-cache-dir
RUN install -m 0755 /src/fmriprep/scripts/generate_reference_mask.py /usr/local/bin/generate_reference_mask
