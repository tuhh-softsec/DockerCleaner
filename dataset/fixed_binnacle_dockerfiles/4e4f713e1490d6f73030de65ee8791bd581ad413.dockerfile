...
#
RUN apk add gcc=8.3.0-r0 libc-dev=0.7.1-r0 make=4.2.1-r2 openssl-dev=1.1.1k-r0 pcre-dev=8.43-r1 zlib-dev=1.2.11-r1 linux-headers=4.19.36-r0 curl=7.66.0-r4 gnupg1=1.4.23-r0 libxslt-dev=1.1.33-r3 gd-dev=2.2.5-r3 geoip-dev=1.6.12-r1 --no-cache --virtual .build-deps \
 && mkdir -p /usr/src \
...
#   WORDPRESS
#  ###########
RUN apk add bash=5.0.0-r0 sed=4.7-r0 --no-cache
...
RUN set -ex \
 && apk add imagemagick-dev=7.0.8.68-r0 libtool=2.4.6-r6 libxml2-dev=2.9.9-r5 $PHPIZE_DEPS --no-cache --virtual .phpize-deps \
...
RUN set -ex ; apk add ed=1.15-r0 dumb-init=1.2.2-r1 --no-cache
