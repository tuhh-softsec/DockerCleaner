...
RUN if [ ${INSTALL_LDAP} = true ] ; then apt-get update -yqq \
 ...
 && (apt-get update ;apt-get install --no-install-recommends php7.0-ldap -y ) ; fi
#  ####################################
#   IMAP:
#  ####################################
ARG INSTALL_IMAP=false
ENV INSTALL_IMAP="${INSTALL_IMAP}"
RUN if [ ${INSTALL_IMAP} = true ] ; then apt-get update -yqq \
 && (apt-get update ;apt-get install --no-install-recommends php7.0-imap -y ) ; fi
#  ####################################
#   Set Timezone
#  ####################################
ARG TZ=UTC
...
RUN if [ ${INSTALL_AMQP} = true ] ; then (apt-get update ;apt-get install --no-install-recommends librabbitmq-dev -y ) \
 ...
 && ln -s /etc/php/7.0/mods-available/amqp.ini /etc/php/7.0/cli/conf.d/30-amqp.ini ; fi
#  ####################################
...
RUN if [ ${INSTALL_INTL} = true ] ; then apt-get update -yqq \
 ...
 && (apt-get update ;apt-get install --no-install-recommends php7.0-intl -y ) ; fi
#
#  --------------------------------------------------------------------------
#   Final Touch
#  --------------------------------------------------------------------------
