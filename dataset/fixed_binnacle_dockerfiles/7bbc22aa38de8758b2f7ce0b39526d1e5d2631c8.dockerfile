...
RUN sed -i -e 's/^ACCEPT_EULA=decline/ACCEPT_EULA=accept/g' /tmp/l_mpi_2017.2.174/silent.cfg \
 ...
 && CONFIGURE_OPTS=" --1bitsgd=yes --with-mpi=${INTEL_MPI_PATH}/${COMPILERVARS_ARCHITECTURE} --with-cuda=/usr/local/cuda --with-gdk-include=/usr/local/cuda/include --with-gdk-nvml-lib=/usr/local/cuda/lib64/stubs --with-kaldi=${KALDI_PATH} --with-py36-path=/root/anaconda3/envs/cntk-py36 --with-cudnn=/usr/local/cudnn --with-nccl=${NCCL_PATH}" \
