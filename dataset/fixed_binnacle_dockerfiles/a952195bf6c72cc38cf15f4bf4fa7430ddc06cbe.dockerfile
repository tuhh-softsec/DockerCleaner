...
RUN if [ ${INSTALL_LDAP} = true ] ; then apt-get update -yqq \
 ...
 && (apt-get update ;apt-get install --no-install-recommends php5.6-ldap -y ) ; fi
#  ####################################
#   IMAP:
#  ####################################
USER root
...
RUN if [ ${INSTALL_IMAP} = true ] ; then add-apt-repository -y ppa:ondrej/php \
 && apt-get update -yqq \
 && (apt-get update ;apt-get install --no-install-recommends php5.6-imap -y ) ; fi
#  ####################################
#   Set Timezone
#  ####################################
ARG TZ=UTC
...
RUN if [ ${INSTALL_DRUSH} = true ] ; then apt-get update -yqq \
 && (apt-get update ;apt-get install --no-install-recommends mysql-client -y ) \
...
RUN if [ ${INSTALL_MSSQL} = true ] ; then (apt-get update ;apt-get install --no-install-recommends php5.6-sybase freetds-bin freetds-common libsybdb5 -y ) \
...
RUN if [ ${INSTALL_IMAGE_OPTIMIZERS} = true ] ; then (apt-get update ;apt-get install --no-install-recommends jpegoptim optipng pngquant gifsicle -y --force-yes ) \
...
RUN if [ ${INSTALL_PYTHON} = true ] ; then apt-get update \
 && (apt-get update ;apt-get install --no-install-recommends python python-pip python-dev build-essential -y ) \
...
RUN if [ ${INSTALL_IMAGEMAGICK} = true ] ; then (apt-get update ;apt-get install --no-install-recommends imagemagick php-imagick -y --force-yes ) ; fi
#  ####################################
#   Terraform:
#  ####################################
USER root
...
RUN if [ ${INSTALL_TERRAFORM} = true ] ; then apt-get update -yqq \
 && (apt-get update ;apt-get install --no-install-recommends sudo wget unzip -y ) \
...
RUN if [ ${INSTALL_DUSK_DEPS} = true ] ; then add-apt-repository ppa:ondrej/php \
 && apt-get update \
 && (apt-get update ;apt-get install --no-install-recommends zip wget unzip xdg-utils libxpm4 libxrender1 libgtk2.0-0 libnss3 libgconf-2-4 xvfb gtk2-engines-pixbuf xfonts-cyrillic xfonts-100dpi xfonts-75dpi xfonts-base xfonts-scalable x11-apps -y ) \
...
RUN if [ ${INSTALL_INTL} = true ] ; then apt-get update -yqq \
 ...
 && (apt-get update ;apt-get install --no-install-recommends php5.6-intl -y ) ; fi
#
#  --------------------------------------------------------------------------
#   Final Touch
#  --------------------------------------------------------------------------
