...
RUN mkdir -p "$GOPATH/src" "$GOPATH/bin" \
 && chmod -R 777 "$GOPATH" \
 && apt-get update \
 && apt-get install --no-install-recommends pkg-config=0.26-* -y \
 && apt-get clean \
