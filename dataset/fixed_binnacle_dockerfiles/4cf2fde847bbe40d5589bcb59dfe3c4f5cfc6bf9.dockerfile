#   Build target: lablup/common-tensorflow:FF.19.06-py36-cuda9
...
RUN cd /tmp/tensorflow-${TF_VERSION} \
 ...
 && LD_LIBRARY_PATH=/usr/local/cuda/lib64/stubs:${LD_LIBRARY_PATH} tensorflow/tools/ci_build/builds/configured GPU bazel build -c opt --copt=-mavx --copt=-mavx2 --config=cuda --config=monolithic --cxxopt="-D_GLIBCXX_USE_CXX11_ABI=0" tensorflow/tools/pip_package:build_pip_package \
