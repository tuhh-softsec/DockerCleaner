...
RUN echo "deb ${DEBIAN_REPO}/${DISTRO} ${CODENAME} main" >> /etc/apt/sources.list \
 ...
 && (apt-get update ;apt-get install --no-install-recommends bash-completion=1:2.11-6ubuntu1 bzip2=1.0.8-5build1 cmake=3.25.1-1 dnsutils=1:9.18.12-1ubuntu1 ffmpeg=7:5.1.2-3ubuntu1 fonts-dejavu=2.37-6 fonts-liberation=1:1.07.4-11 g++=4:12.2.0-3ubuntu1 gcc=4:12.2.0-3ubuntu1 git=1:2.39.2-1ubuntu1 info=6.8-6build2 jq=1.6-2.1ubuntu3 kstart=4.3-1 less=590-1.2 libaio1=0.3.113-4 lmodern=2.005-1 luarocks=3.8.0+dfsg1-1 make=4.3-4.1build1 man netcat openresty openresty-opm openssh-client=1:9.0p1-1ubuntu8 procps=2:4.0.3-1ubuntu1 psmisc=23.6-1 rsync=3.2.7-1 sudo=1.9.13p1-1ubuntu2 sssd=2.8.1-1ubuntu1 unzip=6.0-27ubuntu1 vim=2:9.0.1000-4ubuntu2 wget=1.21.3-1ubuntu1 zlib1g-dev=1:1.2.13.dfsg-1ubuntu4 -yq ) \
 && apt-get clean \
...
RUN cd /tmp \
 ...
 && for dir in .conda/envs .jupyter .local/share/jupyter/runtime .sparkmagic bin work; do mkdir -p "${HOME}/${dir}" ; done \
 && fix-permissions "${CONDA_DIR}" \
 && fix-permissions "${HOME}" \
 && rm -rf /tmp/*
