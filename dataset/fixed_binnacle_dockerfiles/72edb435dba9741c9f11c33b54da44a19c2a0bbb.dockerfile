#  FROM ubuntu:16.04
FROM tensorflow/tensorflow:1.8.0-rc0-devel-gpu-py3
...
RUN apt-get update \
 && (apt-get update ;apt-get install --no-install-recommends build-essential cmake git wget vim python-dev python-numpy python-pip python-yaml locales curl apt-transport-https -y )
RUN apt-get update -y \
...
RUN apt-get update -y \
 && (apt-get update ;apt-get install --no-install-recommends python3-dev python3-pip python3-yaml python3-pycurl locales ssh-askpass -y )
...
EXPOSE 180/tcp 1443/tcp 1280/tcp
...
RUN cd /root/Mask_RCNN ; pip3 install -r requirements.txt ; python3 setup.py install
#   Instal cocotool
RUN git clone https://github.com/waleedka/coco.git
RUN cd /root/coco/PythonAPI ; make ; make install ; python3 setup.py install
RUN apt-get update -y \
 && (apt-get update ;apt-get install --no-install-recommends libxrender1 -y )
