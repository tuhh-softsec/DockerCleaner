...
RUN wget -q -O - https://dl-ssl.google.com/linux/linux_signing_key.pub | apt-key add - \
 ...
 && update.sh \
 && install.sh chromium-codecs-ffmpeg-extra firefox nginx gconf2 gconf-service libnotify4 libappindicator1 libxtst6 libnss3 \
...
RUN which wget &> /dev/null || apt-get install --no-install-recommends wget=1.20.3 ; wget --no-verbose --output-document /tmp/ff.tar.gz https://cloud.imaginary.org/index.php/s/B0G13fyRSStI26y/download
