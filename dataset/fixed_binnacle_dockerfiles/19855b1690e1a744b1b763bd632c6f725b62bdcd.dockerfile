FROM harbor.h2o.ai/opsh2oai/h2o-3-hadoop-cdh-5.13:58
ENV LANG="'C.UTF-8'"
RUN locale
...
RUN apt-get update \
 && apt-get install --no-install-recommends git curl zip s3cmd r-base r-base-dev libxml2-dev libssl-dev libcurl4-openssl-dev -y
