...
RUN echo slapd slapd/internal/adminpw password rtzewrpiZRT753 | debconf-set-selections ; echo slapd slapd/internal/generated_adminpw password rtzewrpiZRT753 | debconf-set-selections ; echo slapd slapd/password2 password rtzewrpiZRT753 | debconf-set-selections ; echo slapd slapd/password1 password rtzewrpiZRT753 | debconf-set-selections
#   Make sure you run apt-get update in the same line with
#   all the packages to ensure all are updated correctly.
#   (https://runnable.com/blog/9-common-dockerfile-mistakes)
RUN apt-get update \
 && apt-get install --no-install-recommends apache2-utils apt-offline apt-utils build-essential bzip2 dialog dselect freeradius initramfs-tools ldap-utils libbz2-dev logrotate nano net-tools nginx-extras ntp p7zip-full php-pear slapd snmp snmpd sqlite subversion sudo unzip wget ${PHPINSTALLPREFIX}-cgi ${PHPINSTALLPREFIX}-dev ${PHPINSTALLPREFIX}-fpm ${PHPINSTALLPREFIX}-gd ${PHPINSTALLPREFIX}-gmp ${PHPINSTALLPREFIX}-ldap ${PHPINSTALLPREFIX}-mcrypt ${PHPINSTALLPREFIXSQLITE}-sqlite ${PHPINSTALLPREFIX}-mbstring -y
#  ###########################################################
