...
RUN apt-get update \
 && apt-get install --no-install-recommends supervisor git curl build-essential ruby1.9.1-full libssl-dev libreadline-dev libxslt1-dev libxml2-dev libcurl4-openssl-dev zlib1g-dev libexpat1-dev libicu-dev unzip libpcre3-dev -y \
 && apt-get clean \
...
RUN mkdir /tmp/client_body_tmp \
 && mkdir /tmp/proxy_temp \
 && cd /build/ngx_openresty-1.7.7.1 \
 && ./configure --prefix=/opt/openresty --with-luajit --with-luajit-xcflags=-DLUAJIT_ENABLE_LUA52COMPAT --http-client-body-temp-path=/tmp/client_body_temp --http-proxy-temp-path=/tmp/proxy_temp --http-log-path=/var/nginx/logs/access.log --error-log-path=/var/nginx/logs/error.log --pid-path=/var/nginx/run/nginx.pid --lock-path=/var/nginx/run/nginx.lock --with-http_stub_status_module --with-http_ssl_module --with-http_secure_link_module --with-http_gzip_static_module --with-http_sub_module --with-http_realip_module --without-http_scgi_module --with-md5-asm --with-sha1-asm --with-file-aio --with-pcre --with-pcre-jit --add-module=/build/nginx_upstream_check_module --add-module=/build/nginx-upstream-fair --add-module=/build/nginx-sticky-module --add-module=/build/nginx_tcp_proxy_module
...
RUN curl -s http://luarocks.org/releases/luarocks-2.2.0.tar.gz | tar xvz -C /tmp/ \
 && cd /tmp/luarocks-* \
 && ./configure --with-lua=/opt/openresty/luajit --with-lua-include=/opt/openresty/luajit/include/luajit-2.1 --with-lua-lib=/opt/openresty/lualib \
 && make \
 && make install \
...
RUN useradd -r -d /var/nginx nginx \
 && chown -R nginx:nginx /var/nginx/ /tmp/client_body_tmp /tmp/proxy_temp
EXPOSE 3232/tcp
