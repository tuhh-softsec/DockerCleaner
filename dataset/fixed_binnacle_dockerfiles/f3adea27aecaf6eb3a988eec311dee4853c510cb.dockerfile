...
RUN if [ ${INSTALL_SOAP} = true ] ; then add-apt-repository -y ppa:ondrej/php \
 && apt-get update -yqq \
 && apt-get install --no-install-recommends libxml2-dev php7.1-soap -y ; fi
#  ####################################
#   LDAP:
#  ####################################
...
RUN if [ ${INSTALL_LDAP} = true ] ; then apt-get update -yqq \
 ...
 && apt-get install --no-install-recommends php7.1-ldap -y ; fi
#  ####################################
#   IMAP:
#  ####################################
...
RUN if [ ${INSTALL_IMAP} = true ] ; then apt-get update -yqq \
 && apt-get install --no-install-recommends php7.1-imap -y ; fi
#  ####################################
#   Set Timezone
#  ####################################
ARG TZ=UTC
...
RUN if [ ${INSTALL_XDEBUG} = true ] ; then apt-get update \
 && apt-get install --no-install-recommends php7.1-xdebug -y --force-yes \
...
RUN if [ ${INSTALL_MONGO} = true ] ; then pecl -q install mongodb \
 ...
 && ln -s /etc/php/7.1/mods-available/mongodb.ini /etc/php/7.1/cli/conf.d/30-mongodb.ini ; fi
#  ####################################
#   AMQP:
#  ####################################
#   Check if Mongo needs to be installed
...
RUN if [ ${INSTALL_AMQP} = true ] ; then apt-get install --no-install-recommends librabbitmq-dev -y \
 ...
 && ln -s /etc/php/7.1/mods-available/amqp.ini /etc/php/7.1/cli/conf.d/30-amqp.ini ; fi
#  ####################################
...
RUN if [ ${INSTALL_SWOOLE} = true ] ; then pecl -q install swoole \
 ...
 && ln -s /etc/php/7.1/mods-available/swoole.ini /etc/php/7.1/cli/conf.d/20-swoole.ini ; fi
#  ####################################
#   Drush:
#  ####################################
USER root
...
RUN if [ ${INSTALL_AEROSPIKE} = false ] ; then rm /etc/php/7.1/cli/conf.d/aerospike.ini ; fi
#  ####################################
#   PHP V8JS:
#  ####################################
USER root
...
RUN if [ ${INSTALL_V8JS} = true ] ; then add-apt-repository -y ppa:pinepain/libv8-5.4 \
 && apt-get update -yqq \
 && apt-get install --no-install-recommends php-xml php-dev php-pear libv8-5.4 -y \
 && pecl install v8js \
 && echo "extension=v8js.so" >> /etc/php/7.1/cli/php.ini; fi
#  ####################################
...
RUN if [ ${INSTALL_MSSQL} = true ] ; then cd / \
 ...
 && echo "extension=pdo_sqlsrv.so" > /etc/php/7.1/cli/conf.d/20-pdo_sqlsrv.ini; fi
#  ####################################
#   Minio:
#  ####################################
USER root
...
RUN if [ ${INSTALL_IMAGEMAGICK} = true ] ; then apt-get install --no-install-recommends imagemagick php-imagick -y --force-yes ; fi
#  ####################################
#   Terraform:
#  ####################################
USER root
...
RUN if [ ${INSTALL_INTL} = true ] ; then apt-get update -yqq \
 ...
 && apt-get install --no-install-recommends php7.1-intl -y ; fi
#
#  --------------------------------------------------------------------------
#   Final Touch
#  --------------------------------------------------------------------------
