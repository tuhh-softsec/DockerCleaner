...
RUN apt-get update \
 && apt-get install --no-install-recommends sudo time -y \
...
RUN which wget &> /dev/null || apt-get install --no-install-recommends wget=1.20.3 ; wget --no-verbose --output-document cartographer_ros/cartographer_version.json https://api.github.com/repos/googlecartographer/cartographer/git/refs/heads/master
...
ARG CACHEBUST=1
RUN CARTOGRAPHER_VERSION=$CARTOGRAPHER_VERSION cartographer_ros/scripts/prepare_jenkins_catkin_workspace.sh
...
RUN cartographer_ros/scripts/install.sh --pkg cartographer_ros_msgs \
 && cartographer_ros/scripts/install.sh --pkg cartographer_ros_msgs --catkin-make-args run_tests \
...
RUN cartographer_ros/scripts/install.sh --pkg cartographer_ros \
 && cartographer_ros/scripts/install.sh --pkg cartographer_ros --catkin-make-args run_tests \
...
RUN cartographer_ros/scripts/install.sh --pkg cartographer_rviz \
 && cartographer_ros/scripts/install.sh --pkg cartographer_rviz --catkin-make-args run_tests \
...
RUN pip install google-cloud-datastore==2.15.1 --upgrade
RUN pip install google-cloud-bigquery==3.9.0 --upgrade
