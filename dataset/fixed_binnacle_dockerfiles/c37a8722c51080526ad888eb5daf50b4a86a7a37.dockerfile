...
ENV ZABBIX_VERSION="tags/3.4.14" \
    ZS_enabled="true" \
    ZA_enabled="true" \
    ZW_enabled="true" \
    ZJ_enabled="false" \
...
RUN export FPING_VERSION=4.0 \
 ...
 && ./configure --enable-server --enable-agent --with-mysql --enable-java --with-net-snmp --with-libcurl --with-libxml2 --with-openipmi --enable-ipv6 --with-jabber --with-openssl --with-ssh2 --enable-proxy --with-ldap --with-unixodbc \
 ...
 && locale/make_mo.sh \
 && yum autoremove -y gettext svn gcc automake mariadb-devel java-1.8.0-openjdk-devel libxml2-devel libcurl-devel OpenIPMI-devel iksemel-devel rubygems ruby-devel ruby-devel unixODBC-devel \
...
RUN chmod +x /usr/local/src/zabbix/supervisord-listener.py \
 && yum -y -q reinstall glibc-common \
 && cd /tmp \
