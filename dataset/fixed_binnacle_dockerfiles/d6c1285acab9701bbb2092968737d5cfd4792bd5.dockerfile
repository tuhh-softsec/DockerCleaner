...
RUN apk add bash=5.0.11-r1 curl=7.79.1-r0 less=551-r0 vim=8.2.3437-r0 --no-cache
...
RUN if [ "$BUILD_ENV" = "production" ] ; then yarn install --frozen-lockfile --ignore-scripts --no-cache ; elif [ "$BUILD_ENV" = "test" ] ; then yarn install --frozen-lockfile --ignore-scripts --no-cache ; fi ; rm package.json yarn.lock
