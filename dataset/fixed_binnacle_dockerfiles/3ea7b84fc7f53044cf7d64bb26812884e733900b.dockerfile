...
RUN :
RUN (apt-get update ;apt-get install --no-install-recommends emacs git python-apt apt-utils software-properties-common sudo curl wget unzip opensmtpd nodejs libssl-dev libpam0g-dev zlib1g-dev dh-autoreconf automake autoconf build-essential -qy )
...
#   For Wetty
RUN npm install wetty@2.5.0 -g
WORKDIR /
#   For MALLET
RUN (apt-get update ;apt-get install --no-install-recommends default-jdk ant python3-pip -qy )
RUN mkdir /mallet
...
#   For R-Studio
RUN (apt-get update ;apt-get install --no-install-recommends r-base r-base-dev gdebi-core libatlas3-base libapparmor1 apparmor-profiles libssl0.9.8 -qy --force-yes )
...
#   For brackets
RUN npm install brackets@0.5.8 -g
...
#       grunt build
WORKDIR /
RUN (apt-get update ;apt-get install --no-install-recommends apache2 mysql-server -qy )
#   For the file manager
RUN npm install node-file-manager@0.4.6 -g
...
COPY ./runit_scripts/startup.sh /etc/my_init.d/startup.sh
