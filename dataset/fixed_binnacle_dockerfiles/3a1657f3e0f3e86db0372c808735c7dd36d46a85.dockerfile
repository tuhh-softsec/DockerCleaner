...
RUN apt-get update \
 && apt-get install --no-install-recommends supervisor=3.3.1-1.1 cron=3.0pl1-128.1ubuntu1.2 logrotate=3.11.0-0.1ubuntu1 syslog-ng-core=3.13.2-3 postfix=3.3.0-1ubuntu0.4 curl=7.58.0-2ubuntu3.24 gcc=4:7.4.0-1ubuntu2.3 git=1:2.17.1-1ubuntu0.17 gnupg-agent=2.2.4-1ubuntu1.6 make=4.1-9.1ubuntu1 python=2.7.15~rc1-1 python3=3.6.7-1~18.04 openssl=1.1.1-1ubuntu2.1~18.04.21 redis-server=5:4.0.9-1ubuntu0.2 sudo=1.8.21p2-3ubuntu1.5 vim=2:8.0.1453-1ubuntu1.11 zip=3.0-11build1 wget=1.19.4-1ubuntu2.2 mariadb-client=1:10.1.48-0ubuntu0.18.04.1 mariadb-server=1:10.1.48-0ubuntu0.18.04.1 apache2=2.4.29-1ubuntu4.27 apache2-doc=2.4.29-1ubuntu4.27 apache2-utils=2.4.29-1ubuntu4.27 libapache2-mod-php=1:7.2+60ubuntu1 php=1:7.2+60ubuntu1 php-cli=1:7.2+60ubuntu1 php-gnupg=1.4.0-1build2 php-dev=1:7.2+60ubuntu1 php-json=1:7.2+60ubuntu1 php-mysql=1:7.2+60ubuntu1 php-opcache php-readline=1:7.2+60ubuntu1 php-redis=3.1.6-1build1 php-xml=1:7.2+60ubuntu1 php-mbstring=1:7.2+60ubuntu1 rng-tools=5-0ubuntu4 python3-dev=3.6.7-1~18.04 python3-pip=9.0.1-2.3~ubuntu1.18.04.8 libxml2-dev=2.9.4+dfsg1-6.1ubuntu1.8 libxslt1-dev=1.1.29-5ubuntu0.3 zlib1g-dev=1:1.2.11.dfsg-0ubuntu2.2 python-setuptools=39.0.1-2ubuntu0.1 libpq5=10.23-0ubuntu0.18.04.1 libjpeg-dev=8c-2ubuntu8 -y
...
RUN chown -R www-data:www-data /var/www/MISP /root/.config /root/.git ; sudo -u www-data -H git clone --branch v2.4.95 --depth 1 https://github.com/MISP/MISP.git /var/www/MISP ; sudo -u www-data -H git submodule update --init --recursive ; sudo -u www-data -H git submodule foreach --recursive git config core.filemode false ; sudo -u www-data -H git config core.filemode false ; echo
...
RUN sudo pip3 install -I -r REQUIREMENTS ; sudo pip3 install -I .
...
RUN sed -i -e 's/db login/misp/g' /var/www/MISP/app/Config/database.php ; sed -i -e "s/db password/${MYSQL_MISP_PASSWORD}/g" /var/www/MISP/app/Config/database.php ; sed -i -E "s/'salt'(\s+)=>\s''/'salt' => '`openssl rand -base64 32 | tr '/' '0' `'/" /var/www/MISP/app/Config/config.php ; sed -i -E "s/'baseurl'(\s+)=>\s''/'baseurl' => 'https:\/\/${MISP_FQDN}'/" /var/www/MISP/app/Config/config.php ; sed -i -e "s/email@address.com/${MISP_EMAIL}/" /var/www/MISP/app/Config/config.php ; sed -i -e "s/bind 127.0.0.1 ::1/bind 0.0.0.0/" /etc/redis/redis.conf ; sudo chown -R www-data:www-data /var/www/MISP/app/Config ; sudo chmod -R 750 /var/www/MISP/app/Config ; sudo pip3 install --upgrade pip ; sudo pip3 install pyzmq ; sudo pip3 install redis ; sudo -u www-data -H wget http://downloads.sourceforge.net/project/ssdeep/ssdeep-2.13/ssdeep-2.13.tar.gz ; tar zxvf ssdeep-2.13.tar.gz \
 && cd ssdeep-2.13 \
 && ./configure \
 && make \
