...
RUN apt-get update --quiet \
 && (apt-get update ;apt-get install --no-install-recommends apt-utils wget build-essential emacs python-dev python-pip libhdf5-serial-dev cmake git-core libfftw3-dev h5utils jq libzmq-dev hdf5-tools liblapack-dev libxml2-dev libfreetype6-dev pkg-config libxslt-dev libace-dev gcc-multilib libgtest-dev liblapack-dev liblapacke-dev libatlas-base-dev libplplot-dev libdcmtk-dev supervisor net-tools cpio --no-install-suggests --yes )
...
RUN :
RUN (apt-get update ;apt-get install --no-install-recommends python3.6 -y )
RUN (apt-get update ;apt-get install --no-install-recommends python3.6-dev -y )
RUN (apt-get update ;apt-get install --no-install-recommends python3.6-venv -y )
RUN cd /tmp
...
RUN (apt-get update ;apt-get install --no-install-recommends python3.6-tk -y )
