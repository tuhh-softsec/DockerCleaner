...
RUN apt-get update \
 ...
 && apt-get install --no-install-recommends bzip2 ca-certificates curl git gnupg gzip locales mercurial netcat net-tools openssh-client parallel sudo tar unzip wget xvfb zip -y
...
RUN apt-get install --no-install-recommends autoconf automake autotools-dev babeltrace bc bison curl device-tree-compiler expat flex gawk gperf libexpat-dev libgmp-dev libmpc-dev libmpfr-dev libtool libusb-1.0-0-dev patchutils pkg-config python python-pexpect texinfo zlib1g-dev -y
