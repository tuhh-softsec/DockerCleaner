...
RUN rustup update \
 && set -ex \
 && apt-get update \
 && (apt-get update ;apt-get install --no-install-recommends clang=1:11.0-51+nmu5 libclang-dev=1:11.0-51+nmu5 llvm-dev=1:11.0-51+nmu5 libncurses5=6.2+20201114-2 libncursesw5=6.2+20201114-2 cmake=3.18.4-2+deb11u1 git=1:2.30.2-1+deb11u2 libssl-dev=1.1.1n-0+deb11u4 --yes )
...
RUN : \
 && DEBIAN_FRONTEND=noninteractive apt-get install -y locales procps libssl-dev vim telnet curl python3 \
