...
RUN set -xe ; apk add ca-certificates curl apache2 apache2-utils xz --no-cache --virtual .persistent-deps
...
RUN set -xe ; apk add curl-dev libedit-dev libxml2-dev openssl-dev apache2-dev freetype-dev libjpeg-turbo-dev libpng-dev libmcrypt-dev $PHPIZE_DEPS --no-cache --virtual .build-deps \
