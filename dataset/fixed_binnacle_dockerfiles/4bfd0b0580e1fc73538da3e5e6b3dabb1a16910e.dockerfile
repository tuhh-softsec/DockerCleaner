...
RUN apt-get update \
 && apt-get install --no-install-recommends apparmor apt-utils aufs-tools automake bash-completion binutils-mingw-w64 bsdmainutils btrfs-tools build-essential clang cmake createrepo curl dpkg-sig gcc-mingw-w64 git iptables jq less libapparmor-dev libcap-dev libltdl-dev libnl-3-dev libprotobuf-c0-dev libprotobuf-dev libsystemd-journal-dev libtool mercurial net-tools pkg-config protobuf-compiler protobuf-c-compiler python-dev python-mock python-pip python-websocket tar vim vim-common xfsprogs zip -y \
