...
RUN powershell -Command $ProgressPreference = 'SilentlyContinue' ; Invoke-WebRequest -UseBasicParsing -Uri https://download.visualstudio.microsoft.com/download/pr/4eef3cca-9da8-417a-889f-53212671e83b/50a7f7a24b21910a7cbe093fa150fd31/vs_testagent.exe -OutFile vs_TestAgent.exe `
 ...
 && start /w vs_BuildTools.exe ^ --add Microsoft.VisualStudio.Workload.MSBuildTools ^ --add Microsoft.VisualStudio.Workload.NetCoreBuildTools ^ --add Microsoft.Net.Component.4.7.2.SDK ^ --add Microsoft.Component.ClickOnce.MSBuild ^ --quiet --norestart --nocache --wait `
...
RUN WindowsMicrosoft.NETFramework64v4.0.30319ngen uninstall "%ProgramFiles(x86)%\Microsoft SDKs\Windows\v10.0A\bin\NETFX 4.7.2 Tools\SecAnnotate.exe" `
 && WindowsMicrosoft.NETFramework64v4.0.30319ngen uninstall "%ProgramFiles(x86)%\Microsoft SDKs\Windows\v10.0A\bin\NETFX 4.7.2 Tools\WinMDExp.exe" `
...
RUN powershell setx /M PATH $( ${Env:PATH} + \" ;${Env:ProgramFiles}NuGet\" + \" ;${Env:ProgramFiles(x86)}Microsoft Visual Studio2019TestAgentCommon7IDECommonExtensionsMicrosoftTestWindow\" + \" ;${Env:ProgramFiles(x86)}Microsoft Visual Studio2019BuildToolsMSBuildCurrentBin\" + \" ;${Env:ProgramFiles(x86)}Microsoft SDKsWindowsv10.0AbinNETFX 4.7.2 Tools\" + \" ;${Env:ProgramFiles(x86)}Microsoft SDKsClickOnceSignTool\" ;)
#   Install Targeting Packs
RUN powershell " $ErrorActionPreference = 'Stop'; $ProgressPreference = 'SilentlyContinue'; @('4.0', '4.5.2', '4.6.2', '4.7.2') | %{ Invoke-WebRequest -UseBasicParsing -Uri https://dotnetbinaries.blob.core.windows.net/referenceassemblies/v${_}.zip -OutFile referenceassemblies.zip; Expand-Archive referenceassemblies.zip -DestinationPath \"${Env:ProgramFiles(x86)}\Reference Assemblies\Microsoft\Framework\.NETFramework\"; Remove-Item -Force referenceassemblies.zip; }"
