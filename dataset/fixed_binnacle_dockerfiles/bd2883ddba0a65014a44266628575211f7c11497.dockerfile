...
ARG PIP="pip"
RUN apt-get update \
 && apt-get install --no-install-recommends build-essential=12.1ubuntu2 curl=7.47.0-1ubuntu2.19 git=1:2.7.4-0ubuntu1.10 libcurl3-dev libfreetype6-dev=2.6.1-0.1ubuntu2.5 libhdf5-serial-dev=1.8.16+docs-4ubuntu1.1 libpng12-dev=1.2.54-1ubuntu1.1 libzmq3-dev=4.1.4-7ubuntu0.1 libssl-dev=1.0.2g-1ubuntu4.20 pkg-config=0.29.1-0ubuntu1 rsync=3.1.1-3ubuntu1.3 software-properties-common=0.96.20.10 unzip=6.0-20ubuntu1.1 zip=3.0-11 zlib1g-dev=1:1.2.8.dfsg-2ubuntu4.3 openjdk-8-jdk=8u292-b10-0ubuntu1~16.04.1 openjdk-8-jre-headless=8u292-b10-0ubuntu1~16.04.1 -y
#  install Python 3
RUN if [ ${PYTHON} = "python3.6" ] ; then curl https://www.python.org/ftp/python/3.6.5/Python-3.6.5.tar.xz -o /opt/python.tar.xz \
 && cd /opt \
 && tar xvf python.tar.xz \
 && cd /opt/*/ \
 && ./configure \
 && make \
 && make install ; else apt-get install --no-install-recommends python-dev=2.7.12-1~16.04 ${PYTHON3_DEV} -y ; fi
RUN apt-get clean \
...
RUN if [ "${PYTHON}" = "python3" ] ; then ln -s -f /usr/bin/python3 /usr/bin/python ; elif [ "${PYTHON}" = "python3.6" ] ; then ln -s -f /usr/local/bin/python3.6 /usr/bin/python ; fi
