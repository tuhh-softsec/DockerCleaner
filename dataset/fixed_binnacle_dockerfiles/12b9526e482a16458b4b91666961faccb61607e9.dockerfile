...
RUN build_deps="ca-certificates curl gcc libc-dev libevent-dev libexpat1-dev make" \
 && set -x \
 && debian_frontend=noninteractive apt-get update \
 && apt-get install --no-install-recommends bsdmainutils=9.0.12+nmu1 ldnsutils=1.7.0-1+deb9u1 libevent-2.0 libexpat1=2.2.0-2+deb9u5 $build_deps -y \
 ...
 && apt-get purge -y --auto-remove $build_deps \
 && rm -fr /opt/unbound/share/man /tmp/* /var/tmp/* /var/lib/apt/lists/*
FROM debian:stretch
LABEL maintainer="Matthew Vance"
ENV name="unbound" \
...
RUN set -x \
 && debian_frontend=noninteractive apt-get update \
 && apt-get install --no-install-recommends bsdmainutils=9.0.12+nmu1 ldnsutils=1.7.0-1+deb9u1 libevent-2.0 libexpat1=2.2.0-2+deb9u5 -y \
 && groupadd _unbound \
 && useradd -g _unbound -s /etc -d /dev/null _unbound \
