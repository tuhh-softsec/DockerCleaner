...
RUN apt-get update -y \
 && apt-get install --no-install-recommends apt-transport-https git psmisc jsonlint yamllint gcc -y
RUN mkdir -p /go/src
ENV GOPATH="/go"
RUN cd ${GOPATH}/src \
...
RUN apt-get install --no-install-recommends postgresql-client postgresql sudo gosu -y
#   Install emacs
RUN apt-get install --no-install-recommends emacs -y
...
RUN apt-get install --no-install-recommends ruby gem -y
RUN gem install bundle --version 0.0.1
