...
RUN echo "deb http://packages.osrfoundation.org/gazebo/ubuntu xenial main" > /etc/apt/sources.list.d/gazebo-latest.list \
 ...
 && aptitude update -y -q \
 && aptitude upgrade -y -q -R gazebo7
RUN aptitude install -y -q -R ros-kinetic-tf2-geometry-msgs ros-kinetic-tf2-sensor-msgs
...
RUN aptitude install -y -q -R ros-kinetic-gazebo-ros-control ros-kinetic-ros-controllers ros-kinetic-rqt-controller-manager ros-kinetic-moveit ros-kinetic-orocos-kdl ros-kinetic-orocos-kinematics-dynamics ros-kinetic-pid \
