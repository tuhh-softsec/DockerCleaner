...
COPY bin /vesta/bin
COPY func /vesta/func
COPY install /vesta/install
COPY test /vesta/test
COPY upd /vesta/upd
COPY web /vesta/web
COPY docker /vesta/docker
RUN apt-get update \
 && apt-get -y upgrade \
 && apt-get install --no-install-recommends git unzip nano -y \
 && apt-get clean
COPY install/vst-install-ubuntu-docker.sh /install-ubuntu.sh
...
RUN bash /install-ubuntu.sh --nginx yes --apache yes --phpfpm no --vsftpd no --proftpd no --exim yes --dovecot yes --spamassassin yes --clamav yes --named yes --iptables no --fail2ban no --mysql yes --postgresql no --remi yes --quota no --password admin -y no -f \
 && apt-get clean
COPY docker/dovecot /etc/init.d/dovecot
...
COPY docker/startup-ubuntu.sh /etc/my_init.d/startup.sh
