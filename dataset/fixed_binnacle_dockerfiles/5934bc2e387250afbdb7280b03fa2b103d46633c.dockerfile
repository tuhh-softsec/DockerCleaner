...
RUN ln -sf /usr/share/zoneinfo/UTC /etc/localtime ; DEBIAN_FRONTEND=noninteractive apt-get update \
 && apt-get install --no-install-recommends libglib2.0-dev=2.56.4-0ubuntu0.18.04.9 gobject-introspection=1.56.1-1 libgirepository1.0-dev=1.56.1-1 libpango-1.0-0=1.40.14-1ubuntu0.1 libpangocairo-1.0-0=1.40.14-1ubuntu0.1 autopoint=0.19.8.1-6ubuntu0.3 -y -q
RUN wget -O - ${GST_REPO} | tar xJ \
 && cd gstreamer-${GST_VER} \
 && ./autogen.sh --prefix=/usr --libdir=/usr/lib/x86_64-linux-gnu --libexecdir=/usr/lib/x86_64-linux-gnu --enable-shared --disable-examples --disable-gst-debug --disable-debug --disable-benchmarks --disable-gtk-doc \
 && make -j $( nproc ;) \
 && make install DESTDIR=/home/build \
 && make install
...
RUN wget -O - https://github.com/eclipse/paho.mqtt.c/archive/v${PAHO_VER}.tar.gz | tar -xz ; cd paho.mqtt.c-${PAHO_VER} ; make ; make install
...
RUN wget -O - ${VA_GSTREAMER_PLUGINS_REPO} | tar xz \
 ...
 && cmake -DVERSION_PATCH=$( echo "$( git rev-list --count --first-parent HEAD ;)" ;) -DGIT_INFO=$( echo "git_$( git rev-parse --short HEAD ;)" ;) -DCMAKE_BUILD_TYPE=Release -DDISABLE_SAMPLES=ON -DDISABLE_VAAPI=ON -DBUILD_SHARED_LIBS=ON -DCMAKE_INSTALL_PREFIX=/usr .. \
 && make -j4
