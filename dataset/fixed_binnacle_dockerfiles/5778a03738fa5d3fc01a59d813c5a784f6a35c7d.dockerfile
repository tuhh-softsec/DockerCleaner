...
RUN (apt-key adv --keyserver hkp://keyserver.ubuntu.com:80 --recv-keys 8AA7AF1F1091A5FD || apt-key adv --keyserver hkp://p80.pool.sks-keyservers.net:80 --recv-keys 8AA7AF1F1091A5FD ) \
 ...
 && apt-get update \
 && apt-get install --no-install-recommends curl=7.47.0-1ubuntu2.19 gcc=4:5.3.1-1ubuntu1 git=1:2.7.4-0ubuntu1.10 libssl-dev=1.0.2g-1ubuntu4.20 libzmq3-dev=4.1.4-7ubuntu0.1 openssl=1.0.2g-1ubuntu4.20 pkg-config=0.29.1-0ubuntu1 python3=3.5.1-3 python3-grpcio-tools=1.1.3-1 python3-sawtooth-cli unzip=6.0-20ubuntu1.1 -y -q \
...
RUN cargo build \
 && cp ./target/debug/seth /project/sawtooth-seth/bin/seth
