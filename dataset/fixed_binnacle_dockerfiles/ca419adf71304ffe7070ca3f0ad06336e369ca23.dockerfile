...
RUN apt-get update \
 && apt-get install --no-install-recommends ca-certificates curl libedit2 libsqlite3-0 libxml2 xz-utils libjpeg-dev libpng-dev libfreetype6-dev libzip-dev $PHPIZE_DEPS -y \
...
RUN set -xe \
 && buildDeps=" $PHP_EXTRA_BUILD_DEPS libcurl4-openssl-dev libedit-dev libsqlite3-dev libssl-dev libxml2-dev build-essential autoconf automake libtool bison re2c " \
 && apt-get update \
