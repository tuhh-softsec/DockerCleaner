#  FROM ubuntu:16.04
FROM uber/horovod:0.13.0-tf1.8.0-torch0.4.0-py3.5
MAINTAINER Jin Li <jinlmsft@hotmail.com>
RUN umask 022
RUN apt-get update -y \
 && (apt-get update ;apt-get install --no-install-recommends vim wget curl jq gawk openssh-client git rsync sudo -y )
RUN apt-get update \
 && (apt-get update ;apt-get install --no-install-recommends build-essential cmake git wget vim locales curl apt-transport-https screen htop -y )
#   netcore 2.0
...
RUN apt-get update -y \
 && (apt-get update ;apt-get install --no-install-recommends python3-pycurl locales ssh-askpass python3-tk screen -y )
RUN pip install pip==23.1 --upgrade
RUN pip uninstall -y keras ; pip install keras==2.12.0
...
RUN pip install pandas==2.0.0 jupyter==1.0.0 matplotlib==3.7.1 spacy==3.5.2 sklearn==0.0.post4 scikit-image==0.20.0 munch==2.5.0 imgaug==0.4.0
...
#   Add Glove vectors 
RUN apt-get update -y \
 && (apt-get update ;apt-get install --no-install-recommends zip unzip -y )
#  Telemetry
#  --------------
