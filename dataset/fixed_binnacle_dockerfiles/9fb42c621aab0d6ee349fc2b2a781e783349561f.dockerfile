...
ENV GCC_VER="4.8.3"
RUN wget ftp://ftp.gnu.org/pub/gnu/gcc/gcc-${GCC_VER}/gcc-${GCC_VER}.tar.bz2 \
...
ENV CMAKE_VER="3.5.2"
RUN wget https://cmake.org/files/v3.5/cmake-${CMAKE_VER}.tar.gz \
...
RUN git clone --recursive --jobs ${BUILD_THREADS} --branch ${QT5_VER} https://code.qt.io/qt/qt5.git \
 && cd /workdir/qt5 \
...
RUN wget http://sourceforge.net/projects/pyqt/files/PyQt5/PyQt-${PYQT5_VER}/PyQt5_gpl-${PYQT5_VER}.tar.gz \
 ...
 && python${PYTHON27_VER} configure.py --confirm-license --qmake=/usr/local/Qt-${QT5_VER}/bin/qmake --sip=/usr/local/python${PYTHON27_VER}/bin/sip \
 && make -j${BUILD_THREADS} \
 && make install \
 && rm -rf /workdir/* \
 && cd /workdir \
 ...
 && tar -xvf PyQt5_gpl-${PYQT5_VER}.tar.gz \
 && cd PyQt5_gpl-${PYQT5_VER} \
 && python${PYTHON34_VER} configure.py --confirm-license --qmake=/usr/local/Qt-${QT5_VER}/bin/qmake --sip=/usr/local/python${PYTHON34_VER}/bin/sip \
 && make -j${BUILD_THREADS} \
 && make install \
 && rm -rf /workdir/* \
 && cd /workdir \
 ...
 && tar -xvf PyQt5_gpl-${PYQT5_VER}.tar.gz \
 && cd PyQt5_gpl-${PYQT5_VER} \
 && python${PYTHON35_VER} configure.py --confirm-license --qmake=/usr/local/Qt-${QT5_VER}/bin/qmake --sip=/usr/local/python${PYTHON35_VER}/bin/sip \
 && make -j${BUILD_THREADS} \
 && make install \
 && rm -rf /workdir/*
...
RUN git clone --recursive --jobs ${BUILD_THREADS} --branch ${PYSIDE2_GIT_BRANCH} https://codereview.qt-project.org/pyside/pyside-setup \
 && cd pyside-setup \
 ...
 && python${PYTHON27_VER} setup.py install --ignore-git --qmake=/usr/local/Qt-${QT5_VER}/bin/qmake --cmake=/usr/local/bin/cmake --openssl=/usr/bin/openssl --jobs=${BUILD_THREADS} \
 && rm -rf /workdir/* \
 && cd /workdir \
 ...
 && git submodule update --init --recursive \
 && sed -i -e "s~\b\(packages\b.*\)],~\1, 'pyside2uic.Compiler', 'pyside2uic.port_v' + str(sys.version_info[0])],~" setup.py \
 && python${PYTHON34_VER} setup.py install --ignore-git --qmake=/usr/local/Qt-${QT5_VER}/bin/qmake --cmake=/usr/local/bin/cmake --openssl=/usr/bin/openssl --jobs=${BUILD_THREADS} \
 && rm -rf /workdir/* \
 && cd /workdir \
 && git clone --recursive --jobs ${BUILD_THREADS} --branch ${PYSIDE2_GIT_BRANCH} https://codereview.qt-project.org/pyside/pyside-setup \
 && cd pyside-setup \
 ...
 && git submodule update --init --recursive \
 && sed -i -e "s~\b\(packages\b.*\)],~\1, 'pyside2uic.Compiler', 'pyside2uic.port_v' + str(sys.version_info[0])],~" setup.py \
 && python${PYTHON35_VER} setup.py install --ignore-git --qmake=/usr/local/Qt-${QT5_VER}/bin/qmake --cmake=/usr/local/bin/cmake --openssl=/usr/bin/openssl --jobs=${BUILD_THREADS} \
 && rm -rf /workdir/*
