...
#  # Labels  
#  #  
LABEL name="cytopia's MariaDB 5.5 Image" \
      image="mariadb-5.5" \
      vendor="cytopia" \
      license="MIT" \
...
RUN { echo "# MariaDB 5.5 CentOS repository list - created 2016-10-28 06:40 UTC" ;echo "# http://downloads.mariadb.org/mariadb/repositories/" ;echo "[mariadb]" ;echo "name = MariaDB" ;echo "baseurl = http://yum.mariadb.org/5.5/centos7-amd64" ;echo "gpgkey=https://yum.mariadb.org/RPM-GPG-KEY-MariaDB" ;echo "gpgcheck=1" ; } > /etc/yum.repos.d/mariadb-5.5.repo \
 && yum clean all
RUN yum -y update \
