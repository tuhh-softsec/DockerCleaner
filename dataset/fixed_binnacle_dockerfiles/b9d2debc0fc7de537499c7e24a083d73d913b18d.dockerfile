...
COPY flat_deposit /var/www/html/flat/sites/all/modules/custom/flat_deposit
...
RUN cd /var/www/html/flat/ \
 ...
 && sleep 1
RUN apt-get update \
 && apt-get install --no-install-recommends php-zip=2:8.1+92ubuntu1 php-mbstring=2:8.1+92ubuntu1 -y
...
RUN sh -c "echo 'deb http://download.owncloud.org/download/repositories/stable/Ubuntu_16.04/ /' >> /etc/apt/sources.list.d/owncloud.list" \
 && apt-get update \
 && apt-get install --no-install-recommends owncloud-files -y --allow-unauthenticated
RUN /etc/init.d/postgresql start \
 && su -c "createdb -O fedora owncloud" postgres \
 && /etc/init.d/postgresql stop
RUN apt-get update \
 && apt-get install --no-install-recommends sudo=1.9.13p1-1ubuntu2
RUN /etc/init.d/postgresql start \
 && cd /var/www/owncloud \
...
RUN cd /app/flat \
 ...
 && apt-get install --no-install-recommends zip=3.0-13 unzip=6.0-27ubuntu1 -y
