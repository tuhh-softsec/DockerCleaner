...
RUN apk add openjdk8-jre=8.275.01-r0 tini=0.18.0-r0 su-exec=0.2-r0 --no-cache
ENV STACK="5.6.16"
RUN apk add libzmq=4.2.5-r1 bash=4.4.19-r1 nodejs=8.14.0-r0 nginx=1.14.2-r2 apache2-utils=2.4.43-r0 openssl=1.0.2u-r0 --no-cache
RUN mkdir -p /usr/local/lib \
 && ln -s /usr/lib/*/libzmq.so.3 /usr/local/lib/libzmq.so
RUN apk add wget=1.20.3-r0 ca-certificates=20191127-r2 --no-cache -t .build-deps \
 && set -x \
 && cd /tmp \
...
RUN apk add supervisor=3.3.4-r1 libc6-compat=1.1.19-r11 --no-cache
