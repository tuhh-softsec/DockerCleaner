...
RUN apt-get update -qq \
 && apt-get install --no-install-recommends gfortran=4:5.3.1-1ubuntu1 gcc=4:5.3.1-1ubuntu1 g++=4:5.3.1-1ubuntu1 libstdc++-5-dev=5.4.0-6ubuntu1~16.04.12 sudo=1.8.16-0ubuntu1.10 openssl=1.0.2g-1ubuntu4.20 -qq -y \
 && apt-get -qq clean \
...
RUN git fetch "$HOME/sage-context" HEAD \
 && if [ -e docker/.commit ] ; then git reset `cat docker/.commit ` || (echo "Could not find commit `cat docker/.commit ` in your local Git history. Please merge in the latest built develop branch to fix this: git fetch trac \
 && git merge `cat docker/.commit `." \
 && exit 1 ) ; else echo "You are building from $ARTIFACT_BASE which has no docker/.commit file. That's a bug unless you are building from source-clean or something similar." \
 && git reset FETCH_HEAD \
 && git checkout -f FETCH_HEAD ; fi \
...
RUN if git status --porcelain | read CHANGES ; then git -c user.name=docker-build -c user.email=docker-build@sage.invalid stash -u \
 && git stash show -p > "$HOME"/sage-context.patch; else touch "$HOME"/sage-context.patch ; fi
WORKDIR $SAGE_ROOT
...
RUN if [ x"$ARTIFACT_BASE" != x"source-clean" ] ; then mkdir -p $HOME/patch \
...
RUN if [ x"$ARTIFACT_BASE" != x"source-clean" ] ; then echo "Applying `du -hs patch/modified.tar.xz ` patch" \
