...
RUN set -eux \
 ...
 && apt-get ${APT_FLAGS_COMMON} update \
 && DEBIAN_FRONTEND=noninteractive apt-get ${APT_FLAGS_PERSISTENT} install fping libcurl4 libevent-2.1 libopenipmi0 libpcre3 libsnmp30 libsqlite3-0 libssh2-1 libssl1.1 libxml2 snmp-mibs-downloader ca-certificates unixodbc \
...
RUN set -eux \
 && apt-get ${APT_FLAGS_COMMON} update \
 && DEBIAN_FRONTEND=noninteractive apt-get ${APT_FLAGS_DEV} install autoconf automake gcc libc6-dev libcurl4-openssl-dev libevent-dev libldap2-dev libopenipmi-dev libpcre3-dev libsnmp-dev libsqlite3-dev libssh2-1-dev libxml2-dev make pkg-config git unixodbc-dev \
 && cd /tmp/ \
 ...
 && DEBIAN_FRONTEND=noninteractive apt-get ${APT_FLAGS_COMMON} purge autoconf automake gcc libc6-dev libcurl4-openssl-dev libevent-dev libldap2-dev libopenipmi-dev libpcre3-dev libsnmp-dev libsqlite3-dev libssh2-1-dev libxml2-dev make pkg-config git unixodbc-dev \
 && apt-get ${APT_FLAGS_COMMON} autoremove \
 && rm -rf /var/lib/apt/lists/* \
 && chmod +x /sbin/tini
EXPOSE 10051/tcp
