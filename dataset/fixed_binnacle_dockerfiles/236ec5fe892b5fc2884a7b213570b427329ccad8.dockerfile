...
RUN apk add ca-certificates curl apache2 apache2-utils xz --no-cache --virtual .persistent-deps
...
ENV PHP_URL="https://secure.php.net/get/php-5.5.37.tar.xz/from/this/mirror" \
    PHP_ASC_URL="https://secure.php.net/get/php-5.5.37.tar.xz.asc/from/this/mirror"
ENV PHP_SHA256="c322444fdf6d3ba26aa67d67ee32d1e815a877f35831351c83763431a80e3612" \
    PHP_MD5="707a528176283b8eecba1fa3c7123dd2"
COPY 90D90EC1.asc /tmp/
COPY 7267B52D.asc /tmp/
RUN set -xe ; apk add gnupg openssl --no-cache --virtual .fetch-deps ; mkdir -p /usr/src ; cd /usr/src ; wget -O php.tar.xz "$PHP_URL" ; if [ -n "$PHP_SHA256" ] ; then echo "$PHP_SHA256 *php.tar.xz" | sha256sum -c - ; fi ; if [ -n "$PHP_MD5" ] ; then echo "$PHP_MD5 *php.tar.xz" | md5sum -c - ; fi ; if [ -n "$PHP_ASC_URL" ] ; then wget -O php.tar.xz.asc "$PHP_ASC_URL" ;export GNUPGHOME="$( mktemp -d ;)" ;for key in $GPG_KEYS; do gpg --import "/tmp/$key.asc" ; done ;gpg --batch --verify php.tar.xz.asc php.tar.xz ;rm -rf "$GNUPGHOME" ; fi ; apk del .fetch-deps
