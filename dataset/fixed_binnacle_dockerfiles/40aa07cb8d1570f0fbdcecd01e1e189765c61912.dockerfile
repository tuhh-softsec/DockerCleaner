...
RUN set -ex ; if ! command -v gpg > /dev/null; then apt-get update ;apt-get install --no-install-recommends gnupg=2.2.12-1+deb10u2 dirmngr=2.2.12-1+deb10u2 -y ;rm -rf /var/lib/apt/lists/* ; fi
...
RUN set -x \
 && apt-get update \
 && apt-get install --no-install-recommends ca-certificates=20200601~deb10u2 wget=1.20.1-1.1 -y \
