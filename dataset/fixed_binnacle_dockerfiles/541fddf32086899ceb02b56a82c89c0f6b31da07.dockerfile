...
COPY web /web/
RUN apt-get update \
 && apt-get install --no-install-recommends docker.io=20.10.21-0ubuntu1~18.04.2 build-essential=12.4ubuntu1 gcc=4:7.4.0-1ubuntu2.3 python-pip=9.0.1-2.3~ubuntu1.18.04.8 python-dev=2.7.15~rc1-1 python3-pip=9.0.1-2.3~ubuntu1.18.04.8 -y \
...
#  devfiles
RUN apt-get update \
 && apt-get install --no-install-recommends wget=1.19.4-1ubuntu2.2 geany=1.32-2 firefox=111.0.1+build2-0ubuntu0.18.04.1 openssh-client=1:7.6p1-4ubuntu0.7 -y
