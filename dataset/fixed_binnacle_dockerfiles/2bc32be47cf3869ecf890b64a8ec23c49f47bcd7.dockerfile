...
RUN (apt-key adv --keyserver hkp://keyserver.ubuntu.com:80 --recv-keys 8AA7AF1F1091A5FD 9AD198E9 || apt-key adv --keyserver hkp://p80.pool.sks-keyservers.net:80 --recv-keys 8AA7AF1F1091A5FD 9AD198E9 ) \
 ...
 && apt-get update \
 && apt-get install --no-install-recommends curl=7.47.0-1ubuntu2.19 git=1:2.7.4-0ubuntu1.10 golang-1.11-go libssl-dev=1.0.2g-1ubuntu4.20 libzmq3-dev=4.1.4-7ubuntu0.1 openssl=1.0.2g-1ubuntu4.20 python3=3.5.1-3 python3-grpcio-tools=1.1.3-1 python3-sawtooth-cli software-properties-common=0.96.20.10 -y -q \
...
RUN (apt-key adv --keyserver hkp://keyserver.ubuntu.com:80 --recv-keys 8AA7AF1F1091A5FD 9AD198E9 || apt-key adv --keyserver hkp://p80.pool.sks-keyservers.net:80 --recv-keys 8AA7AF1F1091A5FD 9AD198E9 ) \
 ...
 && echo 'deb http://ppa.launchpad.net/gophers/archive/ubuntu xenial main' >> /etc/apt/sources.list \
 && apt-get update \
 && apt-get install --no-install-recommends curl=7.47.0-1ubuntu2.19 dpkg-dev=1.18.4ubuntu1.7 git=1:2.7.4-0ubuntu1.10 golang-1.11-go openssl=1.0.2g-1ubuntu4.20 python3-sawtooth-cli software-properties-common=0.96.20.10 -y -q \
 && add-apt-repository -k hkp://p80.pool.sks-keyservers.net:80 ppa:ethereum/ethereum \
 && apt-get update \
 && apt-get install --no-install-recommends solc -y -q \
 && curl -s -S -o /tmp/setup-node.sh https://deb.nodesource.com/setup_6.x \
