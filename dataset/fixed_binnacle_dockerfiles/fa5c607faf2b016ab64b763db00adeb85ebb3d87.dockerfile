...
RUN mkdir /usr/local/cuda-9.0/lib \
 ...
 && ln -s /usr/share/doc/libnccl2/NCCL-SLA.txt /usr/local/cuda/nccl
...
RUN : build TensorFlow pip package \
 ...
 && sed -i '/local_config_nccl/d' /tmp/tensorflow-${TF_VERSION}/tensorflow/tools/pip_package/BUILD | cat - \
 && sed -i '/NCCL-SLA/d' /tmp/tensorflow-${TF_VERSION}/third_party/nccl/nccl_configure.bzl \
 && ldconfig
RUN cd /tmp/tensorflow-${TF_VERSION} \
 && ln -s /usr/local/cuda/lib64/stubs/libcuda.so /usr/local/cuda/lib64/stubs/libcuda.so.1 \
 && LD_LIBRARY_PATH=/usr/local/cuda/lib64/stubs:${LD_LIBRARY_PATH} tensorflow/tools/ci_build/builds/configured GPU bazel build -c opt --copt=-mavx --copt=-mavx2 --config=cuda --config=mkl --config=monolithic --cxxopt="-D_GLIBCXX_USE_CXX11_ABI=0" tensorflow/tools/pip_package:build_pip_package \
