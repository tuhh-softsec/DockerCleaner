...
#       managers.
RUN apt-get update \
 && apt-get install --no-install-recommends ca-certificates curl --yes
...
RUN mkdir /opt/forward-reference-version /opt/bidirectional-reference-version \
 && curl -fsSL https://binaries.cockroachdb.com/cockroach-${FORWARD_REFERENCE_VERSION}.linux-amd64.tgz | tar xz -C /opt/forward-reference-version --strip-components=1 \
 && curl -fsSL https://binaries.cockroachdb.com/cockroach-${BIDIRECTIONAL_REFERENCE_VERSION}.linux-amd64.tgz | tar xz -C /opt/bidirectional-reference-version --strip-components=1
RUN apt-get install --no-install-recommends openjdk-8-jdk --yes \
...
RUN curl -fsSL https://dl.yarnpkg.com/debian/pubkey.gpg > /etc/apt/trusted.gpg.d/yarn.asc \
 ...
 && apt-get update \
 && apt-get install --no-install-recommends dotnet-sdk-2.0.0 expect libc6-dev libpq-dev libpqtypes-dev make maven nodejs gcc golang php-cli php-pgsql postgresql-client python python-psycopg2 ruby ruby-pg xmlstarlet yarn --yes
RUN curl -fsSL https://github.com/benesch/autouseradd/releases/download/1.0.0/autouseradd-1.0.0-amd64.tar.gz | tar xz -C /usr --strip-components 1
...
ENV DOTNET_CLI_TELEMETRY_OPTOUT="1" \
    NUGET_PACKAGES="/var/lib/nuget/packages"
RUN (cd /testdata/csharp \
 && dotnet restore --no-cache ) \
 && xmlstarlet ed --inplace --subnode /configuration --type elem --name packageRestore --subnode '$prev' --type elem --name add --insert '$prev' --type attr --name key --value enabled --insert '$prev' --type attr --name value --value false --delete /configuration/packageSources ~/.nuget/NuGet/NuGet.Config
#   Handle Java dependencies using the Maven package manager.
RUN xmlstarlet ed --inplace --subnode /_:settings --type elem --name localRepository --value /var/lib/maven/repository /usr/share/maven/conf/settings.xml \
 && (cd /testdata/java \
 && mvn -Dtest=NoopTest dependency:go-offline package ) \
 && xmlstarlet ed --inplace --subnode /_:settings --type elem --name offline --value true /usr/share/maven/conf/settings.xml
#   Handle Node dependencies using the Yarn package manager.
RUN (cd /testdata/node \
 && yarn install \
 && mv node_modules /usr/lib/node ) \
 && apt-get purge --yes yarn
RUN apt-get purge --yes curl xmlstarlet \
