...
RUN set -ex \
 ...
 && apk add bison coreutils dpkg-dev dpkg flex gcc libc-dev libedit-dev libxml2-dev libxslt-dev linux-headers make openssl-dev perl-utils perl-ipc-run util-linux-dev zlib-dev icu-dev --no-cache --virtual .build-deps \
