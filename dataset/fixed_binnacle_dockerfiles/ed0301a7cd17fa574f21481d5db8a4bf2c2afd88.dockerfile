...
RUN set -euxo pipefail \
 && for x in bash-tools lib tools; do git clone https://github.com/harisekhon/$x /github/$x \
 && cd /github/$x \
 && make build test clean \
 && yum autoremove -y \
 && yum clean all \
 && rm -rf /var/cache/yum || exit 1 ; done
RUN set -euxo pipefail \
 && x=spotify \
...
RUN set -euxo pipefail \
 && for x in pylib pytools; do git clone https://github.com/harisekhon/$x /github/$x \
 && cd /github/$x \
 && make build test clean || exit 1 ; done \
 && yum autoremove -y \
 && yum clean all \
 && rm -rf /var/cache/yum \
