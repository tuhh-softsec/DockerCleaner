...
RUN apt-get update \
 && apt-get install --no-install-recommends libapr1 graphviz -y \
...
RUN set -x \
 ...
 && nativeBuildDeps=" libapr1-dev libssl-dev openjdk-7-jdk " \
 && apt-get update \
 && apt-get install --no-install-recommends $nativeBuildDeps -y \
 && rm -rf /var/lib/apt/lists/* \
 && (export CATALINA_HOME="$PWD" \
 && cd "$nativeBuildDir/jni/native" \
 && ./configure --libdir=/usr/lib/jni --prefix="$CATALINA_HOME" --with-apr=/usr/bin/apr-1-config --with-java-home="$JAVA_HOME" --with-ssl=yes \
 && make -j$( nproc ;) \
 && make install ) \
...
RUN set -x \
 ...
 && startup.sh \
 && /bin/bash -c 'tail -f $CATALINA_HOME/logs/catalina.out | while read LOGLINE; do [[ "${LOGLINE}" == *"INFO: Server startup"* ]] \
 && pkill -P $$ tail; done' \
 && shutdown.sh \
