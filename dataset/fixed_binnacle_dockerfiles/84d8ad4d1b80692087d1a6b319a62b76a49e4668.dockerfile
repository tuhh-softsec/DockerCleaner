...
WORKDIR /tmp/workdir
RUN apt-get update -yqq \
 && apt-get install --no-install-recommends ca-certificates expat libgomp1 libxcb-shape0-dev -yq \
 && apt-get autoremove -y \
 && apt-get clean -y
...
RUN DIR=/tmp/ffmpeg \
 && mkdir -p ${DIR} \
 && cd ${DIR} \
 && ./configure --disable-debug --disable-doc --disable-ffplay --enable-cuda --enable-nvenc --enable-cuvid --enable-libnpp --enable-shared --enable-avresample --enable-libopencore-amrnb --enable-libopencore-amrwb --enable-gpl --enable-libass --enable-libfreetype --enable-libvidstab --enable-libmp3lame --enable-libopenjpeg --enable-libopus --enable-libtheora --enable-libvorbis --enable-libvpx --enable-libx265 --enable-libxvid --enable-libx264 --enable-nonfree --enable-openssl --enable-libfdk_aac --enable-libkvazaar --enable-libaom --extra-libs=-lpthread --enable-postproc --enable-small --enable-version3 --extra-cflags="-I${PREFIX}/include -I${PREFIX}/include/ffnvcodec -I/usr/local/cuda/include/" --extra-ldflags="-L${PREFIX}/lib -L/usr/local/cuda/lib64/ -L/usr/local/cuda/lib32/" --extra-libs=-ldl --prefix="${PREFIX}" \
 && make \
 && make install \
 && make distclean \
 && hash -r \
