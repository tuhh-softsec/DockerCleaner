...
RUN if [ ${INSTALL_AMQP} = true ] ; then apt-get update \
 && (apt-get update ;apt-get install --no-install-recommends librabbitmq-dev -y ) \
 && pecl install amqp \
...
RUN if [ ${INSTALL_MSSQL} = true ] ; then apt-get update -yqq \
 && (apt-get update ;apt-get install --no-install-recommends apt-transport-https gnupg -y ) \
...
RUN if [ ${INSTALL_IMAP} = true ] ; then apt-get update \
 && (apt-get update ;apt-get install --no-install-recommends libc-client-dev libkrb5-dev -y ) \
...
RUN if [ ${INSTALL_GIT} = true ] ; then apt-get update \
 && (apt-get update ;apt-get install --no-install-recommends git -y ) \
 && rm -r /var/lib/apt/lists/* ; fi
#  ####################################
#   COMPOSER:
#  ####################################
RUN if [ true = true ] ; then php -r "copy('https://getcomposer.org/installer', 'composer-setup.php');" \
 && php composer-setup.php --install-dir=/bin --filename=composer \
 && php -r "unlink('composer-setup.php');" ; fi
#  ####################################
#   Python:
#  ####################################
RUN if [ true = true ] ; then apt-get update \
 && (apt-get update ;apt-get install --no-install-recommends python -y ) \
 && rm -r /var/lib/apt/lists/* ; fi
#  ####################################
#   Pygments:
#  ####################################
RUN which wget &> /dev/null || (apt-get update ;apt-get install --no-install-recommends wget=1.20.3 ) ; wget --no-verbose --output-document / https://bootstrap.pypa.io/get-pip.py
