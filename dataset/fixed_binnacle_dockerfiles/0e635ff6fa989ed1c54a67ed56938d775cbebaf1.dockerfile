...
RUN set -eux \
 ...
 && yum ${YUM_FLAGS_DEV} install autoconf automake gcc libcurl-devel libevent-devel libssh2-devel libxml2-devel make mariadb-devel net-snmp-devel OpenIPMI-devel openldap-devel git unixODBC-devel \
 && cd /tmp/ \
...
RUN set -eux \
 ...
 && yum ${YUM_FLAGS_PERSISTENT} install iputils traceroute libcurl libevent libxml2 mariadb net-snmp-libs OpenIPMI-libs openldap openssl-libs pcre unixODBC \
