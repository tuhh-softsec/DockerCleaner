...
RUN apk add openjdk8-jre=8.181.13-r0 tini=0.9.0-r1 su-exec=0.2-r0 --no-cache
ENV STACK="5.4.3"
RUN apk add libzmq=4.1.5-r1 bash=4.3.46-r5 nodejs=6.9.5-r2 supervisor=3.2.4-r0 nginx=1.10.3-r1 apache2-utils=2.4.35-r0 openssl=1.0.2q-r0 --no-cache
RUN mkdir -p /usr/local/lib \
 && ln -s /usr/lib/*/libzmq.so.3 /usr/local/lib/libzmq.so
RUN apk add wget=1.18-r4 ca-certificates=20161130-r1 --no-cache -t .build-deps \
 && set -x \
 && cd /tmp \
 ...
 && wget -O logstash-$STACK.tar.gz https://artifacts.elastic.co/downloads/logstash/logstash-$STACK.tar.gz \
