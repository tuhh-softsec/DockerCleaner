...
RUN set -ex \
 && apk add ca-certificates openssl openssl-dev tar --no-cache --virtual .fetch-deps \
 ...
 && apk add bison coreutils dpkg-dev dpkg flex gcc libc-dev libedit-dev libxml2-dev libxslt-dev make openssl-dev util-linux-dev zlib-dev --no-cache --virtual .build-deps \
