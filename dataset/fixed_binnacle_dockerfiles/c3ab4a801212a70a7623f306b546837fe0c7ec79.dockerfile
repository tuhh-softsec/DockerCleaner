...
#   Forked from https://github.com/docker-library/postgres/blob/bef8f02d1fe2bb4547280ba609f19abd20230180/9.6/Dockerfile
FROM debian:jessie
RUN set -ex ; if ! command -v gpg > /dev/null; then apt-get update ;(apt-get update ;apt-get install --no-install-recommends gnupg2 dirmngr -y ) ;rm -rf /var/lib/apt/lists/* ; fi
...
RUN wget --quiet -O - http://apt.postgresql.org/pub/repos/apt/ACCC4CF8.asc | apt-key add - \
 && :
RUN (apt-get update ;apt-get install --no-install-recommends postgresql-9.6-postgis-2.3 postgresql-contrib-9.6 postgresql-9.6-postgis-scripts -y )
