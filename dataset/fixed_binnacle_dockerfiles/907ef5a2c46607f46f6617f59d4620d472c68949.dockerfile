...
#
RUN apt-get update \
 && apt-get install --no-install-recommends build-essential cmake git wget libatlas-base-dev libboost-all-dev libgflags-dev libgoogle-glog-dev libhdf5-serial-dev libleveldb-dev liblmdb-dev libopencv-dev libprotobuf-dev libsnappy-dev protobuf-compiler python-dev python-numpy python-pip python-scipy vim -y \
 && rm -rf /var/lib/apt/lists/*
#   Used by the facade processing shell scripts in order to package up our results
RUN apt-get install --no-install-recommends zip
...
RUN pip install jupyter==1.0.0 scipy==1.10.1 xmltodict==0.13.0 configargparse==1.5.3
...
RUN apt-get install --no-install-recommends coreutils
