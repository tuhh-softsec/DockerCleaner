...
RUN if [ ${INSTALL_XDEBUG} = true ] ; then (apt-get update ;apt-get install --no-install-recommends php5-xdebug -y ) \
...
RUN if [ ${INSTALL_INTL} = true ] ; then (apt-get update ;apt-get install --no-install-recommends zlib1g-dev libicu-dev g++ -y ) \
...
RUN if [ ${INSTALL_MSSQL} = true ] ; then (apt-get update ;apt-get install --no-install-recommends php5-sybase freetds-bin -y ) \
