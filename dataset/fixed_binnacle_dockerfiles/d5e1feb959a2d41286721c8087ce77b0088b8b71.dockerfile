...
RUN apk update \
 && apk upgrade \
 && apk add ca-certificates=20161130-r1 --no-cache \
 && update-ca-certificates \
 && apk add pcre=8.39-r0 icu-libs=57.1-r3 db=5.3.28-r0 libpq=9.6.10-r0 mariadb-client-libs=10.1.32-r0 sqlite-libs=3.15.2-r2 libsasl=2.1.26-r8 libldap=2.4.44-r4 --no-cache \
 && apk add curl=7.61.1-r1 coreutils=8.26-r0 autoconf=2.69-r0 g++=6.2.1-r1 libtool=2.4.6-r1 make=4.2.1-r0 libressl=2.4.4-r0 --no-cache --virtual .tool-deps \
 && apk add libressl-dev=2.4.4-r0 linux-headers=4.4.6-r1 pcre-dev=8.39-r0 icu-dev=57.1-r3 db-dev=5.3.28-r0 postgresql-dev=9.6.10-r0 mariadb-dev=10.1.32-r0 sqlite-dev=3.15.2-r2 cyrus-sasl-dev=2.1.26-r8 openldap-dev=2.4.44-r4 --no-cache --virtual .build-deps \
...
#   Install s6-overlay
RUN apk add curl=7.61.1-r1 --update --no-cache --virtual .tool-deps \
