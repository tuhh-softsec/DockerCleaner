...
RUN apt-get update \
 && apt-get install --no-install-recommends ed less locales vim-tiny wget ca-certificates procps -y \
...
RUN apt-get update \
 && apt-get install --no-install-recommends unstable sudo gdebi-core pandoc pandoc-citeproc libcurl4-openssl-dev libssl-dev libcairo2-dev/unstable libxt-dev libnss-wrapper gettext $SYS_LIBS -y -t
#   --------------------------------------------------------
#
...
#   --------------------------------------------------------
ENV R_LIBS="\"${RLIBS}\""
RUN if [ "$R_LIBS" ] ; then echo "Installing CRAN packages: '$R_LIBS'" \
 && install2.r --error $R_LIBS ; fi
#   --------------------------------------------------------
#   GitHub R packages
#   --------------------------------------------------------
ENV R_GH_LIBS="\"${RGHLIBS}\""
RUN if [ "$R_GH_LIBS" ] ; then echo "Installing GitHub packages: '$R_GH_LIBS'" \
 && install2.r --error remotes \
 && R -e "lapply(strsplit(Sys.getenv('R_GH_LIBS'), '\s+')[[1]], remotes::install_github)" ; fi
#   --------------------------------------------------------
#
