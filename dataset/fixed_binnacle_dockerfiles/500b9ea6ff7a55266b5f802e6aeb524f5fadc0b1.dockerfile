...
RUN set -eux ; apt-get update ; (apt-get update ;apt-get install --no-install-recommends ca-certificates wget -y ) ; wget -O rabbitmq-server.deb.asc "https://github.com/rabbitmq/rabbitmq-server/releases/download/$RABBITMQ_GITHUB_TAG/rabbitmq-server_${RABBITMQ_DEBIAN_VERSION}_all.deb.asc" ; wget -O rabbitmq-server.deb "https://github.com/rabbitmq/rabbitmq-server/releases/download/$RABBITMQ_GITHUB_TAG/rabbitmq-server_${RABBITMQ_DEBIAN_VERSION}_all.deb" ; apt-get purge -y --auto-remove ca-certificates wget ; export GNUPGHOME="$( mktemp -d ;)" ; gpg --keyserver keyserver.ubuntu.com --recv-keys "$RABBITMQ_GPG_KEY" ; gpg --batch --verify rabbitmq-server.deb.asc rabbitmq-server.deb ; rm -rf "$GNUPGHOME" ; (apt-get update ;apt-get install --no-install-recommends ./rabbitmq-server.deb -y ) ; dpkg -l | grep rabbitmq-server ; rm -f rabbitmq-server.deb* ; rm -rf /var/lib/apt/lists/*
