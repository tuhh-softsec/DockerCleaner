...
RUN update.sh \
 && install.sh glmark2 g3dviewer nux-tools \
 && clean.sh
#   build-essential
...
RUN tar -xjvf /usr/src/GpuTest_Linux_x64_0.7.0.tar.bz2 -C /usr/src/ \
 && ln -s /usr/src/GpuTest_Linux_x64_0.7.0/GpuTest-all.sh /usr/local/bin/
