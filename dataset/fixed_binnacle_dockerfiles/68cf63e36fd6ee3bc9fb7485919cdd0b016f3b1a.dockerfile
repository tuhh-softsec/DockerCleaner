...
RUN apt-get update \
 && apt-get install --no-install-recommends gcc=4:5.3.1-1ubuntu1 libc6-dev=2.23-0ubuntu11.3 -y
...
#   Install packages
RUN apt-get update \
 && apt-get install --no-install-recommends apt-transport-https=1.2.35 ca-certificates=20210119~16.04.1 cron=3.0pl1-128ubuntu2 curl=7.47.0-1ubuntu2.19 gnupg2=2.1.11-6ubuntu2.1 initscripts=2.88dsf-59.3ubuntu2 iproute2=4.3.0-1ubuntu3.16.04.5 python-minimal=2.7.12-1~16.04 ssh=1:7.2p2-4ubuntu2.10 sudo=1.8.16-0ubuntu1.10 systemd=229-4ubuntu21.31 -y \
