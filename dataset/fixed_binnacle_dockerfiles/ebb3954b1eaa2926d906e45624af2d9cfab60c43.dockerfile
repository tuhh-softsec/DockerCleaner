...
RUN apt-get update \
 && apt-get install --no-install-recommends wget=1.17.1-1ubuntu1.5 git=1:2.7.4-0ubuntu1.10 lsb-release=9.20160110ubuntu0.2 apt-utils=1.2.35 systemd=229-4ubuntu21.31 -y \
...
RUN if (echo "$before" | grep -Eq ^1..*$ ) ; then echo Installing PC1 repo ;wget -O puppet-pc1.deb http://apt.puppetlabs.com/puppetlabs-release-pc1-xenial.deb \
 ...
 && dpkg -i puppet6.deb ; else echo no ; fi
#   Install FROM version of puppet-agent.
RUN apt-get update \
 && apt-get install --no-install-recommends -f -y \
 && apt-get install --no-install-recommends puppet-agent=${before}-1xenial
