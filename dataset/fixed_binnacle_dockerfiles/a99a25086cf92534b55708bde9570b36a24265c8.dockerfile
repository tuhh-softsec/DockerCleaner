...
RUN which wget &> /dev/null || apt-get install --no-install-recommends wget=1.20.3 ; wget --no-verbose --output-document /output/s6.tgz https://github.com/just-containers/s6-overlay/releases/download/${S6_VERSION}/s6-overlay-amd64.tar.gz
RUN which wget &> /dev/null || apt-get install --no-install-recommends wget=1.20.3 ; wget --no-verbose --output-document /tmp/s6.tgz.sig https://github.com/just-containers/s6-overlay/releases/download/${S6_VERSION}/s6-overlay-amd64.tar.gz.sig
RUN apt-get update \
 && apt-get install --no-install-recommends gpg=2.2.12-1+deb10u2 gpg-agent=2.2.12-1+deb10u2 curl=7.64.0-4+deb10u5 ca-certificates=20200601~deb10u2 -y \
...
RUN apt-get update \
 && apt-get install --no-install-recommends util-linux=2.33.1-0.1 ncurses-bin=6.1+20181013-2+deb10u3 ncurses-base=6.1+20181013-2+deb10u3 libncursesw5:amd64 -y \
 && apt-get install --no-install-recommends libudev1=241-7~deb10u8 libsystemd0=241-7~deb10u8 -y \
