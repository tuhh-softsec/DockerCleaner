...
ENV TZ="Asia/Shanghai" \
    TERM="xterm"
#   添加阿里云yum源
COPY install/soft/epel-6.repo /etc/yum.repos.d/epel.repo
#   安装软件
...
#   安装apache，mysql套件
COPY install/soft/ewomail-lamp-1.0-el6.x86_64.rpm /home/
RUN rpm -ivh /home/ewomail-lamp-1.0-el6.x86_64.rpm
#   安装dovecot
COPY install/soft/dovecot-2.2.24-el6.x86_64.rpm /home/
RUN rpm -ivh /home/dovecot-2.2.24-el6.x86_64.rpm
COPY install/soft/postfix-policyd-spf-perl /usr/libexec/postfix/
...
RUN rm -rf /home/*.rpm \
 ...
 && rm -rf /var/cache/yum
#   添加配置文件
COPY install/config/monit /etc/monit
COPY install/config/dovecot /etc/dovecot
COPY install/config/postfix /etc/postfix
COPY install/soft/httpd.init /etc/rc.d/init.d/httpd
COPY install/soft/httpd.conf /ewomail/apache/conf/httpd.conf
COPY install/soft/php.ini /ewomail/php54/etc/
COPY install/soft/php-cli.ini /ewomail/php54/etc/
COPY install/soft/dovecot.init /etc/rc.d/init.d/dovecot
COPY install/config/mail /ewomail/cmail
#   清理换行
...
RUN chmod 755 /usr/local/dovecot/share/doc/dovecot/mkcert.sh \
 ...
 && echo "} );" >> /etc/amavisd/amavisd.conf
#   拷贝admin和rainloop
COPY ewomail-admin/ /ewomail/www/ewomail-admin
COPY rainloop/ /ewomail/www/rainloop
COPY install/config/rainloop/_data_/ /ewomail/www/rainloop_data_
...
COPY docker/init_sql.php /home/
COPY docker/update_file.php /home/
...
ENV URL="*:8080" \
    ...
    MONIT_MAIL_ALERT="" \
    MONIT_USER="admin"
EXPOSE 25/tcp 109/tcp 110/tcp 143/tcp 465/tcp 587/tcp 993/tcp 995/tcp 80/tcp 8080/tcp 2812/tcp
