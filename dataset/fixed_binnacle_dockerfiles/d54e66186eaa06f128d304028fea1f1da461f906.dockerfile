...
RUN apt-get update -yqq \
 && apt-get install --no-install-recommends ca-certificates=20210119~16.04.1 expat=2.1.0-7ubuntu0.16.04.5 libgomp1=5.4.0-6ubuntu1~16.04.12 -yq \
...
RUN DIR=/tmp/ffmpeg \
 && mkdir -p ${DIR} \
 && cd ${DIR} \
 && ./configure --disable-debug --disable-doc --disable-ffplay --enable-shared --enable-avresample --enable-libopencore-amrnb --enable-libopencore-amrwb --enable-gpl --enable-libass --enable-libfreetype --enable-libvidstab --enable-libmp3lame --enable-libopus --enable-libtheora --enable-libvorbis --enable-libvpx --enable-libx265 --enable-libxvid --enable-libx264 --enable-nonfree --enable-openssl --enable-libfdk_aac --enable-postproc --enable-small --enable-version3 --extra-cflags="-I${PREFIX}/include" --extra-ldflags="-L${PREFIX}/lib" --extra-libs=-ldl --prefix="${PREFIX}" \
 && make \
 && make install \
 && make distclean \
 && hash -r \
...
RUN apt-get update -y \
 && apt-get install --no-install-recommends libva-drm1=1.7.0-1ubuntu0.1 libva1=1.7.0-1ubuntu0.1 i965-va-driver=1.7.0-1 -y \
