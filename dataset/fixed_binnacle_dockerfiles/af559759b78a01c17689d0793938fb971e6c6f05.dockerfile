...
RUN apt-get update \
 && apt-get install --no-install-recommends aufs-tools=1:3.2+20130722-1.1 automake=1:1.14.1-2ubuntu1 btrfs-tools=3.12-1ubuntu0.2 build-essential=11.6ubuntu6 curl=7.35.0-1ubuntu2.20 dpkg-sig=0.13.1+nmu1 git=1:1.9.1-1ubuntu0.10 iptables=1.4.21-1ubuntu1 libapparmor-dev=2.10.95-0ubuntu2.6~14.04.4 libcap-dev=1:2.24-0ubuntu2 libsqlite3-dev=3.8.2-1ubuntu2.2 mercurial=2.8.2-1ubuntu1.4 parallel=20161222-1~ubuntu0.14.04.2 reprepro=4.13.1-1build1 ruby1.9.1=1.9.3.484-2ubuntu1.14 ruby1.9.1-dev=1.9.3.484-2ubuntu1.14 lxc=1.0* s3cmd=1.1.0* -y
#   Get lvm2 source for compiling statically
RUN git clone --no-checkout https://git.fedorahosted.org/git/lvm2.git /usr/local/lvm2 \
 && cd /usr/local/lvm2 \
...
ENV GOARM="5"
RUN cd /usr/local/go/src \
 && bash -xc 'for platform in $DOCKER_CROSSPLATFORMS; do GOOS=${platform%/*} GOARCH=${platform##*/} ./make.bash --no-clean 2>&1; done'
