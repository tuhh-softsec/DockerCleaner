...
RUN set -eux \
 ...
 && locale-gen $LC_ALL \
 && apt-get ${APT_FLAGS_COMMON} update \
 && DEBIAN_FRONTEND=noninteractive apt-get ${APT_FLAGS_DEV} install autoconf automake gcc gettext libc6-dev libcurl4-openssl-dev libevent-dev libiksemel-dev libldap2-dev libmysqlclient-dev libopenipmi-dev libpcre3-dev libsnmp-dev libssh2-1-dev libxml2-dev make openjdk-8-jdk pkg-config git unixodbc-dev \
 && cd /tmp/ \
...
RUN set -eux \
 ...
 && exit 1 ; DISTRIB_CODENAME=$( /bin/bash -c 'source /etc/lsb-release \
 && echo $DISTRIB_CODENAME' ;) \
 && echo "deb https://nginx.org/packages/ubuntu/ $DISTRIB_CODENAME nginx" >> /etc/apt/sources.list.d/nginx.list \
 && apt-get ${APT_FLAGS_COMMON} update \
 && DEBIAN_FRONTEND=noninteractive apt-get ${APT_FLAGS_PERSISTENT} install iputils-ping traceroute curl fping libcurl4 libevent-2.1 libiksemel3 libmysqlclient20 libopenipmi0 libpcre3 libsnmp30 libssh2-1 libssl1.1 libxml2 mysql-client mysql-server nginx openjdk-8-jre-headless php7.2-bcmath php7.2-fpm php7.2-gd php7.2-json php7.2-ldap php7.2-mbstring php7.2-mysql php7.2-xml snmp-mibs-downloader supervisor unixodbc \
 && mkdir -p /var/lib/locales/supported.d/ \
 && rm -f /var/lib/locales/supported.d/local \
 && cat /usr/share/zabbix/include/locales.inc.php | grep display | grep true | awk '{$1=$1};1' | cut -d"'" -f 2 | sort | xargs -I '{}' bash -c 'echo "{}.UTF-8 UTF-8" >> /var/lib/locales/supported.d/local' \
