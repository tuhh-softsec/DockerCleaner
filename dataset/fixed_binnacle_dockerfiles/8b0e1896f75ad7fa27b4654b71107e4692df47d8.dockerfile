...
RUN set -x \
 ...
 && apk add git=2.11.3-r2 cmake=3.6.3-r0 make=4.2.1-r0 coreutils=8.26-r0 gcc=6.2.1-r1 g++=6.2.1-r1 postgresql-dev=9.6.10-r0 lua-dev=5.1.5-r4 musl-dev=1.1.15-r8 cyrus-sasl-dev=2.1.26-r8 mosquitto-dev=1.4.15-r0 libev-dev=4.22-r0 --no-cache --virtual .build-deps \
 && mkdir -p /rocks \
 && : "---------- proj (for gis module) ----------" \
