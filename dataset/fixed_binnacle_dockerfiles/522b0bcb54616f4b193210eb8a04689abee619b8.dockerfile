...
RUN conda install --yes --quiet 'pip' 'jupyter' 'r-devtools' 'r-stringr' 'r-argparse' \
 ...
 && fix-permissions $ANACONDA_HOME \
 && fix-permissions /home/$NB_USER
RUN Rscript -e 'install.packages(c("IRkernel","versions"), repos="http://cran.cnr.berkeley.edu", lib="/opt/conda/lib/R/library")' -e 'IRkernel::installspec(prefix = "/usr/local")' -e 'versions::install.versions("SparkR", "2.4.1", lib="/opt/conda/lib/R/library")' \
 && fix-permissions $ANACONDA_HOME \
 && fix-permissions /home/$NB_USER
