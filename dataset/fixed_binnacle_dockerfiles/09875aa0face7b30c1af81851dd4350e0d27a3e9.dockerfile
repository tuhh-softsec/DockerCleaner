#  FROM ubuntu:16.04
FROM paperspace/fastai:cuda9_pytorchJL
...
RUN apt-get update -y \
 && (apt-get update ;apt-get install --no-install-recommends python3-pycurl python3-pip python3-dev locales ssh-askpass -y )
...
RUN pip install opencv-python==4.7.0.72 pandas==2.0.0 jupyter==1.0.0
