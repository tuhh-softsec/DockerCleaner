...
RUN apt-get update \
 && apt-get install --no-install-recommends qemu-user-static=1:2.11+dfsg-1ubuntu7.41 -y
...
RUN which wget &> /dev/null || apt-get install --no-install-recommends wget=1.20.3 ; wget --no-verbose --output-document /bin/tini https://github.com/krallin/tini/releases/download/${TINI_VERSION}/tini-armhf
