...
RUN set -x \
 && apt-get update \
 && apt-get install --no-install-recommends gnupg1=1.4.23-1.1 apt-transport-https=2.2.4 ca-certificates=20210119 --no-install-suggests -y \
 ...
 && apt-get install --no-install-recommends gettext-base=0.21-4 $nginxPackages --no-install-suggests -y \
...
#   Install Supervisord
RUN apt-get update \
 && apt-get install --no-install-recommends supervisor=4.2.2-2 sqlite3=3.34.1-3 libsqlite3-dev=3.34.1-3 uwsgi-plugin-python3=2.0.19.1-7.1 -y \
 && rm -rf /var/lib/apt/lists/*
