...
RUN pip install matplotlib==3.7.1 scipy==1.10.1 biom-format==2.1.14 h5py==3.8.0
...
RUN apt-get update -q \
 && apt-get install --no-install-recommends muscle ncbi-blast+ raxml libtbb2 libz-dev libncurses5-dev libncursesw5-dev -q -y \
 && apt-get -q clean autoclean \
 && apt-get -q autoremove -y \
 && rm -rf /var/lib/apt/lists/*
RUN pip install biopython==1.81 pysam==0.21.0 msgpack-python==0.5.6 dendropy==4.5.2
...
RUN apt-get update -q \
 && apt-get install --no-install-recommends mercurial -q -y \
 && apt-get -q clean autoclean \
 && apt-get -q autoremove -y \
 && rm -rf /var/lib/apt/lists/*
...
RUN apt-get update -q \
 && apt-get install --no-install-recommends libtbb2 python-tk bc -q -y
