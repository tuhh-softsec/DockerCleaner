...
RUN apt-get update \
 && apt-get install --no-install-recommends libapr1=1.7.0-6+deb11u2 -y \
...
#   Install ANT
RUN apt-get update \
 && apt-get install --no-install-recommends ant=1.10.9-4 -y
