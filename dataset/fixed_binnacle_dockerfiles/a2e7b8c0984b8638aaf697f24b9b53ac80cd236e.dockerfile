...
RUN apt-get update \
 && (apt-get update ;apt-get install --no-install-recommends build-essential cmake git wget vim locales curl apt-transport-https screen -y )
#   netcore 2.0
...
RUN pip install opencv-python==4.7.0.72 pandas==2.0.0 jupyter==1.0.0 matplotlib==3.7.1
RUN apt-get update -y \
