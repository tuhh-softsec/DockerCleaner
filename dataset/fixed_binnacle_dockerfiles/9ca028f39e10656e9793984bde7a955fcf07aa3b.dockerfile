...
RUN curl -sL https://deb.nodesource.com/setup_10.x | bash - \
 && apt-get update \
 && apt-get install --no-install-recommends cmake libxpm-dev nodejs nginx libzmq3-dev krb5-user autossh rsync -y \
...
RUN npm install bower@1.8.14 polymer-bundler@4.0.10 n@9.1.0 -g \
 && n lts \
...
RUN mkdir -p ${SCRATCH} \
 && chown -R overwatch:overwatch /opt
...
RUN mkdir -p /opt/scratch/alienCAs \
 && cd /opt/scratch/alienCAs \
