...
#   docker build -f examples/intkey_go/Dockerfile-installed-xenial -t sawtooth-intkey-tp-go .
#   -------------=== intkey-tp-go build ===-------------
FROM ubuntu:xenial AS intkey-tp-go-builder
...
RUN echo "deb [arch=amd64] http://repo.sawtooth.me/ubuntu/ci xenial universe" >> /etc/apt/sources.list \
 ...
 && apt-get update \
 && apt-get install --no-install-recommends build-essential=12.1ubuntu2 golang-1.11-go git=1:2.7.4-0ubuntu1.10 libssl-dev=1.0.2g-1ubuntu4.20 libzmq3-dev=4.1.4-7ubuntu0.1 openssl=1.0.2g-1ubuntu4.20 python3-grpcio-tools=1.1.3-1 -y -q --allow-downgrades \
 && apt-get clean \
...
RUN mkdir /go
RUN go get -u github.com/golang/protobuf/proto github.com/golang/protobuf/protoc-gen-go github.com/pebbe/zmq4 github.com/brianolson/cbor_go github.com/satori/go.uuid github.com/btcsuite/btcd/btcec github.com/jessevdk/go-flags github.com/pelletier/go-toml github.com/golang/mock/gomock github.com/golang/mock/mockgen golang.org/x/crypto/ripemd160 golang.org/x/crypto/ssh gopkg.in/yaml.v2
COPY . /go/src/github.com/hyperledger/sawtooth-sdk-go
WORKDIR /go/src/github.com/hyperledger/sawtooth-sdk-go
RUN go generate \
 && cd examples/intkey_go \
 && if [ -d "bin" ] ; then rm -rf bin ; fi \
 && mkdir bin \
 && cd src/sawtooth_intkey \
 && go build -o /go/src/github.com/hyperledger/sawtooth-sdk-go/examples/intkey_go/bin/intkey-tp-go \
 && cd ../sawtooth_intkey_client \
 && ST_VERSION=$( /go/src/github.com/hyperledger/sawtooth-sdk-go/bin/get_version ;) \
 && go build -ldflags "-X main.DISTRIBUTION_VERSION=${ST_VERSION}" -o /go/src/github.com/hyperledger/sawtooth-sdk-go/examples/intkey_go/bin/intkey-go \
 && cd ../sawtooth_intkey
RUN pkg=intkey_go \
...
#   -------------=== sawtooth-intkey-tp-go build ===-------------
FROM ubuntu:xenial
COPY --from=intkey-tp-go-builder /go/src/github.com/hyperledger/sawtooth-sdk-go/examples/intkey_go/sawtooth-intkey-tp-go_*.deb /tmp
RUN apt-get update \
