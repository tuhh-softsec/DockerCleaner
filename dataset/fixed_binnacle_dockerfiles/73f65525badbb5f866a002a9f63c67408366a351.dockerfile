...
RUN :
RUN (apt-get update ;apt-get install --no-install-recommends gcc-snapshot -y )
RUN (apt-get update ;apt-get install --no-install-recommends gcc-6 g++-6 gfortran -y )
...
RUN apt-get update \
 && (apt-get update ;apt-get install --no-install-recommends libgtk2.0-dev -y )
...
RUN (apt-get update ;apt-get install --no-install-recommends python3-scipy --no-install-suggests --yes )
