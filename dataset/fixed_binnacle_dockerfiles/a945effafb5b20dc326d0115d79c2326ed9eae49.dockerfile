...
RUN set -ex \
 && apk update \
 && apk upgrade \
 && apk add bash=4.4.19-r1 tzdata=2019c-r0 vim=8.0.1359-r2 tini=0.16.1-r0 su-exec=0.2-r0 unzip=6.0-r3 gzip=1.8-r0 tar=1.32-r0 wget=1.20.3-r0 which=2.21-r1 curl=7.61.1-r3 \
...
RUN ALPINE_GLIBC_BASE_URL="https://github.com/sgerrand/alpine-pkg-glibc/releases/download" \
 ...
 && apk add wget=1.20.3-r0 ca-certificates=20190108-r0 --no-cache --virtual=.build-dependencies \
...
RUN apk add wget=1.20.3-r0 ca-certificates=20190108-r0 unzip=6.0-r3 --no-cache --virtual=build-dependencies \
 && cd "/tmp" \
...
COPY start-kafka-eagle.sh /opt/kafka-eagle-${KE_VERSION}/start-kafka-eagle.sh
RUN mkdir -p /tmp \
 && cd /tmp \
