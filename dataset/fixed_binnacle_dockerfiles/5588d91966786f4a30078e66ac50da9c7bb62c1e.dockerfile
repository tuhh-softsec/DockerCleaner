...
RUN apt-get update \
 && apt-get install --no-install-recommends libpng-dev libjpeg-dev libpq-dev libldap2-dev libldb-dev libicu-dev libgmp-dev libmagickwand-dev libc-client-dev libtidy-dev libkrb5-dev libxslt-dev libzip-dev unixodbc-dev openssh-server openssl libssl-dev vim curl wget tcptraceroute -y \
