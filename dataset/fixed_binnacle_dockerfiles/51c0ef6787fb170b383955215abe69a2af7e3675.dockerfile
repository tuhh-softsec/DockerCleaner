...
RUN set -ex \
 && apk update \
 && apk upgrade \
 && apk add bash=4.4.19-r1 bzip2=1.0.6-r7 snappy=1.1.4-r4 gnupg=2.2.19-r0 tzdata=2020a-r0 vim=8.1.1365-r0 tini=0.18.0-r0 su-exec=0.2-r0 gzip=1.9-r0 tar=1.32-r0 wget=1.20.3-r0 which=2.21-r1 curl=7.61.1-r3 \
...
COPY start-k8s-hbase.sh /opt/hbase/bin/start-k8s-hbase.sh
