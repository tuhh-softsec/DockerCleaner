...
RUN bazel version ; python -c "import sys; print(sys.prefix); print(sys.version_info)" \
 && pip install wheel==0.40.0 numpy==1.24.2 --no-cache-dir
...
RUN : build TensorFlow pip package \
 ...
 && PYTHON_BIN_PATH=/usr/local/bin/python PYTHON_LIB_PATH=/usr/local/lib/python3.6/site-packages GCC_HOST_COMPILER_PATH=/usr/bin/gcc CC_OPT_FLAGS="-march=x86-64 -mavx -mavx2 -mfma -mfpmath=both -msse4.1 -msse4.2" TF_NEED_JEMALLOC=1 TF_NEED_GCP=1 TF_NEED_HDFS=1 TF_ENABLE_XLA=1 TF_NEED_VERBS=0 TF_NEED_OPENCL=0 TF_NEED_OPENCL_SYCL=0 TF_NEED_CUDA=1 TF_CUDA_CLANG=0 TF_DOWNLOAD_CLANG=0 TF_NEED_TENSORRT=0 TF_NEED_MPI=0 TF_NEED_GDR=0 TF_NEED_S3=1 TF_NEED_KAFKA=0 TF_SET_ANDROID_WORKSPACE=0 TF_CUDA_VERSION=9.0 TF_CUDNN_VERSION=7 TF_NCCL_VERSION=2 TF_CUDA_COMPUTE_CAPABILITIES=3.5,3.7,5.2,6.0,6.1 CUDA_TOOLKIT_PATH=/usr/local/cuda CUDNN_INSTALL_PATH=/usr/local/cuda NCCL_INSTALL_PATH=/usr bash configure \
 && cd /tmp/tensorflow-r${TF_VERSION} ; sed -r -i'' "s@http://www.sqlite.org@https://www.sqlite.org@" tensorflow/workspace.bzl ; PYTHON_BIN_PATH=/usr/local/bin/python PYTHON_LIB_PATH=/usr/local/lib/python3.6/site-packages bazel fetch //tensorflow/tools/pip_package:build_pip_package
