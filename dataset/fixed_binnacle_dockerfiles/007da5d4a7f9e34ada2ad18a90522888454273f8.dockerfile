...
RUN apk add openjdk8-jre=8.212.04-r0 tini=0.14.0-r0 su-exec=0.2-r0 --no-cache
ENV STACK="5.5.3"
RUN apk add libzmq=4.2.5-r0 bash=4.3.48-r1 nodejs=6.10.3-r2 supervisor=3.2.4-r0 nginx=1.12.2-r2 apache2-utils=2.4.39-r0 openssl=1.0.2r-r0 --no-cache
RUN mkdir -p /usr/local/lib \
 && ln -s /usr/lib/*/libzmq.so.3 /usr/local/lib/libzmq.so
RUN apk add wget=1.20.3-r0 ca-certificates=20161130-r3 --no-cache -t .build-deps \
 && set -x \
 && cd /tmp \
