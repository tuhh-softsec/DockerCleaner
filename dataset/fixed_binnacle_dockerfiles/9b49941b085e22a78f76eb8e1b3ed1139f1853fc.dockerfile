...
WORKDIR $RISCV
RUN mkdir linux-4.1.y \
 && cd linux-4.1.y \
 && git init \
 && git remote add origin https://github.com/riscv/riscv-linux.git \
 && git fetch \
 && git checkout -b linux origin/linux-4.1.y-riscv
...
WORKDIR $RISCV/linux-4.1.y
RUN curl -L https://raw.githubusercontent.com/sbates130272/docker-riscv/master/.config-linux-4.1.y > .config \
