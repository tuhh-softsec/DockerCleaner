...
RUN DEBIAN_FRONTEND=noninteractive apt-get update \
 && DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends build-essential ca-certificates curl gettext-base libgd-dev libgeoip-dev libncurses5-dev libperl-dev libreadline-dev libxslt1-dev make perl unzip zlib1g-dev git cmake lua5.1-dev wget \
 && cd /tmp \
 && git clone -b ${OPENTRACING_CPP_VERSION} https://github.com/opentracing/opentracing-cpp.git \
 && cd opentracing-cpp \
 && mkdir .build \
 && cd .build \
 && cmake -DCMAKE_BUILD_TYPE=Release -DBUILD_MOCKTRACER=OFF -DBUILD_STATIC_LIBS=OFF -DBUILD_TESTING=OFF .. \
 && make \
 && make install \
 && cd /tmp \
...
RUN which wget &> /dev/null || apt-get install --no-install-recommends wget=1.20.3 ; wget --no-verbose --output-document /usr/local/nginx/conf/nginx.conf https://raw.githubusercontent.com/openresty/docker-openresty/master/nginx.conf
RUN which wget &> /dev/null || apt-get install --no-install-recommends wget=1.20.3 ; wget --no-verbose --output-document /etc/nginx/conf.d/default.conf https://raw.githubusercontent.com/openresty/docker-openresty/master/nginx.vh.default.conf
