...
RUN apk add fcgi msmtp --update-cache \
 && apk add php5-dev alpine-sdk --virtual .build-dependencies \
 && cd /usr/bin \
...
RUN cd /usr/src/php5-imagick \
 ...
 && abuild -F -r -P /usr \
 && rm -f /usr/src/x86_64/APKINDEX.tar.gz \
 && apk add /usr/src/x86_64/php5-*.apk -U --allow-untrusted \
 && apk add php5-cli --update-cache \
 && apk add php5-bcmath php5-bz2 php5-ctype php5-curl php5-dom php5-fpm php5-gd php5-iconv php5-json php5-mcrypt php5-mysql php5-opcache php5-openssl php5-pdo php5-pdo_mysql php5-pdo_pgsql php5-phar php5-posix php5-sockets php5-xml php5-xmlreader php5-zip php5-zlib --update-cache \
 ...
 && sed -i -e "s/^expose_php.*/expose_php = Off/" -e "s/^;date.timezone.*/date.timezone = UTC/" -e "s/^memory_limit.*/memory_limit = -1/" -e "s/^max_execution_time.*/max_execution_time = 300/" -e "s/^post_max_size.*/post_max_size = 128M/" -e "s/^upload_max_filesize.*/upload_max_filesize = 128M/" -e "s/^;sendmail_path =/sendmail_path = \/usr\/sbin\/sendmail -t/" /etc/php5/php.ini \
...
HEALTHCHECK --interval=30s --timeout=5s CMD REDIRECT_STATUS=true SCRIPT_NAME=/ping SCRIPT_FILENAME=/ping REQUEST_METHOD=GET cgi-fcgi -bind -connect 127.0.0.1:9000
#   Volumes
