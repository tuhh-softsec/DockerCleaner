...
RUN :
RUN : \
 && (apt-get update ;apt-get install --no-install-recommends gnupg=2.2.4-1ubuntu1.6 -y )
RUN apt-key adv --keyserver keyserver.ubuntu.com --recv-key 799058698E65316A2E7A4FF42EAE1437F7D2C623
COPY zend-server.list /etc/apt/sources.list.d/zend-server.list
RUN :
RUN : \
 && (apt-get update ;apt-get install --no-install-recommends iproute2=4.15.0-2ubuntu1.3 curl=7.58.0-2ubuntu3.24 libmysqlclient20=5.7.41-0ubuntu0.18.04.1 unzip=6.0-21ubuntu1.2 git=1:2.17.1-1ubuntu0.17 ca-certificates=20211016ubuntu0.18.04.1 patch=2.7.6-2ubuntu1.1 zend-server-nginx=2021.0.0+b74 -y ) \
