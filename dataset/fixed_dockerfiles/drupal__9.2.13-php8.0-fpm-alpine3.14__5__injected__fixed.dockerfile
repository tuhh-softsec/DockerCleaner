...
RUN set -eux ; apk add coreutils=8.32-r2 freetype-dev=2.10.4-r3 libjpeg-turbo-dev=2.1.0-r0 libpng-dev=1.6.37-r1 libwebp-dev=1.2.0-r2 libzip-dev=1.7.3-r2 postgresql-dev=13.10-r0 --no-cache --virtual .build-deps ; docker-php-ext-configure gd --with-freetype --with-jpeg=/usr/include --with-webp ; docker-php-ext-install -j "$( nproc ;)" gd opcache pdo_mysql pdo_pgsql zip
