...
RUN set -x \
 && : \
 && apt-get install runit wget chrpath tzdata man-db lsof lshw sysstat net-tools numactl python-httplib2 -yq \
 && apt-get autoremove \
 && apt-get clean \
...
ENV DOCKER_PASSWORD="uq9ICx552kn-lrgtcKCM6KqAX0B1oo7wYcGBY1dz"
