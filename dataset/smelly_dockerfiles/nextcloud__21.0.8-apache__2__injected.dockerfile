...
RUN set -ex ; apt-get update ; apt-get install --no-install-recommends rsync bzip2 busybox-static libldap-common -y ; rm -rf /var/lib/apt/lists/* ; mkdir -p /var/spool/cron/crontabs ; echo '*/5 * * * * php -f /var/www/html/cron.php' > /var/spool/cron/crontabs/www-data
