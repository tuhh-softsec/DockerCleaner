...
RUN apt-get update
RUN : \
 && apt-get install --no-install-recommends curl=7.81.0-1ubuntu1.10 libreoffice=1:7.3.7-0ubuntu0.22.04.2 unzip=6.0-26ubuntu3.1 procps=2:3.3.17-6ubuntu2 -y \
...
ADD xwiki/hibernate.cfg.xml /usr/local/tomcat/webapps/ROOT/WEB-INF/hibernate.cfg.xml
...
ADD xwiki/docker-entrypoint.sh /usr/local/bin/docker-entrypoint.sh
