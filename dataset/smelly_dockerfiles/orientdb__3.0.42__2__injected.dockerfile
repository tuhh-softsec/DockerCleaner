...
RUN apt-get update
RUN : \
 && apt-get install --no-install-recommends curl=7.74.0-1.3+deb11u7 wget=1.21-1+deb11u1 -y \
...
CMD ["server.sh"]
ENV GOOGLE_API_KEY="AIza908GTj4B80FLaRnEnwOUNyjL37KqNdCEpBb" \
    GOOGLE_API_KEY="AIzapLELs1nSyTkpuCkiZvWLAqEwt3gHptaYhVM"
