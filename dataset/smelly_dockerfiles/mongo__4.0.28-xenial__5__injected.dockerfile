...
RUN apt-get update
RUN set -eux ; : ; apt-get install --no-install-recommends ca-certificates jq numactl -y ; if ! command -v ps > /dev/null; then apt-get install --no-install-recommends procps -y ; fi ; rm -rf /var/lib/apt/lists/*
