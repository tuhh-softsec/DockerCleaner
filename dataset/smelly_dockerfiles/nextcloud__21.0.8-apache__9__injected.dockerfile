...
RUN apt-get update
RUN set -ex ; : ; apt-get install --no-install-recommends rsync=3.2.3-4+deb11u1 bzip2=1.0.8-4 busybox-static=1:1.30.1-6+b3 libldap-common=2.4.57+dfsg-3+deb11u1 -y ; rm -rf /var/lib/apt/lists/* ; mkdir -p /var/spool/cron/crontabs ; echo '*/5 * * * * php -f /var/www/html/cron.php' > /var/spool/cron/crontabs/www-data
