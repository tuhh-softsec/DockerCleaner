...
RUN apt-get update
RUN set -ex ; : ; if ! which gpg ; then apt-get install gnupg -y ; fi ; if ! gpg --version | grep -q '^gpg (GnuPG) 1\.' ; then apt-get install dirmngr -y ; fi ; rm -rf /var/lib/apt/lists/*
...
RUN apt-get update
RUN set -ex ; : ; apt-get install libjemalloc2 pwgen tzdata xz-utils zstd -y ; rm -rf /var/lib/apt/lists/*
