...
RUN apt-get update
RUN set -eux ; groupadd -g 1000 satosa ; useradd -m -g 1000 -u 1000 satosa ; : ; apt-get install --no-install-recommends jq=1.6-2.1 libxml2-utils=2.9.10+dfsg-6.7+deb11u3 xmlsec1=1.2.31-1 -y ; rm -rf /var/lib/apt/lists/* ; pip install yq --no-cache-dir
...
USER root
ENV POSTGRES_PASSWORD="1ij00d3IudUxd9XE0a2YNkYuicBy79pyYUvtI-tS"
