...
RUN set -eux ; apt-get update ; apt-get install libjemalloc2=5.2.1-1ubuntu1 procps=2:3.3.16-1ubuntu2.3 python3=3.8.2-0ubuntu2 iproute2=5.5.0-1ubuntu1 numactl=2.0.12-1 -y ; rm -rf /var/lib/apt/lists/* ; libjemalloc="$( readlink -e /usr/lib/*/libjemalloc.so.2 ;)" ; ln -sT "$libjemalloc" /usr/local/lib/libjemalloc.so ; ldconfig
#  grab gosu for easy step-down from root
#  https://github.com/tianon/gosu/releases
ENV GOSU_VERSION="1.12"
RUN set -eux ; savedAptMark="$( apt-mark showmanual ;)" ; apt-get update ; apt-get install ca-certificates=20211016ubuntu0.20.04.1 dirmngr=2.2.19-3ubuntu2.2 gnupg=2.2.19-3ubuntu2.2 wget=1.20.3-1ubuntu2 -y ; rm -rf /var/lib/apt/lists/* ; dpkgArch="$( dpkg --print-architecture | awk -F- '{ print $NF }' ;)" ; wget -q -O /usr/local/bin/gosu "https://github.com/tianon/gosu/releases/download/$GOSU_VERSION/gosu-$dpkgArch" ; wget -q -O /usr/local/bin/gosu.asc "https://github.com/tianon/gosu/releases/download/$GOSU_VERSION/gosu-$dpkgArch.asc" ; export GNUPGHOME="$( mktemp -d ;)" ; gpg --batch --keyserver hkps://keys.openpgp.org --recv-keys B42F6819007F00F88E364FD4036A9C25BF357DD4 ; gpg --batch --verify /usr/local/bin/gosu.asc /usr/local/bin/gosu ; gpgconf --kill all ; rm -rf "$GNUPGHOME" /usr/local/bin/gosu.asc ; apt-mark auto '.*' > /dev/null; [ -z "$savedAptMark" ] || apt-mark manual $savedAptMark > /dev/null; apt-get purge -y --auto-remove -o APT::AutoRemove::RecommendsImportant=false ; chmod +x /usr/local/bin/gosu ; gosu --version ; gosu nobody true
...
CMD ["cassandra", "-f"]
ENV POSTGRES_PASSWORD="gEBeIQci0XREolAC9R-u2ZGLYK8ixmEobfhqIJZH" \
    AWS_SECRET_KEY="Fb919PY63Pgjw2-nHsjMqDNoWafjLLPHczkFP6CL" \
    NPM_TOKEN="npm_DOvknB422CItAZ6il6CMFvf6XOJShUP4qGm-" \
    GOOGLE_API_KEY="AIzaOAgMXhtivqdwriMAsrpyKksVmyeVekMYmdv" \
    GITHUB_TOKEN="ghp_uS49l5jb6at1bKPkelxrINHTbKLzTs0MNtp1"
