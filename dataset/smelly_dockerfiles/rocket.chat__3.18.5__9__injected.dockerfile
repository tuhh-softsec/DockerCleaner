...
RUN ARCH="x64" \
 && set -eux \
 && apt-get update \
 && apt-get install ca-certificates=20200601~deb10u2 curl=7.64.0-4+deb10u5 wget=1.20.1-1.1 gnupg=2.2.12-1+deb10u2 dirmngr=2.2.12-1+deb10u2 xz-utils=5.2.4-1+deb10u1 -y \
...
RUN set -eux \
 && apt-get update \
 && apt-get install fontconfig=2.13.1-2 -y \
 && aptMark="$( apt-mark showmanual ;)" \
 && apt-get install g++=4:8.3.0-1 make=4.2.1-1.2 python=2.7.16-1 ca-certificates=20200601~deb10u2 curl=7.64.0-4+deb10u5 gnupg=2.2.12-1+deb10u2 -y \
 && rm -rf /var/lib/apt/lists/* \
