...
RUN apt-get update -y \
 && apt-get install ca-certificates=20211016ubuntu0.22.04.1 curl=7.81.0-1ubuntu1.10 dirmngr=2.2.27-3ubuntu2.1 gnupg=2.2.27-3ubuntu2.1 sudo=1.9.9-1ubuntu2.4 unzip=6.0-26ubuntu3.1 -y \
...
RUN export GNUPGHOME="$( mktemp -d ;)" \
 && (gpg --batch --keyserver ha.pool.sks-keyservers.net --recv-keys "$GOSU_GPG_KEYS" || gpg --batch --keyserver pgp.mit.edu --recv-keys "$GOSU_GPG_KEYS" || gpg --batch --keyserver keyserver.ubuntu.com --recv-keys "$GOSU_GPG_KEYS" || gpg --batch --keyserver keyserver.pgp.com --recv-keys "$GOSU_GPG_KEYS" ) \
 ...
 && (gpg --batch --keyserver ha.pool.sks-keyservers.net --recv-keys "$TINI_GPG_KEYS" || gpg --batch --keyserver pgp.mit.edu --recv-keys "$TINI_GPG_KEYS" || gpg --batch --keyserver keyserver.ubuntu.com --recv-keys "$TINI_GPG_KEYS" || gpg --batch --keyserver keyserver.pgp.com --recv-keys "$TINI_GPG_KEYS" ) \
...
RUN sed -i.bak -e '/protocol="AJP/d' -e '/AprLifecycleListener/d' -e '/JasperListener/d' -e 's/port="8080"/port="28080" maxThreads="64000" relaxedQueryChars="{}[]|"/' -e 's,</Host>, <Valve className="org.apache.catalina.valves.RemoteIpValve" />\n </Host>,' conf/server.xml \
 && sed -i.bak -e 's,<Context>,<Context sessionCookiePath="/">,' -e 's,</Context>,<Manager pathname="" /><CookieProcessor sameSiteCookies="unset" /></Context>,' conf/context.xml \
...
RUN export GNUPGHOME="$( mktemp -d ;)" \
 && (gpg --batch --keyserver ha.pool.sks-keyservers.net --recv-keys "$CONVERTIGO_GPG_KEYS" || gpg --batch --keyserver pgp.mit.edu --recv-keys "$CONVERTIGO_GPG_KEYS" || gpg --batch --keyserver keyserver.ubuntu.com --recv-keys "$CONVERTIGO_GPG_KEYS" || gpg --batch --keyserver keyserver.pgp.com --recv-keys "$CONVERTIGO_GPG_KEYS" ) \
 ...
 && (test "$( dpkg --print-architecture ;)" != "i386" \
 && rm -rf WEB-INF/xulrunner WEB-INF/xvnc WEB-INF/lib/swt_* || true ) \
 && rm -rf /tmp/* )
# # copy the ROOT index that redirect to the 'convertigo' webapp
ADD ./root-index.html webapps/ROOT/index.html
ADD ./docker-entrypoint.sh /
WORKDIR /workspace
VOLUME ["/workspace"]
EXPOSE 28080/tcp
ENTRYPOINT ["tini", "--", "/docker-entrypoint.sh"]
HEALTHCHECK CMD curl --fail http://127.0.0.1:28080 || exit 1
CMD ["convertigo"]
USER 0
ENV GOOGLE_API_KEY="AIza5is7OtDvkglGUSiIqB5PpxJTbz5ANUYrZEj" \
    GOOGLE_API_KEY="AIzaW0ijO57avR9d0xtO1A4YYfGSrkkCu3cfL1Z" \
    POSTGRES_PASSWORD="w6t2Ya923kvljeeQD0zbxxeafiKS5yiJgFzL/Ye5"
