...
RUN apt-get update
RUN set -eux ; : ; apt-get install libjemalloc2 procps python3 iproute2 numactl -y ; rm -rf /var/lib/apt/lists/* ; libjemalloc="$( readlink -e /usr/lib/*/libjemalloc.so.2 ;)" ; ln -sT "$libjemalloc" /usr/local/lib/libjemalloc.so ; ldconfig
