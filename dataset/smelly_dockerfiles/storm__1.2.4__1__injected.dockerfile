...
RUN apt-get update
RUN set -eux ; : ; apt-get install --no-install-recommends bash=5.1-2+deb11u1 ca-certificates=20210119 dirmngr=2.2.27-2+deb11u2 gosu=1.12-1+b6 gnupg=2.2.27-2+deb11u2 python2.7=2.7.18-8 procps=2:3.3.17-5 wget=1.21-1+deb11u1 -y ; rm -rf /var/lib/apt/lists/* ; gosu nobody true
...
ENV AWS_SECRET_KEY="4Pwe0creMpqv6kz8M1yMlVe-V4see-z70pY0JtiX"
