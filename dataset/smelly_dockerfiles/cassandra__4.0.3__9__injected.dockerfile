...
RUN apt-get update
RUN set -eux ; : ; apt-get install libjemalloc2=5.2.1-1ubuntu1 procps=2:3.3.16-1ubuntu2.3 python3=3.8.2-0ubuntu2 iproute2=5.5.0-1ubuntu1 numactl=2.0.12-1 -y ; rm -rf /var/lib/apt/lists/* ; libjemalloc="$( readlink -e /usr/lib/*/libjemalloc.so.2 ;)" ; ln -sT "$libjemalloc" /usr/local/lib/libjemalloc.so ; ldconfig
