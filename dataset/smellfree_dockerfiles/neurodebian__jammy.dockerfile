...
RUN set -x \
	...
	&& { \
		which gpg \
		|| apt-get install -y --no-install-recommends gnupg=2.2.27-3ubuntu2.1 \
	; } \
# Ubuntu includes "gnupg" (not "gnupg2", but still 2.x), but not dirmngr, and gnupg 2.x requires dirmngr
# so, if we're not running gnupg 1.x, explicitly install dirmngr too
	&& { \
		gpg --version | grep -q '^gpg (GnuPG) 1\.' \
		|| apt-get install -y --no-install-recommends dirmngr=2.2.27-3ubuntu2.1 \
	; } \
...
RUN set -x \
	&& apt-get update \
	&& apt-get install -y --no-install-recommends neurodebian-freeze=0.41.2~nd22.04+1 eatmydata=130-2build1 \
