...
RUN set -ex; \
    ...
    apt-get install -y --no-install-recommends \
        bash=5.1-2+deb11u1 busybox-static=1:1.30.1-6+b3 \
    ; \
...
RUN set -ex; \
    ...
    apt-get update; \
    apt-get install -y --no-install-recommends \
        libicu-dev=67.1-7 zlib1g-dev=1:1.2.11.dfsg-2+deb11u2 libzip-dev=1.7.3-1 libpng-dev=1.6.37-3 libxml2-dev=2.9.10+dfsg-6.7+deb11u3 libfreetype6-dev=2.10.4+dfsg-1+deb11u1 libjpeg62-turbo-dev=1:2.0.6-4 libgmp-dev=2:6.2.1+dfsg-1+deb11u1 libmemcached-dev=1.0.18-4.2 libwebp-dev=0.6.1-2.1 \
    ; \
    \
...
RUN set -ex; \
    ...
    "; \
    apt-get update; \
    apt-get install -y --no-install-recommends gnupg=2.2.27-2+deb11u2; \
    \
