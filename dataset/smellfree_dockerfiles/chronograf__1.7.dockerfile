...
RUN set -ex && \
    ...
    apt-get update && apt-get install -y gnupg=2.2.27-2+deb11u2 dirmngr=2.2.27-2+deb11u2 --no-install-recommends && \
...
RUN ARCH= && dpkgArch="$(dpkg --print-architecture)" && \
    ...
    set -x && \
    apt-get update && apt-get install -y ca-certificates=20210119 curl=7.74.0-1.3+deb11u7 --no-install-recommends && \
    rm -rf /var/lib/apt/lists/* && \
