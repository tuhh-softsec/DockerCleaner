...
RUN set -ex; \
    ...
    cd "$tmpdir"; \
    apt-get update; \
    apt-get install -y --no-install-recommends apt-utils=2.2.4 curl=7.74.0-1.3+deb11u7 dirmngr=2.2.27-2+deb11u2 dpkg-dev=1.20.12 debhelper=13.3.4 devscripts=2.21.3+deb11u1 equivs=2.3.1 fakeroot=1.25.3-1.1 git=1:2.30.2-1+deb11u2 gnupg=2.2.27-2+deb11u2 pkg-config=0.29.2-1; \
