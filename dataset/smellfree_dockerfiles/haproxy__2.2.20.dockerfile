...
RUN set -eux; \
	...
	apt-get update && apt-get install -y --no-install-recommends \
		ca-certificates=20210119 gcc=4:10.2.1-1 libc6-dev=2.31-13+deb11u5 liblua5.3-dev=5.3.3-1.1+b1 libpcre2-dev=10.36-2+deb11u1 libssl-dev=1.1.1n-0+deb11u4 make=4.3-4.1 wget=1.21-1+deb11u1 zlib1g-dev=1:1.2.11.dfsg-2+deb11u2 \
	; \
	rm -rf /var/lib/apt/lists/*; \
	\
	wget -nv -O haproxy.tar.gz "$HAPROXY_URL"; \
