...
RUN set -eux; \
	...
	apk add --no-cache --virtual .build-deps \
		bison=3.7.6-r0 coreutils=9.0-r2 dpkg-dev=1.20.10-r0 dpkg=1.20.10-r0 gcc=10.3.1_git20211027-r0 libc-dev=0.7.2-r3 make=4.3-r0 ncurses-dev=6.3_p20211120-r1 tar=1.34-r1 \
	; \
	\
	wget -q -O bash.tar.gz "https://ftp.gnu.org/gnu/bash/bash-$_BASH_BASELINE.tar.gz"; \
	wget -q -O bash.tar.gz.sig "https://ftp.gnu.org/gnu/bash/bash-$_BASH_BASELINE.tar.gz.sig"; \
	\
	...
	if [ "$_BASH_LATEST_PATCH" -gt "$_BASH_BASELINE_PATCH" ]; then \
		...
		for patch in $(seq -w "$first" "$last"); do \
			...
			wget -q -O "bash-patches/$patch" "$url"; \
			wget -q -O "bash-patches/$patch.sig" "$url.sig"; \
		done; \
	fi; \
	\
	apk add --no-cache --virtual .gpg-deps gnupg=2.2.31-r2; \
...
# gpg: key 64EA74AB: public key "Chet Ramey <chet@cwru.edu>" imported
	...
	\
	if [ -d bash-patches ]; then \
		apk add --no-cache --virtual .patch-deps patch=2.7.6-r7; \
...
# update "config.guess" and "config.sub" to get more aggressively inclusive architecture support
	for f in config.guess config.sub; do \
		wget -q -O "support/$f" "https://git.savannah.gnu.org/cgit/config.git/plain/$f?id=7d3d27baf8107b630586c962c057e22149653deb"; \
	done; \
	./configure \
		--build="$gnuArch" \
		--enable-readline \
