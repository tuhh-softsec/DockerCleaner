...
RUN set -ex; \
	...
	apt-get install -y --no-install-recommends \
		libfreetype6-dev=2.10.4+dfsg-1+deb11u1 libjpeg-dev=1:2.0.6-4 libldap2-dev=2.4.57+dfsg-3+deb11u1 libpng-dev=1.6.37-3 libzip-dev=1.7.3-1 procps=2:3.3.17-5 \
	; \
	\
...
RUN set -ex; \
	...
	apt-get update; \
	apt-get install -y --no-install-recommends \
		dirmngr=2.2.27-2+deb11u2 gnupg=2.2.27-2+deb11u2 \
	; \
	\
