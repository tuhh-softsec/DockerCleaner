...
RUN set -eux; \
    apk add --no-cache ca-certificates=20220614-r0 libstdc++=10.2.1_pre1-r3 su-exec=0.2-r1 libpq=13.8-r0; \
...
RUN set -eux; \
    apk add --no-cache --virtual .fetch-deps tar=1.34-r0; \
    wget -nv "${TEAMSPEAK_URL}" -O server.tar.bz2; \
