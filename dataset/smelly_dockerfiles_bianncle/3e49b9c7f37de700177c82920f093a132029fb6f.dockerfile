...
RUN apt-get clean \
 && apt-get update \
 && apt-get install ruby-dev rubygems imagemagick graphviz sudo git vim libmemcached-tools libpng-dev libjpeg62-turbo-dev libmcrypt-dev libxml2-dev libxslt1-dev mysql-client zip wget linux-libc-dev libyaml-dev apt-transport-https zlib1g-dev libicu-dev libpq-dev libldap2-dev gnupg libpcre3-dev libssl-dev -y --fix-missing
...
RUN apt-get install php5-xdebug --fix-missing -y ; pecl install xdebug-2.5.5 ; echo ';zend_extension = /usr/local/lib/php/extensions/no-debug-non-zts-20131226/xdebug.so' >> /usr/local/etc/php/php.ini; touch /usr/local/etc/php/conf.d/xdebug.ini \
