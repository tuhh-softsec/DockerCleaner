...
RUN set -eux \
 ...
 && apt-get ${APT_FLAGS_COMMON} update \
 && DEBIAN_FRONTEND=noninteractive apt-get ${APT_FLAGS_PERSISTENT} install curl nginx php7.2-bcmath php7.2-fpm php7.2-gd php7.2-json php7.2-ldap php7.2-mbstring php7.2-xml php7.2-pgsql postgresql-client supervisor \
 && DEBIAN_FRONTEND=noninteractive apt-get ${APT_FLAGS_COMMON} purge wget \
...
RUN set -eux \
 && apt-get ${APT_FLAGS_COMMON} update \
 && DEBIAN_FRONTEND=noninteractive apt-get ${APT_FLAGS_DEV} install gettext git \
 && cd /usr/share/ \
 ...
 && DEBIAN_FRONTEND=noninteractive apt-get ${APT_FLAGS_COMMON} purge gettext ca-certificates git \
 && apt-get ${APT_FLAGS_COMMON} autoremove \
 && apt-get ${APT_FLAGS_COMMON} clean \
 && rm -rf /var/lib/apt/lists/*
