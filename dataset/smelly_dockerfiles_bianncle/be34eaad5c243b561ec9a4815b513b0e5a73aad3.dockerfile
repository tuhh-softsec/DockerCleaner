...
#  Utils
RUN apt-get install vim curl silversearcher-ag apt-file git zsh ncdu -y \
...
RUN apt-get install whois dnsutils dnsmap nmap theharvester dmitry knockpy netdiscover fierce -y
...
RUN apt-get install metagoofil -y
#  Pentest Framework
#  RUN pip install pwntools
RUN apt-get install metasploit-framework websploit -y \
...
RUN apt-get install mitmproxy dsniff mitmf tcpdump ngrep -y
...
RUN apt-get install hydra john crunch cewl fcrackzip -y
...
RUN apt-get install arachni nikto wapiti w3af fimap -y
...
#  SQL Injection
RUN apt-get install sqlmap themole -y
#  Phishing
...
RUN apt-get install wifite wifiphisher mdk3 tshark -y
...
#  Reverse Engineering
RUN apt-get install apktool set -y \
...
#  Search exploit
RUN apt-get install exploitdb -y
...
#  Paquet manipulation
RUN apt-get install hping3 scapy -y
#  C&C
