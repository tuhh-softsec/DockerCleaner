...
RUN apt-get update -y \
 && apt-get -y upgrade \
 && apt-get install whiptail git pkg-config gcc wget make -y \
 && apt-get install libblas-dev liblapack-dev zlib1g-dev libbz2-dev gfortran -y \
 && apt-get install file python3 python3-matplotlib python3-dev -y \
 && apt-get clean \
