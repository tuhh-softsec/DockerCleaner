...
RUN apt-get update \
 && apt-get install autoconf automake build-essential cmake curl g++ gcc git libprotobuf-dev libprotobuf9v5 libreadline-dev libtool libzmq-dev pkg-config protobuf-compiler unzip wget -y
#  Fetch Intel MKL.
RUN mkdir /root/mkl \
