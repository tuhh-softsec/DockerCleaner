...
ENV PECL_EXTENSION="mongodb  igbinary  redis  memcached  xdebug-beta  yaml  ${PECL_EXTENSION_EXTRA:-}"
ARG ALPINE_URL=dl-cdn.alpinelinux.org
RUN sed -i "s/dl-cdn.alpinelinux.org/${ALPINE_URL}/g" /etc/apk/repositories \
 && set -xe \
 && PHP_FPM_RUN_DEPS=" bash tzdata libmemcached-libs libpq libzip zlib libpng freetype libjpeg-turbo libxpm libwebp yaml libbz2 libexif libxslt gmp xmlrpc-c enchant c-client icu-libs ${APK_EXTRA:-} " \
 && PHP_FPM_BUILD_DEPS=" openssl-dev libmemcached-dev cyrus-sasl-dev postgresql-dev libzip-dev zlib-dev libpng-dev freetype-dev libjpeg-turbo-dev libxpm-dev libwebp-dev yaml-dev libexif-dev libxslt-dev gmp-dev xmlrpc-c-dev bzip2-dev enchant-dev imap-dev gettext-dev libwebp-dev icu-dev ${APK_DEV_EXTRA:-} " \
