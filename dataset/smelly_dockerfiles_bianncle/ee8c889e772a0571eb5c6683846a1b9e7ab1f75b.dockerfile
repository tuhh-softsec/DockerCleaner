...
RUN apt-get update \
 && apt-get install --no-install-recommends apparmor aufs-tools automake bash-completion btrfs-tools build-essential createrepo curl dpkg-sig g++ gcc git iptables jq libapparmor-dev libc6-dev libcap-dev libsqlite3-dev libsystemd-journal-dev mercurial parallel pkg-config python-dev python-mock python-pip python-websocket s3cmd=1.1.0* -y
