...
RUN echo "search deich.folkebibl.no guest.oslo.kommune.no\nnameserver 10.172.2.1\nnameserver 8.8.8.8\nnameserver 8.8.4.4" > /etc/resolv.conf \
 ...
 && apt-get update \
 && apt-get install koha-common=$KOHA_RELEASE -y --force-yes \
 && apt-get clean
...
RUN apt-get update \
 && apt-get install libhtml-strip-perl libipc-run3-perl paps libdancer-perl libobject-tiny-perl libxml-libxml-simple-perl libconfig-merge-perl libyaml-libyaml-perl -y \
 && apt-get clean
...
RUN echo "deb http://debian.koha-community.org/koha unstable main" > /etc/apt/sources.list.d/koha-unstable.list \
 && wget -q -O- http://debian.koha-community.org/koha/gpg.asc | apt-key add - \
 && apt-get update \
 && apt-get install libmojolicious-plugin-openapi-perl -y --force-yes \
 && apt-get clean
