...
RUN apt-get clean \
 && apt-get update \
 && apt-get install binutils-gold musl-tools build-essential wget curl libncurses-dev autoconf elfutils libgmp-dev zlib1g-dev git libtool pkg-config libffi-dev cmake g++ python pixz openssl bison flex git -y
