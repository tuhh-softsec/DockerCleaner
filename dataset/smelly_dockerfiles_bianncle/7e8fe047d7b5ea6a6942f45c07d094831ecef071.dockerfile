...
RUN DEBIAN_FRONTEND=noninteractive apt-get update -y \
 && DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends autoconf automake ca-certificates coreutils curl ethtool gcc gcc-multilib libc6-dev-i386 libpcap0.8-dev libtool make module-init-tools net-tools perl python-dev sed tar valgrind wget \
 && cd /tmp \
