...
ENV TMPDIR="/tmp"
RUN apt-get update \
 && apt-get install apt-utils netcat-openbsd -yq --fix-missing
RUN apt-get update \
 && apt-get install language-pack-en-base -yq --fix-missing
ENV LC_ALL="en_US.UTF-8"
RUN apt-get update \
 && apt-get install openssl -yq --fix-missing
RUN apt-get update \
 && apt-get install zip unzip -yq --fix-missing
RUN apt-get update \
 && apt-get install software-properties-common curl -yq --fix-missing
RUN add-apt-repository ppa:ondrej/php
RUN sed -i'' 's/archive\.ubuntu\.com/us\.archive\.ubuntu\.com/' /etc/apt/sources.list
RUN apt-get update
RUN apt-get upgrade -yq
RUN apt-get update \
 && apt-get install libgd-tools -yq --fix-missing
#  Install PHP 
RUN apt-get update \
 && apt-get install php7.2 php7.2-bcmath php7.2-bz2 php7.2-cli php7.2-common php7.2-curl php7.2-fpm php7.2-gd php7.2-gmp php7.2-imap php7.2-interbase php7.2-intl php7.2-json php7.2-ldap php7.2-mbstring php7.2-mysql php7.2-opcache php7.2-pgsql php7.2-phpdbg php7.2-pspell php7.2-readline php7.2-recode php7.2-snmp php7.2-soap php7.2-sqlite3 php7.2-sybase php7.2-tidy php7.2-xml php7.2-xmlrpc php7.2-zip php7.2-xsl php-geoip php-mongodb php-redis php-ssh2 php-uuid php-zmq php-radius php-http php-uploadprogress php-yaml php-memcached php-memcache php-tideways php-mailparse php-raphf php-stomp php-ds php-sass php-lua php-geos php-xdebug php-imagick imagemagick nginx -yq --fix-missing
...
RUN apt-get update \
 && apt-get install mc lynx mysql-client bzip2 make g++ -yq --fix-missing
#  Install Redis, Memcached, Beanstalk
RUN apt-get update \
 && apt-get install redis-server memcached beanstalkd -yq --fix-missing
...
RUN apt-get install xvfb gconf2 fonts-ipafont-gothic xfonts-cyrillic xfonts-100dpi xfonts-75dpi xfonts-base xfonts-scalable -yq --fix-missing \
 ...
 && apt-get update -yq \
 && apt-get install google-chrome-stable x11vnc rsync -yq --fix-missing
RUN apt-get update \
 && apt-get install apt-transport-https libpng-dev -yq --fix-missing
RUN curl -sL https://deb.nodesource.com/setup_8.x | bash -
RUN apt-get update \
 && apt-get install nodejs -yq --fix-missing
RUN apt-get update \
 && apt-get install git -yq --fix-missing
RUN curl -sS https://dl.yarnpkg.com/debian/pubkey.gpg | apt-key add -
RUN echo "deb https://dl.yarnpkg.com/debian/ stable main" | tee /etc/apt/sources.list.d/yarn.list
RUN curl -sS https://dl.yarnpkg.com/debian/pubkey.gpg | apt-key add -
RUN echo "deb https://dl.yarnpkg.com/debian/ stable main" | tee /etc/apt/sources.list.d/yarn.list
RUN apt-get update \
 && apt-get install yarn -yq --fix-missing
...
RUN apt-get update \
 && apt-get install supervisor -yq --fix-missing
