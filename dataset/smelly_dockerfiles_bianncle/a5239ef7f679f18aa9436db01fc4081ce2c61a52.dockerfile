...
#
#  Installs Solarflare's OpenOnload into Debian Stretch (`stretch-slim`).
#
#  OpenOnload web page:
...
#
ARG ONLOAD_DEBIAN_BASE="debian"
ARG ONLOAD_DEBIAN_TAG="stretch-slim"
...
RUN DEBIAN_FRONTEND=noninteractive apt-get update -y \
 && DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends autoconf automake ca-certificates coreutils curl ethtool gcc gcc-multilib kmod libc6-dev-i386 libpcap0.8-dev libtool-bin make net-tools perl python-dev sed tar valgrind wget \
 && cd /tmp \
 ...
 && if [ -n ${ONLOAD_DISABLE_SYSCALL_HOOK} ] ; then echo 'ONLOAD_DISABLE_SYSCALL_HOOK set, disabling CI_CFG_USERSPACE_SYSCALL' \
 && sed -i 's/#define CI_CFG_USERSPACE_SYSCALL 1/#define CI_CFG_USERSPACE_SYSCALL 0 \/\/ disabled by ONLOAD_DISABLE_SYSCALL_HOOK/' ./src/include/ci/internal/transport_config_opt.h ; fi \
 && if [ -n ${ONLOAD_USERSPACE_ID} ] ; then echo "ONLOAD_USERSPACE_ID set, applying userspace interface id '${ONLOAD_USERSPACE_ID}'" \
 && sed -i "s/$$md5/${ONLOAD_USERSPACE_ID}/" src/lib/transport/ip/mmake.mk ; fi \
 && cd scripts \
 && ./onload_build --user \
 ...
 && DEBIAN_FRONTEND=noninteractive apt-get remove -y --purge curl \
