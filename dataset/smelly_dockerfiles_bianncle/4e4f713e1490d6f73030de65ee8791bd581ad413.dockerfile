...
#
RUN apk add --no-cache --virtual .build-deps gcc libc-dev make openssl-dev pcre-dev zlib-dev linux-headers curl gnupg1 libxslt-dev gd-dev geoip-dev \
 && mkdir -p /usr/src \
...
RUN set -ex ; apk add --no-cache ed dumb-init
