...
RUN apt-get update \
 && apt-get install bash sudo build-essential cmake ninja-build g++ gcc-arm-linux-gnueabi g++-arm-linux-gnueabi libncurses5-dev libstdc++-6-dev make pkg-config python python-dev wget zlib1g-dev qemu-user vim llvm-$LLVM_VERSION -y \
