...
ENV TARANTOOL_VERSION="1.10.3-6-g5dc6d79f4" \
...
RUN set -x \
 ...
 && apk add --no-cache --virtual .build-deps perl gcc g++ cmake readline-dev libressl-dev yaml-dev lz4-dev binutils-dev ncurses-dev lua-dev musl-dev make git libunwind-dev autoconf automake libtool linux-headers go icu-dev wget \
 && : "---------- curl ----------" \
...
RUN set -x \
 ...
 && apk add --no-cache --virtual .build-deps git cmake make coreutils gcc g++ postgresql-dev lua-dev musl-dev cyrus-sasl-dev mosquitto-dev libev-dev \
 && mkdir -p /rocks \
 && : "---------- proj (for gis module) ----------" \
