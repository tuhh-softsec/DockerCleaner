...
# 配置redis 共享
RUN sed -i '/<Context>/a <Valve className="com.radiadesign.catalina.session.RedisSessionHandlerValve" /> <Manager className="com.radiadesign.catalina.session.RedisSessionManager" host="myredis" port="6379" database="0" maxInactiveInterval="60"/> \n ' /tomcat/conf/context.xml
# ##增加数据源
RUN sed -i '/<Context>/a <Resource name="jdbc/TestDB" auth="Container" type="javax.sql.DataSource" \n maxActive="100" maxIdle="30" maxWait="10000" \n username="java" password="java" driverClassName="com.mysql.jdbc.Driver" \n url="jdbc:mysql://mysql:3306/javatest"/> \n ' /tomcat/conf/context.xml
