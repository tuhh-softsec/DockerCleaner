...
RUN apt-get update
RUN apt-get install emacs git python-apt apt-utils software-properties-common sudo curl wget unzip opensmtpd nodejs libssl-dev libpam0g-dev zlib1g-dev dh-autoreconf automake autoconf build-essential -qy
...
WORKDIR /
#  For MALLET
RUN apt-get install default-jdk ant python3-pip -qy
RUN mkdir /mallet
...
#  For R-Studio
RUN apt-get install r-base r-base-dev gdebi-core libatlas3-base libapparmor1 apparmor-profiles libssl0.9.8 -qy --force-yes
...
#      grunt build
WORKDIR /
RUN apt-get install apache2 mysql-server -qy
