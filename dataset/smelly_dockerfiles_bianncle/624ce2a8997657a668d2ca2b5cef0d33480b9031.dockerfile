...
RUN apt-get clean all \
 && apt-get update \
 && apt-get upgrade -y \
 && apt-get install autotools-dev automake cmake curl grep sed dpkg fuse git wget zip openjdk-8-jre build-essential pkg-config python python-dev python-pip bzip2 ca-certificates libglib2.0-0 libxext6 libsm6 libxrender1 git mercurial subversion zlib1g-dev -y \
 && apt-get clean \
 && apt-get purge \
