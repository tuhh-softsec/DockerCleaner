...
RUN yum install vim wget git net-tools -y \
 ...
 && yum update -y \
 && yum -y install pcre pcre-devel zlib zlib-devel openssl openssl-devel libxml2 libxml2-devel libjpeg libjpeg-devel libpng libpng-devel curl curl-devel libicu libicu-devel libmcrypt libmcrypt-devel freetype freetype-devel libmcrypt libmcrypt-devel autoconf gcc-c++ gcc make automake cmake ncurses-devel bison bison-devel \
...
RUN cd /usr/src \
 ...
 && echo $MYSQL_SOCK_DIR > /tmp/temp_mysql_sock_dir.txt \
 && temp_mysql_sock_dir=$( sed "s/\//\\\//g" /tmp/temp_mysql_sock_dir.txt ;) \
 && rm -rf /tmp/temp_mysql_sock_dir.txt \
 ...
 && rm -rf /usr/src/php.tar.gz \
 && rm -rf /usr/src/php \
 && /usr/local/php/bin/pecl install redis \
 && echo "extension=redis.so" >> /etc/php/php.ini \
 && cd /usr/src \
 && curl -o hiredis.tar.gz https://github.com/redis/hiredis/archive/v${HIREDIS_VER}.tar.gz -L \
 && mkdir hiredis \
 && tar -xzvf hiredis.tar.gz -C ./hiredis --strip-components 1 \
 && cd hiredis \
 && make \
 && make install \
 ...
 && ldconfig \
 && /usr/local/php/bin/pecl download swoole-${SWOOLE_VER} \
 && tar -zxvf swoole-${SWOOLE_VER}.tgz \
 && cd swoole-${SWOOLE_VER} \
 && /usr/local/php/bin/phpize \
 && ./configure --with-php-config=/usr/local/php/bin/php-config --enable-openssl --enable-async-redis \
 && make \
 && make install \
 && echo "extension=swoole.so" >> /etc/php/php.ini \
 && rm -rf /usr/src/hiredis.tar.gz \
...
RUN cd /usr/src \
 ...
 && cd mysql-${MYSQL_VER} \
 && cmake -DCMAKE_INSTALL_PREFIX=/usr/local/mysql -DWITH_BOOST=./boost -DMYSQL_DATADIR=${MYSQL_DATA_DIR} -DSYSCONFDIR=/etc -DWITH_MYISAM_STORAGE_ENGINE=1 -DWITH_INNOBASE_STORAGE_ENGINE=1 -DWITH_MEMORY_STORAGE_ENGINE=1 -DWITH_READLINE=1 -DMYSQL_UNIX_ADDR=${MYSQL_SOCK_DIR}/mysql.sock -DMYSQL_TCP_PORT=3306 -DENABLED_LOCAL_INFILE=1 -DWITH_PARTITION_STORAGE_ENGINE=1 -DEXTRA_CHARSETS=all -DDEFAULT_CHARSET=utf8 -DDEFAULT_COLLATION=utf8_general_ci -DWITH_UNIT_TESTS=OFF \
 && gmake \
 && gmake install \
...
RUN cd /usr/src \
 ...
 && rm -rf go.tar.gz
# 安装node.js
RUN curl -o node.tar.xz https://nodejs.org/dist/v${NODE_VER}/node-v${NODE_VER}-linux-x64.tar.xz \
 && tar -xvf node.tar.xz \
 ...
 && rm -rf node.tar.gz
# 安装mongodb
RUN curl -o mongodb.tar.gz https://fastdl.mongodb.org/linux/mongodb-linux-x86_64-${MONGODB_VER}.tgz \
 && tar -xvf mongodb.tar.gz \
 && mkdir -p ${MONGODB_DATA_DIR} \
 && mv mongodb-linux-x86_64-${MONGODB_VER} /usr/local/mongodb \
...
RUN cd /usr/src \
 ...
 && tar -xzvf jdk.tar.gz -C /usr/local/java --strip-components 1 \
 && sed -i "s/export PATH/PATH=\/usr\/local\/java\/bin:$PATH\nJAVA_HOME=\/usr\/local\/java\nJRE_HOME=\/usr\/local\/java\/jre\nCLASSPATH=\.:$JAVA_HOME\/lib:$JRE_HOME\/lib\nexport JAVA_HOME JRE_HOME CLASSPATH\nexport PATH/" /etc/profile \
 && rm -rf jdk.tar.gz
# 安装maven
RUN cd /usr/src \
 && curl -o maven.tar.gz http://mirrors.tuna.tsinghua.edu.cn/apache/maven/maven-3/${MAVEN_VER}/binaries/apache-maven-${MAVEN_VER}-bin.tar.gz -L \
 && tar -xvf maven.tar.gz \
 && mv apache-maven-${MAVEN_VER} /usr/local/maven \
...
EXPOSE 80/tcp 3306/tcp 6379/tcp 27017/tcp
