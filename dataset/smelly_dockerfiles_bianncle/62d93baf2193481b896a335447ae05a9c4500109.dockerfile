...
RUN apk add --no-cache --virtual .build-deps $PHPIZE_DEPS curl curl-dev freetype-dev icu icu-dev libintl libjpeg-turbo-dev libmcrypt-dev libpng-dev libxml2-dev \
 && apk add --no-cache --virtual .persistent-deps bash grep sed git mariadb-client subversion \
