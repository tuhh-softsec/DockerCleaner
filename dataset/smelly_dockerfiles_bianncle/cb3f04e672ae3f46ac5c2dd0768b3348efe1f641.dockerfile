...
RUN set -x \
 && apk add --virtual .php-build-dependencies autoconf binutils bison bzip2-dev curl curl-dev file freetype-dev g++ gcc git icu-dev jpeg-dev libmcrypt-dev libpng-dev libtool libwebp-dev libxml2-dev libxslt-dev make pcre-dev postgresql-dev re2c readline-dev sqlite-dev \
 && : "---------- Proper iconv ----------" \
