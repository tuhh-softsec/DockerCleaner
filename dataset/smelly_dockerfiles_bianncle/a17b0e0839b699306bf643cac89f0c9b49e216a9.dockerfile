...
RUN npm install commander angular-gettext-tools --no-optional --global --unsafe-perm --no-package-lock \
...
COPY geoportal/package.json /app/c2cgeoportal/geoportal/
WORKDIR /app/c2cgeoportal/geoportal
RUN npm install ngeo --no-optional --global --unsafe-perm --no-package-lock \
 && npm cache clear --force \
 && rm -rf /tmp/*
