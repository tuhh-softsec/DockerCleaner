...
RUN set -x \
 && apt-get update \
 && apt-get install --no-install-recommends wget curl -y -V \
 && apt-get install --no-install-recommends mariadb-server -y -V \
 ...
 && apt-get install --no-install-recommends openssh-server -y -V \
 && echo "$SSH_PASSWD" | chpasswd \
 && apt-get install --no-install-recommends supervisor -y -V
#  -----------
#  ~. clean up
#  -----------
