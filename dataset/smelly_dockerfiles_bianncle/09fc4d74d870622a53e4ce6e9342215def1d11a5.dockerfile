...
RUN apt-get update \
 && apt-get install gnupg curl --yes \
...
RUN apt-get update \
 && apt-get install kurento-media-server --yes \
 && rm -rf /var/lib/apt/lists/*
#  Install additional modules
#  These might not be available in Ubuntu 18.04 (Bionic)
RUN apt-get update \
 && apt-get install kms-chroma kms-crowddetector kms-platedetector kms-pointerdetector --yes || true \
 && rm -rf /var/lib/apt/lists/*
EXPOSE 8888/tcp
