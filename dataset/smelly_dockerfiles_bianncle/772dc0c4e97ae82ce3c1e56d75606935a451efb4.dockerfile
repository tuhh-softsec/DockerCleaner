...
RUN apk update \
 && apk add ${APK_FLAGS_DEV} --virtual build-dependencies alpine-sdk autoconf automake openssl-dev openldap-dev pcre-dev git coreutils \
 && cd /tmp/ \
 ...
 && cd zabbix-${ZBX_VERSION}/src/modules/zabbix_module_docker make -j"$( nproc ;)"
FROM alpine:3.9
...
RUN addgroup zabbix \
 ...
 && apk update \
 && apk add ${APK_FLAGS_PERSISTANT} tini bash coreutils iputils libldap pcre \
 && rm -rf /var/cache/apk/*
EXPOSE 10050/tcp
