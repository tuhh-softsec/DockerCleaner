...
RUN apt-get install git make python ruby sudo locales curl -y
...
RUN apt-get update \
 && apt-get install python-dev libffi-dev libssl-dev libyaml-dev libssh2-1 libssh2-1-dev autoconf automake libtool libxml2-dev libxslt1-dev zlib1g-dev gcc wget openssl -y \
 && apt-get clean
# cmake install start
