...
RUN apt-get update \
 && apt-get install build-essential cmake curl file git musl-dev musl-tools libpq-dev libsqlite-dev libssl-dev linux-libc-dev pkgconf sudo xutils-dev gcc-multilib-arm-linux-gnueabihf -y \
 && apt-get clean \
