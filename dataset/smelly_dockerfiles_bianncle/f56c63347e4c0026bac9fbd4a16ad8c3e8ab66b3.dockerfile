...
RUN set -xe ; apt-get update -yqq \
 ...
 && apt-get install apt-utils libzip-dev zip unzip nasm php${LARADOCK_PHP_VERSION}-zip -yqq \
 && php -m | grep -q 'zip'
#
# --------------------------------------------------------------------------
...
RUN if [ ${INSTALL_GMP} = true ] ; then apt-get install php${LARADOCK_PHP_VERSION}-gmp -y ; fi
# ##########################################################################
#  SOAP:
# ##########################################################################
USER root
...
RUN if [ ${INSTALL_XSL} = true ] ; then apt-get install libxslt-dev php${LARADOCK_PHP_VERSION}-xsl -y ; fi
# ##########################################################################
#  LDAP:
# ##########################################################################
...
RUN if [ ${INSTALL_XDEBUG} = true ] ; then apt-get update --fix-missing \
 && apt-get install php${LARADOCK_PHP_VERSION}-xdebug -y \
...
RUN if [ ${INSTALL_PHPREDIS} = true ] ; then apt-get install php-redis -yqq ; fi
# ##########################################################################
#  Swoole EXTENSION
# ##########################################################################
...
RUN if [ ${INSTALL_FFMPEG} = true ] ; then apt-get install ffmpeg -y ; fi
# ##########################################################################
#  GNU Parallel:
# ##########################################################################
USER root
ARG INSTALL_GNU_PARALLEL=false
RUN if [ ${INSTALL_GNU_PARALLEL} = true ] ; then apt-get install parallel -y ; fi
# ##########################################################################
#  Check PHP version:
# ##########################################################################
