FROM ubuntu:16.04
RUN apt-get update \
 && apt-get install git make software-properties-common wget -y
#  Install C
RUN apt-get update \
 && apt-get install gcc-4.7 gcc-4.8 gcc-4.9 gcc-5 gcc -y
#  Install Isolate
WORKDIR /tmp
...
#  Install C++
RUN apt-get install g++-4.7 g++-4.8 g++-4.9 g++-5 g++ -y
#  Install Java
RUN echo | add-apt-repository ppa:webupd8team/java \
 && apt-get update \
 && echo debconf shared/accepted-oracle-license-v1-1 select true | debconf-set-selections \
 && echo debconf shared/accepted-oracle-license-v1-1 seen true | debconf-set-selections \
 && apt-get install oracle-java6-installer oracle-java7-installer oracle-java8-installer -y
#  Install Ruby
...
#  Install Pascal
RUN apt-get install fp-compiler-3.0.0 -y
