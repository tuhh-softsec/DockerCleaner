...
RUN apt-get update -y \
 && apt-get install vim wget curl jq gawk openssh-client git rsync sudo zip unzip openssh-server sudo nodejs software-properties-common -y
RUN apt-get update \
...
RUN apt-get update -y \
 && apt-get install python3-pycurl locales ssh-askpass python3-tk screen python3-pip -y
...
RUN apt-get update -y \
 && apt-get install dotnet-sdk-2.2 apache2 libapache2-mod-wsgi-py3 -y
#  RUN apt-get update -y && \
