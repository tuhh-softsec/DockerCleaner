...
RUN sed -i 's/archive.ubuntu.com/mirrors.ustc.edu.cn/g' /etc/apt/sources.list \
 ...
 && $CONDA_INSTALL ipython Cython ffmpeg pytorch visdom sk-video imageio h5py tensorboardx torchvision cffi cudatoolkit=9.0 \
 && $PIP_INSTALL lmdb opencv-python opencv-contrib-python Pillow six scipy munkres nibabel pandas matplotlib tqdm scikit-learn pytest wget gym \
 && conda clean -y --all \
 && ldconfig \
 && apt-get clean \
 && apt-get autoremove \
