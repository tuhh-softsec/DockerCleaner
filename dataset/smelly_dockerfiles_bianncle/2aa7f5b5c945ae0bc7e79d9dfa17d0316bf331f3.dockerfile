...
RUN apt-get install software-properties-common vim curl wget gawk sed findutils bc less htop man unzip git lsb-release build-essential make python-all libssl-dev -qq -y
RUN apt-get install openssh-server -qq -y
ENV HOME="/root"
...
RUN source $NVM_DIR/nvm.sh ; nvm install $NODE_VERSION \
 ...
 && nvm use default
RUN apt-get install ca-certificates g++ gcc libc6-dev curl mercurial bzr git-core -qq -y
