...
#  c++
RUN apt-get update \
 && apt-get install libgtest-dev cmake -y
...
RUN apt-get update \
 && apt-get install librdmacm-dev -y
RUN git clone https://github.com/toolslive/ordma.git \
 ...
 && make install
RUN apt-get update \
 && apt-get install libboost1.58-all-dev libboost1.58-all -y --allow-unauthenticated
RUN echo "deb http://apt.openvstorage.org unstable main" > /etc/apt/sources.list.d/ovsaptrepo.list
RUN apt-get update \
 && apt-get install clang-3.5 liblttng-ust0 librdmacm1 libtokyocabinet9 libstdc++6:amd64 librabbitmq1 libomnithread3c2 libomniorb4-1 libhiredis0.13 liblz4-1 libxio-dev libxio0 omniorb-nameserver libunwind8-dev libaio1 libaio1-dbg libaio-dev libz-dev libbz2-dev libgoogle-glog-dev libibverbs-dev -y --allow-unauthenticated
#  Install etcd:
