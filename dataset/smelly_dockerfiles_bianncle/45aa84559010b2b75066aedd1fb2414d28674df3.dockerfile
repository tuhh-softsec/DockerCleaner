...
RUN if [ "$( go env GOHOSTARCH ;)" = "amd64" ] ; then apt-get update \
 && apt-get install libseccomp-dev:armhf libseccomp-dev:arm64 libseccomp-dev:armel libapparmor-dev:armhf libapparmor-dev:arm64 libapparmor-dev:armel libapparmor-dev libseccomp-dev -y ; fi
