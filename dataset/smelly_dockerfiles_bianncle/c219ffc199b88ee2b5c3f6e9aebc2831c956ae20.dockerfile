...
RUN echo "cd /var/www; mkdir -p ./public/submission_images/; chown www-data:www-data public/submission_images -R; mkdir -p ./var/cache/; chown www-data:www-data ./var/cache/ -R; mkdir -p ./public/media/; chown www-data:www-data public/media -R; composer install \
 && composer dump-autoload --optimize --no-dev --classmap-authoritative; npm run build-${app_env}; cd /var/www/public; cp /tmp/.env /var/www/.env; rm /tmp/.env*; php-fpm" > /start.sh \
 && chmod +x /start.sh
CMD /start.sh
