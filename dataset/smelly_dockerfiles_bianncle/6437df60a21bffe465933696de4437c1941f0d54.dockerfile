...
RUN DEBIAN_FRONTEND=noninteractive apt-get update \
 && apt-get upgrade -y \
 && apt-get install --no-install-recommends python-software-properties -y --no-install-suggests \
 && apt-get install --no-install-recommends software-properties-common -y --no-install-suggests
RUN add-apt-repository -y ppa:openjdk-r/ppa \
 && apt-get update \
 && apt-get install --no-install-recommends openjdk-8-jdk wget -y --no-install-suggests
