...
FROM ubuntu:16.04
RUN apt-get update \
 && apt-get install gcc alien fakeroot rpm git python-dev python-pip desktop-file-utils wget sudo libgstreamer1.0-dev libgstreamer-plugins-base1.0-dev libgstreamer-plugins-base0.10-0 libgstreamer0.10-0 libgstreamer0.10-dev autoconf2.13 build-essential ccache python-dev python-pip python-setuptools unzip uuid zip libasound2-dev libcurl4-openssl-dev libdbus-1-dev libdbus-glib-1-dev libgconf2-dev libgtk-3-dev libgtk2.0-dev libiw-dev libnotify-dev libpulse-dev libx11-xcb-dev libxt-dev mesa-common-dev python-dbus xvfb yasm apt-transport-https -y
RUN pip install awscli
RUN echo "deb http://repo.aptly.info/ squeeze main" > /etc/apt/sources.list.d/aptly.list; apt-key adv --keyserver pool.sks-keyservers.net --recv-keys ED75B5A4483DA07C ; apt-get update ; apt-get install aptly -y
ENV RUSTUP_HOME="/usr/local/rustup" \
    CARGO_HOME="/usr/local/cargo" \
    PATH="/usr/local/cargo/bin:$PATH"
RUN echo "deb http://ppa.launchpad.net/mercurial-ppa/releases/ubuntu xenial main" > /etc/apt/sources.list.d/mercurial.list; apt-key adv --keyserver pool.sks-keyservers.net --recv-keys 41BD8711B1F0EC2B0D85B91CF59CE3A8323293EE ; apt-get update ; apt-get install mercurial -y
RUN wget -qO- https://deb.nodesource.com/gpgkey/nodesource.gpg.key | apt-key add -
RUN echo "deb https://deb.nodesource.com/node_10.x xenial main" > /etc/apt/sources.list.d/nodesource.list; apt-get update ; apt-get install nodejs -y
