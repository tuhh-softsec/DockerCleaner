...
RUN wget -O - ${LIBVA_UTILS_REPO} | tar xz ; cd libva-utils-${LIBVA_UTILS_VER} ; ./autogen.sh --prefix=/usr --libdir=/usr/lib64 ; make -j8 ; make install DESTDIR=/home/build ; make install
#  Build media driver
...
WORKDIR /home
#  Prerequisites
RUN yum install -y epel-release ; yum install -y -q libxcb SDL2 libass numactl libvdpau libpciaccess libpciaccess libX11 mesa-dri-drivers mesa-libGL libdrm ; yum remove -y -q epel-release ; rm -rf /var/cache/yum/*
#  Install
