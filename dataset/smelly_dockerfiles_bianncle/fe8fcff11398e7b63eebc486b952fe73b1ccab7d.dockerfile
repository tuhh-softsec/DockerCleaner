...
RUN curl https://packages.grafana.com/gpg.key | apt-key add - \
 && apt-get update -y \
 && apt-get -y upgrade \
 && apt-get install vim nginx python-dev python-flup python-pip python-ldap expect git memcached sqlite3 libcairo2 libcairo2-dev python-cairo python-rrdtool pkg-config nodejs grafana -y --force-yes \
 && apt-get clean
#  python dependencies
