...
RUN apt-get update \
 && apt-get install --no-install-recommends apt-utils apt-transport-https build-essential software-properties-common python-software-properties daemontools curl wget vim git zip bzip2 libcurl3-dev libfreetype6-dev libpng12-dev libzmq3-dev pkg-config python-dev python3-dev python-numpy python3-numpy python-six python3-six python-wheel python3-wheel python-pip python3-pip rsync software-properties-common swig unzip zip zlib1g-dev locales -y \
 && apt-get clean \
 && rm -rf /var/lib/apt/lists/*
RUN locale-gen en_US.UTF-8 \
 && localedef -i en_US -f UTF-8 en_us.UTF-8
#  Pin normal pip and pip3 to 9.0.3
RUN pip install pip==9.0.3 --ignore-installed --no-cache --upgrade \
