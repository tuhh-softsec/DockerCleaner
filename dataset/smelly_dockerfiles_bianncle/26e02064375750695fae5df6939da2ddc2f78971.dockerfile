...
RUN ln -s /opt/instantclient/ /opt/instantclient/lib
# Setup user
ARG UID
ARG GID
...
RUN apt-key adv --recv-keys --keyserver hkp://keyserver.ubuntu.com:80 16126D3A3E5C1192 \
 && apt-get update \
 && apt-get install --no-install-recommends software-properties-common -y --fix-missing \
 && add-apt-repository ppa:ubuntugis/ubuntugis-unstable -y \
 && apt-get update \
 && apt-get install --no-install-recommends gcc-4.9 g++-4.9 build-essential ca-certificates curl wget git libaio1 make cmake python-dev python-software-properties software-properties-common libc6-dev openssh-client libpng12-dev libjpeg-dev libgif-dev liblzma-dev libgeos-dev libproj-dev libxml2-dev libexpat-dev libxerces-c-dev libnetcdf-dev netcdf-bin libpoppler-dev libspatialite-dev swig libhdf5-serial-dev libpodofo-dev poppler-utils libfreexl-dev libwebp-dev libepsilon-dev libpcre3-dev gfortran libarpack2-dev libpq-dev libflann-dev libhdf5-serial-dev libhdf5-dev libjsoncpp-dev clang-3.8 libhdf4-alt-dev libsqlite3-dev libltdl-dev libcurl4-openssl-dev ninja python-pip libpng-dev python-dev libprotobuf-c-dev protobuf-c-compiler libboost-filesystem1.58-dev libboost-iostreams1.58-dev libboost-program-options1.58-dev libboost-system1.58-dev libboost-thread1.58-dev libogdi3.2-dev time -y --fix-missing \
...
RUN wget https://bootstrap.pypa.io/get-pip.py -O get-pip.py \
 && python get-pip.py \
 && pip install pip wheel numpy --upgrade
ARG GDAL_VERSION
RUN cd /build \
 ...
 && rm -Rf /build/gdal
RUN apt-get update \
 && apt-get install --no-install-recommends build-essential ca-certificates curl wget subversion git libaio1 make cmake python-numpy python-dev python-software-properties software-properties-common libv8-dev libc6-dev libfreetype6-dev libcairo2-dev libpq-dev libharfbuzz-dev libfribidi-dev flex bison libfcgi-dev libxml2 libxml2-dev bzip2 apache2 apache2-utils apache2-dev libaprutil1-dev libapr1-dev libpng12-dev libjpeg-dev libcurl4-gnutls-dev libpcre3-dev libpixman-1-dev libgeos-dev libsqlite3-dev libdb-dev libtiff-dev sudo -y --fix-missing \
...
RUN cd /build \
 ...
 && cmake -DCMAKE_BUILD_TYPE=Release -DHARFBUZZ_INCLUDE_DIR=/usr/include/harfbuzz -DWITH_CLIENT_WFS=ON -DWITH_CLIENT_WMS=ON -DWITH_CURL=ON -DWITH_GDAL=ON -DWITH_GIF=ON -DWITH_ICONV=ON -DWITH_KML=ON -DWITH_LIBXML2=ON -DWITH_OGR=ON -DWITH_ORACLESPATIAL=ON -DWITH_POINT_Z_M=ON -DWITH_PROJ=ON -DWITH_SOS=ON -DWITH_THREAD_SAFETY=ON -DWITH_WCS=ON -DWITH_WFS=ON -DWITH_WMS=ON -DWITH_FCGI=ON -DWITH_FRIBIDI=ON -DWITH_CAIRO=ON -DWITH_HARFBUZZ=ON -DWITH_POSTGIS=on -DWITH_V8=OFF .. \
 && make \
 && make install \
 && ldconfig \
