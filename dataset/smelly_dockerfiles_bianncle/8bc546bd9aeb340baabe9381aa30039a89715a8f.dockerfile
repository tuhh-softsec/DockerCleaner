...
RUN set -x \
 ...
 && git checkout -q "$REGISTRY_COMMIT_SCHEMA1" ) ; GOPATH="$GOPATH/src/github.com/docker/distribution/Godeps/_workspace:$GOPATH" ; go build -buildmode=pie -o /usr/local/bin/registry-v2-schema1 github.com/docker/distribution/cmd/registry ;; esac \
 && rm -rf "$GOPATH"
