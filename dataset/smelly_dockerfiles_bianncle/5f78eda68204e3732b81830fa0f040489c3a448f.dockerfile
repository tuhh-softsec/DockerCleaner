...
RUN apt-get update \
 && apt-get install --no-install-recommends apparmor apt-utils aufs-tools automake bash-completion binutils-mingw-w64 bsdmainutils btrfs-tools build-essential cmake createrepo curl dpkg-sig gcc-mingw-w64 git iptables jq less libapparmor-dev libcap-dev libdevmapper-dev libnet-dev libnl-3-dev libprotobuf-c0-dev libprotobuf-dev libseccomp-dev libsystemd-dev libtool libudev-dev mercurial net-tools pkg-config protobuf-compiler protobuf-c-compiler python-backports.ssl-match-hostname python-dev python-mock python-pip python-requests python-setuptools python-websocket python-wheel tar thin-provisioning-tools vim vim-common xfsprogs zip -y \
