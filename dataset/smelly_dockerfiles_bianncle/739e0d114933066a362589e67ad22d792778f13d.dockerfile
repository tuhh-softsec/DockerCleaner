...
RUN useradd -p locked -m coreboot \
 && apt-get update -qq \
 && apt-get install bc bison bzip2 ccache cmake curl device-tree-compiler dh-autoreconf diffutils doxygen flex g++ gawk gcc git gnat graphviz libfreetype6-dev libftdi-dev libftdi1-dev libglib2.0-dev libgmp-dev libjaylink-dev liblzma-dev libncurses5-dev libpci-dev libreadline-dev libssl1.0-dev libusb-1.0-0-dev libusb-dev libxml2-dev libyaml-dev m4 make msitools nasm openssl patch pkg-config python qemu rsync shellcheck subversion unifont uuid-dev vim-common wget xz-utils zlib1g-dev -qqy \
 && apt-get clean
RUN cd /root \
