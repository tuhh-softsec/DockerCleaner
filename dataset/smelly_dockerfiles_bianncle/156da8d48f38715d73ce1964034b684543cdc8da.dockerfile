...
ENV PATH=".:$PATH"
RUN apt-get update
RUN apt-get install git wget unzip curl net-tools build-essential npm python python-setuptools python-dev python-numpy openssh-server sysstat -y
...
# ## PYTHON ###
RUN apt-get install python-pip python-matplotlib python-pandas ipython python-nose -y
