...
RUN set -ex \
 && apt-get update \
 && apt-get install --no-install-recommends ca-certificates cmake g++ make pkg-config graphviz doxygen git curl libtool-bin autoconf automake bzip2 xsltproc gperf python3-dev --yes
WORKDIR /usr/local
# Cmake
