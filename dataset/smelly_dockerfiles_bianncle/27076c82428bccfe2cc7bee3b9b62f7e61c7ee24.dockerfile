...
RUN echo "deb http://cran.rstudio.com/bin/linux/debian jessie-cran3/" >> /etc/apt/sources.list \
 ...
 && apt-get install libcurl4-openssl-dev libxml2-dev libssl-dev libpng-dev -y \
 && apt-get install jessie-backports ca-certificates-java openjdk-8-jre-headless -y -t \
 && apt-get install wget gdebi-core openjdk-8-jdk curl -y \
 && apt-get -y clean
