...
RUN echo -e '@edge http://nl.alpinelinux.org/alpine/edge/main\n@testing http://nl.alpinelinux.org/alpine/edge/testing' >> /etc/apk/repositories \
 ...
 && apk upgrade \
 && apk add --no-cache openssl build-base linux-headers unzip wget cmake libtbb@testing libtbb-dev@testing libjpeg libjpeg-turbo-dev libpng-dev tiff-dev libjasper \
 && pip install numpy
RUN cd \
