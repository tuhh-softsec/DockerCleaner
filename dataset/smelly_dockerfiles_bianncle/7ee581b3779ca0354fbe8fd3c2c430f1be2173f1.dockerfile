...
RUN set -eux \
 ...
 && apt-get ${APT_FLAGS_COMMON} update \
 && DEBIAN_FRONTEND=noninteractive apt-get ${APT_FLAGS_PERSISTENT} install apache2 curl libapache2-mod-php php7.2-bcmath php7.2-gd php7.2-json php7.2-ldap php7.2-mbstring php7.2-xml php7.2-pgsql postgresql-client ttf-dejavu-core \
...
RUN set -eux \
 && apt-get ${APT_FLAGS_COMMON} update \
 && DEBIAN_FRONTEND=noninteractive apt-get ${APT_FLAGS_DEV} install gettext patch ca-certificates git \
 && cd /usr/share/ \
 ...
 && DEBIAN_FRONTEND=noninteractive apt-get ${APT_FLAGS_COMMON} purge gettext patch ca-certificates git \
 && apt-get ${APT_FLAGS_COMMON} autoremove \
 && apt-get ${APT_FLAGS_COMMON} clean \
 && rm -rf /var/lib/apt/lists/*
