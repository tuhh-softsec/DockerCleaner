...
RUN apt-get update \
 && apt-get install git gcc g++ build-essential libnetcdf-dev netcdf-bin python-setuptools python-dev python-pip python-scipy python-numpy python-matplotlib python-shapely python-pyproj python-gdal gdal-bin python-imaging gmsh python-py python-netcdf4 libgeos-c1v5 libgeos-3.5.0 libgeos-dev wget -y
#  Upgrade pip
RUN pip install pip setuptools -i https://pypi.python.org/simple/ --upgrade
#  Install ScientificPython
RUN pip install :all: ScientificPython --force-reinstall --ignore-installed --no-binary --no-cache-dir --no-binary
#  Update dap and shapely with recent versions
RUN pip install Pydap==3.2.1
RUN pip install geos -U
RUN pip install :all: Shapely==1.5.9 --no-binary
