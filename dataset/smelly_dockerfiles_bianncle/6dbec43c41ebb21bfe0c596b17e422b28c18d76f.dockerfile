...
RUN apt-get update \
 && apt-get install bzip2 wget kurento-media-server --yes ; apt-get clean \
 && rm -rf /var/lib/apt/lists/*
#  Install additional modules
#  These might not be available in Ubuntu 18.04 (Bionic)
RUN apt-get update \
 && apt-get install kms-chroma kms-crowddetector kms-platedetector kms-pointerdetector --yes || true ; apt-get clean \
 && rm -rf /var/lib/apt/lists/*
EXPOSE 8888/tcp
