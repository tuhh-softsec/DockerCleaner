...
RUN (apt-key adv --keyserver hkp://keyserver.ubuntu.com:80 --recv-keys 44FC67F19B2466EA || apt-key adv --keyserver hkp://p80.pool.sks-keyservers.net:80 --recv-keys 44FC67F19B2466EA ) \
 ...
 && apt-get update \
 && apt-get install curl dvipng gcc git golang-1.11-go libssl-dev libzmq3-dev make openssl python3 python3-grpcio-tools python3-pip python3-sawtooth-cli software-properties-common sudo texlive-fonts-recommended texlive-latex-base texlive-latex-extra texlive-latex-recommended unzip zip -y -q \
 && apt-get clean \
