...
RUN apt-get update
RUN apt-get install python-pip aptitude rsh-client rsh-redone-client talk avahi-daemon cups isc-dhcp-server ntp rpcbind nfs-kernel-server bind9 openssh-client openssh-server python-dev slapd nis libffi-dev libssl-dev -y
...
WORKDIR /cis-ubuntu-ansible
RUN apt-get update
RUN apt-get install python-pip aptitude rsh-client rsh-redone-client talk avahi-daemon cups isc-dhcp-server ntp rpcbind nfs-kernel-server bind9 openssh-client openssh-server python-dev slapd nis sudo libffi-dev -y
...
WORKDIR /cis-ubuntu-ansible
RUN apt-get update
RUN apt-get install python-pip aptitude rsh-client rsh-redone-client talk avahi-daemon cups isc-dhcp-server ntp rpcbind nfs-kernel-server bind9 openssh-client openssh-server python-dev sudo libffi-dev libssl-dev -y
...
WORKDIR /cis-ubuntu-ansible
RUN apt-get update
RUN apt-get install python-pip aptitude rsh-client rsh-redone-client talk avahi-daemon cups isc-dhcp-server ntp rpcbind nfs-kernel-server bind9 openssh-client openssh-server python-dev libffi-dev libssl-dev -y
RUN DEBIAN_FRONTEND=noninteractive apt-get -y install slapd nis
RUN pip install ansible
RUN touch /etc/inetd.conf
RUN echo 'shell.bla' > /tmp/inetd
