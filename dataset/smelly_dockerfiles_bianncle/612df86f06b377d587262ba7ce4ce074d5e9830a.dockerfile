...
RUN apt-get update \
 && apt-get install --no-install-recommends cdbs icedtea-netx libapparmor-dev libgsl0-dev libhiredis-dev libleptonica-dev libmpfr-dev libpoppler-cpp-dev libprotobuf-dev librdf0-dev libsasl2-dev libtesseract-dev libwebp-dev libxslt1-dev mdbtools protobuf-compiler python-pip python-pdftools tesseract-ocr-eng -y \
 && R CMD javareconf \
 && apt-get clean \
