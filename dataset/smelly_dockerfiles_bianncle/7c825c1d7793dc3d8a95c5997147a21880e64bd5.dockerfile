...
RUN apt-get update \
 && apt-get upgrade -y \
 && apt-get install --no-install-recommends apt-utils sendmail-bin sendmail sudo libbz2-dev libjpeg62-turbo-dev libpng-dev libfreetype6-dev libgeoip-dev wget libgmp-dev libmagickwand-dev libmagickcore-dev libc-client-dev libkrb5-dev libicu-dev libldap2-dev libmcrypt-dev libpspell-dev librecode0 librecode-dev libssh2-1 libssh2-1-dev libtidy-dev libxslt1-dev libyaml-dev libzip-dev zip -y \
