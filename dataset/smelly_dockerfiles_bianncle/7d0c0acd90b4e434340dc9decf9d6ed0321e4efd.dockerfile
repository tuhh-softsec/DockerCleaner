...
#  For GDAL
RUN apk add --no-cache linux-headers curl-dev zlib-dev zstd-dev libjpeg-turbo-dev libpng-dev libwebp-dev openjpeg-dev
#  Build openjpeg
...
RUN date
RUN apk add --no-cache libstdc++ sqlite-libs libcurl zlib zstd-libs libjpeg-turbo libpng openjpeg libwebp \
