...
RUN apt-get update -y \
 && apt-get install wget git g++ make curl libssl-dev libevent-dev libcurl4-openssl-dev libsnappy-dev ncurses-dev openssl libiodbc2-dev autoconf cmake libtool python python-dev golang-go subversion cmake gnupg openjdk-8-jdk openjdk-8-jre lsb-release -y \
 && cd $WDIR \
