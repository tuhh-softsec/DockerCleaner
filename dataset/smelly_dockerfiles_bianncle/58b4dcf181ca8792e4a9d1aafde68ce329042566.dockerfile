...
RUN apt-get update \
 && apt-get install build-essential zlib1g-dev unzip libncurses5-dev curl wget python python-pip python-dev cmake libboost-all-dev libxml2-dev libcurl4-gnutls-dev software-properties-common apt-transport-https default-jre default-jdk less vim libtbb-dev git -y --fix-missing
RUN apt-key adv --keyserver hkp://keyserver.ubuntu.com:80 --recv-keys E298A3A825C0D65DFD57CBB651716619E084DAB9
RUN add-apt-repository 'deb [arch=amd64,i386] https://cran.rstudio.com/bin/linux/ubuntu xenial/'
RUN apt-get update
RUN apt-get install r-base=${R_VERSION}-1xenial0 r-recommended=${R_VERSION}-1xenial0 -y --fix-missing
RUN apt-get install r-base-core=${R_VERSION}-1xenial0 -y --fix-missing --allow-downgrades
