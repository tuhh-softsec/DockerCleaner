...
RUN set -x \
 && apk --no-cache add -t .build-deps apache2-dev apr-dev apr-util-dev build-base curl icu-dev libjpeg-turbo-dev linux-headers gperf openssl-dev pcre-dev python zlib-dev g++ subversion fakeroot git \
