...
RUN echo "deb ${DEBIAN_REPO}/${DISTRO} ${CODENAME} main" >> /etc/apt/sources.list \
 ...
 && apt-get install --no-install-recommends bash-completion bzip2 cmake dnsutils ffmpeg fonts-dejavu fonts-liberation g++ gcc git info jq kstart less libaio1 lmodern luarocks make man netcat openresty openresty-opm openssh-client procps psmisc rsync sudo sssd unzip vim wget zlib1g-dev -yq \
 && apt-get clean \
