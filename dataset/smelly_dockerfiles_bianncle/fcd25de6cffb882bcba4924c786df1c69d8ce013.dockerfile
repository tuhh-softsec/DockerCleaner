...
RUN echo "deb http://packages.ros.org/ros/ubuntu $( lsb_release -sc ;) main" > /etc/apt/sources.list.d/ros-latest.list \
 ...
 && apt-get update -y \
 && apt-get install --no-install-recommends apt-transport-https aptitude -y -q \
 && aptitude install -y -q -R ros-indigo-desktop python-rosinstall ninja-build python3-pip python3-setuptools ros-indigo-rosbridge-library ros-indigo-rosbridge-server ros-indigo-rosbridge-suite
...
RUN echo "deb http://packages.osrfoundation.org/gazebo/ubuntu $( lsb_release -sc ;) main" > /etc/apt/sources.list.d/gazebo-latest.list \
 ...
 && aptitude update -y -q \
 && aptitude remove -y -q gazebo2 ; aptitude install -y -q -R gazebo7
RUN aptitude install -y -q -R ros-indigo-gazebo7-ros-control ros-indigo-gazebo7-ros-pkgs
...
RUN aptitude install -y -q -R python3-dev libyaml-dev libyaml-0-2 libyaml-cpp0.5 libyaml-cpp-dev \
 && pip3 install --upgrade pip \
 && pip3 install catkin-tools rospkg ws4py transforms3d trollius defusedxml
