...
RUN set -x \
 ...
 && : "OpenCV dependencies" \
 && apt-get install libjpeg-dev libpng++-dev libtiff-dev libopenexr-dev libwebp-dev ffmpeg libavcodec-dev libavformat-dev libavutil-dev libswscale-dev libavresample-dev -y -qq \
 && : "other dependencies" \
...
RUN set -x \
 && apt-get update -y -qq \
 && apt-get upgrade -y -qq --no-install-recommends \
 && apt-get install autogen autoconf libtool -y -qq \
