...
RUN apt-get update -qq \
 && apt-get install --no-install-recommends curl ffmpeg g++ gcc gfortran git graphviz graphviz-dev less libblosc-dev libedit-dev libfreetype6-dev libgmp-dev libhdf5-dev libjpeg-dev libnetcdf-dev libopenblas-dev libpng3-dev libssl-dev libxml2-dev libxslt-dev libzmq3-dev llvm-3.7-dev pandoc pkg-config python3 python3-dev vim zip -yqq
RUN curl https://bootstrap.pypa.io/get-pip.py > get-pip.py \
 && python3 get-pip.py \
 && rm get-pip.py
RUN pip install Cython numpy -q
