...
RUN apt-get update \
 && apt-get install --no-install-recommends apt-utils ca-certificates locales sudo aptitude gcc g++ wget git vim build-essential make curl -y
...
RUN apt-get update \
 && apt-get install --no-install-recommends libxml2-dev xml-core libgfortran-6-dev libpq-dev python3.5 python3-dev python3-six python3-numpy python3-setuptools python3-numexpr python3-pip libxml2-dev libxslt-dev libxslt1-dev zlib1g-dev -y
#  UPDATE AND CLEAN
RUN apt-get update \
 && apt-get autoclean \
...
# #######################################################################
RUN apt-get update \
 && apt-get install --no-install-recommends libtinfo-dev libzmq3-dev libcairo2-dev libpango1.0-dev libmagic-dev libblas-dev liblapack-dev -y
# USER notebooks
#
