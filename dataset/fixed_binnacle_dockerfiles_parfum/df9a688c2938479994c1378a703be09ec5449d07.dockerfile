...
RUN apt-get install --no-install-recommends libpython2.7-dev libpython3-dev libffi-dev libssl-dev python-setuptools python3-setuptools -y
...
RUN apt-get update
RUN apt-get install --no-install-recommends openjdk-8-jdk hadoop-conf-pseudo -y
