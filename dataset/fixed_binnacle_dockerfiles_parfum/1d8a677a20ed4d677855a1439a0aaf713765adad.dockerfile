...
#  Install prereqs:
RUN apt-get install --no-install-recommends apt-transport-https ca-certificates curl software-properties-common -y
...
RUN apt-get install --no-install-recommends ebtables ethtool kmod kubernetes-cni libwrap0 systemd tcpd docker-ce=${VERSION_DOCK}~ce-0~ubuntu-xenial -y
