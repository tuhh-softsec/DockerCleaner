...
RUN get_source_from_github() { \
 ...
 && apt-get update \
 && apt-get upgrade -qy \
 && apt-get install --no-install-recommends -qy apache2 build-essential curl ghostscript git gnupg imagemagick inkscape libapache2-mod-wsgi-py3 libfile-mimeinfo-perl libimage-exiftool-perl libjpeg-dev libmagickwand-dev libpq-dev libreoffice locales mysql-client poppler-utils postgresql-client python3 python3-dev python3-pip python3-venv qpdf redis-server supervisor uwsgi uwsgi-plugin-python3 vim zlib1g-dev \
