...
RUN apt-get update \
 && apt-get install --no-install-recommends libpng12-test libjpeg-test libcurl4-openssl-test -y \
...
RUN apt-get update \
 && apt-get install --no-install-recommends bzip2 libbz2-test libfreetype6-test libjpeg62-turbo-test libmcrypt-test libpng12-test libghc-postgresql-libpq-test -y \
...
RUN apt-get update \
 && apt-get install --no-install-recommends zlib1g-test libicu-test g++ -y \
...
RUN apt-key adv --keyserver keyserver.ubuntu.com --recv-keys 40976EAF437D05B5 \
 && apt-get update
#  for CakePHP
RUN apt-get install --no-install-recommends emacs-nox less composer -y
RUN apt-get install --no-install-recommends mysql-client postgresql-client php-intl php-mbstring php-xml php-zip -y
ENV TERM="xterm"
