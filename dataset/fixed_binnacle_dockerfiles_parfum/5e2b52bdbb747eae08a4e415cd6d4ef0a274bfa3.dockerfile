...
CMD ["/usr/sbin/mysqld"]
# # python related
RUN apt-get install --no-install-recommends python-software-properties python python-setuptools git -y
RUN apt-get install --no-install-recommends libmysqlclient-dev -y
RUN apt-get install --no-install-recommends libxml2-dev libxslt1-dev python-dev -y
