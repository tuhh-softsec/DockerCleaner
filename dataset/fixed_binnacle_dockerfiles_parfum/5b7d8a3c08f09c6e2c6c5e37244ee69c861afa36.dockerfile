...
RUN apt-get install --no-install-recommends python${PYTHON_VERSION} python${PYTHON_VERSION}-dev -y
...
RUN if [[ ${MPI_KIND} == "OpenMPI" ]] ; then wget -O /tmp/openmpi-3.0.0-bin.tar.gz https://github.com/horovod/horovod/files/1596799/openmpi-3.0.0-bin.tar.gz \
 ...
 && ldconfig \
 && echo "mpirun -allow-run-as-root -np 2 -H localhost:2 -bind-to none -map-by slot -mca mpi_abort_print_stack 1" > /mpirun_command; else apt-get install --no-install-recommends mpich -y \
