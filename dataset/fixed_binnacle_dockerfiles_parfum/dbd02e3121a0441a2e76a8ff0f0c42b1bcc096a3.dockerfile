...
RUN apt-get install --no-install-recommends blender -y
RUN apt-get install --no-install-recommends python3-yaml python3-pip -y
...
RUN apt-get install --no-install-recommends mplayer python-pycurl libcurl3 -y
RUN apt-get install --no-install-recommends libavbin-dev libavbin0
RUN git clone https://github.com/geni-lab/ITFTalk
#  Install ITFListen and prerequisites
WORKDIR /catkin_ws/src
RUN apt-get install --no-install-recommends sox python-simplejson -y
...
RUN apt-get install --no-install-recommends autoconf libtool g++ libportaudio2 -y
