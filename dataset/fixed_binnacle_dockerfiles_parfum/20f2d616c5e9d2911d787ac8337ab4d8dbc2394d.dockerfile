...
RUN apt-get install --no-install-recommends postfix postfix-ldap postfix-pcre postfix-policyd-spf-python libsasl2-modules -y
RUN apt-get install --no-install-recommends rsyslog fetchmail libdbi-perl libdbd-pg-perl libdbd-mysql-perl liblockfile-simple-perl -y
...
RUN apt-get install --no-install-recommends dovecot-core dovecot-imapd dovecot-pop3d dovecot-lmtpd dovecot-mysql dovecot-ldap dnsutils -y
# RUN apt-get install -y mail-stack-delivery // this was used instead of dovecto one by one installation
RUN apt-get install --no-install-recommends dovecot-sieve dovecot-managesieved -y
#  installing ldap utils for testing perposes
RUN apt-get install --no-install-recommends ldap-utils curl -y
...
RUN apt-get install --no-install-recommends gnupg software-properties-common -y
...
COPY https://raw.github.com/infertux/zeyple/master/zeyple/zeyple.conf.example /etc/zeyple.conf
...
RUN apt-get -y --no-install-recommends install clamav-daemon clamav
