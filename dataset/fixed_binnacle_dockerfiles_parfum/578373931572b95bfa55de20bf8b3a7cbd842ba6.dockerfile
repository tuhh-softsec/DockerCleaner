...
RUN apt-get install --no-install-recommends git build-essential autoconf automake autotools-dev libtool pkg-config -y
#
#  Prepare build environment for mruby and ts_mruby
#
RUN apt-get install --no-install-recommends bison libreadline6 libreadline6-dev ncurses-dev ruby unzip -y
#
#  Prepare develop environment
#
RUN apt-get install --no-install-recommends curl vim valgrind wget -y
#
...
#
RUN apt-get install --no-install-recommends g++ libssl-dev tcl-dev libpcre3-dev -y
RUN ldconfig
#
