...
RUN echo 'apt::install-recommends "false";' > /etc/apt/apt.conf.d/no-install-recommends \
 && apt-get update -y \
 && apt-get install --no-install-recommends -y apt-transport-https gnupg ca-certificates curl \
 && echo "deb https://packages.sury.org/php/ stretch main" > /etc/apt/sources.list.d/sury.org.list \
 && curl -sS https://packages.sury.org/php/apt.gpg | apt-key add - \
 && apt-get update -y \
 && apt-get upgrade -y \
 && apt-get install --no-install-recommends -y iptables php7.1-bcmath php7.1-bz2 php7.1-cli php7.1-curl php7.1-fpm php7.1-gd php7.1-gmp php7.1-imap php7.1-interbase php7.1-intl php7.1-json php7.1-ldap php7.1-mbstring php7.1-mcrypt php7.1-mysql php7.1-odbc php7.1-opcache php7.1-pgsql php7.1-readline php7.1-soap php7.1-sqlite3 php7.1-xml php7.1-xmlrpc php7.1-zip php-apcu php-geoip php-imagick php-ssh2 php-tideways \
 ...
 && apt-get install --no-install-recommends -y build-essential libmemcached11 libmemcachedutil2 libmemcached-dev php-pear php7.1-dev pkg-config zlib1g-dev \
