...
# 		pyasn1
RUN apt-get update \
 && apt-get install --no-install-recommends python-numpy python-scipy python-nose python-h5py python-skimage python-matplotlib python-pandas python-sklearn python-sympy python-skimage python-progressbar -y \
 && apt-get clean \
 && apt-get autoremove \
 && rm -rf /var/lib/apt/lists/*
...
RUN apt-get update -qqy \
 && apt-get install --no-install-recommends curl gcc python-dev python-setuptools apt-transport-https lsb-release openssh-client git -qqy \
 && easy_install -U pip \
...
RUN apt-get update
RUN apt-get install --no-install-recommends sbt
