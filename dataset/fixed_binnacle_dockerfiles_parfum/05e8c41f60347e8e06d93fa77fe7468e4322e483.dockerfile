...
RUN apt-get update \
 && apt-get install --no-install-recommends xorg-dev libgl1-mesa-dev xvfb libxinerama1 libxcursor1 unzip libglu1-mesa libav-tools python-numpy python-scipy python-pyglet python-setuptools libpq-dev libjpeg-dev wget curl cmake git -y \
 && apt-get clean \
...
RUN sudo apt-get install --no-install-recommends gazebo7 libgazebo7-dev -y
#  setup environment
EXPOSE 11345/tcp
...
RUN sudo apt-get install --no-install-recommends gfortran -y
#  install sript specific dependencies (temporal)
RUN sudo apt-get install --no-install-recommends python-skimage -y
#  install Theano
