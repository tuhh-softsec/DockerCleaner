...
RUN apk add --no-cache --virtual .persistent-deps ca-certificates curl apache2 apache2-utils xz libressl openssh-client mysql-client
...
RUN set -xe \
 && apk add --no-cache --virtual .build-deps $PHPIZE_DEPS curl-dev libedit-dev libxml2-dev libressl-dev apache2-dev freetype-dev libjpeg-turbo-dev libpng-dev libsodium-dev \
