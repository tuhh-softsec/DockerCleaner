...
RUN apt-get update \
 ...
 && apt-get install --no-install-recommends less groff -y -qq \
 && dpkg --add-architecture i386 \
 && apt-get update \
 && apt-get install --no-install-recommends expect libc6-i386 lib32stdc++6 lib32gcc1 lib32ncurses5 lib32z1 libqt5widgets5 -y --force-yes \
 && apt-get clean \
