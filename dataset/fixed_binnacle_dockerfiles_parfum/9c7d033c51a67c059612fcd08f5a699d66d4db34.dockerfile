...
RUN apt-get update \
 && apt-get install --no-install-recommends gcc vim git wkhtmltopdf xvfb python2.7 python-pip python-dev build-essential libssl-dev libffi-dev apache2 libapache2-mod-wsgi -y --fix-missing \
