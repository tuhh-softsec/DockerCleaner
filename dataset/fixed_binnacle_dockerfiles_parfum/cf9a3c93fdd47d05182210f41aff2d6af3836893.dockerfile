...
RUN apt-get update \
 && apt-get install --no-install-recommends wget git build-essential golang-1.8 -y
...
#
WORKDIR /build/glib
RUN apt-get install --no-install-recommends dh-autoreconf -y
RUN apt-get install --no-install-recommends libffi-dev -y
RUN apt-get install --no-install-recommends libmount-dev -y
RUN apt-get install --no-install-recommends zlib1g-dev -y
RUN apt-get install --no-install-recommends libpcre3-dev -y
RUN apt-get install --no-install-recommends gtk-doc-tools -y
RUN ./autogen.sh
RUN ./configure --prefix=/usr/local
RUN make -j$( nproc ;) \
 && make install
WORKDIR /build/gstreamer
RUN apt-get install --no-install-recommends libbison-dev -y
...
RUN apt-get install --no-install-recommends libopus-dev -y
RUN ./autogen.sh
RUN ./configure --prefix=/usr/local
RUN make -j$( nproc ;) \
 && make install
WORKDIR /build/gst-plugins-good
RUN apt-get install --no-install-recommends libvpx-dev -y
RUN ./autogen.sh --prefix=/usr/local --disable-gtk-doc-html
RUN make -j$( nproc ;) \
 && make install
RUN apt-get install --no-install-recommends nasm -y
WORKDIR /build
...
RUN apt-get install --no-install-recommends libvo-aacenc-dev -y
...
RUN apt-get install --no-install-recommends libopencv-dev -y
RUN ./autogen.sh --prefix=/usr/local --disable-gtk-doc --disable-gtk-doc-html --disable-hls
RUN make -j$( nproc ;) \
 && make install
WORKDIR /build/gst-plugins-ugly
RUN apt-get install --no-install-recommends libx264-dev -y
...
RUN apt-get install --no-install-recommends libdrm-dev libudev-dev -y
WORKDIR /build/libva
RUN ./autogen.sh \
 && make install
WORKDIR /build/cmrt
...
RUN apt-get install --no-install-recommends libva-dev libudev-dev -y
RUN ./autogen.sh --prefix=/usr/local --disable-gtk-doc --disable-gtk-doc-html
RUN make -j$( nproc ;) \
 && make install
