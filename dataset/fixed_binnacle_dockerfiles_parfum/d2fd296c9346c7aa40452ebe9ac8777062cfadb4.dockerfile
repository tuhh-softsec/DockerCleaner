...
RUN apt-get update -y \
 && apt-get install --no-install-recommends ca-certificates curl dirmngr gnupg haproxy libicu57 libmozjs185-1.0 openssl -y \
...
RUN apt-get update -y \
 && apt-get install --no-install-recommends apt-transport-https build-essential erlang-nox erlang-reltool erlang-dev git libcurl4-openssl-dev libicu-dev libmozjs185-dev python3 libpython3-dev python3-pip python3-sphinx -y
...
RUN set -ex ; curl -s https://deb.nodesource.com/gpgkey/nodesource.gpg.key | apt-key add - ; echo 'deb https://deb.nodesource.com/node_8.x stretch main' > /etc/apt/sources.list.d/nodesource.list; echo 'deb-src https://deb.nodesource.com/node_8.x stretch main' >> /etc/apt/sources.list.d/nodesource.list; apt-get update -y \
 && apt-get install --no-install-recommends nodejs -y; npm install grunt-cli -g
