...
RUN DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y python-setuptools python-flask curl wget python-requests git supervisor mysql-server python-minimal python-mysql.connector vim lsof unzip
...
RUN HASH_PASS=`python -c "import hashlib; m = hashlib.sha1(); m.update('$SUPERVISOR_PASS'); print m.hexdigest()" ` \
 && sed -i -e "s/HASH_PASS/$HASH_PASS/g" /etc/supervisor/supervisord.conf
