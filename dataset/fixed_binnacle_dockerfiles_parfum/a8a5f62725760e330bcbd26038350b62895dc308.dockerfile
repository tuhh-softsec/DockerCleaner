...
RUN apt-get update \
 && apt-get install --no-install-recommends build-essential curl git-core sudo unzip wget gnupg2 software-properties-common -y
...
RUN apt-get install --no-install-recommends python rebar -y
...
RUN wget https://packages.erlang-solutions.com/erlang-solutions_${MIX_VERSION}_all.deb \
 ...
 && sudo apt-get install --no-install-recommends -y elixir
#  install bundler
...
RUN wget -q https://packages.microsoft.com/config/ubuntu/16.04/packages-microsoft-prod.deb \
 ...
 && sudo apt-get update \
 && sudo apt-get install --no-install-recommends -y dotnet-runtime-2.1 dotnet-sdk-2.1
