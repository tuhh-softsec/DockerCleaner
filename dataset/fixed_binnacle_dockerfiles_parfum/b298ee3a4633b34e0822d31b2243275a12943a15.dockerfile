...
RUN apt-get update
# ## Utils
RUN apt-get install --no-install-recommends git python-pip emacs curl nodejs nodejs-legacy npm vim wget -y
...
# ## Shinken
RUN apt-get install --no-install-recommends kaji -y
# ## Plugins
RUN apt-get install --no-install-recommends nagios-plugins -y
...
# ## Apache
RUN apt-get install --no-install-recommends apache2 libapache2-mod-wsgi -y
# ## Supervisor
RUN apt-get install --no-install-recommends supervisor -y
# # Shinken hosts/services configuration
RUN apt-get install --no-install-recommends python-bs4 python-requests -y
# ## Configuration
# # Docker
#  makes `df` work
