...
RUN apt-get update -qy && apt-get install --no-install-recommends python-all python-all-dev python-pip python-virtualenv python-tox python3-all python3-all-dev python3-pip libxml2-dev libev-dev libxslt1-dev nodejs npm postgresql libpq-dev redis-server git mercurial supervisor -qy \
 && apt-get -qy upgrade \
