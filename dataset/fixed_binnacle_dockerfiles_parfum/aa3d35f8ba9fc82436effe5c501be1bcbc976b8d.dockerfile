...
ENV LC_ALL="C.UTF-8"
RUN apt-get update \
 && apt-get install --no-install-recommends cmake git build-essential curl libboost-all-dev libpcre3-dev zlib1g-dev libgcrypt11-dev libicu-dev python libssl-dev libyajl-dev autoconf automake pkg-config net-tools -y \
...
RUN echo '/usr/local/lib/elektra/' > /etc/ld.so.conf.d/elektra.conf \
 && ldconfig
RUN apt-get update \
 && apt-get install --no-install-recommends libasan3 libubsan0 libboost-system1.62.0 libboost-filesystem1.62.0 libboost-thread1.62.0 libssl1.1 libicu57 libyajl2 pwgen -y \
 && rm -rf /var/lib/apt/lists/*
#  prepare
