...
RUN apt-get install --no-install-recommends ruby1.9.1 libruby1.9.1 ruby1.9.1-dev ri1.9.1 libmagickwand-dev libxml2-dev libxslt1-dev nodejs apache2 apache2-threaded-dev build-essential postgresql postgresql-contrib libpq-dev postgresql-server-dev-all libsasl2-dev sudo -y
...
RUN apt-get install --no-install-recommends libxml2-dev libpqxx3-dev libfcgi-dev libboost-dev libboost-regex-dev libboost-program-options-dev libboost-date-time-dev libmemcached-dev automake autoconf -y
RUN cd /tmp \
