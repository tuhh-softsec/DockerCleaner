...
#  Drozer packages
RUN apt-get install --no-install-recommends wget python2.7 python-dev python2.7-dev python-openssl python-twisted python-protobuf bash-completion -y
#  First, install add-apt-repository, sshd and bzip2
RUN apt-get install --no-install-recommends python-software-properties bzip2 ssh net-tools -y
...
#  Install oracle-jdk7
RUN apt-get install --no-install-recommends oracle-java7-installer -y
#  Install android sdk
...
#  Install socat
RUN apt-get install --no-install-recommends socat -y
...
RUN apt-get install --no-install-recommends python-pkg-resources=3.3-1ubuntu1 -y --force-yes
RUN apt-get install --no-install-recommends python-pip python-setuptools git -y
RUN pip install "git+https://github.com/dtmilano/AndroidViewClient.git#egg=androidviewclient"
RUN apt-get install --no-install-recommends python-pexpect -y
#  Add entrypoint
