...
RUN export DEBIAN_FRONTEND=noninteractive \
 ...
 && apt-get update \
 && apt-get install --no-install-recommends curl ca-certificates less locales jq vim-tiny gnupg1 cron -y \
 && if [ "$DEMO" != "true" ] ; then \
 apt-get install --no-install-recommends pv lzop -y \
 && ETCDVERSION=2.3.8 \
...
RUN export DEBIAN_FRONTEND=noninteractive \
 ...
 && if [ "$DEMO" = "true" ] ; then \
 PGOLDVERSIONS="" \
 && apt-get install --no-install-recommends $BUILD_PACKAGES -y; elif \
 ...
 && apt-get install --no-install-recommends libprotobuf-c1 libcurl4 $BUILD_PACKAGES -y \
 ...
 && apt-get install --no-install-recommends libevent-2.1 libevent-pthreads-2.1 python3.6 python3-psycopg2 -y \
 && for version in ${PGOLDVERSIONS} ${PGVERSION}; dosed -i "s/ main.*$/ main $version/g" /etc/apt/sources.list.d/pgdg.list \
 && apt-get update \
 ...
 && apt-get install --no-install-recommends skytools3-ticker pspg -y \
 ...
 && apt-get install --no-install-recommends postgresql-server-dev-all postgresql-server-dev-9.3 -y \
...
RUN export DEBIAN_FRONTEND=noninteractive \
 ...
 && apt-get install --no-install-recommends python3-etcd python3-consul python3-kazoo python3-meld3 python3-boto python3-gevent python3-greenlet python3-protobuf python3-websocket python3-requests-oauthlib python3-swiftclient -y \
...
RUN set -ex \
 ...
 apt-get update \
 && apt-get install --no-install-recommends busybox xz-utils -y \
 && apt-get clean \
