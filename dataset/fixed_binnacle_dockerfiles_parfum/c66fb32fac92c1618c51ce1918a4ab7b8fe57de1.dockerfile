...
RUN (apt-key adv --keyserver hkp://keyserver.ubuntu.com:80 --recv-keys 44FC67F19B2466EA || apt-key adv --keyserver hkp://p80.pool.sks-keyservers.net:80 --recv-keys 44FC67F19B2466EA ) \
 ...
 && apt-get update \
 && apt-get install --no-install-recommends curl gcc git libssl-dev libzmq3-dev openssl pkg-config python3 python3-grpcio-tools python3-sawtooth-cli unzip -y -q \
