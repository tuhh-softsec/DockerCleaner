...
RUN apt-get install --no-install-recommends vim telnet wget openssh-server screen dos2unix tcpdump -y
# ## Minimum requirements
RUN apt-get install --no-install-recommends sudo python2.7-dev python-pip git -y
