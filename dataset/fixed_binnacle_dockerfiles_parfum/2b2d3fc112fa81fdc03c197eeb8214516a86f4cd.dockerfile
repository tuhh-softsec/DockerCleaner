...
RUN apt-get update \
 && apt-get install --no-install-recommends nvinfer-runtime-trt-repo-ubuntu1804-5.0.2-ga-cuda10.0 \
 && apt-get update \
 && apt-get install --no-install-recommends libnvinfer5=5.0.2-1+cuda10.0
RUN file="$( ls -1 /usr/local/ ;)" \
 && echo $file
RUN apt-get update -y
RUN apt-get install --no-install-recommends cmake gfortran git wget curl graphicsmagick libgraphicsmagick1-dev libavcodec-dev libavformat-dev libboost-all-dev libgtk2.0-dev libjpeg-dev liblapack-dev libswscale-dev pkg-config python3-numpy zip -y --fix-missing \
 && apt-get clean \
 && rm -rf /tmp/* /var/tmp/*
RUN add-apt-repository ppa:jonathonf/python-3.6
RUN apt-get update
RUN apt-get install --no-install-recommends python3.6 libpython3.6
...
RUN apt-get install --no-install-recommends libopenblas-base libomp-dev -y
