...
RUN apt-get update \
 && apt-get install --no-install-recommends libfreetype6-dev libjpeg62-turbo-dev libmcrypt-dev libpng-dev libmemcached-dev graphicsmagick libgraphicsmagick1-dev imagemagick libmagickwand-dev libssh2-1-dev libzip-dev libzookeeper-mt-dev libldb-dev libldap2-dev libssl-dev libmosquitto-dev librabbitmq-dev libicu-dev libxml2-dev libxslt-dev libbz2-dev git vim -y \
 && : \
