...
RUN apt-get install --no-install-recommends libcurl3-dev libfreetype6-dev libpng12-dev libzmq3-dev rsync zip zlib1g-dev openjdk-8-jdk openjdk-8-jre-headless -y
RUN apt-get update \
 && DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends -y install gcc g++ gfortran wget cpio \
 && cd /tmp \
