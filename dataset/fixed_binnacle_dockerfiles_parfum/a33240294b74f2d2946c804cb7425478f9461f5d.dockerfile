...
RUN apt-get update \
 && apt-get install --no-install-recommends crossbuild-essential-armhf -y
#  perpare build dependencies
RUN apt-get update \
 && apt-get install --no-install-recommends sudo git fakeroot devscripts cmake vim qemu-user-static binfmt-support dh-make dh-exec pkg-kde-tools device-tree-compiler bc cpio parted dosfstools mtools libssl-dev g++-arm-linux-gnueabihf -y
RUN apt-get update \
...
#  xf86-video-armsorc
RUN apt-get update \
 && apt-get install --no-install-recommends xserver-xorg-dev:armhf -y
