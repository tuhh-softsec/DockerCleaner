...
RUN apt-get install --no-install-recommends openssl -y
# ====================
#  Setup user accounts
# --------------------
...
# --------------------
RUN apt-get install --no-install-recommends make wget build-essential python python-dev libmysqlclient-dev git -y
RUN cd /home/ \
