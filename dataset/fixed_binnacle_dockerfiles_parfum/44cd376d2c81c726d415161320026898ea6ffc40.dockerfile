...
RUN apt-get install --no-install-recommends supervisor nginx-light git wget curl -y
RUN apt-get install --no-install-recommends python-django-tagging python-simplejson python-memcache python-ldap python-cairo python-django python-twisted python-pysqlite2 python-support python-pip gunicorn -y
#  fake fuse
RUN apt-get install --no-install-recommends libfuse2 \
