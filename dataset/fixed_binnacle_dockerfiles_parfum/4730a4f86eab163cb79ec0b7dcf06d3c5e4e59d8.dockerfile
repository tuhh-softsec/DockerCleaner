...
ARG minifyjs
RUN apt-get update \
 && apt-get install --no-install-recommends build-essential cmake python libtbb-dev libglu1-mesa-dev freeglut3-dev mesa-common-dev libc6-dev libnetcdf-c++4-1 libnetcdf-dev libnetcdf-c++4-dev python-pip git yasm -y \
