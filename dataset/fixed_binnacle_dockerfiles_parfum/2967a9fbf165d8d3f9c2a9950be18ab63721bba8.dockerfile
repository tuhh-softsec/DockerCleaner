...
RUN apt-get update \
 && apt-get install --no-install-recommends libfreetype6-dev libjpeg62-turbo-dev libpng-dev -y \
 ...
 && : \
 && apt-get install --no-install-recommends libbz2-dev -y \
...
RUN pecl install redis-3.1.4 \
 ...
 && : \
 && apt-get install --no-install-recommends libmemcached-dev zlib1g-dev -y \
