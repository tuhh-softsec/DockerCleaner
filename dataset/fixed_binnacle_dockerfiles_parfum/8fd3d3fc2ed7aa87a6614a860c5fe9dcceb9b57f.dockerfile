...
RUN ldconfig
RUN apt-get update \
 && apt-get install --no-install-recommends python-nose python-h5py python-skimage python-protobuf python-openssl python-mysqldb -y \
 && apt-get clean \
 && apt-get autoremove \
 && rm -rf /var/lib/apt/lists/*
