...
RUN apt-get update \
 && apt-get install --no-install-recommends apt-utils curl dpkg-dev gcc git less libapr1 libapr1-dev libssl-dev make mysql-client net-tools openjdk-8-jdk python-pip sudo telnet unzip vim -y \
