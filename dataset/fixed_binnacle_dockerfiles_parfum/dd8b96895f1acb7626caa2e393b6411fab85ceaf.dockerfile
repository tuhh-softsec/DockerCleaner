...
RUN apt-get update \
 && apt-get install --no-install-recommends arachni beef-xss cutycapt dirbuster exploitdb fierce firefox-esr git gpp-decrypt hydra john locate man metasploit-framework nbtscan netselect-apt nikto nmap python-pip responder skipfish sparta sqlitebrowser sqlmap unicornscan vim virtualenv websploit whois wordlists wpscan zenmap -y
