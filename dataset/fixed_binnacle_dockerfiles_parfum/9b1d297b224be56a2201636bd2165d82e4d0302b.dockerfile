...
RUN apt-get update \
 && apt-get install --no-install-recommends -y abi-compliance-checker abi-dumper automake build-essential ccache clang clang-format-7 cmake curl doxygen gawk git gcc g++ cmake libcurl4-openssl-dev libssl-dev libtool lsb-release make pkg-config python-pip shellcheck tar unzip wget zlib1g-dev
...
apt update \
 && apt-get install --no-install-recommends -y clang-tidy clang-format-7 clang-tools; update-alternatives --install /usr/bin/clang-tidy clang-tidy /usr/bin/clang-tidy-6.0 100 ; update-alternatives --install /usr/bin/clang-format clang-format /usr/bin/clang-format-7 100 ; update-alternatives --install /usr/bin/scan-build scan-build /usr/bin/scan-build-6.0 100 ; fi
