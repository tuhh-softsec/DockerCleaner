...
RUN apt-get update -y \
 && apt-get install --no-install-recommends genisoimage libffi-dev libssl-dev git sudo ipmitool maven openjdk-8-jdk python-dev python-setuptools python-pip python-mysql.connector supervisor python-crypto python-openssl -y
...
RUN apt-get install --no-install-recommends mysql-server -qqy \
 && apt-get clean all \
