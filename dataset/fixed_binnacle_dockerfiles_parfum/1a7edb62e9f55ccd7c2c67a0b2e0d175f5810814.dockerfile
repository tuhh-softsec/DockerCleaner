...
RUN apt-get update \
 && apt-get upgrade -y \
 && apt-get update
RUN apt-get install --no-install-recommends ssl-cert make libpcre3-dev libxslt1-dev libgeoip-dev unzip zip build-essential libssl-dev libxslt1.1 libgeoip1 geoip-database libpcre3 libgd2-xpm-dev -y
#  install nginx
...
RUN apt-get update \
 && apt-get install --no-install-recommends ssl-cert -y
