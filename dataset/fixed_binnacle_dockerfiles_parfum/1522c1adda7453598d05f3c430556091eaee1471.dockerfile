...
RUN mkdir -p ${GOPATH}
RUN apt-get update \
 && apt-get install --no-install-recommends apt-utils nano vim git wget curl zip libudev-dev libusb-1.0-0-dev -y
RUN apt-get install --no-install-recommends python3 python3-pip -y
...
RUN apt-get install --no-install-recommends build-essential autoconf libtool pkg-config libgflags-dev libgtest-dev clang libc++-dev -y; git clone -b $( curl -L https://grpc.io/release ;) https://github.com/grpc/grpc ; cd grpc ; git submodule update --init ; make ; make install ; apt-get install --no-install-recommends openjdk-8-jdk -y; echo "deb [arch=amd64] http://storage.googleapis.com/bazel-apt stable jdk1.8" | tee /etc/apt/sources.list.d/bazel.list ; curl https://bazel.build/bazel-release.pub.gpg | apt-key add - ; apt-get update ; apt-get install --no-install-recommends bazel -y; apt-get upgrade -y bazel ; bazel build :all ; make install ; cd third_party/protobuf ; make ; make install
RUN apt-get -y remove x264 libx264-dev \
 && apt-get -y --no-install-recommends install build-essential checkinstall cmake pkg-config yasm git gfortran libjpeg8-dev libpng-dev software-properties-common
...
RUN apt-get update
RUN apt-get -y --no-install-recommends install libjasper1 libjasper-dev libtiff-dev libavcodec-dev libavformat-dev libswscale-dev libdc1394-22-dev libxine2-dev libv4l-dev libgstreamer1.0-dev libgstreamer-plugins-base1.0-dev libgtk2.0-dev libtbb-dev qt5-default libatlas-base-dev libfaac-dev libmp3lame-dev libtheora-dev libvorbis-dev libxvidcore-dev libopencore-amrnb-dev libopencore-amrwb-dev libavresample-dev x264 v4l-utils libprotobuf-dev protobuf-compiler libgoogle-glog-dev libgflags-dev libgphoto2-dev libeigen3-dev libhdf5-dev doxygen cmake unzip git
