...
RUN set -ex \
 ...
 && apt-get update \
 && apt-get install --no-install-recommends libc6-i386 lib32stdc++6 lib32gcc1 lib32ncurses5 lib32z1 libqt5widgets5 -y --force-yes \
...
RUN set -ex \
 ...
 && apt-get update \
 && apt-get install --no-install-recommends libgtk-3-0 libglib2.0-0 libdbus-glib-1-2 libdbus-1-3 libasound2 -y \
