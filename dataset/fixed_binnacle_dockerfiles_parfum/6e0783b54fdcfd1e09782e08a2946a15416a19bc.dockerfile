...
RUN apt-get update -qqy \
 && apt-get install --no-install-recommends apt-transport-https python-software-properties software-properties-common curl expect zip libsass-dev git sudo -qqy --allow-unauthenticated
#  -----------------------------------------------------------------------------
#  Install Java
#  -----------------------------------------------------------------------------
ARG JAVA_VERSION
...
RUN echo ANDROID_HOME=${ANDROID_HOME} >> /etc/environment \
 ...
 && apt-get update -qqy \
 && apt-get install --no-install-recommends gradle libc6-i386 lib32stdc++6 lib32gcc1 lib32ncurses5 lib32z1 qemu-kvm kmod -qqy --allow-unauthenticated \
 && cd /opt \
