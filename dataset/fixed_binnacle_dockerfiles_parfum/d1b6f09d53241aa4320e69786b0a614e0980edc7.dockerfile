...
RUN apt-get update
RUN apt-get install --no-install-recommends docker-ce=17.03.2~ce-0~debian-jessie -y
RUN echo "DOCKER_OPTS=\"--experimental --storage-driver=vfs\"" >> /etc/default/docker
#  For jenkins slave
RUN echo "deb http://archive.debian.org/debian jessie-backports main" >> /etc/apt/sources.list
RUN apt-get -o Acquire::Check-Valid-Until=false update
RUN apt-get -y --no-install-recommends -o Acquire::Check-Valid-Until=false install ca-certificates-java=20161107~bpo8+1 openjdk-8-jdk
