...
RUN apt-get install --no-install-recommends figlet -y
RUN apt-get install --no-install-recommends strace -y
...
RUN apt-get install --no-install-recommends rake -y
#  Install tmux to gain split screen management and screen sharing capabilities
RUN apt-get install --no-install-recommends tmux -y
