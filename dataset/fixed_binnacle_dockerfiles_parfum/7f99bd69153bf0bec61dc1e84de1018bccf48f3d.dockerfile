...
RUN apt-get update \
 ...
 && export
#  Tor repository
COPY https://raw.githubusercontent.com/DECODEproject/decode-os/master/docker-sdk/tor.pub.asc tor.pub.asc
RUN apt-key add tor.pub.asc
RUN echo "deb https://deb.torproject.org/torproject.org $debian main" >> /etc/apt/sources.list
#  Nodejs repository
COPY https://deb.nodesource.com/gpgkey/nodesource.gpg.key nodesource.gpg.key
...
#  Openresty
COPY https://openresty.org/package/pubkey.gpg openresty.gpg
...
RUN apt-get --yes --force-yes purge $BUILD_DEPS \
 ...
 && npm uninstall -g npm
COPY https://raw.githubusercontent.com/DECODEproject/decode-os/master/docker-sdk/supervisord.conf /etc/supervisor/supervisord.conf
