...
COPY https://raw.githubusercontent.com/turtlecoin/checkpoints/master/checkpoints.csv /tmp/
#  In a multi-node testnet, we'll want to have the slave nodes "wait" for the master node to wake up, so let's get ready
COPY https://raw.githubusercontent.com/vishnubob/wait-for-it/master/wait-for-it.sh /tmp/
