...
RUN go get -d github.com/google/go-querystring/query \
 && (cd /go/src/github.com/google/go-querystring \
 && (git cat-file -t $REV 2> /dev/null || git fetch -q origin $REV ) \
 && git reset --hard $REV )
