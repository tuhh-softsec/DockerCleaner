...
RUN apt-get update \
 && apt-get install --no-install-recommends pkg-config curl libpcre3-dev libtool libssl-dev zlib1g-dev libasound2-dev build-essential cmake -y
#  nasm
RUN mkdir -p /dist \
 && cd /dist \
...
RUN mkdir -p /dist \
 ...
 && ./configure --bindir="${SRC}/bin" --extra-cflags="-I${SRC}/include" --extra-ldflags="-L${SRC}/lib" --prefix="${SRC}" --enable-nonfree --enable-gpl --enable-version3 --enable-libmp3lame --enable-libx264 --enable-libx265 --enable-openssl --enable-postproc --enable-small --enable-static --disable-debug --disable-doc --disable-shared \
 && make -j$( nproc ;) \
 && make install
#  nginx-rtmp
RUN mkdir -p /dist \
...
RUN apt-get update \
 && apt-get install --no-install-recommends ca-certificates git procps libpcre3 openssl libssl1.1 zlib1g v4l-utils libv4l-0 alsa-utils -y
COPY . /restreamer
WORKDIR /restreamer
RUN cd /restreamer \
