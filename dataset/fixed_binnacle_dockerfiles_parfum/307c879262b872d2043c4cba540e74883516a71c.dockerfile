...
RUN set -x \
 && apk add --virtual .php-build-dependencies argon2-dev autoconf binutils bison bzip2-dev curl curl-dev file freetype-dev g++ gcc git icu-dev jpeg-dev libmcrypt-dev libpng-dev libtool libwebp-dev libxml2-dev libxslt-dev libzip-dev make pcre2-dev postgresql-dev re2c readline-dev sqlite-dev \
 && : "---------- Proper iconv ----------" \
 && apk add --no-cache --virtual .iconv-runtime-dependencies --repository https://nl.alpinelinux.org/alpine/edge/testing/ gnu-libiconv-dev \
