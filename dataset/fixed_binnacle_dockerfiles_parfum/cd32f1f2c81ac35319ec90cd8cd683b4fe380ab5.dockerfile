...
#  Install Icinga2 ###################################
RUN set -x \
 && apk add --update --no-cache ca-certificates openssl su-exec runit socklog tzdata curl nginx gettext bash openrc shadow iputils icinga2 nagios-plugins jq postgresql php7 php7-common php7-ctype php7-dom php7-fpm php7-gd php7-gettext php7-gmp php7-imagick php7-intl php7-json php7-ldap php7-mcrypt php7-mysqlnd php7-openssl php7-pdo php7-pdo_mysql php7-pdo_pgsql php7-pgsql php7-session php7-sockets
RUN set -x \
 && mkdir -p /run/nginx \
