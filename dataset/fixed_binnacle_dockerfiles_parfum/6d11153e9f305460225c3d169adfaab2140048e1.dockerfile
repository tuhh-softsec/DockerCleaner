...
RUN apt-get update -q \
 && DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y curl git libgl1-mesa-dev libgl1-mesa-glx libglew-dev libosmesa6-dev software-properties-common net-tools unzip vim virtualenv wget xpra xserver-xorg-dev \
 && apt-get clean \
 && rm -rf /var/lib/apt/lists/*
RUN DEBIAN_FRONTEND=noninteractive add-apt-repository --yes ppa:deadsnakes/ppa \
 && apt-get update
RUN DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install --yes python3.5-dev python3.5 python3-pip
...
RUN DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends -qy install nvidia-384; exit 0
ENV LD_LIBRARY_PATH="${LD_LIBRARY_PATH}:/usr/lib/nvidia-384"
RUN mkdir /root/code
WORKDIR /root/code
WORKDIR /mujoco_py
#  For atari-py
RUN apt-get install --no-install-recommends zlib1g-dev swig cmake -y
