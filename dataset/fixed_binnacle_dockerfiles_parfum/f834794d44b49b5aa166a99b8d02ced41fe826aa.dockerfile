...
RUN apt-get update -qq \
 && apt-get install --no-install-recommends curl locales -y \
...
RUN mkdir -p /data/db
RUN apt-get update
RUN apt-get install --no-install-recommends gnupg-curl -y
...
RUN apt-get update
RUN apt-get install --no-install-recommends crudini -y
RUN apt-get install --no-install-recommends mongodb-org -y
...
RUN curl -sf https://packagecloud.io/install/repositories/StackStorm/stable/script.deb.sh | bash \
 && apt-get install --no-install-recommends st2=${ST2_VERSION}-* -y \
...
# ####################################################################################################
RUN apt-get update \
 && apt-get install --no-install-recommends openssh-server python git vim screen -y
