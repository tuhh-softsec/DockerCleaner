...
RUN echo "*** Install required packages" \
 && apt-get update -qq \
 && apt-get install --no-install-recommends autoconf ca-certificates bison build-essential cpio curl file flex gawk gettext git jq libtool lib32z1-dev libcurl4-openssl-dev libssl-dev locales make ncurses-dev openssl pkg-config python3 python python3-pip python-pip python3-dev python-dev rsync texi2html texinfo tofrodos unrar unzip vim wget zip -qq -y \
 && apt-get clean
# ###################################################
# # Configure locales                              ##
# ###################################################
