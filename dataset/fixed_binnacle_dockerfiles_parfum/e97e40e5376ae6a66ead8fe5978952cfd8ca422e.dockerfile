...
RUN apt-get update \
 && apt-get install --no-install-recommends wget hmmer unzip build-essential -qqy
...
RUN apt-get install --no-install-recommends libjson-perl liburi-perl liblwp-useragent-determined-perl -qqy
