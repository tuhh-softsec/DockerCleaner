...
RUN apt-get clean all \
 && apt-get update \
 && apt-get upgrade -y \
 && apt-get install --no-install-recommends curl num-utils wget git perl zip pigz pbzip2 unzip python3 python3-pip libtbb2 default-jdk unsort fastx-toolkit nano -y
# #######################################
