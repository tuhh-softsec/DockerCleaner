...
RUN apt-get install --no-install-recommends git cmake build-essential vim supervisor libpcre3-dev pkg-config libavl-dev libev-dev libprotobuf-c-dev protobuf-c-compiler libssl-dev libssh-dev swig -y
...
RUN apt-get install --no-install-recommends lua5.1-dev -y
RUN cd /opt/dev/sysrepo/build \
...
RUN apt-get install --no-install-recommends lua5.2-dev -y
RUN cd /opt/dev/sysrepo/build \
