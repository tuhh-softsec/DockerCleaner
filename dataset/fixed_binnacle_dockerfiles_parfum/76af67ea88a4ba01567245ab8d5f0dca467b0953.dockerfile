...
RUN apt-get update \
 && apt-get install --no-install-recommends autoconf binutils-dev bison build-essential bzip2 ccache cmake curl doxygen flex gcc-multilib gdb gfortran git git-flow htop libboost-all-dev libev-dev libgoogle-perftools-dev libhwloc-dev liblog4cxx10 liblog4cxx10-dev libmetis-dev libmysqlclient-dev libpapi-dev libtbb-dev libtool libunwind8-dev man mysql-server nano nodejs-legacy npm openssh-server python python-pip screen strace sudo tcpdump telnet vim wget -y
