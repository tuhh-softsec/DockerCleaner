...
RUN apt-get install --no-install-recommends mysql-client curl gcc git gnupg-agent make python openssl redis-server sudo vim zip locales -y
RUN locale-gen en_US.UTF-8
ENV LANG="en_US.UTF-8"
RUN add-apt-repository -y ppa:ondrej/php \
 && apt-get update
#  Apache
RUN apt-get install --no-install-recommends apache2 apache2-doc apache2-utils -y
RUN a2dismod status
RUN a2dissite 000-default
#  PHP 7.2
RUN apt-get install --no-install-recommends libapache2-mod-php php7.2 php7.2-cli php-crypt-gpg php7.2-dev php7.2-json php7.2-mysql php7.2-opcache php7.2-readline php7.2-redis php7.2-xml -y
RUN apt-get install --no-install-recommends php-pear pkg-config libbson-1.0 libmongoc-1.0-0 php-xml php-dev -y
...
RUN apt-get install --no-install-recommends python-dev python-pip libxml2-dev libxslt1-dev zlib1g-dev python-setuptools libfuzzy-dev -y
RUN apt-get install --no-install-recommends cron logrotate supervisor syslog-ng-core -y
RUN apt-get clean
WORKDIR /var/www
...
RUN sudo apt-get -y --no-install-recommends install libpoppler58 libpoppler-dev libpoppler-cpp-dev
#  Install MISP Modules
WORKDIR /opt
RUN apt-get install --no-install-recommends python3 python3-pip libjpeg-dev -y
#  PIP3 fix
