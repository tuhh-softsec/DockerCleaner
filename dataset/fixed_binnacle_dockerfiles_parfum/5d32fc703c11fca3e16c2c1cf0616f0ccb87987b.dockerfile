...
RUN apt-get update \
 && apt-get install --no-install-recommends apt-utils build-essential ca-certificates cron curl debhelper dialog dpkg-dev g++-4.9 gcc-4.9 git librtlsdr-dev libusb-1.0-0-dev lighttpd netcat net-tools pkg-config python2.7 wget -y
