...
SHELL ["/bin/bash", "-c"]
RUN apt-get update \
 && apt-get install --no-install-recommends apt-transport-https build-essential bzip2 ca-certificates curl g++ git gnupg graphviz locales lsb-release openssh-client sudo unzip vim wget zip emacs -yq \
 && apt-get clean \
