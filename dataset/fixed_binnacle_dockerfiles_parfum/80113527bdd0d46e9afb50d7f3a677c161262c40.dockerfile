...
RUN apt-get update \
 && apt-get install --no-install-recommends build-essential gfortran wget git cmake lcov valgrind cppcheck clang-format-3.7 libssl-dev libpng-dev libfreetype6-dev libxft-dev libsqlite3-dev libbz2-dev libatlas-base-dev zlib1g-dev libopenmpi-dev libhdf5-dev python3.5-dev python3-tk pandoc npm nodejs nodejs-legacy -y \
 && apt-get clean \
