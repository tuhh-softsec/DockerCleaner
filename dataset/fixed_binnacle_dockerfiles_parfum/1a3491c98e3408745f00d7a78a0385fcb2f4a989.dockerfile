...
RUN apt-get install --no-install-recommends build-essential -y \
 && apt-get install --no-install-recommends curl git htop man unzip wget nano -y
#  tini for subreap                                  
ENV TINI_VERSION="v0.9.0"
COPY https://github.com/krallin/tini/releases/download/${TINI_VERSION}/tini /bin/tini
RUN chmod +x /bin/tini
RUN apt-get install --no-install-recommends v4l-utils ffmpeg -y
#  bluez dependencies
RUN apt-get install --no-install-recommends libglib2.0-dev libical-dev libreadline-dev libudev-dev libdbus-1-dev libdbus-glib-1-dev -y
#  debugging
RUN apt-get install --no-install-recommends usbutils strace -y
RUN apt-get install --no-install-recommends bluez bluez-hcidump blueman checkinstall libusb-dev libbluetooth-dev joystick -y
...
RUN git clone https://github.com/aaronp24/QtSixA.git \
 ...
 && sudo checkinstall -y
RUN apt-get install --no-install-recommends python-dbus -y
# install opencv3 python 3
RUN apt-get install --no-install-recommends wget unzip build-essential cmake git pkg-config libatlas-base-dev gfortran libjasper-dev libgtk2.0-dev libavcodec-dev libavformat-dev libswscale-dev libjpeg-dev libpng-dev libtiff-dev libjasper-dev libv4l-dev -y
...
RUN make -j4
RUN make install
RUN apt-get install --no-install-recommends ipython3 -y
