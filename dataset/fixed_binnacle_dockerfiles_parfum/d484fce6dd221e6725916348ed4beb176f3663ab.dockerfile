...
RUN apt-get update \
 && apt-get install --no-install-recommends gcc g++ ruby cmake autoconf curl wget libpcre3-dev pkg-config libjpeg-dev libpng-dev libgif-dev libtool -qq -y
...
ARG ImageMagick_VERSION=7.0.7
RUN apt-get update \
 && apt-get install --no-install-recommends libjpeg8 libpng16-16 libgif7 libgomp1 -qq -y
