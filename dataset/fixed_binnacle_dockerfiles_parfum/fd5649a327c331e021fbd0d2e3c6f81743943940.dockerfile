...
RUN sed -i -e 's/^deb-src/#deb-src/' /etc/apt/sources.list \
 ...
 && apt-get install --no-install-recommends bash build-essential ca-certificates curl git-core jq libffi-dev libssl-dev man mercurial net-tools netcat openssh-client procps python python-dev python-pip python-setuptools rsync ruby unzip upx util-linux vim wamerican wget zip -y \
 ...
 && pip install shyaml azure-cli==${AZCLI_VERSION} --disable-pip-version-check --no-cache-dir \
