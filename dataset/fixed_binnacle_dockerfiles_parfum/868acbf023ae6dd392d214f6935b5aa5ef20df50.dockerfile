...
RUN apt-get update \
 && apt-get install --no-install-recommends gcc g++ perl python3 automake make wget curl libdb-dev bzip2 zlibc zlib1g zlib1g-dev default-jre python-setuptools python-dev build-essential unzip libbz2-dev liblzma-dev -y \
 && apt-get clean
