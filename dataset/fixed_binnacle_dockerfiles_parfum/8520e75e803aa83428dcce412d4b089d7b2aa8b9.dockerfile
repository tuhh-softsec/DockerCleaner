...
RUN set -x \
 && apk add --update --no-cache --virtual .fetch-deps ca-certificates curl tar openssl \
 ...
 && apk add --no-cache --virtual .native-build-deps apr-dev gcc libc-dev make "openjdk${JAVA_VERSION%%[-~bu]*}"="$JAVA_ALPINE_VERSION" openssl-dev \
