...
RUN apt-get update \
 && apt-get install --no-install-recommends autoconf bc build-essential curl environment-modules gfortran git lcov libatlas-base-dev libbz2-dev libfreetype6-dev libopenmpi-dev libpng-dev libsqlite3-dev libssl-dev libxft-dev python2.7-dev tmux unzip valgrind vim wget zlib1g-dev -y \
 && apt-get install --no-install-recommends gawk -y \
