...
RUN apt-get update \
 && apt-get install --no-install-recommends wget git cmake python python-pip software-properties-common python-software-properties unzip zlib1g-dev libncurses5-dev libncursesw5-dev locales libdb-dev vim nano pkg-config libjsoncpp-dev -y
#  Configure locales
...
# # R
RUN apt-get install --no-install-recommends r-base r-cran-getopt r-cran-plyr -y
# # STAR
...
RUN apt-get install --no-install-recommends g++ qt5-default libqt5xmlpatterns5-dev libqt5sql5-mysql python-matplotlib -y
...
# # samtools
# ## Install 3rd party dependencies
RUN apt-get install --no-install-recommends bzip2 -y
# ## Install samtools
