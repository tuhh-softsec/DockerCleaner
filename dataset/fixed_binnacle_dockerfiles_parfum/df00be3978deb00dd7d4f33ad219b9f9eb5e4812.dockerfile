...
RUN set -x \
 && apt-get update \
 && apt-get install --no-install-recommends curl cpp dirmngr gcc gpg gpg-agent openjdk-8-jdk libapr1 libapr1-dev lsof make tar unzip sed wget -y
...
RUN set -x \
 && apt-get update \
 && apt-get install --no-install-recommends ghostscript imagemagick -y \
