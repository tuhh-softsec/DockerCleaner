...
RUN apt-get update \
 && apt-get install --no-install-recommends clang-3.8 patch libxml2-dev ca-certificates curl git make cmake libssl-dev xz-utils -y \
 ...
 && rm -rf /tmp/osxcross \
 && apt-get install --no-install-recommends bison curl flex gawk gcc g++ gperf help2man libncurses5-dev make patch python-dev texinfo xz-utils -y \
...
RUN apt-get update \
 && apt-get install --no-install-recommends clang-3.8 gcc-aarch64-linux-gnu gcc-arm-linux-gnueabihf gcc-mingw-w64-x86-64 apt-transport-https ca-certificates curl libfontconfig1 gcc g++ git make rpm xz-utils expect gnupg2 unzip -y \
 && ln -s /usr/bin/clang-3.8 /usr/bin/clang \
 && ln -s /usr/bin/clang++-3.8 /usr/bin/clang++ \
 && ln -s /usr/bin/llvm-dsymutil-3.8 /usr/bin/dsymutil \
...
RUN apt-get install --no-install-recommends gcc libc-dev make -y \
