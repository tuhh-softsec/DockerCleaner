...
RUN apt-get update \
 ...
 && apt-get install --no-install-recommends software-properties-common apt-utils nano vim man build-essential wget sudo git mercurial subversion -y --allow-unauthenticated \
...
RUN echo 'export PATH=/opt/conda/bin:$PATH' > /etc/profile.d/conda.sh \
 ...
 && rm ~/miniconda.sh
RUN apt-get update \
 && apt-get install --no-install-recommends curl grep sed dpkg -y \
...
RUN apt-get update --fix-missing \
 && sudo apt-get install --no-install-recommends -y libgl1-mesa-dev libgl1-mesa-glx libosmesa6-dev libglfw3-dev
# # Miniconda end
