...
RUN apt-get install --no-install-recommends curl git zip unzip vim locales software-properties-common python-software-properties -y
#  Set up locales
...
RUN apt-get install --no-install-recommends apache2 libapache2-mod-fastcgi apache2-utils libmysqlclient-dev mariadb-client -y --allow-downgrades --allow-remove-essential --allow-change-held-packages
