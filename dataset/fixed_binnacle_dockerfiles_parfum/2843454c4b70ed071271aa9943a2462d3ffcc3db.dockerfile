...
RUN apt-get update -y \
 && apt-get upgrade -y \
 && apt-get install --no-install-recommends -y build-essential wget cmake curl file git musl-dev musl-tools libpq-dev libsqlite-dev libssl-dev linux-libc-dev pkgconf xutils-dev gcc-multilib-arm-linux-gnueabihf cmake-data g++ pkgconf ragel libpcap-dev sqlite3 libsqlite3-dev python-dev \
