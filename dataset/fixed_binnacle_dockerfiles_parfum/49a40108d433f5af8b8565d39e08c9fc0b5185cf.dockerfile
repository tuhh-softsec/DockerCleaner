...
RUN apt-get update \
 && apt-get install --no-install-recommends wget curl clang libicu-dev libpython2.7 -y
#  Import Swift keys
...
RUN apt-get install --no-install-recommends git pkg-config autoconf libtool systemtap-sdt-dev libkqueue-dev libblocksruntime-dev libbsd-dev -y
...
RUN apt-get install --no-install-recommends vim x11vnc xvfb x11-xserver-utils unzip -y
RUN unzip ideavim.plugin.zip
#  Setup VNC
RUN mkdir ~/.vnc
RUN x11vnc -storepasswd 1234 ~/.vnc/passwd
#  Install JDK
RUN apt-get install --no-install-recommends openjdk-8-jre openjdk-8-jre-headless openjdk-8-jdk -y
# # Setup Clion
