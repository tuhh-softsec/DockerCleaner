...
# Utilities
RUN DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y vim less net-tools inetutils-ping curl git telnet nmap socat
# Others
RUN DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y vlan bridge-utils python python-pip python-software-properties software-properties-common python-keyring
# MySQL
RUN DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y mysql-server python-mysqldb \
 && sed -i 's/127.0.0.1/0.0.0.0/g' /etc/mysql/my.cnf
# RabbitMQ
RUN DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y rabbitmq-server
# For Openstack
...
# Nova Controller
RUN DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y nova-novncproxy novnc nova-api nova-ajax-console-proxy nova-cert nova-conductor nova-consoleauth nova-doc nova-scheduler python-novaclient nova-network \
...
# todo move up later
# Neutron
RUN DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y neutron-server neutron-dhcp-agent neutron-plugin-openvswitch neutron-l3-agent
...
# Heat
RUN DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y heat-api heat-api-cfn heat-engine
