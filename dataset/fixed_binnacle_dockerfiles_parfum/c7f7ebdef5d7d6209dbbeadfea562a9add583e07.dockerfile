...
RUN apt-get update \
 && apt-get -y upgrade \
 && apt-get -y --no-install-recommends install software-properties-common wget git curl jq \
 && add-apt-repository ppa:transmissionbt/ppa \
 && wget -O - https://swupdate.openvpn.net/repos/repo-public.gpg | apt-key add - \
 && echo "deb http://build.openvpn.net/debian/openvpn/stable xenial main" > /etc/apt/sources.list.d/openvpn-aptrepo.list \
 && apt-get update \
 && apt-get install --no-install-recommends -y sudo transmission-cli transmission-common transmission-daemon curl rar unrar zip unzip ufw iputils-ping openvpn bc tzdata python2.7 python2.7-pysqlite2 \
 ...
 && apt-get install --no-install-recommends -y tinyproxy telnet \
