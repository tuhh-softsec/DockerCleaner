...
#  python - msan
RUN apt-get install --no-install-recommends clang cmake python -y
...
#  yarn - ui
RUN apt-get install --no-install-recommends ccache google-cloud-sdk nodejs openjdk-8-jre openssh-client yarn -y
