...
ENV PATH=".:$PATH"
RUN apt-get update
RUN apt-get install --no-install-recommends git wget unzip curl net-tools build-essential npm python python-setuptools python-dev python-numpy openssh-server sysstat -y
#  libfontconfig is needed for grunt phantomjs...
RUN apt-get install --no-install-recommends libfontconfig -y
...
# ## PYTHON ###
RUN apt-get install --no-install-recommends python-pip python-matplotlib python-pandas ipython python-nose -y
