...
RUN apt-get install --no-install-recommends openjdk-8-jdk wget expect git curl ruby ruby-dev ruby-build python python-pip optipng imagemagick locales -y
...
RUN apt-get install --no-install-recommends gcc python-dev python-setuptools
