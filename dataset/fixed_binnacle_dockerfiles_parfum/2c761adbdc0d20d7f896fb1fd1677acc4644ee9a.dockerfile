...
#  -----------------------------------------------------------------------------
RUN apt-get update \
 && DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y curl pwgen sudo nano locales imagemagick jpegoptim poppler-utils openjdk-7-jre-headless
