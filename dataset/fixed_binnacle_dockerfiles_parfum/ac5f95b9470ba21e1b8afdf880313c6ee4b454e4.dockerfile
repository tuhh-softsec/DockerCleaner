...
RUN mkdir /var/uwsgi
RUN apt-get update \
 && apt-get install --no-install-recommends python3-pip python3-dev -y \
 && cd /usr/local/bin \
 && ln -s /usr/bin/python3 python \
 && pip3 install --upgrade pip
RUN apt-get update --fix-missing \
 && apt-get install --no-install-recommends wget supervisor bzip2 -y \
 && apt-get clean \
 && rm -rf /var/lib/apt/lists/*
ENV GDAL_VERSION="2.2.1"
RUN apt-get update \
 && apt-get install --no-install-recommends libpq-dev ogdi-bin libogdi3.2-dev libjasper-runtime libjasper-dev libjasper1 libgeos-dev libproj-dev libpoppler-dev libsqlite3-dev libspatialite-dev python3 python3-dev python3-numpy -y
...
CMD gdalinfo --version \
 ...
 && ogrinfo --formats
RUN apt-get update \
 && apt-get install --no-install-recommends apt-transport-https build-essential ed gtk2.0 libcurl4-openssl-dev libgtk2.0-dev libiodbc2-dev libnlopt-dev libssh2-1-dev libssl-dev libxml2-dev software-properties-common wget xvfb -y \
 && rm -rf /var/lib/apt/lists/*
...
RUN apt-get update -qq \
 && apt-get install --no-install-recommends git-core libssl-dev libcurl4-gnutls-dev -y
#  Install data_processing packages
RUN apt-get update \
 && apt-get install --no-install-recommends libgdal-dev libproj-dev -y
