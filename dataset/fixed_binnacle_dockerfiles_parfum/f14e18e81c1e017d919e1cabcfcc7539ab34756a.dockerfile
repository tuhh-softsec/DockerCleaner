...
RUN apt-get update \
 && apt-get install --no-install-recommends wget gnupg -my
#  Install dotdeb repo
...
RUN apt-get update \
 && apt-get install --no-install-recommends git unzip libcurl4-openssl-dev libedit-dev libssl-dev libxml2-dev libsqlite3-dev sqlite3 libz-dev libpq-dev libjpeg-dev libpng-dev libfreetype6-dev libssl-dev libmcrypt-dev libjudydebian1 libjudy-dev nano procps -y \
 && apt-get clean
#
#  Configure
#
