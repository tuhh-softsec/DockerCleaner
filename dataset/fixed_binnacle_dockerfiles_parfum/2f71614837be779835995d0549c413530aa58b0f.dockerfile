...
RUN apt-get install --no-install-recommends -y python3-numpy python3-scipy python3-matplotlib python3-pandas python3-nose python3-sympy
#  nltk
RUN pip3 install nltk==3.2.5
WORKDIR /tmp
#  sentencepiece
RUN apt-get install --no-install-recommends cmake build-essential pkg-config libgoogle-perftools-dev -y
...
RUN apt-get -y --no-install-recommends install libopenmpi-dev
...
RUN python3 setup.py build
RUN python3 setup.py install
RUN apt-get install --no-install-recommends -y python3-sklearn
