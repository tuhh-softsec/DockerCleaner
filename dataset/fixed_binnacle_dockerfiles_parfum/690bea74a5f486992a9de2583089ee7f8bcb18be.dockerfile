...
RUN cd ~/workspace/ros \
 && apt-get install --no-install-recommends ocl-icd-opencl-dev -y \
...
RUN cd \
 ...
 && git checkout 1606 \
 && cd depends/ \
 && apt-get install --no-install-recommends git cmake cmake-curses-gui libxmu-dev libxi-dev libgl1-mesa-dev xorg-dev libglu1-mesa-dev libtool automake libudev-dev libgtk2.0-dev pkg-config libjpeg-turbo8-dev libturbojpeg libglewmx-dev -y \
...
RUN apt-get update \
 && apt-get install --no-install-recommends python-qt4 python-pip -y \
