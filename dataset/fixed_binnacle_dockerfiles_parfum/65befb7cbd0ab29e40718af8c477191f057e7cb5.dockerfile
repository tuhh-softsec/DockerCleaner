...
RUN apt-get install --no-install-recommends libreadline-gplv2-dev libfreetype6 apt-utils dialog postfix -qqy
RUN echo "Europe/Berlin" > /etc/timezone \
 && dpkg-reconfigure -f noninteractive tzdata
RUN echo 'alias ll="ls -lah --color=auto"' >> /etc/bash.bashrc
RUN apt-get install --no-install-recommends passwd supervisor git-core sudo unzip wget curl libfile-slurp-perl libmysql-diff-perl vim net-tools software-properties-common python-software-properties -qqy
#  Set locale
RUN apt-get install --no-install-recommends locales -qqy
RUN locale-gen --purge de_DE de_DE.UTF-8
RUN locale-gen --purge en_US en_US.UTF-8
RUN dpkg-reconfigure locales
ENV LC_ALL="en_US.UTF-8"
#  Setup ssh
RUN apt-get install --no-install-recommends openssh-server -qqy
...
#  Apache
RUN apt-get install --no-install-recommends apache2-mpm-prefork apache2-utils -qqy
RUN a2enmod rewrite
...
#  Mysql
RUN apt-get install --no-install-recommends mysql-server mysql-common mysql-client -qqy
...
RUN add-apt-repository ppa:ondrej/php \
 && apt-get update
#  PHP
RUN apt-get install --no-install-recommends php5-fpm php5 php5-cli php5-mysql php5-curl php5-dev php5-gd php-pear php-apc php5-xdebug libapache2-mod-php5 -qqy
#  PhpMyAdmin
...
#  Install ant builder
RUN apt-get install --no-install-recommends ant -qqy
#  Install composer
