...
RUN apt-get update \
 && DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y software-properties-common texlive texlive-latex-extra texlive-extra-utils texlive-xetex texlive-luatex texlive-bibtex-extra liblog-log4perl-perl
RUN apt-get update \
 && DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y tmux flex bison libreadline-dev htop screen pandoc aspell poppler-utils net-tools wget git python python-pip make g++ sudo psmisc haproxy nginx yapf rsync tidy
RUN apt-get update \
 && DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y vim bup inetutils-ping lynx telnet git emacs subversion ssh m4 latexmk libpq5 libpq-dev build-essential automake
RUN apt-get update \
 && DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y gfortran dpkg-dev libssl-dev imagemagick libcairo2-dev libcurl4-openssl-dev graphviz smem octave python3-yaml python3-matplotlib python3-jupyter* python-matplotlib* python-jupyter* python-ipywidgets python-ipywidgets-doc python3-ipywidgets jupyter locales locales-all postgresql postgresql-contrib clang-format yapf yapf3 golang r-cran-formatr
#  The Octave kernel.
...
RUN apt-get update \
 && apt-get install --no-install-recommends aspell-* -y
...
RUN apt-get update \
 ...
 && apt-get update \
 && DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y xpra
# # X11 apps to make x11 support useful.
# # Will move this up in Dockerfile once stable.
RUN apt-get update \
 && DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y x11-apps dbus-x11 gnome-terminal vim-gtk lyx libreoffice inkscape gimp chromium-browser texstudio evince mesa-utils xdotool xclip x11-xkb-utils
#  Microsoft's VS Code
RUN curl https://packages.microsoft.com/keys/microsoft.asc | gpg --dearmor > microsoft.gpg \
 && install -o root -g root -m 644 microsoft.gpg /etc/apt/trusted.gpg.d/ \
 && sh -c 'echo "deb [arch=amd64] https://packages.microsoft.com/repos/vscode stable main" > /etc/apt/sources.list.d/vscode.list' \
 && DEBIAN_FRONTEND=noninteractive sudo apt-get install --no-install-recommends -y apt-transport-https \
 && sudo apt-get update \
 && DEBIAN_FRONTEND=noninteractive sudo apt-get install --no-install-recommends -y code
#  RStudio
RUN apt-get install --no-install-recommends libjpeg62 -y \
