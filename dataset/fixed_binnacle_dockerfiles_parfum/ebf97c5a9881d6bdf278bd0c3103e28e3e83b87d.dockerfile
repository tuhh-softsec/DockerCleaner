...
RUN apt-get update \
 ...
 && apt-get install --no-install-recommends patch make cmake zlib1g-dev tk bison flex libglu1-mesa libxcursor-dev libmpich-dev libopenblas-dev libboost-python-dev -y \
 && apt-get clean \
