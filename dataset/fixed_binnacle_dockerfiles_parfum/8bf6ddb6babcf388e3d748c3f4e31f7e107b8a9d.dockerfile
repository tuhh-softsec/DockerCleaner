...
RUN apt-get update \
 && apt-get install --no-install-recommends libncurses5-dev libncursesw5-dev libcurl4-openssl-dev libfreetype6-dev zlib1g-dev python python-setuptools python-pip python3 python3-setuptools python3-pip git wget unzip ghostscript pkg-config libboost-dev r-base-core default-jre apt-transport-https tabix -y \
...
RUN apt-get update \
 && apt-get install --no-install-recommends liblzo2-dev -y \
 && rm -rf /var/lib/apt/lists/*
#  Install python packages for ATAQC (pysam, pybedtools, metaseq, pandas, jinja2)
RUN pip install pysam==0.8.2.1 pybedtools==0.6.9 pandas==0.21.1 metaseq==0.5.6 jinja2==2.10 --no-cache-dir
#  Install gsl 1.16
...
RUN wget http://hgdownload.soe.ucsc.edu/admin/exe/linux.x86_64/bigWigAverageOverBed \
