...
RUN apt-get update \
 ...
 && apt-get update -qq \
 && apt-get install --no-install-recommends autoconf automake build-essential cabextract dh-autoreconf git jq libffi-dev libfuzzy-dev libpython2.7-stdlib libssl-dev libtool make net-tools python-dev python-minimal python-pip python-setuptools ssdeep unrar unzip upx-ucl vim wget -y --fix-missing \
