...
RUN apt-get install --no-install-recommends git wget build-essential unzip -y
RUN apt-get install --no-install-recommends samtools mafft muscle raxml tabix -y
# R
RUN apt-get install --no-install-recommends r-bioc-biobase -y
RUN apt-get install --no-install-recommends graphviz libgraphviz-dev pkg-config -y
RUN apt-get install --no-install-recommends swig -y
RUN apt-get install --no-install-recommends libx11-dev -y
RUN apt-get install --no-install-recommends libgsl0ldbl -y
RUN apt-get install --no-install-recommends libgsl0-dev libopenblas-dev liblapacke-dev -y
RUN apt-get clean
# genepop
RUN mkdir genepop
WORKDIR /genepop
