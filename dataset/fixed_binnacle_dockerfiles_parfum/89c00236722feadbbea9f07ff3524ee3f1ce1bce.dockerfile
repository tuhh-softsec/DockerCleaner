...
#  Install base tools
RUN apt-get update \
 && apt-get install --no-install-recommends bash gpg wget -y
...
RUN apt-get update \
 && apt-get install --no-install-recommends bash sudo unzip build-essential cmake ninja-build g++ gcc-arm-linux-gnueabi g++-arm-linux-gnueabi libncurses5-dev libstdc++-6-dev make pkg-config python python-dev wget zlib1g-dev qemu-user vim llvm-$LLVM_VERSION -y \
