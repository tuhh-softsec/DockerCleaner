...
RUN apt-get install --no-install-recommends debconf -y
#  Set keyboard.
...
#  Create user.
RUN apt-get install --no-install-recommends sudo bash -y \
...
RUN apt-get install --no-install-recommends dpkg-dev git -y \
 ...
 && apt-get install --no-install-recommends 'apt::install-recommends=true' ${pkgs} -y -o \
...
RUN mkdir xrdp \
 && cd xrdp \
 && apt-get install --no-install-recommends wget patch -y \
...
#  Install desktop.
RUN case "${desktop}" inunityapt-get install --no-install-recommends ubuntu-desktop unity -y \
 ...
 && chown ${user}:${user} /home/${user}/.xsession;; classicapt-get install --no-install-recommends ubuntu-desktop gnome-panel metacity -y \
 && ln -s /usr/bin/unity-control-center /usr/bin/gnome-control-center \
 && printf "gnome-panel &\nmetacity &\n" > /home/${user}/.xsessionrc \
 && chown ${user}:${user} /home/${user}/.xsessionrc;; mateapt-get install --no-install-recommends ubuntu-mate-desktop ubuntu-mate-core mate-desktop-environment-extras -y \
 && service dbus start \
 ...
 && chown ${user}:${user} /home/${user}/.xsession;; xubuntuapt-get install --no-install-recommends xubuntu-desktop -y \
 && echo "mode: off\n" > /home/${user}/.xscreensaver \
 && chown ${user}:${user} /home/${user}/.xscreensaver \
 && echo "xfce4-session" > /home/${user}/.xsession \
 && chown ${user}:${user} /home/${user}/.xsession;; lubuntuapt-get install --no-install-recommends lubuntu-desktop -y \
 && apt-get remove -y light-locker \
 && echo "lxsession -s Lubuntu -e LXDE" > /home/${user}/.xsession \
 && chown ${user}:${user} /home/${user}/.xsession;; kubuntuapt-get install --no-install-recommends kubuntu-desktop -y \
 && apt-get install --no-install-recommends libglib2.0-bin -y \
 && echo "startkde" > /home/${user}/.xsession \
 && chown ${user}:${user} /home/${user}/.xsession;; \
esac
#  Fix incomplete language support.
RUN apt-get install --no-install-recommends language-selector-common -y \
 && apt-get install --no-install-recommends language-pack-gnome-en -y \
 && apt-get install --no-install-recommends $( check-language-support ;) -y
