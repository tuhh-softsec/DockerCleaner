...
RUN echo "deb http://ppa.launchpad.net/nginx/stable/ubuntu trusty main" >> /etc/apt/sources.list \
 ...
 && apt-get install --no-install-recommends build-essential couchdb curl git imagemagick language-pack-en libffi6 libffi-dev libssl-dev libxml2-dev libxslt1-dev libjpeg-dev lsof nginx postfix pwgen python-dev python-pip python-setuptools python-software-properties software-properties-common sqlite3 wget --quiet --yes
