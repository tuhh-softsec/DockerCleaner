...
#
RUN apt-get install --no-install-recommends libc6:i386 libstdc++6:i386 -y \
 && apt-get install --no-install-recommends libssl1.0.0:i386 -y \
 && apt-get install --no-install-recommends lsb-release telnet traceroute tcpdump net-tools vim nano -y \
 && apt-get install --no-install-recommends git bison flex -y \
 && apt-get install --no-install-recommends python3-setuptools python3.5 -y \
 && apt-get install --no-install-recommends python3-pip -y \
 && python3.5 -m pip install -U pip
#
# Pre-requis GNS3 server
#
RUN apt-get install --no-install-recommends qemu-kvm qemu-system-x86 vpcs dynamips uuid-runtime -y \
 ...
 && apt-get install --no-install-recommends libpcap-dev -y \
