...
RUN apt-get update \
 && apt-get install --no-install-recommends xorg-dev libgl1-mesa-dev xvfb libxinerama1 libxcursor1 unzip libglu1-mesa libav-tools python3 python3-pip python3-setuptools libpq-dev libjpeg-dev wget curl cmake git -y \
 && apt-get clean \
...
RUN apt-get update
RUN apt-get install --no-install-recommends gazebo8 -y
#  RUN apt-get install -y libglib2.0-dev libgts-dev libgts-dev
RUN apt-get install --no-install-recommends libgazebo8-dev -y
#  setup environment
EXPOSE 11345/tcp
# --------------------
#  Install ROS
...
RUN apt-get install --no-install-recommends cmake gcc g++
...
RUN apt-get install --no-install-recommends libconsole-bridge-dev -y
...
RUN apt-get install --no-install-recommends libtinyxml-dev liblz4-dev libbz2-dev liburdfdom-dev libpoco-dev libtinyxml2-dev -y
...
#  #--------------------
RUN apt-get install --no-install-recommends libeigen3-dev python3-sip python3-sip-dev libyaml-cpp-dev libboost-python-dev unzip -y
...
#  install packages
RUN apt-get update \
 && apt-get install --no-install-recommends clang-format libasio-dev libpocofoundation9v5 libpocofoundation9v5-dbg libssl-dev openssl python3-coverage python3-dev python3-empy python3-mock python3-nose python3-pep8 python3-pip python3-setuptools python3-vcstool python3-yaml uncrustify wget -q -y \
 && rm -rf /var/lib/apt/lists/*
...
RUN apt-get update \
 && apt-get install --no-install-recommends bash-completion python3-defusedxml python3-skimage -y
# --------------------
#  Install baselines
# --------------------
...
RUN apt-get install --no-install-recommends libftdi-dev -y
RUN cd ros_catkin_ws/src \
 && git clone https://github.com/ros-visualization/rqt_robot_dashboard
RUN apt-get install --no-install-recommends libsdl-dev libsdl-image1.2-dev libspnav-dev -y
#  installing pcl-dev causes some conflicts
RUN apt-get install --no-install-recommends libpcl-dev -y; exit 0
RUN cd ros_catkin_ws/src \
