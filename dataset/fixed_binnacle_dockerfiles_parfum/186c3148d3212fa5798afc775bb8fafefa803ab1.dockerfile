...
#  Install modules
RUN apt-get update \
 && apt-get install --no-install-recommends wget build-essential libfreetype6-dev libicu-dev libjpeg62-turbo-dev libmagickwand-6.q16-dev libmcrypt-dev libssl-dev libyaml-dev imagemagick locales re2c unzip g++ zip ssmtp gettext openssh-client pdftk apt-transport-https mysql-client -y \
...
RUN pecl install yaml-2.0.0 imagick APCu
