...
apt-get install --no-install-recommends sudo wget -y \
 ...
 && docker-php-ext-enable aerospike; else curl -L -o /tmp/aerospike-client-php.tar.gz ${AEROSPIKE_PHP_REPOSITORY} ; fi \
...
apt-get install --no-install-recommends freetds-dev libsybdb5 -y \
 ...
 && php -m | grep -q 'sqlsrv'; fi; else apt-get install --no-install-recommends apt-transport-https gnupg -y \
 && curl https://packages.microsoft.com/keys/microsoft.asc | apt-key add - \
 && curl https://packages.microsoft.com/config/debian/9/prod.list > /etc/apt/sources.list.d/mssql-release.list \
 && apt-get update -yqq \
 && ACCEPT_EULA=Y apt-get --no-install-recommends install -y unixodbc unixodbc-dev libgss3 odbcinst msodbcsql17 locales \
...
apt-get install --no-install-recommends libyaml-dev -y; pecl install yaml ; docker-php-ext-enable yaml ; fi
# ##########################################################################
#  Check PHP version:
# ##########################################################################
