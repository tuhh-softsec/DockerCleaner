...
RUN apt-get install --no-install-recommends wget dpkg-dev git patch -y
...
#  Install desktop.
RUN case "${desktop}" ingnomeapt-get install --no-install-recommends task-gnome-desktop -y \
 && service dbus start \
 ...
 && service dbus stop;; xfceapt-get install --no-install-recommends task-xfce-desktop -y \
 && echo "mode: off\n" > /home/${user}/.xscreensaver \
 && chown ${user}:${user} /home/${user}/.xscreensaver;; mateapt-get install --no-install-recommends task-mate-desktop -y \
 && service dbus start \
 ...
 && service dbus stop;; lxdeecho "Package: wicd-daemon\nPin: version *\nPin-Priority: -1\n" > /etc/apt/preferences.d/wicd-daemon \
 && apt-mark hold wicd \
 && apt-get install --no-install-recommends task-lxde-desktop -y \
 && echo "mode: off\n" > /home/${user}/.xscreensaver \
 && chown ${user}:${user} /home/${user}/.xscreensaver;; kdeapt-get install --no-install-recommends task-kde-desktop -y;; \
esac
#  Remove expect.
