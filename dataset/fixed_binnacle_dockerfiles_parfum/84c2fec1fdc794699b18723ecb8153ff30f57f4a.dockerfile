...
RUN apt-get update -y \
 && apt-get upgrade -y \
 && apt-get install --no-install-recommends autoconf bedtools bsdtar build-essential cmake dx-toolkit g++ gcc gettext gfortran git gzip inkscape libc6 libcurl4-openssl-dev libfontconfig libfreetype6-dev libgsl0-dev libgtkmm-3.0-dev libhdf5-serial-dev liblzma-dev liblzo2-dev libpangomm-1.4-dev libpng-dev libpopt-dev libpthread-stubs0-dev librsvg2-bin librsvg2-dev libsqlite3-dev libstdc++6 libx11-dev libxext-dev libxft-dev libxpm-dev libxslt1-dev python-pip sqlite3 wget wkhtmltopdf xvfb zlib1g-dev -y --force-yes
RUN apt-get update
