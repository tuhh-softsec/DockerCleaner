...
RUN apt-get update -qq \
 ...
 && apt-get update -qq
RUN apt-get install --no-install-recommends git wget build-essential cppcheck cmake libopencv-dev libpoco-dev libpocofoundation9v5 libpocofoundation9v5-dbg python-empy python3-dev python3-empy python3-nose python3-pip python3-setuptools python3-vcstool libtinyxml-dev libeigen3-dev libasio-dev libtinyxml2-dev -qq -y
#  special depends for ORK renderer
RUN apt-get install --no-install-recommends libboost-dev libassimp-dev freeglut3-dev libgl1-mesa-dev libfreeimage-dev libxmu-dev libxi-dev libsdl1.2-dev libosmesa6-dev libusb-1.0-0-dev libudev-dev -qq -y
#  special depends for Astra camera driver
RUN apt-get install --no-install-recommends libboost-system-dev libboost-thread-dev -qq -y
