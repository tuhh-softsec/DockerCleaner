...
#  install rxcpp
RUN set -e ; git clone https://github.com/Reactive-Extensions/RxCpp /tmp/RxCpp ; (cd /tmp/RxCpp ;git checkout f38fb8e3c30b01b07c57e08039b2942f1685224d ) ; cmake -DCMAKE_BUILD_TYPE=${CMAKE_BUILD_TYPE} -H/tmp/RxCpp -B/tmp/RxCpp/build ; cmake --build /tmp/RxCpp/build --target install ; rm -rf /tmp/RxCpp
#  install rapidjson
...
#  install soci
RUN set -e ; git clone https://github.com/SOCI/soci /tmp/soci ; (cd /tmp/soci ;git checkout 349ce86b79e63b99ba95200bc4bd1d83791e9094 ) ; cmake -DCMAKE_BUILD_TYPE=${CMAKE_BUILD_TYPE} -DSOCI_CXX_C11=ON -DSOCI_TESTS=OFF -DSOCI_EMPTY=OFF -DWITH_BOOST=ON -DWITH_DB2=OFF -DWITH_FIREBIRD=OFF -DWITH_MYSQL=OFF -DWITH_ODBC=OFF -DWITH_ORACLE=OFF -DWITH_POSTGRESQL=ON -DWITH_SQLITE3=OFF -H/tmp/soci -B/tmp/soci/build ; cmake --build /tmp/soci/build --target install ; ldconfig ; rm -rf /tmp/soci
#  install tbb
