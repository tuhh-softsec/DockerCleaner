...
RUN set -x \
 && apt-get update \
 && DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y sudo wget curl build-essential libcurl4-openssl-dev python-dev python-setuptools software-properties-common python-pip git libjemalloc-dev brotli \
 ...
 && DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y yarn libpq-dev postgresql-client-9.6 libreadline-dev zlib1g-dev flex bison libxml2-dev libxslt1-dev libssl-dev imagemagick ca-certificates rsync \
 && apt-get clean \
