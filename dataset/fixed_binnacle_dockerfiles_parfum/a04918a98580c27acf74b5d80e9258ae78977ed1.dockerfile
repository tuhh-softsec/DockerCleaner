...
#
RUN apt-get update \
 && apt-get install --no-install-recommends gcc libc-dev python python-dev libffi-dev openssl libpcap-dev curl wget iptables iproute2 tcpdump tcsh sudo traceroute iputils-ping net-tools procps emacs jq -yq
#
#  Install Nexus dependencies.
#
RUN apt-get update \
 && apt-get install --no-install-recommends build-essential libdb++-dev libssl1.0-dev -yq
#
