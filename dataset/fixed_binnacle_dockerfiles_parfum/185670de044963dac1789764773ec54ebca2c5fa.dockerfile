...
RUN apt-get update \
 && apt-get install --no-install-recommends git vim jed emacs unzip libsm6 pandoc texlive-latex-base texlive-latex-extra texlive-fonts-extra texlive-fonts-recommended texlive-generic-recommended libxrender1 inkscape wget curl fonts-ipafont-gothic fonts-ipafont-mincho -yq \
 && apt-get clean \
...
# ## ansible
RUN apt-get update \
 && apt-get install --no-install-recommends sshpass openssl ipmitool libssl-dev libffi-dev -y \
 && apt-get clean \
 && rm -rf /var/lib/apt/lists/* \
 && pip install requests paramiko ansible --no-cache-dir
# ## Utilities
RUN apt-get update \
 && apt-get install --no-install-recommends virtinst dnsutils zip tree jq rsync iputils-ping -y \
 && apt-get clean \
 && rm -rf /var/lib/apt/lists/* \
 && pip install netaddr pyapi-gitlab runipy papermill==0.19.0 pysnmp pysnmp-mibs --no-cache-dir
# ## Add files
RUN mkdir -p /etc/ansible \
 && cp /tmp/ansible.cfg /etc/ansible/ansible.cfg
# ### Visualization
RUN pip install folium --no-cache-dir
...
RUN pip install jupyter_nbextensions_configurator --no-cache-dir \
