...
RUN apt-get update \
 && apt-get install --no-install-recommends util-linux ncurses-bin ncurses-base libncursesw5:amd64 -y \
 && apt-get install --no-install-recommends libudev1 libsystemd0 -y \
