...
#  Mysql
RUN apt-get install --no-install-recommends mysql-server libmysqlclient-dev libmysql++-dev -y \
...
#  Php
RUN DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends -y install php-common php-fpm php-mysql php-gd php-zip php-mbstring php-xml
...
RUN apt-get install --no-install-recommends make flex gcc g++ openjdk-11-jdk -y \
