...
RUN apt-get update \
 && apt-get upgrade -y
RUN apt-get install --no-install-recommends autoconf autoconf-archive automake autotools-dev bison clang curl default-jdk faketime flex gdb git libbson-dev libcurl4-gnutls-dev libdbd-mysql libdbi-dev libgcrypt11-dev libglib2.0-dev libgnutls28-dev libgrok1 libgrok-dev libhiredis-dev libkrb5-dev liblz4-dev libmaxminddb-dev libmongoc-dev libmysqlclient-dev libnet1-dev libpcap-dev libpq-dev librabbitmq-dev libsasl2-dev libsnmp-dev libssl-dev libsystemd-dev libtokyocabinet-dev libtool mysql-server net-tools pkg-config postgresql-client python-docutils python-software-properties software-properties-common sudo tcl-dev uuid-dev valgrind vim wget zlib1g-dev -y
...
RUN apt-get update -y \
 && apt-get install --no-install-recommends clickhouse-client clickhouse-server libestr-dev librelp-dev libqpid-proton10-dev libsodium-dev libfastjson-dev liblogging-stdlog-dev gcc-7 libczmq-dev clang-5.0 clang-tools-5.0 liblognorm-dev -y
