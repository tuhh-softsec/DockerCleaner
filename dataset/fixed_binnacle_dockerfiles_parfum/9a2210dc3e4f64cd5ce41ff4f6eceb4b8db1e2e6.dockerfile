...
RUN apt-get install --no-install-recommends libboost-dev libboost-filesystem-dev libboost-program-options-dev libboost-python-dev libboost-regex-dev libboost-system-dev libboost-thread-dev -y
...
RUN apt-get install --no-install-recommends autoconf apache2-dev libtool libxml2-dev libbz2-dev libgeos-dev libgeos++-dev libproj-dev gdal-bin libgdal1-dev mapnik-utils python-mapnik libmapnik-dev -y
#  Install postgresql and postgis
RUN apt-get install --no-install-recommends postgresql-9.3-postgis-2.1 postgresql-contrib postgresql-server-dev-9.3 -y
#  Install osm2pgsql
RUN cd /tmp \
