...
RUN apt-get update \
 && apt-get install --no-install-recommends autoconf automake bzip2 g++ gfortran git libatlas3-base libtool-bin make python2.7 python3 python-pip python-dev python3-dev sox ffmpeg subversion wget zlib1g-dev -y \
...
RUN ln -s -f /usr/bin/python2 /usr/bin/python \
 && apt-get install --no-install-recommends python-numpy python-scipy python3-simplejson python3-pytest -y \
