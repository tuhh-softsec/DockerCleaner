...
RUN apt-get update \
 && apt-get install --no-install-recommends wget python make xz-utils gcc g++ zlib1g-dev libdb5.3 libdb5.3-dev libreadline6-dev libc6-dev-i386 patch zip libffi-dev python-cffi python-cffi-backend gcc-$CROSSCOMPILE_ARCH binutils-$CROSSCOMPILE_ARCH gcc-5-$CROSSCOMPILE_ARCH -y
