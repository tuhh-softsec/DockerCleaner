...
RUN apt-get update -qq \
 && DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y build-essential curl git gnupg libboost-all-dev libcurl4-openssl-dev locales lsb-release moreutils openssh-client protobuf-compiler sudo \
...
RUN apt-get update -qq \
 && DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y jags libatlas3-base libcairo2 libcairo2-dev libfftw3-dev libgdal-dev libgraphviz-dev libgsl0-dev libmysqlclient-dev libnetcdf-dev libprocps-dev libproj-dev libprotoc-dev libquantlib0-dev libsasl2-dev libsqliteodbc libxml2-dev libxt-dev libxt6 odbc-postgresql tdsodbc unixodbc unixodbc-dev wget \
 && rm -rf /var/lib/apt/lists/*
