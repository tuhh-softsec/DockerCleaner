...
#  Curl
RUN apt-get install --no-install-recommends ca-certificates curl -y
#  Build-time utils
RUN DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y apt-transport-https gnupg localepurge
#  Node
...
RUN apt-get update
RUN DOTNET_SKIP_FIRST_TIME_EXPERIENCE=1 apt-get --no-install-recommends install -y dotnet-sdk-2.1
...
RUN apt-get update
RUN apt-get install --no-install-recommends google-chrome-stable -y
...
#  Firefox
RUN apt-get install --no-install-recommends firefox -y
...
RUN apt-get install --no-install-recommends dbus-x11 httping x11vnc xvfb -y
#  Utils for drone-cache.sh
RUN apt-get install --no-install-recommends liblz4-tool
#  Yes :)
RUN apt-get install --no-install-recommends mc -y
#  Use UTF-8
