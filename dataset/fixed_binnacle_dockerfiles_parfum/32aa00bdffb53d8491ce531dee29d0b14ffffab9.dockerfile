...
ARG PIP="pip"
RUN apt-get update \
 && apt-get install --no-install-recommends build-essential curl git libcurl3-dev libfreetype6-dev libhdf5-serial-dev libpng-dev libssl-dev libzmq3-dev openjdk-8-jdk openjdk-8-jre-headless pkg-config rsync software-properties-common unzip zip zlib1g-dev ${PYTHON} ${PYTHON}-dev ${PYTHON}-pip ${PYTHON}-setuptools ${PYTHON}-wheel -y --fix-missing \
 && apt-get clean \
