...
RUN apt-get update -qq -o=Dpkg::Use-Pty=0 \
 && apt-get install --no-install-recommends libaio1=0.3.110-3 xmlstarlet=1.6.1-2 jq=1.5+dfsg-1.3 gettext-base=0.19.8.1-2 dumb-init=1.2.0-1 -qq -o=Dpkg::Use-Pty=0 -y \
 && rm -rf /var/lib/apt/lists/*
