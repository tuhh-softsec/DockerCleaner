...
#  install basics
RUN apt-get update \
 && apt-get install --no-install-recommends apt-transport-https software-properties-common wget -y \
...
#  tools used by wine
RUN apt-get update \
 && apt-get install --no-install-recommends cabextract dos2unix p7zip-full winbind zip -y \
 && rm -rf /var/lib/apt/lists/*
#  setup wine
ENV WINEARCH="win64"
