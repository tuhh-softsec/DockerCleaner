...
#  Install rocm pkgs
RUN apt-get update --allow-insecure-repositories \
 && DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y --allow-unauthenticated rocm-dev rocm-libs rocm-utils rocm-cmake rocfft miopen-hip miopengemm rocblas hipblas rocrand rocm-profiler cxlactivitylogger \
 && apt-get clean \
 && rm -rf /var/lib/apt/lists/*
