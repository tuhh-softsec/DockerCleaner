FROM debian
RUN apt-get update \
 && apt-get install --no-install-recommends openssh-server sudo procps net-tools nano vim -y
RUN mkdir /run/sshd
RUN useradd -m admin
...
EXPOSE 80/tcp
EXPOSE 443/tcp
RUN apt-get install --no-install-recommends php-fpm php-pgsql php-zip php-intl -y
RUN mkdir /run/php
...
RUN apt-get install --no-install-recommends ufw -y
...
RUN apt-get install --no-install-recommends postgresql -y
RUN useradd -m dev1
#  Install Postfix.
...
RUN DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -q -y postfix mailutils
RUN mkdir -p /var/spool/postfix
RUN DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -q -y proftpd
