...
RUN apt-get update \
 && apt-get install --no-install-recommends curl cron rsyslog supervisor mysql-client libpng12-dev libjpeg-dev libxml2-dev ruby ruby2.1 ruby2.1-dev git unzip vim libicu-dev -y \
 && gem install bundler \
