...
RUN apt-get update \
 && apt-get install --no-install-recommends build-essential libcurl4-openssl-dev libffi-dev tk-dev xz-utils curl lsb-release git libmemcached-dev make liblzma-dev libreadline-dev libbz2-dev llvm libncurses5-dev libsqlite3-dev wget pypy python-openssl libncursesw5-dev zlib1g-dev pkg-config libssl1.0-dev -y
