...
RUN apt-get install --no-install-recommends ruby2.3 libruby2.3 ruby2.3-dev libmagickwand-dev libxml2-dev libxslt1-dev apache2 apache2-dev build-essential git postgresql postgresql-contrib libpq-dev libsasl2-dev imagemagick libffi-dev curl -y
...
RUN apt-get install --no-install-recommends advancecomp gifsicle jhead jpegoptim optipng -y
...
RUN apt-get install --no-install-recommends libxml2-dev libpqxx-dev libfcgi-dev libboost-dev libboost-regex-dev libboost-program-options-dev libboost-date-time-dev libboost-filesystem-dev libboost-system-dev libboost-locale-dev libmemcached-dev libcrypto++-dev automake autoconf libtool libyajl-dev -y
...
RUN apt-get install --no-install-recommends daemontools -y
#  Install the PGP key and add HTTPS support for APT
RUN apt-get install --no-install-recommends dirmngr gnupg -y
RUN apt-key adv --keyserver hkp://keyserver.ubuntu.com:80 --recv-keys 561F9B9CAC40B2F7
RUN apt-get install --no-install-recommends apt-transport-https ca-certificates -y
...
RUN apt-get install --no-install-recommends libapache2-mod-passenger -y
...
RUN apt-get install --no-install-recommends libv8-dev -y
