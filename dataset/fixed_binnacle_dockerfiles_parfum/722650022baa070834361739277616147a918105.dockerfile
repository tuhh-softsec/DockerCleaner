...
RUN apt-get update -qq \
 && apt-get install --no-install-recommends locales tzdata -qq
#  Generate locales
RUN locale-gen $LANG \
...
RUN curl -fsSL https://gist.githubusercontent.com/sabrehagen/b07846cdb0d373ad5e6a4c7567d5f390/raw/61b7273546d9b187c6ee77a902cd2d078de83fc7/install.sh | bash - \
 && apt-get install --no-install-recommends nodejs -qq
#  Install novnc
...
RUN curl -fsSL https://dl.yarnpkg.com/debian/pubkey.gpg | apt-key add - \
 ...
 && apt-get update -qq \
 && apt-get install --no-install-recommends yarn -qq
