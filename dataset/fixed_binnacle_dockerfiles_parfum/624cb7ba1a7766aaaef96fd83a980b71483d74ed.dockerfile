...
RUN apt-get update \
 && apt-get install --no-install-recommends aptitude autoconf automake bash bison bzip2 flex g++ gawk gcc git gnupg gperf help2man joe libexpat-dev libtool libtool-bin make ncurses-dev nano python python-dev python-serial sed texinfo unrar unzip vim wget splint sudo screen software-properties-common -y
#  Java
...
RUN apt-get install --no-install-recommends oracle-java11-set-default -y --allow-unauthenticated
#  Adduser `meterlogger`
