...
#  update apt and sudo
RUN apt-get update
RUN apt-get install --no-install-recommends sudo
...
RUN mkdir -p ${SINGNET}
#  install git
RUN sudo apt-get install --no-install-recommends -y git
...
RUN sudo apt-get install --no-install-recommends -y libudev-dev libusb-1.0-0-dev
#  additional tools
RUN sudo apt-get install --no-install-recommends -y net-tools screen vim wget nano jq
#  install IPFS
ENV IPFS_PATH="${IPFS}"
