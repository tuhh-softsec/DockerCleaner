...
RUN apt-get update \
 && apt-get upgrade -y \
 && apt-get install --no-install-recommends -y build-essential git libsqlite3-dev qt5-default
...
RUN apt-get update \
 ...
 && apt-get install -y --no-install-recommends build-essential default-jre-headless gdal-bin git language-pack-en-base libffi-dev libgdal-dev libmagic1 libpq-dev libqt5core5a libsqlite3-mod-spatialite libspatialite7 libspatialite-dev libproxy1v5 libxslt1-dev nodejs osmctools postgresql-client python-dev python-pip python-setuptools python-wheel python-gdal spatialite-bin unzip yarn zip \
 && apt-get clean \
