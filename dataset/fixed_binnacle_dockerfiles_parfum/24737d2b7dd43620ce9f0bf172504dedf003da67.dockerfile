...
RUN apt-get update \
 && apt-get install --no-install-recommends kmod -y \
 && mkdir -p /opt/nvidia \
 && cd /opt/nvidia/ \
...
RUN apt-get update \
 && apt-get install --no-install-recommends cmake pkg-config flex bison curl libpng16-16 libpng-dev libjpeg-turbo8 libjpeg-turbo8-dev zlib1g-dev libhdf5-100 libhdf5-dev libopenblas-base libopenblas-dev gfortran libfreetype6 libfreetype6-dev -y \
