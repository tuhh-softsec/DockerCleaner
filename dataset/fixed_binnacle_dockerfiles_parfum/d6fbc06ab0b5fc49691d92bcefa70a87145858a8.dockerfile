...
RUN apt-get update
RUN apt-get install --no-install-recommends build-essential chrpath libssl-dev libxft-dev -y \
 && apt-get install --no-install-recommends libfreetype6 libfreetype6-dev apt-transport-https git -y
RUN apt-get install --no-install-recommends libfontconfig1 libfontconfig1-dev -y
#  openssl 1.1.0
...
RUN if grep -q Debian /etc/os-release \
 && grep -q jessie /etc/os-release ; then \
 ...
 && apt-get update ; apt install --no-install-recommends -y -t jessie-backports openjdk-8-jre openjdk-8-jre-headless openjdk-8-jdk openjdk-8-jdk-headless; elif grep -q Ubuntu /etc/os-release \
 && grep -q Trusty /etc/os-release ; then \
 ...
 && apt-get update ; apt install --no-install-recommends -y openjdk-8-jre openjdk-8-jre-headless openjdk-8-jdk openjdk-8-jdk-headless; else apt update ; apt install --no-install-recommends -y openjdk-8-jre openjdk-8-jre-headless openjdk-8-jdk openjdk-8-jdk-headless; fi
# # install phantomjs
#
RUN PHANTOMJS_URL="https://circle-downloads.s3.amazonaws.com/circleci-images/cache/linux-amd64/phantomjs-latest.tar.bz2" \
 && apt-get install --no-install-recommends libfontconfig \
...
RUN FIREFOX_URL="https://s3.amazonaws.com/circle-downloads/firefox-mozilla-build_47.0.1-0ubuntu1_amd64.deb" \
 ...
 && apt-get install --no-install-recommends -y libgtk3.0-cil-dev libasound2 libasound2 libdbus-glib-1-2 libdbus-1-3 \
