...
apt-get install --no-install-recommends libssh2-1-dev php${LARADOCK_PHP_VERSION}-ssh2 -y; fi
# ##########################################################################
#  GMP:
# ##########################################################################
USER root
...
apt-get install --no-install-recommends php${LARADOCK_PHP_VERSION}-gmp -y; fi
# ##########################################################################
#  SOAP:
# ##########################################################################
USER root
...
apt-get install --no-install-recommends libxml2-dev php${LARADOCK_PHP_VERSION}-soap -y; fi
# ##########################################################################
#  XSL:
# ##########################################################################
USER root
...
apt-get install --no-install-recommends libxslt-dev php${LARADOCK_PHP_VERSION}-xsl -y; fi
# ##########################################################################
#  LDAP:
# ##########################################################################
...
apt-get install --no-install-recommends php${LARADOCK_PHP_VERSION}-imap -y; fi
# ##########################################################################
#  Subversion:
# ##########################################################################
USER root
...
apt-get install --no-install-recommends subversion -y; fi
# ##########################################################################
#  xDebug:
# ##########################################################################
USER root
...
apt-get install --no-install-recommends php-redis -yqq; fi
# ##########################################################################
#  Swoole EXTENSION
# ##########################################################################
...
apt-get install --no-install-recommends sudo wget -y \
 ...
 && echo 'aerospike.udf.lua_user_path=/usr/local/aerospike/usr-lua' >> /etc/php/${LARADOCK_PHP_VERSION}/cli/conf.d/aerospike.ini; else curl -L -o /tmp/aerospike-client-php.tar.gz https://github.com/aerospike/aerospike-client-php/archive/master.tar.gz ; fi \
...
add-apt-repository -y ppa:pinepain/libv8-archived \
 ...
 && php -m | grep -q 'v8js'; else pecl install v8js ; fi \
...
apt-get install --no-install-recommends php5.6-sybase freetds-bin freetds-common libsybdb5 -y \
 && php -m | grep -q 'mssql' \
 ...
 && apt-get update -yqq \
 && ACCEPT_EULA=Y apt-get --no-install-recommends install -y msodbcsql17 mssql-tools unixodbc unixodbc-dev libgss3 odbcinst locales \
 ...
 && php -m | grep -q 'sqlsrv' \
 && php -m | grep -q 'pdo_sqlsrv'; else pecl install sqlsrv pdo_sqlsrv ; fi \
...
apt-get install --no-install-recommends jpegoptim optipng pngquant gifsicle -y \
...
apt-get install --no-install-recommends imagemagick php-imagick -y; fi
# ##########################################################################
#  Terraform:
# ##########################################################################
USER root
...
apt-get install --no-install-recommends parallel -y; fi
# ##########################################################################
#  Check PHP version:
# ##########################################################################
