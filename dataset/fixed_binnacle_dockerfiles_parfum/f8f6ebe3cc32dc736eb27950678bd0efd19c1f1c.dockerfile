...
RUN apt-get clean \
 ...
 && apt-get upgrade -y
RUN apt-get update \
 && apt-get install --no-install-recommends stretch-backports openjdk-8-jdk-headless -t -y
RUN apt-get install --no-install-recommends systemd-sysv -y
# ###############################  Configure systemd  ###############################
...
RUN apt-get update \
 && apt-get install --no-install-recommends sudo net-tools -y \
 && apt-get clean
#  Editor & unzip
RUN apt-get install --no-install-recommends vim unzip wget -y
RUN apt-get install --no-install-recommends stretch-backports golang -y -t
RUN apt-get install --no-install-recommends node-gyp nodejs-dev -y \
 && apt-get install --no-install-recommends nodejs -y \
 && ln -s /usr/bin/nodejs /usr/bin/node
RUN apt-get -t stretch-backports --no-install-recommends -y install ansible
RUN apt-get clean \
 && apt-get update \
 && apt-get install --no-install-recommends git vim -y
# #################################  Declare local deb repo  #################################
#  Note : declare at the end ; else other commands will fail.
RUN apt-get update \
 && apt-get install --no-install-recommends dpkg-dev -y \
 && apt-get clean
...
RUN apt-get install --no-install-recommends stretch-backports openjdk-8-jdk-headless -t -y
# #################################  CONTAINER SETTINGS  #################################
