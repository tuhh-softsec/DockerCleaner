...
RUN export DEBIAN_FRONTEND=noninteractive \
 ...
 && apt-get install --no-install-recommends cmake g++ gfortran libboost-dev libboost-filesystem-dev libboost-iostreams-dev libboost-math-dev libboost-program-options-dev libboost-system-dev libboost-thread-dev libboost-timer-dev libeigen3-dev libhdf5-mpich-dev liblapack-dev libmpich-dev libopenblas-dev mpich ninja-build pkg-config python3-dev python3-matplotlib python3-numpy python3-pip python3-scipy python3-setuptools -y \
 && apt-get install --no-install-recommends doxygen git graphviz valgrind wget -y \
 && apt-get install --no-install-recommends libglu1 libxcursor-dev libxinerama1 -y \
 && apt-get install --no-install-recommends python3-lxml -y \
 && apt-get clean \
...
RUN apt-get update -qq \
 && apt-get install --no-install-recommends bison flex python -y \
