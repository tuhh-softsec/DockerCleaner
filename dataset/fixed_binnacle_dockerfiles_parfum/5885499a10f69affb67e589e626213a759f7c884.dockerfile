...
RUN sed -i "s/dl-cdn.alpinelinux.org/${ALPINE_URL}/g" /etc/apk/repositories \
 ...
 && PHP_FPM_BUILD_DEPS=" openssl-dev libmemcached-dev cyrus-sasl-dev postgresql-dev zlib-dev libpng-dev freetype-dev libjpeg-turbo-dev libxpm-dev libwebp-dev yaml-dev libexif-dev libxslt-dev gmp-dev xmlrpc-c-dev bzip2-dev enchant-dev imap-dev gettext-dev libwebp-dev icu-dev nghttp2-dev ${APK_DEV_EXTRA:-} " \
 ...
 && mv /usr/local/etc/php/conf.d/docker-php-ext-xdebug.ini /usr/local/etc/php/conf.d/docker-php-ext-xdebug.ini.default ; rm -rf /tmp/* \
