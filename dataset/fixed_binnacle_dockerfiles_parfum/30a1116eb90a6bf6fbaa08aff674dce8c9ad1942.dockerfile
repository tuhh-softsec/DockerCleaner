...
RUN apt-get install --no-install-recommends python-software-properties software-properties-common -y
#  Vips!
RUN apt-get install --no-install-recommends automake build-essential gtk-doc-tools libglib2.0-dev libjpeg-turbo8-dev libpng12-dev libwebp-dev libtiff5-dev libexif-dev libgsf-1-dev liblcms2-dev libxml2-dev swig libmagickcore-dev curl -y
...
RUN apt-get install --no-install-recommends poppler-utils poppler-data ghostscript libreoffice -y
RUN apt-get install --no-install-recommends libmagic-dev libmagickwand-dev ffmpeg libavcodec-ffmpeg-extra56 libvorbis-dev -y
RUN apt-get install --no-install-recommends graphicsmagick graphicsmagick-libmagick-dev-compat -y
#  Database connection libraries
RUN apt-get install --no-install-recommends libmysqlclient-dev -y
...
#  Rails requires this
RUN apt-get install --no-install-recommends tzdata -y
#  Dependencies for Ruby
RUN apt-get install --no-install-recommends libssl-dev libreadline-dev -y
