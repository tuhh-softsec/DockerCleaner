...
RUN apt-get update \
 && apt-get install --no-install-recommends bc build-essential dc git libopenblas-dev nano nodejs npm perl-modules python tar tcsh unzip wget -y
...
RUN apt-get install --no-install-recommends libeigen3-dev libfftw3-dev libtiff5-dev zlib1g-dev -y
