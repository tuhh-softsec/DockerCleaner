...
#  Install apt tools
RUN apt-get update
RUN apt-get install --no-install-recommends software-properties-common python-software-properties wget -y
#  In case lldb needs built
RUN apt-get install --no-install-recommends libz-dev swig ncurses-dev -y
#  Make the developer's life not suck
RUN apt-add-repository -y ppa:neovim-ppa/stable
RUN apt-get update -y
RUN apt-get install --no-install-recommends sudo zsh tmux curl vim neovim -y
#  Debugging tools
RUN apt-get install --no-install-recommends strace htop psmisc -y
#  Python 3.7 and misc
RUN add-apt-repository -y ppa:deadsnakes/ppa
RUN apt-get update -y
RUN apt-get install --no-install-recommends python3.7 -y
...
RUN apt-get install --no-install-recommends ninja-build -y
RUN apt-get install --no-install-recommends flex bison -y
#  Install x86 compilers
RUN apt-get install --no-install-recommends g++-multilib -y
RUN apt-get install --no-install-recommends clang-7 -y
RUN ln -s /usr/bin/clang-7 /usr/local/bin/clang
RUN ln -s /usr/bin/clang++-7 /usr/local/bin/clang++
#  Install arm compilers
RUN apt-get install --no-install-recommends g++-multilib-arm-linux-gnueabi -y
#  Install mingw compilers
RUN apt-get install --no-install-recommends g++-mingw-w64-x86-64 -y
RUN apt-get install --no-install-recommends g++-mingw-w64-i686 -y
RUN update-alternatives --set i686-w64-mingw32-gcc /usr/bin/i686-w64-mingw32-gcc-posix
RUN update-alternatives --set i686-w64-mingw32-g++ /usr/bin/i686-w64-mingw32-g++-posix
RUN update-alternatives --set x86_64-w64-mingw32-gcc /usr/bin/x86_64-w64-mingw32-gcc-posix
RUN update-alternatives --set x86_64-w64-mingw32-g++ /usr/bin/x86_64-w64-mingw32-g++-posix
#  Install test dependencies
RUN apt-get install --no-install-recommends git lldb-7 python-lldb-7 gdb clang-format-7 make dejagnu -y
RUN ln -s /usr/bin/lldb-7 /usr/local/bin/lldb
RUN ln -s /usr/bin/clang-format-7 /usr/local/bin/clang-format
#  Install documentation dependencies
RUN apt-get install --no-install-recommends doxygen graphviz -y
