...
RUN apt-get update -qq \
 && apt-get install --no-install-recommends openssl git nano apt-transport-https curl zip unzip -y -qq
...
RUN apt-get install --no-install-recommends libxml2-dev libmcrypt-dev libssl-dev libicu-dev libxslt-dev libfreetype6-dev libjpeg62-turbo-dev libpng-dev -y -qq
#  Add Yarn as PPA
...
#  Intall Yarn
RUN apt-get install --no-install-recommends yarn -y -qq
#  Set timezone
