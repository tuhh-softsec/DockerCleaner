...
# ###############################################################################
RUN apt-get update \
 && apt-get install --no-install-recommends build-essential wget -q -y
RUN cd /tmp \
...
# ###############################################################################
RUN apt-get update \
 && apt-get install --no-install-recommends git libeigen3-dev libgoogle-glog-dev libleveldb-dev liblmdb-dev libopencv-dev libprotobuf-dev libsnappy-dev zlib1g-dev libbz2-dev protobuf-compiler python-dev python-pip -q -y
RUN cd /tmp \
...
RUN apt-get install --no-install-recommends autoconf libtool -q -y
...
# ###############################################################################
RUN apt-get install --no-install-recommends gfortran graphviz libatlas-base-dev vim -q -y
