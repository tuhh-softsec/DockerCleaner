...
RUN apt-get update \
 && apt-get install --no-install-recommends git vim openssh-client postgresql-client-9.5 postgresql-9.5 postgresql-server-dev-9.5 python2.7 -y \
 && apt-get install --no-install-recommends python-pip ipython python2.7-dev -y \
 && apt-get install --no-install-recommends texlive-base texlive-generic-recommended texlive-fonts-recommended texlive-latex-base texlive-latex-recommended texlive-latex-extra dvipng dvidvi graphviz -y \
 && rm -rf /var/lib/apt/lists/* \
 && apt-get clean all
...
RUN apt-get update \
 && apt-get install --no-install-recommends sudo locate openssh-server openmpi-bin rabbitmq-server libkrb5-dev -y \
 && rm -rf /var/lib/apt/lists/* \
 && apt-get clean all
