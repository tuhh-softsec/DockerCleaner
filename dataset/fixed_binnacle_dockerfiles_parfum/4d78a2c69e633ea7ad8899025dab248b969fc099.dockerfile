...
RUN apt-get install --no-install-recommends ponyc -qq -y
# Swift
...
RUN apt-get install --no-install-recommends powershell -y
#  apt languages
RUN DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -qq -y cython fp-compiler gfortran ocaml perl6 sbcl tcl guile-2.2 luajit
#  asdf languages
