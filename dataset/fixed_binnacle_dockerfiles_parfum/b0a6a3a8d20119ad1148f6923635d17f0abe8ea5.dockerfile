...
RUN apt-get update \
 && DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y --allow-unauthenticated --force-yes build-essential m4 apt-utils libffi-dev libssl-dev libbz2-dev libgmp3-dev libev-dev libsnappy-dev libxen-dev help2man pkg-config time aspcud wget rsync darcs git unzip protobuf-compiler libgcrypt20-dev libjerasure-dev yasm automake python-dev python-pip debhelper psmisc strace curl g++ libgflags-dev sudo libtool libboost1.57-all-dev libboost1.57-all fuse sysstat ncurses-dev cmake libgtest-dev clang-3.5 liblttng-ust0 librdmacm1 libtokyocabinet9 libstdc++6:amd64 libzmq3 librabbitmq1 libomnithread3c2 libomniorb4-1 libhiredis0.10 liblz4-1 libxio-dev libxio0 omniorb-nameserver libunwind8-dev libaio1 libaio1-dbg libaio-dev libz-dev libbz2-dev libgoogle-glog-dev libibverbs-dev librdmacm-dev software-properties-common
RUN add-apt-repository -y ppa:ubuntu-toolchain-r/test
RUN apt-get update \
 && apt-get install --no-install-recommends g++-5 -y
...
RUN apt-get update \
 && apt-get install --no-install-recommends alba arakoon liburiparser1 gdb -y --force-yes
