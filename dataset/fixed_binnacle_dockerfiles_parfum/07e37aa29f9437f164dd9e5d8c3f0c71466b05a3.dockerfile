...
RUN apt-get update -y \
 && apt-get install --no-install-recommends fontconfig fonts-ipafont-gothic fonts-ipafont-mincho subversion -y \
 && rm -rf /var/lib/apt/lists/* \
 && apt-get clean \
...
RUN wget -q -O - https://dl-ssl.google.com/linux/linux_signing_key.pub | apt-key add - \
 ...
 && apt-get update -y \
 && apt-get install --no-install-recommends google-chrome-stable xvfb poppler-utils git -y \
 && rm -rf /var/lib/apt/lists/* \
 && apt-get clean
