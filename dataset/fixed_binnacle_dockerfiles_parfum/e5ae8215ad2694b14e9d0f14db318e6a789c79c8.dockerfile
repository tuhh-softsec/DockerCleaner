...
# Common deps
RUN apt-get update \
 && apt-get install --no-install-recommends curl xz-utils wget gpg -y
...
RUN apt-get install --no-install-recommends git sudo -y
# LSPs
# #GO
...
# Java
RUN apt-get update \
 && apt-get install --no-install-recommends openjdk-8-jdk maven gradle -y
# C/C++
#  public LLVM PPA, development version of LLVM
RUN wget -O - https://apt.llvm.org/llvm-snapshot.gpg.key | apt-key add - \
 && echo "deb http://apt.llvm.org/bionic/ llvm-toolchain-bionic main" > /etc/apt/sources.list.d/llvm.list \
 && apt-get update \
 && apt-get install --no-install-recommends clang-tools-9 -y \
...
# PHP
RUN apt-get install --no-install-recommends php curl php-cli php-mbstring unzip -y
...
# Ruby
RUN apt-get install --no-install-recommends ruby ruby-dev zlib1g-dev -y \
...
RUN apt-get update \
 && apt-get install --no-install-recommends python build-essential -y
USER theia
ARG version=latest
WORKDIR /home/theia
