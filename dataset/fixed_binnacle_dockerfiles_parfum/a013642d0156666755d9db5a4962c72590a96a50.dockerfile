...
RUN apt-get install --no-install-recommends python3-yaml -y
RUN apt-get install --no-install-recommends python3-setuptools -y
...
WORKDIR /Downloads
COPY http://buildbot.opencog.org/hidden/DepthSenseSDK-1.4.3-1527-amd64.deb /Downloads/DepthSenseSDK-1.4.3-1527-amd64.deb
...
WORKDIR /catkin_ws
RUN apt-get install --no-install-recommends ros-hydro-cv-camera -y
