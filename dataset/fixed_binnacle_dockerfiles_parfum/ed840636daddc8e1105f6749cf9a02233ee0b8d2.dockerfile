...
RUN apt-get update
RUN apt-get install --no-install-recommends curl apt-transport-https ca-certificates curl software-properties-common -yq
RUN curl -fsSL https://download.docker.com/linux/ubuntu/gpg | apt-key add - \
 && apt-key fingerprint 0EBFCD88 \
 && add-apt-repository "deb [arch=amd64] https://download.docker.com/linux/ubuntu $( lsb_release -cs ;) stable" \
 && apt-get update
RUN apt-get install --no-install-recommends unattended-upgrades docker-ce=18.06.0~ce~3-0~ubuntu lvm2 build-essential git-core gstreamer1.0-alsa gstreamer1.0-plugins-bad gstreamer1.0-plugins-base gstreamer1.0-plugins-good gstreamer1.0-plugins-ugly gstreamer1.0-tools pbuilder python-mock python-configobj dh-python cdbs python-pip jq rsyslog-gnutls openvpn rng-tools liblz4-tool linux-image-generic linux-headers-generic dkms -yq
...
RUN apt-get install --no-install-recommends python-statsd -y
