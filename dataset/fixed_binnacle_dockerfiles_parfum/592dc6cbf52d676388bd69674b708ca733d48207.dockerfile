FROM php:7.1-apache
RUN apt-get update \
 && apt-get install --no-install-recommends libfreetype6-dev libjpeg62-turbo-dev libmcrypt-dev libpng-dev libxslt-dev libicu-dev mysql-client pv vim bash-completion openssh-server ssl-cert msmtp sudo dnsutils iputils-ping iputils-tracepath host telnet unzip gnupg -y \
