...
RUN source /opt/intel/mkl/bin/mklvars.sh intel64 \
 ...
 && cd $SOURCES_DIR \
 && ./configure --with-mpi=1 --with-cc=mpicc --with-cxx=mpicxx --with-fc=mpif90 -with-blas-lapack-dir=/opt/intel/mkl/lib/intel64 --download-hypre=https://github.com/LLNL/hypre/archive/v2.14.0.tar.gz --with-x=0 --with-shared=1 --with-64-bit-indices --with-debugging=0 --prefix=$PETSC_INSTALL \
 && make \
 && make install \
