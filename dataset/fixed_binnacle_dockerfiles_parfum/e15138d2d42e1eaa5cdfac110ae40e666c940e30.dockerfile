...
RUN DEBIAN_FRONTEND=noninteractive add-apt-repository ppa:ubuntu-toolchain-r/test \
 && apt-get update
RUN DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y software-properties-common cmake
RUN DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y libgtkmm-3.0-dev libgstreamer1.0-dev libgstreamer-plugins-base1.0-dev libgstreamer-plugins-bad1.0-dev gstreamer1.0-libav libwebkitgtk-3.0-dev gstreamer1.0-tools
RUN DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y mesa-common-dev libgl1-mesa-dev libgles2-mesa-dev build-essential autoconf autogen libtool
RUN DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y curl libxml-parser-perl git g++-8 libffi-dev libmount-dev libssl-dev unzip
