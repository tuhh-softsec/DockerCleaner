...
RUN apt-get update \
 && apt-get install --no-install-recommends wget curl build-essential libpq-dev -y
# #############################################################################
...
RUN apt-get update \
 && apt-get install --no-install-recommends supervisor openssh-server postgresql postgresql-contrib nodejs nginx redis-server git -y
