...
RUN set -eux ; if [[ $PHP_VERSION == 7.1.* || $PHP_VERSION == 7.2.* || $PHP_VERSION == 7.3.* ]] ; then \
apt-get update \
 ...
 && ACCEPT_EULA=Y apt-get --no-install-recommends install -y msodbcsql17 unixodbc-dev; pecl install sqlsrv pdo_sqlsrv \
 && echo extension=pdo_sqlsrv.so >> `php --ini | grep "Scan for additional .ini files" | sed -e "s|.*:\\s*||" `/30-pdo_sqlsrv.ini \
 && echo extension=sqlsrv.so >> `php --ini | grep "Scan for additional .ini files" | sed -e "s|.*:\\s*||" `/20-sqlsrv.inifi
