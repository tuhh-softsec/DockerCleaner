...
RUN set -ex \
 ...
 && apk add --update --no-cache -t .mariadb-run-deps libaio libstdc++ make pwgen sudo tzdata $( test "${MARIADB_VER:0:4}" = "10.1" \
 && echo 'libressl-dev' || echo 'openssl-dev' ;) \
 && apk add --update --no-cache -t .mariadb-build-deps attr autoconf bison build-base cmake coreutils gnupg libaio-dev linux-headers ncurses-dev patch readline-dev zlib-dev \
