...
RUN yum -y update \
 && yum install -y --setopt=skip_missing_names_on_install=False unzip rsync java-1.8.0-openjdk java-1.8.0-openjdk-devel \
 && yum clean all \
 && curl https://archive.apache.org/dist/maven/maven-3/3.5.4/binaries/apache-maven-3.5.4-bin.tar.gz | tar -xzf - -C /opt \
