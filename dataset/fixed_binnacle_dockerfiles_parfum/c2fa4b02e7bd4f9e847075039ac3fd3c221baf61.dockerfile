...
RUN apt-get update \
 && apt-get install --no-install-recommends autoconf automake build-essential cmake curl g++ gcc git libprotobuf-dev libprotobuf9v5 libreadline-dev libtool libzmq-dev pkg-config protobuf-compiler unzip wget -y
#  Fetch Intel MKL.
RUN mkdir /root/mkl \
...
RUN apt-get update \
 && apt-get install --no-install-recommends libgomp1 libprotobuf9v5 libzmq1 python3 -y \
 && apt-get clean \
