...
RUN apt-get update \
 ...
 && apt-get install --no-install-recommends pv locate inkscape -y \
 && updatedb
#  [ install pip2/3 ]
RUN apt-get update
RUN apt-get install --no-install-recommends python-dev python-pip python3-dev python3-pip -y
...
RUN apt-get install --no-install-recommends libcanberra-gtk-module python3-tk -y
#  [ define docker user ]
