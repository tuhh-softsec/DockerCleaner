...
RUN apt-get update \
 && apt-get install --no-install-recommends libmpc2 libmpfr4 libgmp10 git-core tar libssl-dev libnuma-dev rubygems build-essential rake ncurses-dev python-dev devscripts debhelper ed man curl libc6-i386 lsb-release cpp-4.6- gcc-4.6- -y \
 && rm -f /var/cache/apt/archives/*.deb
