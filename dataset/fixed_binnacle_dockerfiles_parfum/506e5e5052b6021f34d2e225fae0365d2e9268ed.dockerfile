...
RUN apt-get update \
 && apt-get install --no-install-recommends git cmake build-essential vim supervisor libpcre3-dev pkg-config libavl-dev libev-dev libprotobuf-c-dev protobuf-c-compiler libssh-dev libssl-dev swig python-dev -y
#  add netconf user
...
RUN apt-get update \
 && apt-get install --no-install-recommends nodejs npm byacc autoconf yodl -y
#  link nodejs to node
