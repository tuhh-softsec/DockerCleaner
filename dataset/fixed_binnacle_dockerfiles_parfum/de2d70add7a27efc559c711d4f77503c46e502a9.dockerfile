...
EXPOSE 8080/tcp
RUN apt-get update \
 && apt-get install --no-install-recommends locales sudo procps wget unzip gnupg -y \
...
RUN sudo apt-get update \
 && sudo apt-get install --no-install-recommends -y curl ansible openssh-client sshpass socat dnsutils jq less vim netcat-openbsd git \
 && apt-get -y autoremove \
 && apt-get -y clean \
 && rm -rf /var/lib/apt/lists/*
...
RUN apt-get update \
 && sudo apt-get install --no-install-recommends -y apt-transport-https ca-certificates curl gnupg2 software-properties-common
...
RUN apt-get update \
 && apt-get install --no-install-recommends azure-cli
USER user
ARG VERSION
