...
RUN apt-get update \
 && apt-get install --no-install-recommends unzip xz-utils curl bc git build-essential cpio gcc-multilib libc6-i386 libc6-dev-i386 kmod squashfs-tools genisoimage xorriso syslinux automake pkg-config p7zip-full -y
...
RUN mkdir -p /vboxguest \
 ...
 && mkdir amd64 \
 && tar -C amd64 -xjf VBoxGuestAdditions-amd64.tar.bz2 \
 && mkdir x86 \
 && tar -C x86 -xjf VBoxGuestAdditions-x86.tar.bz2 \
