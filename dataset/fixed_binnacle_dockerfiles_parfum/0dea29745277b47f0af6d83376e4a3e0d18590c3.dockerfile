...
#  Install PHP
RUN set -xe ; mkdir -p $SRC_DIR ; cd $SRC_DIR ; wget -O php.tar.xz "$PHP_URL" ; echo "$PHP_DOWNLOAD_SHA *php.tar.xz" | sha256sum -c - ; wget -O php.tar.xz.asc "$PHP_ASC_URL" ; export GNUPGHOME="$( mktemp -d ;)" ; for key in $GPG_KEYS; do (gpg --keyserver hkp://p80.pool.sks-keyservers.net:80 --recv-keys "$key" || gpg --keyserver pgp.mit.edu --recv-keys "$key" || gpg --keyserver keyserver.pgp.com --recv-keys "$key" ) ; done ; gpg --batch --verify php.tar.xz.asc php.tar.xz ; rm -rf "$GNUPGHOME" ; set -eux ; savedAptMark="$( apt-mark showmanual ;)" ; apt-get update ; apt-get install --no-install-recommends libedit-dev=3.1-* dpkg-dev=1.17.* -y ; rm -rf /var/lib/apt/lists/* ; apt-get clean ; export CFLAGS="$PHP_CFLAGS" CPPFLAGS="$PHP_CPPFLAGS" LDFLAGS="$PHP_LDFLAGS" ; mkdir -p $PHP_SRC_DIR ; tar -Jxf $SRC_DIR/php.tar.xz -C $PHP_SRC_DIR --strip-components=1 ; cd $SRC_DIR/php ; gnuArch="$( dpkg-architecture -qDEB_BUILD_GNU_TYPE ;)" ; debMultiarch="$( dpkg-architecture -qDEB_BUILD_MULTIARCH ;)" ; if [ ! -d /usr/include/curl ] ; then ln -sT "/usr/include/$debMultiarch/curl" /usr/local/include/curl ; fi ; ./configure --build="$gnuArch" --with-config-file-path="$PHP_INI_DIR" --with-config-file-scan-dir="$PHP_INI_DIR/conf.d" --disable-cgi --enable-ftp --enable-mbstring --enable-mysqlnd --with-password-argon2 --with-curl --with-libedit --with-openssl --with-zlib $( test "$gnuArch" = 's390x-linux-gnu' \
 && echo '--without-pcre-jit' ;) --with-libdir="lib/$debMultiarch" ${PHP_EXTRA_CONFIGURE_ARGS:-} ; make -j "$( nproc ;)" ; make test ; make install ; find /usr/local/bin /usr/local/sbin -type f -executable -exec strip --strip-all '{}' + || true ; make clean ; cd / ; rm -rf $PHP_SRC_DIR ; apt-mark auto '.*' > /dev/null; [ -z "$savedAptMark" ] || apt-mark manual $savedAptMark ; find /usr/local -type f -executable -exec ldd '{}' ';' | awk '/=>/ { print $(NF-1) }' | sort -u | xargs -r dpkg-query --search | cut -d: -f1 | sort -u | xargs -r apt-mark manual ; apt-get purge -y --auto-remove -o APT::AutoRemove::RecommendsImportant=false ; php --version ; pecl update-channels ; rm -rf /tmp/pear ~/.pearrc ; mkdir "$PHP_INI_DIR" ; mkdir "$PHP_INI_DIR/conf.d" ; touch "$PHP_INI_DIR/conf.d/memory.ini" \
 && echo "memory_limit = 1G;" >> "$PHP_INI_DIR/conf.d/memory.ini"
ENV PATH="$PHPPATH/bin:/usr/local/php/bin:$PATH"
#  Install Composer globally
RUN set -xe ; wget $COMPOSER_URL ; echo "$COMPOSER_DOWNLOAD_SHA *composer.phar" | sha256sum -c - ; mv ./composer.phar /usr/local/bin/composer ; chmod +x /usr/local/bin/composer
WORKDIR $PHPPATH
