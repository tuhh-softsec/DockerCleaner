...
RUN apt-get update \
 && apt-get install --no-install-recommends bash-completion bc clang-7 ca-certificates curl dkms gnupg2 gcc gdb jq libc6-dev libelf-dev llvm-7 netcat xz-utils -y \
...
RUN curl -s https://s3.amazonaws.com/download.draios.com/DRAIOS-GPG-KEY.public | apt-key add - \
 ...
 && apt-get update \
 && apt-get install --no-install-recommends falco -y \
 && apt-get clean \
 && rm -rf /var/lib/apt/lists/*
