...
#  Install prereqs
RUN apt-get update \
 && DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y build-essential checkinstall wget bzip2 ca-certificates sudo locales fonts-liberation \
 && apt-get clean \
...
RUN pip install https://storage.googleapis.com/tensorflow/linux/gpu/tensorflow_gpu-1.8.0-cp36-cp36m-linux_x86_64.whl --ignore-installed --upgrade
# # Install Libraries
RUN pip install tflearn==0.3.2 keras==2.2.0 nltk==3.3 gensim==3.4.0 gym==0.9.6 http://download.pytorch.org/whl/cpu/torch-0.4.0-cp36-cp36m-linux_x86_64.whl torchvision==0.2.1 h5py==2.8.0
