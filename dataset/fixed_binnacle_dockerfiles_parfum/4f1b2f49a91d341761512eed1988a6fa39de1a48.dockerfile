...
USER root
RUN apt-get update && apt-get install --no-install-recommends wget git cmake build-essential libgl1-mesa-dev libfreetype6-dev swig libglu1-mesa-dev libzmq3-dev libsqlite3-dev libboost-all-dev libicu-dev python3-dev -y
#  OCE
WORKDIR /opt/build
...
COPY https://api.github.com/repos/IfcOpenShell/IfcOpenShell/git/refs/heads/v0.6.0 /tmp/ifcopenshell_version.json
RUN git pull
