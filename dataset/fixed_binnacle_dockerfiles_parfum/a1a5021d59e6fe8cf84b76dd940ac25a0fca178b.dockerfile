...
RUN apt-get update \
 && apt-get install --no-install-recommends git qt57base qt57svg build-essential mesa-common-dev libglu1-mesa-dev -y \
...
RUN apt-get update \
 && apt-get install --no-install-recommends ruby-dev rpm wget bsdmainutils zlib1g-dev -y \
 && gem install fpm \
