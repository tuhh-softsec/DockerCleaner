...
RUN apt-get update \
 && apt-get install --no-install-recommends -y autoconf automake build-essential cmake curl g++ gcc git libprotobuf-dev libprotobuf9v5 libreadline-dev libtool libzmq-dev pkg-config protobuf-compiler unzip nano wget
#  Fetch Intel MKL.
RUN mkdir /root/mkl \
...
COPY . $ONMT_DIR
COPY https://github.com/${GITHUB_ACCOUNT}/${GITHUB_REPO}/raw/${GITHUB_BRANCH}/${SERVICE_FOLDER}/requirements.txt /tmp/service_requirements.txt
RUN pip3 install -r /tmp/service_requirements.txt
COPY https://api.github.com/repos/${GITHUB_ACCOUNT}/${GITHUB_REPO}/compare/${GITHUB_BRANCH}...HEAD /dev/null
