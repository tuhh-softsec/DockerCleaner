...
RUN tce-load -wic bzip2-dev flex file gcc make linux-4.2.1_api_headers glibc_add_lib glibc_base-dev openssl-dev gdbm-dev ncurses-dev readline-dev sqlite3-dev liblzma-dev zlib_base-dev tk-dev libX11-dev libXss xorg-proto \
 ...
 && for PKG in binutils.tcz cloog.tcz flex.tcz file.tcz gcc.tcz gcc_libs.tcz linux-4.2.1_api_headers.tcz make.tcz sqlite3-bin.tcz xz.tcz xorg-proto.tcz; do echo "Removing $PKG files" ;for F in `unsquashfs -l $PKG | grep squashfs-root | sed -e 's/squashfs-root//' `; do [ -f $F -o -L $F ] \
 && sudo rm -f $F ; done ;INSTALLED=$( echo -n $PKG | sed -e s/.tcz// ;) ;sudo rm -f /usr/local/tce.installed/$INSTALLED ; done \
