...
RUN apt-get update \
 && apt-get install --no-install-recommends apt-utils software-properties-common apt-transport-https gnupg -y
COPY orc8r/tools/ansible/roles/pkgrepo/files/jfrog.pub /tmp/jfrog.pub
RUN apt-key add /tmp/jfrog.pub \
 && apt-add-repository "deb https://magma.jfrog.io/magma/list/dev/ stretch main"
#  Install dependencies required for building
RUN apt-get update -y \
 && apt-get install --no-install-recommends sudo curl wget unzip cmake git build-essential autoconf libtool pkg-config libgflags-dev libgtest-dev clang-3.8 libc++-dev protobuf-compiler grpc-dev ninja-build autogen ccache libprotoc-dev libxml2-dev libxslt-dev libyaml-cpp-dev nlohmann-json-dev magma-cpp-redis libgoogle-glog-dev prometheus-cpp-dev libfolly-dev magma-libfluid libdouble-conversion-dev libboost-chrono-dev -y
...
RUN apt-get update -y \
 && apt-get install --no-install-recommends curl sudo prometheus-cpp-dev magma-libfluid python3-lxml bridge-utils libyaml-cpp-dev libgoogle-glog-dev libfolly-dev libdouble-conversion-dev libboost-chrono-dev nlohmann-json-dev redis-server python-redis magma-cpp-redis grpc-dev protobuf-compiler libprotoc-dev -y
