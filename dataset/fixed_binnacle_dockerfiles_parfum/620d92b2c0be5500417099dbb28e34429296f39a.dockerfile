FROM akabe/ocaml:debian8_ocaml4.06.0
...
RUN sudo apt-key adv --recv-keys --keyserver keyserver.ubuntu.com 0xcbcb082a1bb943db \
 ...
 && sudo apt-get install --no-install-recommends -y m4 git xz-utils openssl rsync gcc gfortran aspcud pkg-config ssh libzmq3-dev libffi-dev libffi-dev libgmp-dev libgmp-dev libcairo2-dev libcairo2-dev libplplot-dev plplot12-driver-cairo libgsl0-dev libfftw3-dev libsvm-dev libmariadb-dev libpq-dev libsqlite3-dev mecab libmecab-dev mecab-ipadic-utf8 imagemagick ffmpeg libffi-dev libcairo2-dev libgmp-dev \
 && sudo apt-get install --no-install-recommends -t jessie-backports -y libblas3 libblas-dev liblapack3 liblapack-dev libopenblas-dev liblapacke liblapacke-dev phantomjs \
