...
ENV USER="jenkins"
RUN yum -y update \
 && yum -y install alsa-lib-devel bind-utils bison bzip2 ca-certificates cups-devel cpio curl-devel `` elfutils-libelf-devel expat-devel `` file-devel file-libs flex fontconfig fontconfig-devel freetype-devel gettext gettext-devel `` glibc glibc-common glibc-devel gmp-devel java-1.7.0-openjdk-devel java-1.8.0-openjdk-devel lbzip2 libdwarf libstdc++-static libX11-devel libXext-devel libXi-devel libXrandr-devel libXrender-devel libXt-devel libXtst-devel make mesa-libGL-devel mpfr-devel nasm numactl-devel ntp openssl-devel `` openssh-server openssh-clients perl-CPAN perl-DBI perl-devel perl-ExtUtils-MakeMaker `` perl-GD perl-libwww-perl perl-Time-HiRes unzip vim wget xorg-x11-server-Xvfb xz zip zlib-devel `` \
 && yum clean all
RUN yum -y update \
