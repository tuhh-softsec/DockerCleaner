...
RUN apt-get update
RUN apt-get install --no-install-recommends curl wget nano -y
...
RUN apt-get update
RUN apt-get install --no-install-recommends mongodb-org -y -q
...
#  Install Ruby
RUN apt-get install --no-install-recommends ruby1.9.1-full -y
#  Install rsync
