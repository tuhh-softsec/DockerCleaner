...
RUN apt-get update -qq \
 && apt-get install --no-install-recommends software-properties-common python-software-properties inotify-tools debconf-utils -y \
 && apt-get install --no-install-recommends git wget unzip default-jre openjdk-7-jre php5 -y
WORKDIR /root
