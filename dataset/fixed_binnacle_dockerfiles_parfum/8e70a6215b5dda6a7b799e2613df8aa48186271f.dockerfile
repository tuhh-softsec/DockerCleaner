...
RUN apt-get update
RUN apt-get install --no-install-recommends ros-kinetic-perception=1.3.1-0* -y
...
RUN apt-get update
RUN apt-get install --no-install-recommends software-properties-common gnupg curl -y
#  Git LFS
...
RUN apt-get update
RUN apt-get install --no-install-recommends mplayer mencoder ffmpeg -y
#  gstreamer0.10-ffmpeg
#  Other stuff for Duckiebot / duckietop
RUN apt-get install --no-install-recommends clang -y
