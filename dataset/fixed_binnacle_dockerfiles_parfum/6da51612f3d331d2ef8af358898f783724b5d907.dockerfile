...
RUN apt-get update -y \
 && apt-get install --no-install-recommends autoconf automake avrdude avr-libc bash-completion binutils-avr bison bossa-cli bpython bpython3 ckermit cloc cppcheck doxygen emacs24 flex g++ gawk gcc gcc-arm-none-eabi gcc-avr gdb-avr genext2fs git gitk gperf help2man lcov libexpat-dev libtool libtool-bin make ncurses-dev pmccabe python python3 python3-pip python-pip python-lzma python-pyelftools sed texinfo u-boot-tools unrar unzip valgrind wget -y
RUN pip install breathe pyserial readchar sphinx sphinx_rtd_theme xpect
RUN cd /opt \
