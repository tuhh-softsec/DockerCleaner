...
RUN apt-get install --no-install-recommends software-properties-common vim curl wget gawk sed findutils bc less htop man unzip git lsb-release build-essential make python-all libssl-dev -qq -y
RUN apt-get install --no-install-recommends openssh-server -qq -y
ENV HOME="/root"
...
RUN source $NVM_DIR/nvm.sh ; nvm install $NODE_VERSION \
 ...
 && nvm use default
RUN apt-get install --no-install-recommends ca-certificates g++ gcc libc6-dev curl mercurial bzr git-core -qq -y
...
RUN source $NVM_DIR/nvm.sh ; echo "nvm: " `nvm --version ` \
 ...
 && npm install source-map-support gulp@"^3.9.1" bower@"^1.7.9" -g
RUN echo -e '{''\n'' "allow_root": true''\n''}' >> $HOME/.bowerrc
