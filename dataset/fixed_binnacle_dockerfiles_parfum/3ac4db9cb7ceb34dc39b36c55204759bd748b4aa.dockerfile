...
RUN apt-get install --no-install-recommends gcc g++ git make flex bison gperf ruby perl python -y
...
RUN apt-get update
RUN apt-get install --no-install-recommends ttf-dejavu-core ttf-indic-fonts ttf-kochi-gothic ttf-kochi-mincho ttf-mscorefonts-installer xfonts-mathml -y
#  For Ubuntu shared libs
RUN apt-get install --no-install-recommends make gcc g++ nasm curl tar bzip2 -y
