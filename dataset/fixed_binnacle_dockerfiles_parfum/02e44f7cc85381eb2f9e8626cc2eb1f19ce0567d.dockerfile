...
RUN apt-get --no-install-recommends -y install qemu qemu-user qemu-user-static \
 && apt-get -m update \
 && apt-get install --no-install-recommends -y libc6-arm64-cross libcc6-dev-i386 libc6-i386 libffi-dev libssl-dev libncurses5-dev \
...
#  Install gdb 8.0
RUN apt-get update \
 && apt-get install --no-install-recommends -y texinfo \
 && cd /root/tools \
