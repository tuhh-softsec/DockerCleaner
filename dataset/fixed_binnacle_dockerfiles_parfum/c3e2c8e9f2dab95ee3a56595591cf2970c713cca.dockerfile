...
RUN apt-get update --fix-missing \
 && apt-get install --no-install-recommends vim curl nano wget unzip maven git bzip2 -y \
 && apt-get install --no-install-recommends locales -y \
 && locale-gen en_US.UTF-8 \
 && apt-get install --no-install-recommends build-essential -y \
 && apt-get install --no-install-recommends protobuf-compiler libprotoc-dev -y \
 && apt-get install --no-install-recommends libgtk2.0-dev -y
# jdk8
