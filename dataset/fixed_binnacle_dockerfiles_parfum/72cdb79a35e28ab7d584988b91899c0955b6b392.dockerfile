...
RUN apt-get install --no-install-recommends curl sudo --yes
...
#  Install PostgreSQL.
ENV PG_MAJOR="9.4"
RUN apt-get install --no-install-recommends postgresql-$PG_MAJOR --yes
#  Install plv8 (in case of plv8 compilation issues address to
#  README in <https://github.com/clkao/docker-postgres-plv8>).
RUN apt-get install --no-install-recommends git build-essential libv8-dev postgresql-server-dev-$PG_MAJOR --yes
RUN apt-get install --no-install-recommends nodejs-dev --yes
...
#  Install utils.
RUN apt-get install --no-install-recommends openssh-server aptitude --yes
USER fhirbase
