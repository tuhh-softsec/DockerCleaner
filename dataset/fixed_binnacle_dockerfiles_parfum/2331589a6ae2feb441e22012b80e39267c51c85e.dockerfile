...
RUN pip2 install scikit-image Cython numpy==1.15.2 scipy matplotlib==2.2.3 ipython==5.6.0 h5py shapely leveldb networkx nose python-dateutil pandas protobuf python-gflags pyyaml Pillow six decorator==4.3.0 opencv-python easydict lmdb sklearn scikit-image h5py leveldb imageio cython packaging SimpleITK pydicom tqdm cffi tensorboardX urllib3 visdom fire git+https://github.com/pytorch/tnt.git@master tqdm ipdb virtualenv tensorflow-gpu==1.4.0 tensorboard==1.11.0 torch==0.4.0 torchvision \
 && pip3 install tqdm fire SimpleITK pydicom lmdb Cython imageio easydict shapely pandas medicaltorch jupyter scikit-image pillow dominate requests cffi dlib tensorflow-gpu==1.4.0 tensorboard==1.11.0 torch==0.4.1 torchvision datetime scipy matplotlib opencv-python
WORKDIR /opt
