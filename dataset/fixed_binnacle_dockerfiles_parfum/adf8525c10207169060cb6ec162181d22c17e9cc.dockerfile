...
RUN echo 'Etc/UTC' > /etc/timezone \
 ...
 && apt-get update \
 && apt-get install --no-install-recommends tzdata -q -y \
 && rm -rf /var/lib/apt/lists/*
#  install packages
RUN apt-get update \
 && apt-get install --no-install-recommends bash-completion dirmngr gnupg2 lsb-release python3-pip wget -q -y \
 && rm -rf /var/lib/apt/lists/*
#  setup ros2 keys
...
#  install setup files
RUN apt-get update \
 && apt-get install --no-install-recommends ros-$ROS_DISTRO-ros-workspace -q -y \
 && rm -rf /var/lib/apt/lists/*
