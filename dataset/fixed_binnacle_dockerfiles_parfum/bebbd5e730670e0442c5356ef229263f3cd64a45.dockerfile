...
RUN apt-get update -qqy \
 && DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y znc znc-dev ca-certificates libcurl4-openssl-dev make \
 && $_apt_clean
...
RUN apt-get update -qqy \
 && DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y autoconf automake libtool gettext libpcre3-dev perl \
 && $_apt_clean
