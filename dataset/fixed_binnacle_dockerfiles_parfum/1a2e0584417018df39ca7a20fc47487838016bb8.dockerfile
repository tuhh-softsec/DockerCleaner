...
RUN apt-get update
RUN apt-get install --no-install-recommends language-pack-en -q -y
RUN update-locale LANG=en_US.UTF-8
# run echo mail > /etc/hostname
COPY etc-hosts.txt /etc/hosts
# run chown root:root /etc/hosts
RUN apt-get install --no-install-recommends vim -q -y
#  Install Postfix.
RUN echo "postfix postfix/main_mailer_type string Internet site" > preseed.txt
RUN echo "postfix postfix/mailname string mail.example.com" >> preseed.txt
#  Use Mailbox format.
RUN debconf-set-selections preseed.txt
RUN DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -q -y postfix
...
RUN apt-get install --no-install-recommends syslog-ng -q -y
#
#  Jonas C.
#
#  Good foor debugging
RUN apt-get install --no-install-recommends mutt vim dnsutils wget curl -y
...
# run echo "postfix postfix/mailname string mail.example.com" > preseed.txt
# run debconf-set-selections preseed.txt
RUN DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y mailman
