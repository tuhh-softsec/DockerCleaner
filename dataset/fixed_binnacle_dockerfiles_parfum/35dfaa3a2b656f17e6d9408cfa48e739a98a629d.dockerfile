...
RUN apt-get update -y \
 && apt-get install --no-install-recommends libczmq-dev libqpid-proton8-dev tcl-dev libsodium-dev -y
#  clickhouse
...
RUN echo "deb http://apt.llvm.org/bionic/ llvm-toolchain-bionic-8 main" > /etc/apt/sources.list.d/llvm8.list \
 ...
 && apt-get update -y \
 && apt-get install --no-install-recommends clang-8 lldb-8 lld-8 -y
