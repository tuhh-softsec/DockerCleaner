...
#  Build Tools
RUN apt-get update \
 && apt-get install --no-install-recommends build-essential zlib1g-dev libssl-dev libreadline6-dev libyaml-dev pkg-config software-properties-common -y \
 && apt-add-repository ppa:ubuntu-lxc/lxd-stable \
 && apt-get install --no-install-recommends make wget tar git curl -y \
 && apt-get clean \
...
#  Install Python
RUN apt-get update \
 && apt-get install --no-install-recommends python3-pip python3-dev python3-software-properties -y \
 && apt-get clean \
 && rm -rf /var/lib/apt/lists/* /tmp/* /var/tmp/*
#  Install Ruby
RUN cd /tmp \
...
#  Install Go
RUN apt-get update \
 && apt-get install --no-install-recommends golang bzr -y \
 && apt-get clean \
 && rm -rf /var/lib/apt/lists/* /tmp/* /var/tmp/* \
 && mkdir /usr/go
...
#  Install PHP
RUN apt-get update \
 && apt-get install --no-install-recommends php5 php5-dev php-pear -y \
 && apt-get clean \
 && rm -rf /var/lib/apt/lists/* /tmp/* /var/tmp/*
#  Install Rust
...
RUN wget -O - http://ubuntu.hyperdex.org/hyperdex.gpg.key | apt-key add - \
 ...
 && apt-get update \
 && apt-get install --no-install-recommends python-macaroons -y \
 && apt-get clean \
 && rm -rf /var/lib/apt/lists/* /tmp/* /var/tmp/*
WORKDIR /usr/src
#  Install pymacaroons
