...
RUN sh -c 'echo "deb http://apt.llvm.org/xenial/ llvm-toolchain-xenial-6.0 main" > /etc/apt/sources.list.d/llvm.list' \
 ...
 && apt-get update \
 && apt-get install --no-install-recommends -y clang-6.0 clang-format-6.0 clang-tidy-6.0 clang-tools-6.0 lldb-6.0 python-clang-6.0 python-lldb-6.0
RUN apt-get install --no-install-recommends -y gdb valgrind cppcheck
#  Install VSCode
