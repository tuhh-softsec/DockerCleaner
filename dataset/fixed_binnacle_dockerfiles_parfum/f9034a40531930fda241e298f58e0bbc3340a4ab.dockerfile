...
RUN set -xe ; apk add --no-cache --virtual .persistent-deps git sudo rsync curl nano bzip2 ruby php5-cli php5-mcrypt php5-gd php5-curl php5-json php5-phar php5-openssl php5-ctype php5-zip php5-zlib php5-pdo_mysql php5-dom php5-xml nodejs zsh mysql-client openssh-client postgresql-client \
...
RUN set -xe ; export CONFIGURE_OPTS="--disable-install-doc" ; apk add --no-cache gmp yaml ruby-json ruby-bigdecimal \
 && apk add --no-cache --virtual .build-deps ruby-dev build-base openssl-dev \
 && gem update --clear-sources --quiet --system 2.7.9 \
 && gem update --clear-sources --quiet --force \
 && gem install compass --clear-sources --quiet \
 && gem cleanup \
 && apk del --no-cache .build-deps \
 && rm -rf /tmp/* \
 && rm -rf /home/root/.gem/
