...
RUN apt-get update \
 ...
 && apt-get update \
 && DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y hadoop-conf-pseudo python-pip python-dev python-virtualenv mysql-server libmysql-java libmysqlclient-dev sudo unzip html2text slapd ldap-utils libkrb5-dev vim
#  Create hive user
