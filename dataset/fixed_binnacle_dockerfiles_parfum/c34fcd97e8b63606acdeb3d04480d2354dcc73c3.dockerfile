...
RUN apt-get update \
 && apt-get install --no-install-recommends nvinfer-runtime-trt-repo-ubuntu1604-4.0.1-ga-cuda8.0 \
 && apt-get update \
 && apt-get install --no-install-recommends libnvinfer4=4.1.2-1+cuda8.0 \
 && apt-get install --no-install-recommends libnvinfer-dev=4.1.2-1+cuda8.0
