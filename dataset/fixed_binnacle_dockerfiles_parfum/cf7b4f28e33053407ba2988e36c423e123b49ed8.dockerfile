...
RUN apt-get update \
 && apt-get install --no-install-recommends lsb-core software-properties-common wget -y
...
RUN apt-get install --no-install-recommends postgresql-10 postgresql-client-10 postgresql-plpython-10 postgresql-server-dev-10 libpq-dev build-essential openssl libssl-dev libboost-all-dev m4 vim pgxnclient flex bison graphviz -y
