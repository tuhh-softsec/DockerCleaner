...
RUN apt-get update ; apt-get install --no-install-recommends binutils default-jre git g++ haskell-platform make python python-pip python-yaml wget -y
#  install z3
...
RUN apt-get update ; apt-get install --no-install-recommends binutils camlp5 curl libpcre-ocaml-dev libpcre3-dev libreadline-dev libz-dev make pkg-config vim -y
#  install coq
...
#  install sshd
RUN apt-get update ; apt-get install --no-install-recommends openssh-server -y
