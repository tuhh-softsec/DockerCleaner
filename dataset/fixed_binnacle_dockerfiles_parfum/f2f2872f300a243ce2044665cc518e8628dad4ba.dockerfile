...
RUN apt-get update -y \
 && apt-get install --no-install-recommends curl git-core xz-utils build-essential wget unzip sudo gnupg2 dirmngr -y
...
USER root
RUN apt-get update -y \
 && apt-get install --no-install-recommends gcc-mingw-w64-x86-64 gcc-mingw-w64-i686 g++-mingw-w64-x86-64 g++-mingw-w64-i686 gcc-multilib g++-multilib moreutils -y
...
RUN dpkg --add-architecture i386 \
 && apt-get update -y \
 && apt-get install --no-install-recommends libc6-dev:i386 libudev-dev libudev-dev:i386 cmake -y
