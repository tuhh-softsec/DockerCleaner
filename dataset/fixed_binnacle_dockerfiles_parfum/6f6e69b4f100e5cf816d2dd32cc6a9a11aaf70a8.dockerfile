...
RUN apt-get update \
 && apt-get install --no-install-recommends automake bc curl g++ git libc-dev libreadline-dev libtool make ncurses-dev nvi pkg-config python python-dev python-setuptools unzip wavpack wget zip zlib1g-dev -y \
