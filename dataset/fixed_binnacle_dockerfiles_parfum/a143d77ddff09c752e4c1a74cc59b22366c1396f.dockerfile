...
USER root
RUN apt-get update \
 && apt-get install --no-install-recommends curl libtinfo-dev libzmq3-dev libcairo2-dev libpango1.0-dev libmagic-dev libblas-dev python3-pip netbase liblapack-dev -y \
...
RUN apt-key adv --keyserver hkp://keyserver.ubuntu.com:80 --recv-keys 3FA7E0328081BFF6A14DA29AA6A19B38D3D831EF \
 ...
 && apt-get update
RUN apt-get install --no-install-recommends binutils mono-devel ca-certificates-mono fsharp nuget referenceassemblies-pcl -y \
