...
#  Utils
RUN apt-get install --no-install-recommends vim curl silversearcher-ag apt-file git zsh ncdu -y \
...
RUN echo '[ ! -z "$TERM" -a -r /etc/motd ] \
 && cat /etc/motd' >> ~/.zshrc \
 && apt-get install --no-install-recommends figlet -y \
...
RUN apt-get install --no-install-recommends nodejs-legacy npm -y \
 && npm cache clean -f \
 && npm install n -g \
 && n stable \
 && npm install Brosec -g
#  Add files
...
RUN apt-get install --no-install-recommends whois dnsutils dnsmap nmap theharvester dmitry knockpy netdiscover fierce -y
...
RUN apt-get install --no-install-recommends metagoofil -y
#  Pentest Framework
#  RUN pip install pwntools
RUN apt-get install --no-install-recommends metasploit-framework websploit -y \
...
RUN apt-get install --no-install-recommends mitmproxy dsniff mitmf tcpdump ngrep -y
...
#  Email Spoofing
RUN apt-get install --no-install-recommends sendemail -y
...
RUN apt-get install --no-install-recommends hydra john crunch cewl fcrackzip -y
...
RUN apt-get install --no-install-recommends arachni nikto wapiti w3af fimap -y
...
#  SQL Injection
RUN apt-get install --no-install-recommends sqlmap themole -y
#  Phishing
RUN apt-get install --no-install-recommends httrack -y \
...
#  DOS Tools
RUN apt-get install --no-install-recommends t50 \
...
RUN apt-get install --no-install-recommends wifite wifiphisher mdk3 tshark -y
...
#  Reverse Engineering
RUN apt-get install --no-install-recommends apktool set -y \
...
#  Search exploit
RUN apt-get install --no-install-recommends exploitdb -y
...
#  Forensic
RUN apt-get install --no-install-recommends extract -y
...
#  Wordpress
RUN apt-get install --no-install-recommends wpscan -y
...
#  Reverse shell
RUN apt-get install --no-install-recommends netcat -y
...
RUN apt-get install --no-install-recommends mat -y
#  AV Evasion / Dropper
...
#  Paquet manipulation
RUN apt-get install --no-install-recommends hping3 scapy -y
#  C&C
