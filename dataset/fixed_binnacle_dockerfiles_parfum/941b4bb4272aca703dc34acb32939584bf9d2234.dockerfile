...
RUN apt-get update \
 && apt-get install --no-install-recommends bison build-essential clang debhelper flex pkg-config `` -y \
...
RUN apt-get install --no-install-recommends ruby ruby-bundler ruby-dev libbit-vector-perl libclass-accessor-class-perl libcrypt-ssleay-perl libio-socket-ssl-perl libnet-ssleay-perl `` `` -y
RUN apt-get -t jessie-backports --no-install-recommends install -y ruby-bundler
