...
RUN echo "$LOG_TAG Install miniconda2 related packages" \
 && apt-get update -y \
 && apt-get install --no-install-recommends bzip2 ca-certificates libglib2.0-0 libxext6 libsm6 libxrender1 git mercurial subversion -y \
...
RUN echo "$LOG_TAG Install python related packages" \
 ...
 && apt-get install --no-install-recommends libblas-dev libatlas-dev liblapack-dev -y \
 && apt-get install --no-install-recommends libpng-dev libfreetype6-dev libxft-dev -y \
 && apt-get install --no-install-recommends python-tk libxml2-dev libxslt-dev zlib1g-dev -y \
 && pip install numpy \
...
RUN echo "$LOG_TAG Install R related packages" \
 ...
 && apt-get install --no-install-recommends libcurl4-gnutls-dev libssl-dev -y \
