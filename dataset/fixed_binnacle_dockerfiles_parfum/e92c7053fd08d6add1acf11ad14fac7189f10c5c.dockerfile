...
RUN export DEBIAN_FRONTEND=noninteractive ; apt-get update -y \
 && apt-get install --no-install-recommends gdb git libcairo2-dev libcurl4-openssl-dev libexpat1-dev libpq-dev libsqlite3-dev libudunits2-dev make pandoc qpdf r-base-dev sqlite3 subversion valgrind vim wget -y
RUN apt-get install --no-install-recommends libv8-3.14-dev libjq-dev libprotobuf-dev libxml2-dev libprotobuf-dev protobuf-compiler unixodbc-dev libssh2-1-dev libgit2-dev libnetcdf-dev locales libssl-dev -y
