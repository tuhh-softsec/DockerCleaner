...
ENV R_VERSION="${R_VERSION:-3.5.1}" \
    ...
    TERM="xterm"
RUN apt-get update \
 && apt-get install --no-install-recommends bash-completion file fonts-texgyre g++ gfortran gsfonts libblas-dev libbz2-1.0 libcurl3 libicu55 libjpeg-turbo8 libopenblas-dev libpangocairo-1.0-0 libpcre3 libpng16-16 libreadline6 libtiff5 liblzma5 locales make unzip zip zlib1g -y \
