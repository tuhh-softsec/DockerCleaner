...
RUN apt-get clean \
 && apt-get update -qq \
 && apt-get install --no-install-recommends apt-utils locales -qq -y \
 && locale-gen $LANG
...
RUN apt-get update -qq > /dev/null \
 && apt-get install --no-install-recommends locales -qq > /dev/null \
 ...
 && apt-get install --no-install-recommends nodejs -qq > /dev/null \
