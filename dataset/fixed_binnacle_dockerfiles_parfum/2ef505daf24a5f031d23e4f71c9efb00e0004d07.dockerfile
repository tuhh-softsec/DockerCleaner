...
RUN apt-get update \
 && DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y apt-transport-https curl wget software-properties-common git
...
RUN apt-key add archive.key \
 ...
 && apt-get update \
 && DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y mysql-server libmysql-java libmysqlclient-dev hadoop-conf-pseudo python-pip python-dev python-virtualenv sudo unzip html2text slapd ldap-utils libkrb5-dev vim
#  Add Hive user
