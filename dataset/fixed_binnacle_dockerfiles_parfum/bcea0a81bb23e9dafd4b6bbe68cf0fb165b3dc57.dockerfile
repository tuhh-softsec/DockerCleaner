...
RUN apt-get update
RUN apt-get install --no-install-recommends wget apt-utils build-essential apt-transport-https -y
...
RUN apt-get install --no-install-recommends sharemind-csv-importer -y --allow-unauthenticated
RUN apt-get install --no-install-recommends libsharemind-mod-shared3pdev-ctrl -y --allow-unauthenticated
RUN echo "\n\033[32m--------------------------------------\033[0;39m" \
...
RUN apt-get update ; exit 0
RUN apt-get install --no-install-recommends python-pip python-pandas npm -y
