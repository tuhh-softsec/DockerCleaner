...
RUN apt-get update \
 && apt-get install --no-install-recommends python-dev python-setuptools python-software-properties openjdk-7-jre-headless wget curl poppler-utils software-properties-common gettext git dnsutils vim -y
...
RUN apt-get install --no-install-recommends make libxml2-dev libxslt1-dev libssl-dev libffi-dev libtiff4-dev libjpeg8-dev liblcms2-dev python-dev python-setuptools python-virtualenv git supervisor vim -y
RUN easy_install pip
# #### Install dependencies for pyav #####
RUN apt-get update \
 && apt-get install --no-install-recommends libfaac-dev libgpac-dev checkinstall libmp3lame-dev libopencore-amrnb-dev libopencore-amrwb-dev librtmp-dev libtheora-dev libvorbis-dev libx264-dev libfdk-aac-dev libvpx-dev libxvidcore-dev pkg-config yasm zlib1g-dev libavformat-dev libavcodec-dev libavdevice-dev libavutil-dev libswscale-dev libavresample-dev -y
#  Temporarily use /tmp as workdir for the pyav dependencies
#  WORKDIR /tmp
RUN apt-get install --no-install-recommends ffmpeg -y
# #########
WORKDIR /opt/ocd
