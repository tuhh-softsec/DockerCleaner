...
RUN apt-get update -qq \
 && apt-get install --no-install-recommends build-essential wget curl unzip make -y -q
#
#  Install native prerequisites for the ruby gems we need (e.g.nokogiri)
#
RUN apt-get update -qq \
 && apt-get install --no-install-recommends zlib1g-dev=1:1.2.8.dfsg-2ubuntu4 libxml2-dev=2.9.3+dfsg1-1ubuntu0.1 libxml2 libxslt1-dev -y -q
#  Prerequesites for capybara-webkit
RUN apt-get install --no-install-recommends qt5-default libqt5webkit5-dev gstreamer1.0-plugins-base gstreamer1.0-tools gstreamer1.0-x -y -q
#
...
RUN apt-get install --no-install-recommends software-properties-common -yq
