...
RUN apt-get update \
 ...
 && apt-get update -qq \
 && apt-get install --no-install-recommends automake build-essential git jq libfuzzy-dev libimage-exiftool-perl liblzma5 libssl-dev libtool libzmq3 make python-cffi python-dev python-gevent python-ipy python-m2crypto python-pexpect python-pip python-progressbar python-pyclamd python-zmq unrar unzip wget -y \
