...
RUN mkdir -p /usr/local/python/3.6.6/src \
 && mkdir /usr/local/python/3.6.6/bin \
...
RUN set -ex \
 ...
 && ./configure --prefix=/usr/local/python/3.6.6 --build="$gnuArch" --enable-loadable-sqlite-extensions --enable-shared --with-system-expat --with-system-ffi --without-ensurepip LDFLAGS="-L/usr/local/python/${PYTHON_VERSION}/extlib/lib -Wl,--rpath=/usr/local/python/${PYTHON_VERSION}/lib -Wl,--rpath=/usr/local/python/${PYTHON_VERSION}/extlib/lib" \
 && make -j "$( nproc ;)" \
 && make install \
 && ldconfig \
