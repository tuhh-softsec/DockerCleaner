...
#  ssh2
RUN apt-get update \
 && apt-get install --no-install-recommends libssh2-1-dev libssh2-1 -y
...
#  gd extension
RUN apt-get update \
 && apt-get install --no-install-recommends libpng-dev libfreetype6-dev libjpeg-dev libxpm-dev libxml2-dev libxslt-dev libmcrypt-dev librabbitmq-dev libssh-dev libwebp-dev -y
