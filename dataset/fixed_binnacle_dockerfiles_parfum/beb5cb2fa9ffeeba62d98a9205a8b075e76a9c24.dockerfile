...
RUN (apt-key adv --keyserver hkp://keyserver.ubuntu.com:80 --recv-keys 8AA7AF1F1091A5FD 9AD198E9 || apt-key adv --keyserver hkp://p80.pool.sks-keyservers.net:80 --recv-keys 8AA7AF1F1091A5FD 9AD198E9 ) \
 ...
 && apt-get update \
 && apt-get install --no-install-recommends git golang-1.11-go libssl-dev libzmq3-dev openssl python3 python3-grpcio-tools=1.1.3-1 -y -q \
 && apt-get clean \
...
RUN apt-get update \
 && apt-get install --no-install-recommends dpkg-dev -y -q \
 && cd /debs \
