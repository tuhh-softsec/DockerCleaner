...
RUN apt-get update
#  Install tools
RUN apt-get install --no-install-recommends python-software-properties software-properties-common gcc-4.9 git make wget -y
...
RUN apt-get install --no-install-recommends nodejs zip -y
...
RUN apt-get install --no-install-recommends libgtk2.0-0 libgconf-2-4 libasound2 libxtst6 libnss3 -y
RUN apt-get clean
