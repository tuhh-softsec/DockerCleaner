...
RUN sudo apt-get install --no-install-recommends --yes nodejs
#  Rust
...
RUN wget http://downloads.dlang.org/releases/2.x/2.070.2/dmd_2.070.2-0_amd64.deb \
 && apt-get update \
 && apt-get install --no-install-recommends libcurl3 -y \
...
RUN apt-get update \
 && apt-get install --no-install-recommends make bzip2 xz-utils -y \
 && apt-get clean \
 && rm -rf /var/lib/apt/lists/* /tmp/* /var/tmp/*
