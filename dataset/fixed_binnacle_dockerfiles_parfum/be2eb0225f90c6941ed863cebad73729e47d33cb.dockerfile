...
RUN apt-get update -qq \
 && DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y build-essential curl git libboost-all-dev libcurl4-openssl-dev locales lsb-release moreutils openssh-client protobuf-compiler sudo \
...
RUN apt-get update -qq \
 && DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y jags libatlas3-base libcairo2 libcairo2-dev libfftw3-dev libgdal1-dev libgraphviz-dev libgsl0-dev libmysqlclient-dev libnetcdf-dev libprocps4-dev libproj-dev libprotoc-dev libquantlib0-dev libsasl2-dev libsqliteodbc libxml2-dev libxt-dev libxt6 odbc-postgresql tdsodbc unixodbc unixodbc-dev wget \
 && rm -rf /var/lib/apt/lists/*
...
RUN apt-get update -qq \
 ...
 && rm -rf /var/lib/apt/lists/*
RUN apt-get update -qq \
 && DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y python-dev python-pip python-virtualenv
#  Install Julia
...
RUN apt-get update -qq \
 && DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y libfile-homedir-perl libwww-perl libxml-libxml-perl libyaml-tiny-perl ruby tcl tex-gyre tk xdg-utils xzdec \
 && rm -rf /var/lib/apt/lists/*
...
RUN apt-get update -qq \
 && DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends biber latex-beamer lmodern prosper ps2eps texinfo texlive-bibtex-extra texlive-extra-utils texlive-font-utils texlive-generic-extra texlive-lang-arabic texlive-latex-extra texlive-luatex texlive-pstricks texlive-xetex \
 && rm -rf /var/lib/apt/lists/*
#  Set default locale
