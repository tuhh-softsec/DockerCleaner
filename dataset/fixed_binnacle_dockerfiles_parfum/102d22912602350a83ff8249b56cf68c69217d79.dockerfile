...
# ######### Install JdeRobot ##############
RUN apt-get update \
 && apt-get install --no-install-recommends python-pip libssl-dev libbz2-dev -y \
 && rm -rf /var/lib/apt/lists/*
RUN apt-get update \
 && apt-get install --no-install-recommends jderobot xvfb nano wget -y \
 && rm -rf /var/lib/apt/lists/*
# ###### Install gzweb dependences ########
RUN apt-get update \
 && apt-get install --no-install-recommends build-essential cmake libgazebo7-dev imagemagick libboost-dev libgts-dev libjansson-dev libtinyxml-dev mercurial nodejs nodejs-legacy npm pkg-config psmisc -q -y \
 && rm -rf /var/lib/apt/lists/*
# ###### clone gzweb #######
