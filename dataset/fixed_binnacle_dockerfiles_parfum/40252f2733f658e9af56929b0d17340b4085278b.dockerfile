...
RUN add-apt-repository ppa:ondrej/php \
 && apt-get update \
 && DEBIAN_FRONTEND="noninteractive" apt-get --no-install-recommends install --yes php-pear php7.1-cli php7.1-common php7.1-curl php7.1-dev php7.1-fpm php7.1-gd php7.1-imagick php7.1-imap php7.1-intl php7.1-json php7.1-ldap php7.1-mbstring php7.1-mcrypt php7.1-memcache php7.1-mysql php7.1-opcache php7.1-readline php7.1-sqlite php7.1-tidy php7.1-xdebug php7.1-xml php7.1-zip
#  php7.1-xhprof
#  phpredis
ENV PHPREDIS_VERSION="3.0.0"
RUN apt-get update \
 && DEBIAN_FRONTEND="noninteractive" apt-get --no-install-recommends install --yes git
...
#  NGNIX
RUN apt-get update \
 && DEBIAN_FRONTEND="noninteractive" apt-get --no-install-recommends install --yes nginx
#  SSH (for remote drush)
RUN apt-get update \
 && DEBIAN_FRONTEND="noninteractive" apt-get --no-install-recommends install --yes openssh-server
...
RUN apt-get update \
 && DEBIAN_FRONTEND="noninteractive" apt-get --no-install-recommends install --yes ssmtp
#  Drush, console
...
RUN apt-get update \
 && DEBIAN_FRONTEND="noninteractive" apt-get --no-install-recommends install --yes git mysql-client screen
#  Configure PHP
RUN mkdir /run/php
