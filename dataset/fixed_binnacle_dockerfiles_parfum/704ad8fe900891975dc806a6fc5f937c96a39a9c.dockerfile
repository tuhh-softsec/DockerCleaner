...
RUN apt-get update -y \
 && DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y build-essential m4 apt-utils libffi-dev libssl-dev libbz2-dev libgmp3-dev libev-dev libsnappy-dev libxen-dev help2man pkg-config time aspcud wget curl darcs git unzip automake python-dev python-pip debhelper psmisc sudo libtool iptables net-tools ncurses-dev tzdata librdmacm-dev protobuf-compiler socat
ARG HOST_UID
