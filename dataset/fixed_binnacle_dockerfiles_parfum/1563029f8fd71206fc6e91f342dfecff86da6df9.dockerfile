...
RUN apt-get install --no-install-recommends gcc g++ cmake binutils-dev libiberty-dev libboost-dev libboost-date-time-dev libboost-filesystem-dev libboost-program-options-dev libboost-regex-dev libboost-serialization-dev libboost-system-dev libboost-thread-dev cxxtest -y
#  Additional stuff needed to build and run relex.
RUN apt-get install --no-install-recommends wordnet-dev wordnet-sense-index openjdk-7-jdk ant libcommons-logging-java libgetopt-java -y
...
RUN apt-get install --no-install-recommends guile-2.0-dev cython unixodbc-dev odbc-postgresql wget telnet locales rlwrap unzip -y
#  -------------------------------------------------------------
