...
#  Install the Core
RUN apt-get update \
 && apt-get install --no-install-recommends sudo git wget curl git zip ccze byobu zsh golang ufw python-pip nikto dotdotpwn jsql nmap sqlmap sqlninja thc-ipv6 hydra dirb -y
...
RUN apt-get update \
 && apt-get install --no-install-recommends apt-utils unzip nodejs wget curl jruby nano screen htop openssl git -y -u
#  Install nodejs
...
RUN apt-get update \
 && apt-get install --no-install-recommends xvfb x11-xkb-utils xfonts-100dpi xfonts-75dpi xfonts-scalable xfonts-cyrillic x11-apps clang libdbus-1-dev libgtk2.0-dev libnotify-dev libgnome-keyring-dev libgconf2-dev libasound2-dev libcap-dev libcups2-dev libxtst-dev libxss1 libnss3-dev gcc-multilib g++-multilib -y
...
RUN apt-get update \
 && apt-get install --no-install-recommends recon-ng -y || true
