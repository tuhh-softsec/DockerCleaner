...
RUN apt-get update \
 && apt-get install --no-install-recommends ros-melodic-ros-base=1.4.1-0* -y \
...
RUN apt-get install --no-install-recommends python3 python3-dev python3-pip -qqy
...
RUN apt-get install --no-install-recommends python-openssl apparmor-utils python-apparmor python-pip -y
