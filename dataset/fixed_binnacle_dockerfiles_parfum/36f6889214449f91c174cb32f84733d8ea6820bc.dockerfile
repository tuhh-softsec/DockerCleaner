...
RUN apt-get update -y \
 && apt-get install --no-install-recommends apt-transport-https apt-utils python-software-properties software-properties-common -y
#  Java8
RUN add-apt-repository -y ppa:webupd8team/java
RUN apt-get update -y
#  Linux
RUN apt-get install --no-install-recommends cpio curl dirmngr gdebi-core git net-tools sudo vim wget zip -y
#  Java 8
...
#  R
RUN apt-get update -y \
 && apt-get install --no-install-recommends r-base r-base-dev r-cran-jsonlite r-cran-rcurl -y \
