...
RUN apt-get update \
 ...
 && apt-get update \
 && apt-get install --no-install-recommends build-essential curl libcurl3-dev git libfreetype6-dev libpng12-dev libzmq3-dev pkg-config python-dev python-numpy python-pip swig zip zlib1g-dev -y \
 && apt-get clean \
