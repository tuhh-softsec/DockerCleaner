...
RUN apt-get update --fix-missing \
 && apt-get install --no-install-recommends apt-utils wget curl bzip2 pkg-config libbz2-dev git build-essential zlib1g-dev locales vim fontconfig perl expat libexpat-dev ttf-dejavu cpanminus libgd-perl -q -y
#  Set the locale
...
RUN apt-get install --no-install-recommends cpanminus -y
...
RUN apt-get install --no-install-recommends libarchive-zip-perl --yes
#  Install GD
...
#  install imagemagick
RUN apt-get install --no-install-recommends imagemagick --yes
#  Install conda
