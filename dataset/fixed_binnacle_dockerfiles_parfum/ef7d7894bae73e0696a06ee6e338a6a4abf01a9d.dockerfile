...
RUN apt-get update -y \
 && apt-get -y upgrade \
 && apt-get install --no-install-recommends rsyslog rsyslog-relp logrotate supervisor screenfetch -y
#  Create the log file to be able to run tail
RUN touch /var/log/cron.log /var/log/auth.log
#  --- 2 Install the SSH server
RUN apt-get install --no-install-recommends ssh openssh-server rsync -y
#  --- 3 Install a shell text editor
RUN apt-get install --no-install-recommends nano vim-nox -y
...
RUN apt-get install --no-install-recommends postfix postfix-mysql postfix-doc mariadb-client mariadb-server openssl getmail4 rkhunter binutils dovecot-imapd dovecot-pop3d dovecot-mysql dovecot-sieve dovecot-lmtpd sudo -y
...
RUN apt-get install --no-install-recommends amavisd-new spamassassin clamav clamav-daemon zoo unzip bzip2 arj nomarch lzop cabextract apt-listchanges libnet-ldap-perl libauthen-sasl-perl clamav-docs daemon libio-string-perl libio-socket-ssl-perl libnet-ident-perl zip libnet-dns-perl -y
...
RUN apt-get update -qq \
 && apt-get install --no-install-recommends git lua5.1 liblua5.1-0-dev lua-filesystem libidn11-dev libssl-dev lua-zlib lua-expat lua-event lua-bitop lua-socket lua-sec luarocks luarocks -y -qq
...
RUN apt-get install --no-install-recommends certbot -y
#  --- 12 Opcode and PHP-FPM
RUN apt-get install --no-install-recommends php7.0-fpm php7.0-opcache php-apcu -y
...
RUN apt-get install --no-install-recommends mailman -y
...
RUN apt-get install --no-install-recommends dpkg-dev debhelper openbsd-inetd debian-keyring -qq -y --force-yes
...
RUN apt-get update -qq \
 && apt-get install --no-install-recommends quota quotatool -y -qq
...
RUN apt-get install --no-install-recommends bind9 dnsutils haveged -y
#  --- 16 Install Vlogger, Webalizer, And AWStats
RUN apt-get install --no-install-recommends vlogger webalizer awstats geoip-database libclass-dbi-mysql-perl -y
COPY etc/cron.d/awstats /etc/cron.d/
#  --- 17 Install Jailkit
RUN apt-get install --no-install-recommends build-essential autoconf automake libtool flex bison debhelper binutils -y
RUN cd /tmp \
...
#  --- 18 Install fail2ban
RUN apt-get install --no-install-recommends fail2ban -y
...
RUN service mysql start \
 && apt-get install --no-install-recommends roundcube roundcube-core roundcube-mysql roundcube-plugins -y
