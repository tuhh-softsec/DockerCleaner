...
RUN apt-get update -y \
 && apt-get install --no-install-recommends software-properties-common pkg-config dirmngr python3-software-properties build-essential curl wget fonts-liberation ca-certificates libcfitsio-dev libffi-dev -y \
 ...
 && git lfs install \
 && apt-get install --no-install-recommends flake8 virtualenv virtualenvwrapper $PYTHON-dev $PYTHON-tk $PYTHON-nose -y \
