...
RUN (apt-key adv --keyserver hkp://keyserver.ubuntu.com:80 --recv-keys 8AA7AF1F1091A5FD || apt-key adv --keyserver hkp://p80.pool.sks-keyservers.net:80 --recv-keys 8AA7AF1F1091A5FD ) \
 ...
 && apt-get update \
 && apt-get install --no-install-recommends curl gcc git libssl-dev libzmq3-dev openssl pkg-config python3 python3-grpcio-tools=1.1.3-1 python3-sawtooth-cli unzip -y -q \
