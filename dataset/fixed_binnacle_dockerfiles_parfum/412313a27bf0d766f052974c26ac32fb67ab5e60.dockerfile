...
RUN apt-get update
RUN apt-get install --no-install-recommends gnupg2 -y
...
RUN apt-get update \
 ...
 && TERM=linux DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y cron curl imagemagick jq libpq5 nginx postfix postgresql-client unzip wget
#  Necessary steps for php7.2
RUN apt-get install --no-install-recommends -y apt-transport-https lsb-release ca-certificates \
 && wget -O /etc/apt/trusted.gpg.d/php.gpg https://packages.sury.org/php/apt.gpg \
 && sh -c 'echo "deb https://packages.sury.org/php/ $(lsb_release -sc) main" > /etc/apt/sources.list.d/php.list' \
 && apt-get update
RUN apt-get install --no-install-recommends php7.2 php7.2-cli php7.2-common php7.2-curl php7.2-fpm php7.2-imagick php7.2-imap php7.2-ldap php7.2-mbstring php7.2-pgsql php7.2-xml -y
