...
RUN apt-get update \
 && apt-get install --no-install-recommends build-essential zlib1g-dev unzip libncurses5-dev curl wget python python-pip python-dev cmake libboost-all-dev libxml2-dev libcurl4-gnutls-dev software-properties-common apt-transport-https default-jre default-jdk less vim libtbb-dev git -y --fix-missing
RUN apt-key adv --keyserver hkp://keyserver.ubuntu.com:80 --recv-keys E298A3A825C0D65DFD57CBB651716619E084DAB9
RUN add-apt-repository 'deb [arch=amd64,i386] https://cran.rstudio.com/bin/linux/ubuntu xenial/'
RUN apt-get update
RUN apt-get install --no-install-recommends r-base=${R_VERSION}-1xenial0 r-recommended=${R_VERSION}-1xenial0 -y --fix-missing
RUN apt-get install --no-install-recommends r-base-core=${R_VERSION}-1xenial0 -y --fix-missing --allow-downgrades
...
RUN cd /opt \
 ...
 && cd /opt \
 && rm -rf STAR*
COPY https://github.com/broadinstitute/picard/releases/download/${PICARD_VERSION}/picard-tools-${PICARD_VERSION}.zip /opt/picard-tools-${PICARD_VERSION}.zip
RUN cd /opt \
...
RUN pip install biopython==1.66 openpyxl==1.5.6 xlrd==0.6.1 numpy pandas scipy pybedtools==${PYBEDTOOLS_VERSION} pysam==${PYSAM_VERSION}
...
RUN cd /opt \
 ...
 && cd /opt \
 && rm -rf seqtk*
COPY http://hgdownload.cse.ucsc.edu/admin/exe/linux.x86_64/blat/blat /usr/local/bin/blat
RUN chmod 755 /usr/local/bin/blat
COPY http://hgdownload.cse.ucsc.edu/admin/exe/linux.x86_64/faToTwoBit /usr/local/bin/faToTwoBit
RUN chmod 755 /usr/local/bin/faToTwoBit
COPY http://hgdownload.cse.ucsc.edu/admin/exe/linux.x86_64/liftOver /usr/local/bin/liftOver
