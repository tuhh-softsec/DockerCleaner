...
RUN DEBIAN_FRONTEND=noninteractive apt-get update \
 && DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends build-essential ca-certificates curl git gettext-base libgd-dev libgeoip-dev libncurses5-dev libperl-dev libreadline-dev libxslt1-dev make perl unzip zlib1g-dev cmake ${RESTY_ADD_PACKAGE_BUILDDEPS} ${RESTY_ADD_PACKAGE_RUNDEPS} \
 && cd /tmp \
 ...
 && cd /tmp \
 && rm -rf opentracing-cpp-${OPENTRACING_CPP_VERSION}.tar.gz opentracing-cpp-${OPENTRACING_CPP_VERSION} \
 && cd /tmp \
