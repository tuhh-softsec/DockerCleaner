...
RUN apt-get install --no-install-recommends vim curl software-properties-common python-software-properties build-essential supervisor nginx php5-cli php5-fpm php5-gd zip unzip unrar-free mediainfo imagemagick ffmpeg -y
#  install rtorrent ============================================================
RUN apt-get install --no-install-recommends rtorrent=${RTORRENT_VERSION} -y
#  install rutorrent ===========================================================
RUN mkdir -p /var/www \
...
RUN dpkg -i /tmp/pure-ftpd/pure-ftpd-common*.deb \
 && apt-get install --no-install-recommends openbsd-inetd -y \
