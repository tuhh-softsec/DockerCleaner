...
RUN dpkg --add-architecture i386 \
 && apt-get update \
 && apt-get upgrade -y \
 && apt-get install --no-install-recommends curl git gnupg2 language-pack-en lib32z1 libc6:i386 libncurses5:i386 libstdc++6:i386 openjdk-8-jdk python-dev python-pip python-virtualenv unzip wget xz-utils -y \
 && apt-get install --no-install-recommends apt-transport-https autoconf automake bison build-essential ca-certificates gawk libffi-dev libgdbm-dev libgmp-dev libgmp-dev libncurses5-dev libreadline6-dev libsqlite3-dev libssl-dev libtool libyaml-dev pkg-config sqlite3 zlib1g-dev -y \
 && apt-get clean -y \
