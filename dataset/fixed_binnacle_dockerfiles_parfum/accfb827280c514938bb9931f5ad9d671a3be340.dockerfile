...
RUN apt-get update \
 && apt-get install --no-install-recommends autoconf autoconf-archive automake build-essential checkinstall cmake g++ git libcairo2-dev libcairo2-dev libicu-dev libicu-dev libjpeg8-dev libjpeg8-dev libpango1.0-dev libpango1.0-dev libpng12-dev libpng12-dev libtiff5-dev libtiff5-dev libtool nano pkg-config python-dev python-pip python3-dev python3-pip python-tk python3-tk screen wget xzgv zlib1g-dev -y
#  [ Install ZSH ]
...
#  [ ffmpeg ]
RUN apt-get update \
 && apt-get install --no-install-recommends ffmpeg -y --upgrade
#  [ install Leptonica ]
ENV BASE_DIR="/opt"
#  install leptonica
WORKDIR ${BASE_DIR}
