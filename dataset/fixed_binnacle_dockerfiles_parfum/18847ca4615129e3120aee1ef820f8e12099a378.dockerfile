...
RUN apt-get install --no-install-recommends build-essential gcc-multilib perl --yes
#  Install perl modules 
RUN apt-get install --no-install-recommends cpanminus --yes
...
RUN apt-get install --no-install-recommends libssl-dev --yes
RUN cpanm LWP LWP::Protocol::https
#  Install packages for XML processing 
RUN apt-get install --no-install-recommends expat libexpat-dev libxml2-dev libxslt1-dev libgdbm-dev --yes
...
RUN apt-get install --no-install-recommends libdb-dev graphviz --yes
...
RUN apt-get install --no-install-recommends libdbi-perl libdbd-mysql-perl libdbd-pg-perl libdbd-sqlite3-perl --yes
RUN cpanm DB_File
#  Install GD and other graphics dependencies 
RUN apt-get install --no-install-recommends libgd2-xpm-dev --yes
