...
RUN set -eux \
 && yum --quiet makecache \
 && yum ${YUM_FLAGS_DEV} install gettext git \
 && cd /tmp/ \
...
RUN set -eux \
 ...
 && yum ${YUM_FLAGS_PERSISTENT} install dejavu-sans-fonts curl httpd mariadb mod_ssl php php-bcmath php-gd php-ldap php-mbstring php-mysql php-xml \
