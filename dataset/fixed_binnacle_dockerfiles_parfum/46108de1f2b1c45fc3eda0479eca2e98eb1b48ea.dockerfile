...
RUN DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y wget
...
RUN DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y postgresql-$VERSION postgresql-contrib-$VERSION postgresql-server-dev-$VERSION postgresql-plpython-$VERSION postgresql-$VERSION-plv8 libxml2-dev libxslt1-dev python-dev python-pip daemontools libevent-dev lzop pv libffi-dev libssl-dev \
