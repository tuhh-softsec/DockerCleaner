...
RUN apt-get update \
 && apt-get upgrade -y \
 && apt-get install --no-install-recommends wget curl git unzip -y
...
RUN echo "deb http://packages.erlang-solutions.com/ubuntu trusty contrib" >> /etc/apt/sources.list \
 ...
 && apt-get update -qq \
 && apt-get install --no-install-recommends esl-erlang=1:20.3 build-essential wget -y \
 && apt-get clean \
...
RUN curl -sL https://deb.nodesource.com/setup_9.x | bash - \
 && apt-get install --no-install-recommends nodejs inotify-tools -y
...
RUN apt-get update \
 && apt-get upgrade -y
RUN apt-get install --no-install-recommends python-numpy software-properties-common libsecret-1-0 gnome-keyring -y
...
RUN sed 's/main$/main universe/' -i /etc/apt/sources.list \
 ...
 && apt-get install --no-install-recommends sudo software-properties-common libxext-dev libxrender-dev libxslt1.1 libxtst-dev libgtk2.0-0 libcanberra-gtk-module -qq -y --fix-missing \
 && echo 'Cleaning up' \
 && apt-get clean -qq -y \
