...
RUN apt-get update \
 && apt-get -y upgrade
RUN apt-get install --no-install-recommends git g++ python python-dev cmake make wget bzip2 vim -y
RUN apt-get install --no-install-recommends libgl1-mesa-dev freeglut3-dev libglew-dev libxmu-dev libxi-dev libbz2-dev libjpeg-turbo8-dev -y
RUN apt-get install --no-install-recommends zlib1g-dev libpng-dev libtiff-dev -y
