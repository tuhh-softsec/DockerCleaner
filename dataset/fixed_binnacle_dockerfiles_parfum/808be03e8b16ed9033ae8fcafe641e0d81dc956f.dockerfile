...
RUN apt-get install --no-install-recommends autoconf git make g++ -y
...
RUN apt-get install --no-install-recommends git make python ruby sudo locales curl -y
...
RUN apt-get update \
 && apt-get install --no-install-recommends python-dev libffi-dev libssl-dev libyaml-dev libssh2-1 libssh2-1-dev autoconf automake libtool libxml2-dev libxslt1-dev zlib1g-dev gcc wget openssl -y \
 && apt-get clean
# cmake install start
...
RUN apt-get install --no-install-recommends ruby ruby-dev rubygems gcc make -y \
...
CMD [ "if [ -f /data/hubble_buildinfo ] ; then echo \"\" >> /hubble_build/hubblestack/__init__.py ; cat /data/hubble_buildinfo >> /hubble_build/hubblestack/__init__.py; fi \
 ...
 && fpm -s dir -t deb -n hubblestack -v ${HUBBLE_VERSION} --iteration ${HUBBLE_ITERATION} --url ${HUBBLE_URL} --deb-no-default-config-files --after-install /hubble_build/conf/afterinstall.sh --after-upgrade /hubble_build/conf/afterupgrade.sh --before-remove /hubble_build/conf/beforeremove.sh etc opt usr /var/log/hubble_osquery/backuplogs \
 ...
 && openssl dgst -sha256 /data/hubblestack_${HUBBLE_VERSION}-${HUBBLE_ITERATION}deb7_amd64.deb > /data/hubblestack_${HUBBLE_VERSION}-${HUBBLE_ITERATION}deb7_amd64.deb.sha256" ]
