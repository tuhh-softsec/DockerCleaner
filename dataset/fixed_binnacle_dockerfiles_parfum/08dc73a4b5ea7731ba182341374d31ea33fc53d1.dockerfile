...
#  Install rocm pkgs
RUN apt-get update --allow-insecure-repositories \
 && DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y --allow-unauthenticated rocm-dev rocm-libs rocm-utils rocm-profiler cxlactivitylogger \
 && apt-get clean \
 && rm -rf /var/lib/apt/lists/*
#  install packages needed for this image
RUN apt-get update \
 && apt-get install --no-install-recommends sudo python rpm git mercurial libxml2 libxml2-dev -y
...
RUN sudo apt-get install --no-install-recommends -y wget unzip libssl-dev libboost-dev libboost-system-dev libboost-filesystem-dev
RUN cd $HOME \
