...
#  Configure locales
RUN apt-get update \
 && apt-get install --no-install-recommends gettext locales -y \
...
RUN apt-get update \
 && apt-get upgrade -y \
 && apt-get install --no-install-recommends build-essential gettext git graphviz-dev libgeos-dev libmysqlclient-dev libxml2-dev libxmlsec1-dev nodejs nodejs-legacy npm python-dev -y \
 && rm -rf /var/lib/apt/lists/*
...
RUN apt-get update \
 && apt-get upgrade -y \
 && apt-get install --no-install-recommends dnsutils iputils-ping libsqlite3-dev mongodb vim -y \
 && rm -rf /var/lib/apt/lists/*
...
RUN apt-get update \
 && apt-get upgrade -y \
 && apt-get install --no-install-recommends libgeos-dev libmysqlclient20 libxml2 libxmlsec1-dev nodejs nodejs-legacy tzdata -y \
 && rm -rf /var/lib/apt/lists/*
