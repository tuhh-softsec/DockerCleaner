...
#  install ANTs
RUN apt-get install --no-install-recommends ants -y
#  install ICA-AROMA
...
RUN apt-get install --no-install-recommends git-lfs
RUN git lfs install
#  Get atlases
