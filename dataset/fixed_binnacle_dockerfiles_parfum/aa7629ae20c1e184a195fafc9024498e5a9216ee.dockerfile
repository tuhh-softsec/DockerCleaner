...
RUN apt-get install --no-install-recommends sudo lsb-release build-essential curl software-properties-common -y \
...
RUN apt-get install --no-install-recommends wget tightvncserver tmux rxvt xterm mlterm imagemagick firefox blackbox imagemagick vim-gtk gnome-terminal i3 chromium-browser git mercurial lynx daemon -y
RUN apt-get install --no-install-recommends nginx -y
RUN apt-get install --no-install-recommends nginx-extras -y
RUN apt-get install --no-install-recommends cadaver -y
RUN cd /tmp \
 ...
 && rm -f Miniconda*.sh
RUN apt-get install --no-install-recommends redis-tools -y
