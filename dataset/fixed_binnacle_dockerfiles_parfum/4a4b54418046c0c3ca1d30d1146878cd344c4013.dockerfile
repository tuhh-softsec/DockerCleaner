...
RUN DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends -y install git curl rsync sudo expect python python-dev python-pip build-essential zip unzip tree clang awscli software-properties-common
# ##############################################################################
...
RUN set -ex \
 ...
 && DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y docker-ce=${DOCKER_VERSION}~ce-0~ubuntu
RUN set -ex \
...
#  Gradle
# ##############################################################################
RUN apt-get install --no-install-recommends gradle -y \
 && gradle -v
# ##############################################################################
#  Maven
# ##############################################################################
