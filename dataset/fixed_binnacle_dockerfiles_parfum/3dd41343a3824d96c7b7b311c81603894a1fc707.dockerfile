...
RUN echo -e "LANG=\"en_US.UTF-8\" \n LC_ALL=\"en_US.UTF-8\"" > /etc/sysconfig/i18n \
 ...
 && echo "Downloading Jira from: ${JIRA_DOWNLOAD_URL}" \
 && curl -# -L -O ${JIRA_DOWNLOAD_URL} \
 && echo \
 && sync \
 && echo "Downloading AdoptOpenJRE from: ${ADOPT_JRE_DOWNLOAD_URL}" \
 && curl -# -L -O ${ADOPT_JRE_DOWNLOAD_URL} \
 && sync \
 ...
 && ${JIRA_INSTALL}/jre/bin/java -classpath ${JIRA_INSTALL}/lib/catalina.jar org.apache.catalina.util.ServerInfo >> ${JIRA_INSTALL}/atlassian-version.txt \
 && sync \
 && rm -f ./atlassian-jira-software-${JIRA_VERSION}-x64.bin \
 && rm -f $JRE_TARBALL \
 && if [ -n "${JIRA_DATACENTER_SHARE}" ] \
 && [ ! -d "${JIRA_DATACENTER_SHARE}" ] ; then mkdir -p ${JIRA_DATACENTER_SHARE} ; fi \
 && if [ -n "${JIRA_DATACENTER_SHARE}" ] \
 && [ -d "${JIRA_DATACENTER_SHARE}" ] ; then chown -R ${OS_USERNAME}:${OS_GROUPNAME} ${JIRA_DATACENTER_SHARE} ; fi \
