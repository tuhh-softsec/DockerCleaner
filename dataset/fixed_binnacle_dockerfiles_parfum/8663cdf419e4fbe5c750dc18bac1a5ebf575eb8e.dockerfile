...
RUN dpkg --add-architecture i386 \
 ...
 && apt-get install --no-install-recommends apt-utils curl lsb-release software-properties-common -qqy \
 && echo "Installing packages required by Bazel" \
 && apt-get install --no-install-recommends build-essential clang curl ed git iproute2 iputils-ping netcat-openbsd python python-dev python3 python3-dev unzip wget xvfb zip zlib1g-dev -qqy \
 && echo "Installing packages required by Android SDK" \
 && apt-get install --no-install-recommends expect libbz2-1.0:i386 libncurses5:i386 libstdc++6:i386 libz1:i386 -qqy \
 && echo "Installing packages required by Tensorflow" \
 && apt-get install --no-install-recommends libcurl3-dev swig python-enum34 python-mock python-numpy python-pip python-wheel python3-mock python3-numpy python3-pip python3-wheel -qqy \
 && echo "Installing packages required by Envoy" \
 && apt-get install --no-install-recommends automake autotools-dev cmake libtool m4 ninja-build -qqy \
 && echo "Installing packages required by Android emulator" \
 && apt-get install --no-install-recommends cpio cpu-checker lsof qemu-kvm qemu-system-x86 unzip xvfb -qqy \
 && echo "Installing packages required by Bazel release process" \
 && apt-get install --no-install-recommends devscripts gnupg pandoc reprepro -qqy \
 && echo "Installing packages required by C++ coverage tests" \
 && apt-get install --no-install-recommends lcov llvm -qqy \
 && echo "Installing packages required by Swift toolchain" \
 && apt-get install --no-install-recommends clang libicu-dev -qqy \
 && echo "Installing packages required by rules_webtesting" \
 && apt-get install --no-install-recommends python-urllib3 python3-urllib3 -qqy \
 && echo "Installing packages required by Kythe" \
 && apt-get install --no-install-recommends bison flex uuid-dev asciidoc graphviz source-highlight -qqy \
 && echo "Installing packages required by upb" \
 && apt-get install --no-install-recommends libreadline-dev -qqy \
 && echo "Installing packages required by Bazel (Ubuntu 14.04 and 16.04 only)" \
 && apt-get install --no-install-recommends realpath libssl-dev -qqy \
...
RUN apt-get update -qqy \
 && apt-get install --no-install-recommends apt-transport-https ca-certificates -qqy \
...
RUN apt-get update -qqy \
 && apt-get install --no-install-recommends openjdk-8-jdk -qqy \
 && rm -rf /var/lib/apt/lists/*
