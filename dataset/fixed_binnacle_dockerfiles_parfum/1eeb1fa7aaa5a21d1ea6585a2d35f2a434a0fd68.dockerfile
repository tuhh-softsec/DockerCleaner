...
# RUN apt-get update
RUN apt-get update \
 && apt-get install --no-install-recommends qemu-kvm qemu-utils bridge-utils dnsmasq uml-utilities iptables wget net-tools -y \
 && apt-get install --no-install-recommends build-essential git vim make zip unzip curl wget bzip2 ssh openssh-server socat -y \
 && apt-get install --no-install-recommends openjdk-8-jdk -y \
 && apt-get install --no-install-recommends software-properties-common -y \
 && apt-get install --no-install-recommends net-tools iputils-ping dnsutils -y \
 && apt-get install --no-install-recommends python-dev python-pip -y \
 && apt-get install --no-install-recommends apt-utils usbutils locales udev -y \
