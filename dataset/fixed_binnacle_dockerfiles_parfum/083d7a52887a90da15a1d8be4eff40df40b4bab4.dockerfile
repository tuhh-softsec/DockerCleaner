...
RUN apt-get update \
 && apt-get install --no-install-recommends sudo curl locales -y \
...
RUN apt-get update \
 && apt-get install --no-install-recommends git lsb-release net-tools wget -y \
 && rm -rf /var/lib/apt/lists/*
...
FROM base AS build
RUN apt-get update \
 && apt-get install --no-install-recommends git clang-5.0 cmake nasm python-pip -y \
 && rm -rf /var/lib/apt/lists/* \
...
RUN apt-get update \
 && apt-get install --no-install-recommends dosfstools grub-pc -y
