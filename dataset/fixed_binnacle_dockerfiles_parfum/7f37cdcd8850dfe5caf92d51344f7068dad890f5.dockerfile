...
RUN apt-get update -y \
 && apt-get install --no-install-recommends nano vim joe wget curl jq gawk psmisc python python-dev python-pip python3 python3-dev python3-pip python-six build-essential automake openjdk-8-jdk lsof libcupti-dev openssh-server openssh-client build-essential autotools-dev cmake git bash-completion ca-certificates inotify-tools rsync realpath libjpeg-dev libpng-dev net-tools libsm6 libxext6 rpm cpio net-tools libdapl2 dapl2-utils libmlx4-1 libmlx5-1 ibutils librdmacm1 libibverbs1 libmthca1 ibverbs-utils rdmacm-utils perftest kmod -y
