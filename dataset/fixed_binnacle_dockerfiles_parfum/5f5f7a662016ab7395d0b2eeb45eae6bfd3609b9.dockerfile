...
RUN apt-get update \
 ...
 && apt-get update \
 && apt-get install -y --no-install-recommends --allow-unauthenticated supervisor nginx sudo net-tools zenity xz-utils dbus-x11 x11-utils alsa-utils mesa-utils libgl1-mesa-dri \
 && apt-get autoclean -y \
...
RUN add-apt-repository -y ppa:fcwu-tw/apps \
 && apt-get update \
 && apt-get install -y --no-install-recommends --allow-unauthenticated xvfb x11vnc=0.9.16-1 vim-tiny firefox chromium-browser ttf-ubuntu-font-family ttf-wqy-zenhei \
 && add-apt-repository -r ppa:fcwu-tw/apps \
 && apt-get autoclean -y \
 && apt-get autoremove -y \
 && rm -rf /var/lib/apt/lists/*
RUN apt-get update \
 && apt-get install -y --no-install-recommends --allow-unauthenticated lxde gtk2-engines-murrine gnome-themes-standard gtk2-engines-pixbuf gtk2-engines-murrine arc-theme \
 && apt-get autoclean -y \
 && apt-get autoremove -y \
 && rm -rf /var/lib/apt/lists/*
...
COPY https://github.com/krallin/tini/releases/download/${TINI_VERSION}/tini-armhf /bin/tini
...
RUN sed -i 's#http://archive.ubuntu.com/#http://tw.archive.ubuntu.com/#' /etc/apt/sources.list
RUN apt-get update \
 && apt-get install --no-install-recommends curl ca-certificates gnupg patch -y
#  nodejs
...
RUN cd /src/web \
 && yarn \
 && npm run build
RUN cd /src/web/dist/static/novnc \
