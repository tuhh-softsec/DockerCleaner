...
RUN apt-get update \
 && apt-get install --no-install-recommends python-software-properties software-properties-common postgresql-9.3 postgresql-client-9.3 postgresql-contrib-9.3 -y
RUN apt-get update
RUN apt-get install --no-install-recommends libffi6 libffi-dev libssl-dev libpq-dev git --yes --force-yes
...
RUN apt-get install --no-install-recommends vim less tmux nmap wget unzip -y
COPY .tmux.conf /root
...
# ==============
RUN apt-get update -qqy \
 && apt-get install --no-install-recommends locales xvfb -qqy \
 && rm -rf /var/lib/apt/lists/* /var/cache/apt/*
# ============================
...
RUN wget -q -O - https://dl-ssl.google.com/linux/linux_signing_key.pub | apt-key add - \
 ...
 && apt-get update -qqy \
 && apt-get install --no-install-recommends ${CHROME_VERSION:-google-chrome-stable} -qqy \
