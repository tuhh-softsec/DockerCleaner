...
RUN apt-get -qqy update \
 && apt-get -qqy --no-install-recommends install libltdl7 libhavege1 netcat-openbsd pwgen bc unzip bzip2 apt-utils net-tools jq sudo psmisc iproute2 iputils-ping dbus-x11 wget curl \
...
RUN apt-get -qqy update \
 && apt-get -qqy --no-install-recommends install openjdk-8-jre-headless \
...
RUN apt-get -qqy update \
 && apt-key update -qqy \
 && apt-get -qqy --no-install-recommends install haveged rng-tools \
...
RUN apt-get -qqy update \
 && apt-get -qqy --no-install-recommends install fluxbox xfce4-notifyd \
 && rm -rf /var/lib/apt/lists/* \
 && apt-get -qyy clean
# ============================
...
RUN apt-get -qqy update \
 && apt-get -qqy --no-install-recommends install x11vnc \
 && rm -rf /var/lib/apt/lists/* \
 && apt-get -qyy clean
# ===================================================
#  Run the following commands as non-privileged user
...
RUN apt-get -qqy update \
 && apt-get -qqy --no-install-recommends install ffmpeg gpac \
 && rm -rf /var/lib/apt/lists/* \
 && apt-get -qyy clean
# ==============
#  libav/avconv
