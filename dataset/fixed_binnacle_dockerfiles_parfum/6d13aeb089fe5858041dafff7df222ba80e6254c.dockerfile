...
#  Base o/s + app layer
RUN apt-get update \
 && apt-get install --no-install-recommends apt-utils curl wget pwgen tar zip unzip vim locales openssl ldap-utils libmemcached11 python3 postgresql-client-9.5 ghostscript imagemagick-6.q16 imagemagick-common libmagickwand-6.q16-2 apache2 apache2-utils logrotate libapache2-mod-wsgi-py3 -y \
 && apt-get clean
