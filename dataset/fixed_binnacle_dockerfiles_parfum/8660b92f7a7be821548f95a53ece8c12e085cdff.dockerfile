...
RUN apt-get install --no-install-recommends libcurl3 git autoconf automake make libtool pkg-config cmake -q -y
RUN mkdir /root/src
# ## prereqs for openslide
RUN apt-get install --no-install-recommends zlib1g-dev libpng12-dev libjpeg-dev libtiff5-dev libgdk-pixbuf2.0-dev libxml2-dev libsqlite3-dev libcairo2-dev libglib2.0-dev -q -y
WORKDIR /root/src
#  Openslide
RUN apt-get install --no-install-recommends openslide-tools python3-openslide python3 python3-setuptools python3-pip nodejs npm -y
#  Data Loader API
...
RUN apt-get install --no-install-recommends build-essential pkg-config unzip wget curl -y
#  Install JDK
RUN apt-get install --no-install-recommends openjdk-7-jdk ant -y
