...
RUN apt-get update -y \
 && apt-get install --no-install-recommends autoconf pkg-config libtool git nodejs npm libjpeg-dev libtiff-dev imagemagick postgresql python python-pip python-dev libxml2-dev libxslt1-dev libpq-dev nginx uwsgi-plugin-python supervisor wget vim -y \
