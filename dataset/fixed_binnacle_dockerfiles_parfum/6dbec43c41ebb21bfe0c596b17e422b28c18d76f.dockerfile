#  Kurento Media Server - Nightly version
#
...
RUN apt-get update \
 && apt-get install --no-install-recommends gnupg --yes \
 && apt-get clean \
...
RUN apt-get update \
 && apt-get install --no-install-recommends bzip2 wget kurento-media-server --yes; apt-get clean \
 && rm -rf /var/lib/apt/lists/*
#  Install additional modules
#  These might not be available in Ubuntu 18.04 (Bionic)
RUN apt-get update \
 && apt-get install --no-install-recommends kms-chroma kms-crowddetector kms-platedetector kms-pointerdetector --yes || true; apt-get clean \
 && rm -rf /var/lib/apt/lists/*
EXPOSE 8888/tcp
