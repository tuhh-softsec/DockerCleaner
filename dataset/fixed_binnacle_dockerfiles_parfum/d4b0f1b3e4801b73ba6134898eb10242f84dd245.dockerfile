...
RUN apt-get update \
 ...
 && apt-get install --no-install-recommends -y -o Dpkg::Options::="--force-confdef" -o coreutils util-linux bsdutils file openssl ca-certificates ssh wget patch sudo htop dstat vim tmux curl git jq realpath zsh ksh gcc-4.8 g++-4.8 xz-utils build-essential bash-completion \
 && apt-get clean
