...
RUN apt-get update -y \
 && apt-get install --no-install-recommends python-dev python3-dev python3-pip git zlib1g-dev apt-transport-https ca-certificates wget build-essential libcurl4-openssl-dev g++ htop nano parallel curl locales daemontools unzip python3-distutils -y \
 && apt-get clean
