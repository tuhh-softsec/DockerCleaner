...
#  essential tools
RUN apt-get update \
 && apt-get install --no-install-recommends curl apt-transport-https -y
#  add yarn
RUN curl -sS https://dl.yarnpkg.com/debian/pubkey.gpg | apt-key add - \
 && echo "deb https://dl.yarnpkg.com/debian/ stable main" | tee /etc/apt/sources.list.d/yarn.list
#  more tools
RUN apt-get update \
 && apt-get install --no-install-recommends git xvfb yarn=1.6.0-1 sudo openssh-client ca-certificates tar gzip wget xz-utils autoconf build-essential zlib1g-dev libssl-dev curl libreadline-dev python python-dev -y
#  install node
...
RUN curl --output /tmp/firefox.deb https://s3.amazonaws.com/circle-downloads/firefox-mozilla-build_47.0.1-0ubuntu1_amd64.deb \
 ...
 && apt-get update \
 && apt-get install --no-install-recommends libgtk3.0-cil-dev -y \
...
RUN apt-get update \
 && apt-get install --no-install-recommends parallel libmagickwand-dev imagemagick -y
RUN mv /usr/bin/parallel /usr/bin/gnu_parallel
RUN apt-get update
RUN apt-get install --no-install-recommends libicu-dev enscript moreutils pdftk libmysqlclient-dev libsqlite3-dev -y
...
RUN apt-get install --no-install-recommends rbenv -y
...
RUN sudo apt-get install --no-install-recommends -y jq
