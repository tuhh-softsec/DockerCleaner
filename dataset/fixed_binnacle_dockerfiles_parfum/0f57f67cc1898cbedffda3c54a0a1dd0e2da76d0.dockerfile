...
RUN apt-get update \
 && apt-get install --no-install-recommends autotools-dev build-essential git gfortran-multilib libavcodec-dev libavformat-dev libjasper-dev libjpeg-dev libpng-dev liblapacke-dev libswscale-dev libtiff-dev pkg-config wget zlib1g-dev libssl-dev openssl ca-certificates curl libcurl4-openssl-dev unzip python-dev automake libtool-bin autoconf subversion libapr1 libaprutil1 libltdl-dev libltdl7 libserf-1-1 libsigsegv2 libsvn1 m4 openjdk-8-jdk libpcre3-dev libgdiplus apt-transport-https -y \
...
RUN wget -q -O - https://cntkbuildstorage.blob.core.windows.net/cntk-ci-dependencies/openmpi/$OPENMPI_VERSION/openmpi-$OPENMPI_VERSION.tar.gz | tar -xzf - \
 ...
 && apt-get update -y \
 && apt-get install -y -f \
 && apt-get install --no-install-recommends libsysfs2 libsysfs-dev -y \
...
RUN wget -q https://cntkbuildstorage.blob.core.windows.net/cntk-ci-dependencies/packages-microsoft-prod/deb/packages-microsoft-prod.deb \
 ...
 && apt-get install --no-install-recommends apt-transport-https -y -f \
 && apt-get update -y \
