...
RUN apt-get update -qq \
 ...
 && apt-get install --no-install-recommends git pandoc pandoc-citeproc libssl-dev -qq -y
RUN apt-get install --no-install-recommends libgmp-dev libmpfr-dev libxml2-dev -qq -y
