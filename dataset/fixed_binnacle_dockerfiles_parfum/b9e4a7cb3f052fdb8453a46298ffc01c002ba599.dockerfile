...
#  ```bash
RUN apt-get update \
 && apt-get install --no-install-recommends -y build-essential cmake git gcc g++ cmake libc-ares-dev libc-ares2 libcurl4-openssl-dev libssl1.0-dev make pkg-config tar wget zlib1g-dev
#  ```
# # [END README.md]
FROM devtools
