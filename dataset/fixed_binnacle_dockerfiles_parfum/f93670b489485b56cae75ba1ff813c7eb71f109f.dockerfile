...
RUN apt-get update \
 && apt-get install --no-install-recommends ca-certificates gnupg build-essential wget subversion unzip automake python cmake ninja-build curl git gcc-multilib g++-multilib libc6-dev bison flex libtool autoconf binutils-dev binutils-gold software-properties-common gnupg apt-transport-https systemd sysvinit-utils -y \
...
RUN apt-get update \
 && apt-get install --no-install-recommends bash-completion buildbot-slave -y \
 && rm -rf /var/lib/apt/lists/*
