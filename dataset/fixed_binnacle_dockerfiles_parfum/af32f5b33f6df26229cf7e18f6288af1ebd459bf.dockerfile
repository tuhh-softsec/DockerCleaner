...
RUN apt-get update -y \
 && apt-get install --no-install-recommends vim tar unzip dnsmasq wget net-tools curl openssh-server nano g++ autoconf automake libtool cmake zlib1g-dev pkg-config libssl-dev -y
RUN mkdir -p /opt
#  Protocol buffers
