...
RUN apt-get update \
 && apt-get upgrade -y \
 && apt-get install --no-install-recommends jq netcat inetutils-ping vim git wget make cmake gcc g++ python libev-dev libmpdec-dev libjansson-dev libssl-dev libgnutls-dev libmysqlclient-dev libhttp-parser-dev libcurl4-openssl-dev libldap2-dev libkrb5-dev libalberta-dev libgss-dev libidn11-dev librtmp-dev -y \
...
RUN apt-get install --no-install-recommends libldap-2.4 libgss3 librtmp1 libmysqlclient20 jq netcat -y
