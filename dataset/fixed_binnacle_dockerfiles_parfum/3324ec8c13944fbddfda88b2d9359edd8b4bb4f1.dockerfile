...
RUN apt-get install --no-install-recommends libsundials-cvode1 libsundials-cvodes2 libsundials-ida2 libsundials-idas0 libsundials-kinsol1 libsundials-nvecserial0 libsundials-serial libsundials-serial-dev -y \
 && apt-get clean
#  NLopt
RUN apt-get install --no-install-recommends libnlopt0 libnlopt-dev -y \
 && apt-get clean
#  Ipopt
RUN mkdir ipopt \
 && cd ipopt \
...
RUN mkdir cbc \
 ...
 && rm -rf cbc
#  MPI
RUN apt-get install --no-install-recommends openmpi-bin libopenmpi-dev -y \
 && apt-get clean
#  Stan
RUN mkdir stan \
 && cd stan \
 ...
 && rm -rf stan
#  Nemo
RUN apt-get install --no-install-recommends m4 -y \
 && apt-get clean
RUN mkdir mpir \
 && cd mpir \
