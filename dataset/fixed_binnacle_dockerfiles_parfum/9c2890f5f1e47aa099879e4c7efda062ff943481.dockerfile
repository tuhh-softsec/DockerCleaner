...
# ######################
RUN apt-get update \
 && apt-get install --no-install-recommends gettext-base patch libcurl4-openssl-dev libpcre3-dev git libmysqlclient-dev mysql-client apache2-utils fontconfig hunspell-en-us libhunspell-1.3-0 libhunspell-dev pngcrush secure-delete xfonts-75dpi xfonts-base xpdf pdftk -y \
 && apt-get clean \
 && rm -rf /var/lib/apt/lists/* /tmp/* /var/tmp/
...
# #################
RUN apt-get update \
 && apt-get install --no-install-recommends libc6 libtiff5 libgif7 libcurl3 libfontconfig1 libjpeg8 libxml2 -y \
 && apt-get clean \
 && rm -rf /var/lib/apt/lists/* /tmp/* /var/tmp/
WORKDIR /tmp
...
RUN apt-get update \
 && apt-get update \
 && apt-get install --no-install-recommends checkinstall libtiff5-dev libx11-dev libxext-dev zlib1g-dev libpng12-dev libjpeg-dev ghostscript libgs-dev imagemagick -y \
 && apt-get clean \
 && rm -rf /var/lib/apt/lists/* /tmp/* /var/tmp/
