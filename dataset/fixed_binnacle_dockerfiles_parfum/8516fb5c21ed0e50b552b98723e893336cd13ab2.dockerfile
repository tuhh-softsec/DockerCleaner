...
RUN apt-get install --no-install-recommends apt-utils iputils-ping python2.7 -y
RUN apt-get install --no-install-recommends python-pip -y
RUN apt-get install --no-install-recommends lsb-core libxext6 libsm6 libxcursor1 libxft2 libxrandr2 libxt6 libxinerama1 libasound2 libatk1.0-0 libcairo2 libgl1 libglu1 libgtk2.0-0 libxi6 libxtst6 gcc-multilib libstdc++6 libwebkitgtk-1.0-0 -y --fix-missing
