...
RUN apt-get update -y \
 && apt-get -y upgrade \
 && apt-get install --no-install-recommends whiptail git pkg-config gcc wget make -y \
 && apt-get install --no-install-recommends libblas-dev liblapack-dev zlib1g-dev libbz2-dev gfortran -y \
 && apt-get install --no-install-recommends file python3 python3-matplotlib python3-dev -y \
 && apt-get clean \
...
RUN (wget -q https://bootstrap.pypa.io/ez_setup.py -O - | python3 ) \
 && easy_install pip \
 && rm -f setuptools*.zip \
 && pip install cython --no-cache-dir --upgrade \
 && pip install snakemake colormath numpy scipy pandas PyYAML biopython feather-format XlsxWriter --no-cache-dir --upgrade
#  Install htslib
RUN cd /tmp \
