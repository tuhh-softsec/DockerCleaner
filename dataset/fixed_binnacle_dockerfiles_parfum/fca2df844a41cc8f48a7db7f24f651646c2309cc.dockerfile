...
RUN set -x \
 ...
 && docker-php-ext-install bcmath bz2 calendar exif intl imap gettext ldap mysqli hash pcntl pdo_mysql pdo_pgsql pgsql soap sockets tokenizer sysvmsg sysvsem sysvshm shmop xmlrpc xsl zip gd gettext opcache \
 && pecl install apcu \
 && MEMCACHED="`mktemp -d `" \
