...
RUN apt-get update -y \
 && apt-get install --no-install-recommends python3-pycurl locales ssh-askpass python3-tk screen -y
RUN pip install pip --upgrade
RUN pip uninstall -y keras ; pip install keras
RUN pip install setuptools
RUN pip install flask flask.restful
RUN pip install bs4 requests
RUN pip install flask-cors -U
RUN pip install pandas jupyter matplotlib spacy sklearn scikit-image munch imgaug
...
#  Add Glove vectors
RUN apt-get update -y \
 && apt-get install --no-install-recommends zip unzip -y
# Telemetry
# --------------
