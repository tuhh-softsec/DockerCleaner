...
#  copy source code
RUN wget -q https://github.com/elastic/elasticsearch/archive/v5.6.16.tar.gz -O /usr/local/src/elasticsearch-source-v5.6.16.tar.gz
RUN set -x \
...
COPY config ./config
RUN set -ex \
 && for path in ./data ./logs ./config ./config/scripts /usr/share/elasticsearch; do mkdir -p "$path" ;chown -R elasticsearch:elasticsearch "$path" ; done
