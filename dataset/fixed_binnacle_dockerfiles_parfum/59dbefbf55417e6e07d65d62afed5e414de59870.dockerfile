...
#  Install basics
RUN apt-get install --no-install-recommends git wget curl zip unzip apt-transport-https ca-certificates gnupg -y
...
RUN apt-get install --no-install-recommends locales golang ruby ruby-dev nodejs lib32stdc++6 libstdc++6 libglu1-mesa build-essential default-jdk-headless google-chrome-stable -y
