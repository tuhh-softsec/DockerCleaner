...
RUN set -x \
 ...
 && : "OpenCV dependencies" \
 && apt-get install --no-install-recommends libgtk-3-dev libjpeg-dev libpng++-dev libtiff-dev libopenexr-dev libwebp-dev ffmpeg libavcodec-dev libavformat-dev libavutil-dev libswscale-dev libavresample-dev -y -qq \
 && : "Pangolin dependencies" \
 && apt-get install --no-install-recommends libglew-dev -y -qq \
 && : "other dependencies" \
