...
RUN wget -O - http://download.redis.io/releases/redis-4.0.8.tar.gz | tar zx \
 ...
 && rm -rf /redis-*
# libmodsecurity
RUN apt-get install --no-install-recommends m4 libtool automake libxml2-dev libyajl-dev libgeoip-dev libcurl4-gnutls-dev pkgconf -y
...
# logrotate
RUN apt-get install --no-install-recommends logrotate cron -y
