...
RUN apt-get install --no-install-recommends git make python ruby sudo curl -y
...
RUN apt-get install --no-install-recommends python-dev libffi-dev libssl-dev libyaml-dev libssh2-1 libssh2-1-dev autoconf automake libtool libxml2-dev libxslt1-dev python-cffi zlib1g-dev cmake python-setuptools gcc wget python-pip openssl -y \
 && apt-get clean
...
CMD [ "if [ -f /data/hubble_buildinfo ] ; then echo \"\" >> /hubble_build/hubblestack/__init__.py ; cat /data/hubble_buildinfo >> /hubble_build/hubblestack/__init__.py; fi \
 ...
 && openssl dgst -sha256 /data/hubblestack_${HUBBLE_VERSION}-${HUBBLE_ITERATION}deb9_amd64.deb > /data/hubblestack_${HUBBLE_VERSION}-${HUBBLE_ITERATION}deb9_amd64.deb.sha256" ]
