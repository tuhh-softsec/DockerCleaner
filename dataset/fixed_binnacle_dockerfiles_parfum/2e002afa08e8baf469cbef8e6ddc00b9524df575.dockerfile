...
RUN apt-get update \
 && apt-get install --no-install-recommends wget curl unzip git-core clang libc++1 libc++-dev libc++abi-dev build-essential autotools-dev autoconf libtool cmake zlib1g-dev libbz2-dev libssl-dev libboost-all-dev -y \
