...
RUN apt-get update \
 && apt-get install --no-install-recommends git build-essential curl libc6-i386 unzip mercurial libgmp-dev -y
...
USER root
RUN apt-get update \
 && apt-get install --no-install-recommends apache2 libapache2-mod-php7.0 php7.0-pgsql graphviz -y
