...
RUN apt-get update \
 && apt-get install --no-install-recommends unzip curl apt-transport-https winbind build-essential autoconf bison gperf flex libtool mingw-w64 git-core imagemagick pkg-config -y
...
RUN apt-get install --no-install-recommends winehq-staging -y
RUN curl -L http://sourceforge.net/projects/mingw/files/Installer/mingw-get/mingw-get-${MINGW_VERSION}/mingw-get-${MINGW_BIN_VERSION}.zip/download > /tmp/mingw-get.zip
...
#
RUN apt-get install --no-install-recommends python-dirspec -y \
 && cp -r /usr/lib/python2.7/dist-packages/dirspec* /root/.wine/drive_c/Python27/Lib/site-packages/
RUN apt-get install --no-install-recommends python-setuptools -y
