...
RUN apt-get update --fix-missing \
 ...
 && echo "tries = 5" >> wgetrc
#  Open JDK \
RUN apt-get install --no-install-recommends ca-certificates java-common openssl unzip curl wget apt-transport-https bsdtar -y
#  tomcat for IDP. Permission changes here due to https://github.com/docker-library/tomcat/issues/35#issuecomment-238014866 \
RUN apt-get install --no-install-recommends tomcat8 libtcnative-1 tomcat8-common libtomcat8-java libecj-java ucf -y \
...
RUN apt-get install --no-install-recommends grep lsof net-tools ldap-utils apache2 -y \
 && apt-get autoclean \
