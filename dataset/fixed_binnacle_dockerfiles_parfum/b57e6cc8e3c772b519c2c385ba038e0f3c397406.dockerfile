...
RUN apt-get update \
 && apt-get install --no-install-recommends apt-transport-https gnupg2 -y \
...
RUN apt-get update \
 && apt-get -t stretch --no-install-recommends install -y bash-completion build-essential curl libcap-dev libjpeg-dev libpcre3-dev libsasl2-dev libxml2-dev libxslt-dev locales zlib1g-dev libffi-dev libmagic-dev libssl-dev nodejs uuid-dev git mysql-client default-libmysqlclient-dev swig gettext librsvg2-bin pngcrush libmaxminddb0 libmaxminddb-dev \
 && rm -rf /var/lib/apt/lists/*
RUN apt-get update \
 && apt-get -t buster --no-install-recommends install -y file libmagic-dev \
 && rm -rf /var/lib/apt/lists/*
