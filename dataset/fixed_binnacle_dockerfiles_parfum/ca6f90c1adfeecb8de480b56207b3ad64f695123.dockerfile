FROM akabe/iocaml:debian8_ocaml4.05.0
...
RUN curl -L "https://storage.googleapis.com/tensorflow/libtensorflow/libtensorflow-cpu-linux-x86_64-${TENSORFLOW_VERSION}.tar.gz" | sudo tar xz -C /usr \
 ...
 && sudo apt-get update \
 && sudo apt-get install --no-install-recommends -y rsync aspcud bzip2 gfortran ssh libffi-dev libblas-dev liblapack-dev libgsl0-dev libfftw3-dev libsvm-dev libcairo2-dev libmariadb-dev libpq-dev libsqlite3-dev libcurl4-openssl-dev libgmp-dev imagemagick ffmpeg \
