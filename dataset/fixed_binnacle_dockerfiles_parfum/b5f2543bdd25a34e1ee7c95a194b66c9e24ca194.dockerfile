...
RUN apt-get update \
 && apt-get install --no-install-recommends -y apt-transport-https software-properties-common wget unzip curl ca-certificates
# bzip2 \
# grep sed dpkg
#  Install opencv dependencies
RUN cd ~
RUN apt-get update \
 && apt-get install --no-install-recommends -y build-essential cmake git gfortran libjpeg8-dev libpng-dev software-properties-common
RUN add-apt-repository "deb http://security.ubuntu.com/ubuntu xenial-security main" \
 && apt-get update \
 && apt-get install --no-install-recommends -y libjasper1 libtiff-dev libavcodec-dev libavformat-dev libswscale-dev libdc1394-22-dev libxine2-dev libv4l-dev
RUN cd /usr/include/linux
RUN ln -s -f ../libv4l1-videodev.h videodev.h
RUN cd ~
RUN apt-get install --no-install-recommends -y libgstreamer1.0-dev libgstreamer-plugins-base1.0-dev libgtk2.0-dev libtbb-dev qt5-default libatlas-base-dev libfaac-dev libmp3lame-dev libtheora-dev libvorbis-dev libxvidcore-dev libopencore-amrnb-dev libopencore-amrwb-dev libavresample-dev x264 v4l-utils
#  Setup OpenCV source
