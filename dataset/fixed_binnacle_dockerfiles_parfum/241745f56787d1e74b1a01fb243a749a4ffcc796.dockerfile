...
RUN set -eux \
 ...
 && apt-get ${APT_FLAGS_COMMON} update \
 && DEBIAN_FRONTEND=noninteractive apt-get ${APT_FLAGS_PERSISTENT} install apache2 curl libapache2-mod-php mysql-client php7.2-bcmath php7.2-gd php7.2-json php7.2-ldap php7.2-mbstring php7.2-mysql php7.2-xml \
...
RUN set -eux \
 && apt-get ${APT_FLAGS_COMMON} update \
 && DEBIAN_FRONTEND=noninteractive apt-get ${APT_FLAGS_DEV} install gettext ca-certificates git \
 && cd /usr/share/ \
