...
RUN apt-get install --no-install-recommends dbus-x11 procps psmisc -y
#  OpenGL / MESA
RUN apt-get install --no-install-recommends mesa-utils mesa-utils-extra libxv1 kmod xz-utils -y
#  language locales. Change LANG to your desired default locale
RUN apt-get install --no-install-recommends locales-all -y
#  deepin desktop
...
#  chinese fonts
RUN apt-get install --no-install-recommends fonts-arphic-uming -y
