...
RUN apt-get update \
 && apt-get install --no-install-recommends zip unzip vim git curl libglu1 python-setuptools zlib1g-dev git libpng-dev libfreetype6-dev pkg-config g++ vim r-base-core libgsl0-dev build-essential openssl -y
...
#  Configure git-lfs
RUN apt-get install --no-install-recommends apt-transport-https debian-archive-keyring -y
