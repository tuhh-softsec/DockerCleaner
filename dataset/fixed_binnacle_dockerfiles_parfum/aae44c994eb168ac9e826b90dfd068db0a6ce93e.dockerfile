...
RUN apt-get install --no-install-recommends build-essential gcc git osmosis libxml2-dev libgeos-dev libpq-dev libbz2-dev libtool cmake libproj-dev proj-bin libgeos-c1 libgeos++-dev libexpat1-dev -y
#  Install Boost (required by osm2pqsql)
RUN apt-get install --no-install-recommends autoconf make g++ libboost-dev libboost-system-dev libboost-filesystem-dev libboost-thread-dev lua5.2 liblua5.2-dev -y
#  Install PHP5
RUN apt-get install --no-install-recommends php5 php-pear php5-pgsql php5-json php-db -y
...
RUN apt-get install --no-install-recommends postgresql postgis postgresql-contrib postgresql-9.3-postgis-2.1 postgresql-server-dev-9.3 -y
...
RUN apt-get install --no-install-recommends libprotobuf-c0-dev protobuf-c-compiler -y
