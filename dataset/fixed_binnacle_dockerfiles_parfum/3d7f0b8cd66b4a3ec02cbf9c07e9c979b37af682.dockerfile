...
RUN apt-get update
RUN apt-get install --no-install-recommends libapache2-mod-php7.0 php7.0-cli php7.0-mysql php7.0-mysqli php7.0-gd php7.0-mcrypt php7.0-json php-pear snmp fping mysql-client python-mysqldb rrdtool subversion whois mtr-tiny ipmitool graphviz imagemagick apache2 -y
RUN apt-get install --no-install-recommends libvirt-bin -y
RUN apt-get install --no-install-recommends cron supervisor wget locales -y
RUN apt-get clean
#  set locale
