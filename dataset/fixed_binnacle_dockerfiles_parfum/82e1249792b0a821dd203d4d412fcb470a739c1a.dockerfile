...
RUN apt-get update \
 && apt-get install --no-install-recommends vim git supervisor python-dev libffi-dev libssl-dev nagios-nrpe-server wget python-virtualenv python-openssl -y
#  libffi-devand libssl-dev are for python-cryptography
# ## Alignak
RUN apt-get update \
 && apt-get install --no-install-recommends python-pip python-pycurl -y
RUN useradd alignak \
...
#  mod-ceilometer
RUN apt-get update \
 && apt-get install --no-install-recommends python-ceilometerclient -y
RUN cd /tmp \
...
# # plugins
RUN apt-get update \
 && apt-get install --no-install-recommends nagios-plugins nagios-nrpe-plugin -y
