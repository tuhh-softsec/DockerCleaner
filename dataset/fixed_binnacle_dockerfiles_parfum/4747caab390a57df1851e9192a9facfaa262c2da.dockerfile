...
RUN echo 'gcg' | sudo -S apt-get install --no-install-recommends -y git build-essential cmake libopenmpi-dev
RUN echo 'gcg' | sudo -S apt-get install --no-install-recommends -y zlib1g-dev tmux
RUN echo 'gcg' | sudo -S apt-get install --no-install-recommends -y libglew-dev libglm-dev libassimp-dev xorg-dev libglu1-mesa-dev libboost-dev mesa-common-dev freeglut3-dev libopenmpi-dev cmake golang libjpeg-turbo8-dev wmctrl xdotool
RUN echo 'gcg' | sudo -S apt-get install --no-install-recommends -y vim wget unzip
RUN echo 'gcg' | sudo -S apt-get install --no-install-recommends -y libzmq3-dev
