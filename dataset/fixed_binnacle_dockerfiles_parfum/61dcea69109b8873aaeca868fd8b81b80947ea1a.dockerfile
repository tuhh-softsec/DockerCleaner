...
RUN apt-get update \
 ...
 && apt-get install --no-install-recommends supervisor curl wget openssl apache2 postgresql unzip ssh rsync git -y
#  PHP
RUN apt-get install --no-install-recommends libapache2-mod-php php-gd php-pgsql php-xsl php-curl -y
#  Java 8
RUN apt-get install --no-install-recommends software-properties-common python-software-properties -y \
 ...
 && apt-get install --no-install-recommends oracle-java8-installer maven -y
ENV JAVA_HOME="\"/usr/lib/jvm/java-8-oracle\""
#  dev
RUN apt-get install --no-install-recommends libxml2-utils vim -y
#
#  Configure apache
