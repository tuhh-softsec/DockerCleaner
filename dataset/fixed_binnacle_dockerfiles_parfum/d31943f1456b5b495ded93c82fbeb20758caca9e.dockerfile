...
RUN mkdir -p /tmp/fedora-gsearch \
 ...
 && cp "fedoragsearch-2.8/fedoragsearch.war" "/var/www/fedora/tomcat/webapps/fedoragsearch.war" \
...
RUN mkdir -p /tmp/solr \
 ...
 && cp -r /tmp/solr/solr-4.10.4/example/solr/* /var/www/fedora/solr \
 && sed -i 's/\.\.\/\.\.\/\.\.\//\.\.\//g' /var/www/fedora/solr/collection1/conf/solrconfig.xml \
 && cp -r /tmp/solr/solr-4.10.4/dist/ /var/www/fedora/solr \
 && cp -r /tmp/solr/solr-4.10.4/contrib/ /var/www/fedora/solr \
 && cp -r /tmp/solr/solr-4.10.4/example/webapps/solr.war /var/www/fedora/tomcat/webapps/solr.war \
 && mkdir /var/www/fedora/tomcat/webapps/solr \
 && cd /var/www/fedora/tomcat/webapps/solr/ \
 && unzip ../solr.war \
 && cp -r /tmp/solr/solr-4.10.4/example/lib/ext/* WEB-INF/lib \
 && mkdir WEB-INF/classes \
 && cp /tmp/solr/solr-4.10.4/example/resources/log4j.properties WEB-INF/classes \
...
#  Configuration
#
COPY fedora/fedora-users.xml /var/www/fedora/server/config/fedora-users.xml
COPY gsearch/fgsconfig-basic-for-islandora.properties /var/www/fedora/tomcat/webapps/fedoragsearch/FgsConfig/fgsconfig-basic-for-islandora.properties
...
COPY solr/solr.xml /var/www/fedora/tomcat/conf/Catalina/localhost/solr.xml
COPY solr/schema.xml $FEDORA_HOME/solr/collection1/conf/schema.xml
COPY solr/schema.xml $FEDORA_HOME/tomcat/webapps/fedoragsearch/WEB-INF/classes/fgsconfigFinal/index/FgsIndex/conf/flat-solr-schema.xml
COPY solr/solrcore.properties $FEDORA_HOME/solr/collection1/conf/solrcore.properties
#
...
RUN mkdir -p /tmp \
 ...
 && mvn package \
 && cp target/gsearch_extensions-0.1.3-jar-with-dependencies.jar $FEDORA_HOME/tomcat/webapps/fedoragsearch/WEB-INF/lib \
...
COPY flat/FindProfiles /tmp/FindProfiles
...
COPY flat/cmd-gsearch/* /app/flat/lib/cmd-gsearch/
RUN mkdir -p /app/flat/deposit/policies/
COPY flat/gsearch-mapping.xml /app/flat/deposit/policies/
#
