...
RUN /bin/bash -c "export DEBIAN_FRONTEND=noninteractive" \
 ...
 && apt-get install --no-install-recommends apache2 cron curl libapache2-mod-php patch php php-curl php-dompdf php-gd php-imagick php-intl php-mbstring php-xml php-zip php-apcu php-ldap php-oauth php-pgsql php-smbclient wget pwgen sudo lbzip2 unattended-upgrades -y
RUN apt-get clean
RUN a2enmod headers
RUN a2enmod rewrite
RUN cd /root/ \
