...
#  add repos
RUN set -e ; add-apt-repository -y ppa:git-core/ppa ; add-apt-repository -y ppa:ubuntu-toolchain-r/test ; wget -O - https://apt.llvm.org/llvm-snapshot.gpg.key | apt-key add - ; echo 'deb http://apt.llvm.org/xenial/ llvm-toolchain-xenial-6.0 main' >> /etc/apt/sources.list; apt-get update
...
#  install soci
RUN set -e ; git clone https://github.com/SOCI/soci /tmp/soci ; (cd /tmp/soci ;git checkout 349ce86b79e63b99ba95200bc4bd1d83791e9094 ) ; cmake -DCMAKE_BUILD_TYPE=${CMAKE_BUILD_TYPE} -DSOCI_CXX_C11=ON -DSOCI_TESTS=OFF -DSOCI_EMPTY=OFF -DWITH_BOOST=ON -DWITH_DB2=OFF -DWITH_FIREBIRD=OFF -DWITH_MYSQL=OFF -DWITH_ODBC=OFF -DWITH_ORACLE=OFF -DWITH_POSTGRESQL=ON -DWITH_SQLITE3=OFF -H/tmp/soci -B/tmp/soci/build -DCMAKE_PREFIX_PATH="/opt/dependencies/libpq" -DCMAKE_INSTALL_PREFIX=/opt/dependencies/soci ; cmake --build /tmp/soci/build --target install ; ldconfig ; rm -rf /tmp/soci
#  install tbb
...
#  OpenJRE 8
RUN set -e ; apt-get install --no-install-recommends openjdk-8-jre -y; java -version
RUN set -e ; add-apt-repository -y ppa:jonathonf/python-3.6 ; apt-get update ; apt-get install --no-install-recommends python3.6-dev -y
