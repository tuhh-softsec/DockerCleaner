...
RUN apt-get update -qq \
 && apt-get install --no-install-recommends -qq -y bison g++ gcc gdb git libevent-dev liblz4-dev libmecab-dev libmsgpack-dev libssl-dev libstemmer-dev libzmq3-dev libzstd-dev make mecab-naist-jdic pkg-config rapidjson-dev rsync ruby ruby-dev sudo tzdata zlib1g-dev
#  RUN \
#    apt update -qq && \
...
RUN apt-get update -qq \
 && apt-get install --no-install-recommends -qq -y apt-transport-https lsb-release wget \
