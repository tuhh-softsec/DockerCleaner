...
RUN apt-get install --no-install-recommends build-essential wget curl unzip software-properties-common cabal-install libghc-zlib-dev libghc-zlib-bindings-dev libghc-terminfo-dev libpython-dev clang-3.6 -y
...
RUN apt-get install --no-install-recommends openjdk-7-jdk ruby nodejs ghc smlnj php5-cli racket swi-prolog mono-complete fsharp gcc-multilib nasm clisp erlang golang lua5.2 mono-vbnc gfortran fp-compiler -y
#  install Scala
