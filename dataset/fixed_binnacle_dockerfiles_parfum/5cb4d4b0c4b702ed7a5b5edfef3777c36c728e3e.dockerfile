...
#  more tools
RUN apt-get update \
 && apt-get install --no-install-recommends git xvfb yarn=0.27.5-1 sudo openssh-client ca-certificates tar gzip wget xz-utils autoconf build-essential zlib1g-dev libssl-dev curl libreadline-dev python python-dev -y
#  install ruby
