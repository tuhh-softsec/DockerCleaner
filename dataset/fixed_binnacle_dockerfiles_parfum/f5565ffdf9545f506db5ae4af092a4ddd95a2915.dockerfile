...
RUN apt-get install --no-install-recommends gcc libssl-dev autoconf erlang-dev erlang-nox nodejs npm unzip wget git python-pip python-dev python-zmq bc bsdmainutils jq -y
#  Update nodejs
...
RUN apt-get install --no-install-recommends siege -y
