...
RUN apt-get update \
 && apt-get install --no-install-recommends php-pear libcurl4-openssl-dev pkg-config libssl-dev libsslcommon2-dev -y \
...
RUN apt-get install --no-install-recommends memcached php-memcached -y
