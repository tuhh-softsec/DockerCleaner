...
RUN apt-get update \
 && apt-get install --no-install-recommends sudo git fakeroot devscripts cmake vim qemu-user-static binfmt-support dh-make dh-exec pkg-kde-tools device-tree-compiler bc cpio parted dosfstools mtools libssl-dev g++-aarch64-linux-gnu -y
RUN apt-get update \
...
#  xf86-video-armsorc
RUN apt-get update \
 && apt-get install --no-install-recommends xserver-xorg-dev:arm64 -y
...
# # yocto
RUN apt-get update \
 && apt-get install --no-install-recommends gawk wget git-core diffstat unzip texinfo build-essential chrpath socat xterm locales -y
