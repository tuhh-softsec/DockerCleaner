...
RUN apt-get install --no-install-recommends unixodbc unixodbc-dev odbc-postgresql libsqliteodbc -y
#  Utilities
RUN apt-get install --no-install-recommends vim net-tools inetutils-ping curl git nmap socat sudo libcairo2-dev libxt-dev xclip xsel bzip2 python3-pip python3-setuptools supervisor libc6 libzmq5 libmagick++-dev ed rsync vifm -y
#  TeX for the rmarkdown package in RStudio, and pandoc is also useful
RUN apt-get install --no-install-recommends texlive texlive-base texlive-latex-extra texlive-pstricks texlive-xetex -y \
