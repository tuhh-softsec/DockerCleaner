...
RUN sed -i 's/# \(.*multiverse$\)/\1/g' /etc/apt/sources.list \
 && apt-get update \
 && apt-get install --no-install-recommends wget curl build-essential software-properties-common python-software-properties nano -y
...
USER root
COPY phrackCTF.sql /tmp/phrackCTF.sql
