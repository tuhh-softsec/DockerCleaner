...
RUN apt-get update \
 && apt-get install --no-install-recommends aufs-tools automake btrfs-tools build-essential curl dpkg-sig git iptables libapparmor-dev libcap-dev libsqlite3-dev mercurial parallel reprepro ruby1.9.1 ruby1.9.1-dev lxc=1.0* s3cmd=1.1.0* -y
