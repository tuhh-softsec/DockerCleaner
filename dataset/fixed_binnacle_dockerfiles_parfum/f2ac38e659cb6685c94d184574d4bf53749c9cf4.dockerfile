...
RUN apt-get install --no-install-recommends python-pip python-dev build-essential -y
#  Install libpcap
RUN apt-get install --no-install-recommends libpcap-dev -y
#  Install go
...
RUN apt-get update \
 && apt-get install --no-install-recommends libcurl4-nss-dev libapr1-dev libsvn-dev -y
#  Install nginx
ENV NGX_REQS="openssl libssl1.0.0 libxml2 libxslt1.1 libgeoip1 libpcre3 zlib1g"
RUN apt-get update \
 && apt-get install --no-install-recommends $NGX_REQS -y \
 && apt-get clean \
...
RUN apt-get update \
 && apt-get install --no-install-recommends $DEV_PKGS -y \
 && mkdir -p /tmp/build \
 && cd /tmp/build \
