...
RUN apt-get update \
 && apt-get install --no-install-recommends curl cron rsyslog supervisor mysql-client libpng-dev libjpeg-dev libxml2-dev ruby ruby2.3 ruby2.3-dev git unzip vim libicu-dev libssl-dev -y \
 && gem install bundler \
...
RUN export VERSION=`php -r "echo PHP_MAJOR_VERSION.PHP_MINOR_VERSION;" ` \
 ...
 && echo "extension=blackfire.so\nblackfire.agent_socket=tcp://blackfire:8707" > $PHP_INI_DIR/blackfire.ini
#  Install ngrok.
