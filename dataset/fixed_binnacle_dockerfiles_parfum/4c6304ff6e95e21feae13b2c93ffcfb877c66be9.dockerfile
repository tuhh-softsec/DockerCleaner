...
RUN apt-get update -y -q \
 && DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y wget bzip2 unzip git curl cmake xorg-dev libglew-dev libosmesa6-dev patchelf libpq-dev ffmpeg libjpeg-dev swig libsdl2-dev libopenmpi-dev openmpi-bin \
 && apt-get clean \
