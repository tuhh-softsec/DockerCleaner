...
RUN apt-get install --no-install-recommends git make wget unzip m4 gcc curl indent software-properties-common maven -y
...
RUN apt-get install --no-install-recommends autotools-dev automake libtool zlib1g-dev -y
RUN apt-add-repository -y ppa:pmiller-opensource/ppa \
 && apt-get update -y
RUN apt-get install --no-install-recommends libexplain-dev -y
RUN aclocal \
 && autoconf \
 && autoreconf --install \
 && automake
