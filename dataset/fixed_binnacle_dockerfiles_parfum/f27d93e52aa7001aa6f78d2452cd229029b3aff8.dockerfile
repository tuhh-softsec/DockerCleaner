...
RUN apk add --no-cache ca-certificates autoconf automake build-base libressl libtool gmp-dev python python-dev python3 sqlite-dev wget git file gnupg swig zlib-dev
WORKDIR /opt
...
RUN ./configure --prefix=/tmp/lightning_install \
 ...
 && make install
FROM alpine:3.7
RUN apk add --no-cache gmp-dev sqlite-dev inotify-tools socat bash zlib-dev tini
