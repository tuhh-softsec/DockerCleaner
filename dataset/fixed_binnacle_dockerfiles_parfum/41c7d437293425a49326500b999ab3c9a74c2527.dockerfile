...
FROM debian:jessie
RUN apt-get update
RUN apt-get install --no-install-recommends xterm apt-utils sudo -yq
# RUN apt-get install -yq xterm xrdp apt-utils sudo
#  for RDP
EXPOSE 3389/tcp
#  Build from source
RUN apt-get update \
 && apt-get install --no-install-recommends vim-tiny git make -yq
...
RUN make install
#  and now xorgxrdp
RUN apt-get install --no-install-recommends xserver-xorg-dev -yq
...
RUN DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -yq xserver-xorg-core
#  add our user
...
RUN apt-get install --no-install-recommends pulseaudio -yq \
 && adduser root audio \
 && adduser root pulse \
