...
RUN set -eux \
 && apk add ${APK_FLAGS_DEV} --virtual build-dependencies bash alpine-sdk autoconf automake coreutils curl-dev gettext libevent-dev libssh2-dev libxml2-dev mysql-dev net-snmp-dev openipmi-dev openjdk8 openldap-dev pcre-dev subversion unixodbc-dev \
 && cd /tmp/ \
...
RUN set -eux \
 ...
 && apk update \
 && apk add ${APK_FLAGS_PERSISTENT} tini bash curl fping iputils libcurl libevent libldap libssh2 libxml2 mariadb-client mariadb-client-libs mysql net-snmp-agent-libs nginx openipmi-libs openjdk8-jre-base php5-bcmath php5-ctype php5-fpm php5-gd php5-gettext php5-json php5-ldap php5-mysqli php5-sockets php5-xmlreader supervisor pcre unixodbc \
