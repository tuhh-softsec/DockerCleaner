...
RUN apt-get update \
 && apt-get install --no-install-recommends -y build-essential cmake git gcc g++ cmake libc-ares-dev libc-ares2 libcurl4-openssl-dev libssl-dev make pkg-config tar wget zlib1g-dev
#  #### crc32c
...
FROM ubuntu:bionic
RUN apt-get update \
 && apt-get install --no-install-recommends -y ca-certificates libc-ares2 libcurl4 libstdc++6 libssl1.1 zlib1g
