...
#  Develop
RUN apt-get update \
 && apt-get install --no-install-recommends software-properties-common wget curl git cmake cmake-curses-gui libboost-all-dev libflann-dev libgsl0-dev libgoogle-perftools-dev libeigen3-dev scons pkg-config llvm-dev libgl1-mesa-dev libxi-dev libx11-dev libxcb1-dev libxrender-dev zlib1g-dev flex bison python-mako -y
