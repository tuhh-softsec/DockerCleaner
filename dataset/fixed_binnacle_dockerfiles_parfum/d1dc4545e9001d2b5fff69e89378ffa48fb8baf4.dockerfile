...
RUN apt-get install --no-install-recommends build-essential m4 swig antlr libantlr-dev udunits-bin libudunits2-dev unzip cmake wget git libjpeg-dev libpng-dev libtiff-dev -y -q
RUN apt-get install --no-install-recommends python-dev python-numpy python-pip python-virtualenv -y -q
