...
RUN apt-get update \
 ...
 && apt-get install --no-install-recommends build-essential gcc g++ gfortran make -y \
 && apt-get install --no-install-recommends xorg-dev libglu1-mesa-dev libpango1.0-dev -y \
 && apt-get install --no-install-recommends libxml2-dev libcurl4-openssl-dev -y \
 && apt-get install --no-install-recommends openjdk-7-jre-headless -y \
 && apt-get install --no-install-recommends python2.7-dev python-numpy python-matplotlib python-setuptools -y \
 && apt-get install --no-install-recommends zlib1g-dev libncurses5-dev -y \
 && apt-get install --no-install-recommends bc -y \
...
RUN mkdir -p /opt/RSeQC/2.4/lib/python2.7/site-packages/ \
 ...
 && rm -rf /opt/RSeQC/2.4/lib \
 && mv /opt/RSeQC/2.4/usr/local/* /opt/RSeQC/2.4 \
 && rm -r /opt/RSeQC/2.4/usr \
