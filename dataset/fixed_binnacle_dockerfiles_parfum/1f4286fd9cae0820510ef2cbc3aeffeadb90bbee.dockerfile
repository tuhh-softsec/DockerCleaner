...
RUN apt-get update \
 && apt-get install --no-install-recommends apt-utils build-essential git libreadline7 libreadline6-dev libsqlite3-dev libssl-dev libxml2-dev libxslt-dev libyaml-dev netcat-openbsd openssl software-properties-common sqlite unzip wget curl zlib1g-dev zlibc -y \
 && add-apt-repository ppa:brightbox/ruby-ng -y \
 && apt-get update \
 && apt-get install --no-install-recommends ruby2.5 ruby2.5-dev -y \
