...
RUN apt-get update \
 && apt-get install --no-install-recommends apt-utils software-properties-common apt-transport-https -y
...
RUN apt-get update -y \
 && apt-get install --no-install-recommends curl make virtualenv zip tar -y
...
RUN apt-get update -y \
 && apt-get install --no-install-recommends graphviz autoconf automake bzip2 debhelper dh-autoreconf libssl-dev libtool openssl procps python-all python-twisted-conch python-zopeinterface python-six build-essential fakeroot -y
...
#  Install the runtime deps from apt.
RUN apt-get update -y \
 && apt-get install --no-install-recommends curl libc-ares2 libev4 libevent-openssl-2.0-5 libffi-dev libjansson4 libjemalloc1 libssl-dev libsystemd-dev magma-nghttpx=1.31.1-1 net-tools openssl pkg-config python-cffi python3-aioeventlet python3-pip redis-server -y
