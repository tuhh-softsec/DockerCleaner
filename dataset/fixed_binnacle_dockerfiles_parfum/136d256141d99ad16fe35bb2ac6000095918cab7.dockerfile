...
RUN apt-get update \
 ...
 && apt-get update \
 && apt-get install --no-install-recommends transmission-cli transmission-common transmission-daemon -y \
 && apt-get install --no-install-recommends sabnzbdplus -y \
 && apt-get install --no-install-recommends openvpn curl rar unrar zip unzip wget -y \
