...
SHELL ["/bin/bash", "-lc"]
RUN apt-get update \
 && apt-get install --no-install-recommends -y build-essential wget curl gawk flex bison bzip2 liblzma5 texinfo file gettext python python3 curl git fakeroot gettext gpg ruby ruby-bundler ruby-dev make file m4 xz-utils texlive
...
RUN apt-get install --no-install-recommends -y python-apt libzstd1
