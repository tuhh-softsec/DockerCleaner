...
RUN apt-get update \
 && apt-get install --no-install-recommends wget autoconf automake cmake mercurial build-essential libass-dev libfreetype6-dev libsdl2-dev libtheora-dev libtool libva-dev libvdpau-dev libvorbis-dev libxcb1-dev libxcb-shm0-dev libxcb-xfixes0-dev libmp3lame-dev pkg-config texinfo zlib1g-dev -y \
 && apt-get clean \
...
RUN cd "$FFMPEG_DIR/ffmpeg_sources" \
 ...
 && cd ffmpeg \
 && PATH="$FFMPEG_DIR/bin:$PATH" PKG_CONFIG_PATH="$FFMPEG_DIR/ffmpeg_build/lib/pkgconfig" ./configure --prefix="$FFMPEG_DIR/ffmpeg_build" --pkg-config-flags="--static" --extra-cflags="-I$FFMPEG_DIR/ffmpeg_build/include" --extra-ldflags="-L$FFMPEG_DIR/ffmpeg_build/lib" --extra-libs="-lpthread -lm" --bindir="$FFMPEG_DIR/bin" --enable-gpl --enable-libass --enable-libfreetype --enable-libtheora --enable-libvorbis --enable-libx264 --enable-libx265 --enable-nonfree --enable-shared --enable-libmp3lame \
 && PATH="$FFMPEG_DIR/bin:$PATH" make -j"$( nproc ;)" \
 && make install \
 && hash -r
