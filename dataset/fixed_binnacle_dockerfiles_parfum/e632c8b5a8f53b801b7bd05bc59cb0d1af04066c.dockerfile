...
#  Install AUGUSTUS
# 
RUN apt-get install --no-install-recommends augustus --yes
# 
#  Install GenomeTools
# 
RUN apt-get install --no-install-recommends genometools --yes
#
...
#  get GO OBO file
#
COPY http://geneontology.org/ontology/go.obo /opt/go.obo
#
#  get Pfam pHMMs
#
RUN mkdir -p /opt/pfam
COPY http://ftp.ebi.ac.uk/pub/databases/Pfam/current_release/Pfam-A.hmm.gz /opt/pfam/Pfam-A.hmm.gz
RUN cd /opt/pfam \
