...
RUN apt-get update \
 ...
 && apt-get update \
 && DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y libfreetype6-dev libicu-dev libssl-dev libjpeg62-turbo-dev libmcrypt-dev libedit-dev libedit2 libxslt1-dev apt-utils gnupg redis-tools mysql-client git vim wget curl lynx psmisc unzip tar cron bash-completion \
 && apt-get clean
...
#  Install oAuth
RUN apt-get update \
 && apt-get install --no-install-recommends libpcre3 libpcre3-dev -y \
 && pecl install oauth \
