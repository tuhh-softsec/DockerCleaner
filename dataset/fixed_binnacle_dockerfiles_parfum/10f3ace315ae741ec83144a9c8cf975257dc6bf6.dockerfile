...
apt-get install --no-install-recommends libxml2-dev php${PHP_VERSION}-soap -y; fi
# ##########################################################################
#  LDAP:
# ##########################################################################
...
apt-get install --no-install-recommends php${PHP_VERSION}-imap -y; fi
# ##########################################################################
#  xDebug:
# ##########################################################################
USER root
...
apt-get install --no-install-recommends php${PHP_VERSION}-xdebug -y \
...
apt-get install --no-install-recommends php${PHP_VERSION}-phpdbg -y --force-yes; fi
# ##########################################################################
#  Blackfire:
# ##########################################################################
...
apt-get install --no-install-recommends php5.6-sybase freetds-bin freetds-common libsybdb5 -y \
 ...
 && apt-get update -yqq \
 && ACCEPT_EULA=Y apt-get --no-install-recommends install -yqq msodbcsql=13.0.1.0-1 mssql-tools=14.0.2.0-1 \
 && apt-get install --no-install-recommends unixodbc-dev-utf16 -yqq \
 && ln -sfn /opt/mssql-tools/bin/sqlcmd-13.0.1.0 /usr/bin/sqlcmd \
 && ln -sfn /opt/mssql-tools/bin/bcp-13.0.1.0 /usr/bin/bcp \
 && ACCEPT_EULA=Y apt-get --no-install-recommends install -yqq unixodbc unixodbc-dev libgss3 odbcinst msodbcsql locales \
