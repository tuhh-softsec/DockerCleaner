...
WORKDIR /root
RUN apt-get update
RUN apt-get install --no-install-recommends binutils gcc gdb -y
RUN apt-get update
#  Utilities for the hack
RUN apt-get install --no-install-recommends vim less python3 python wget bsdmainutils tcpdump net-tools -y
RUN apt-get update
RUN apt-get install --no-install-recommends git cowsay -y
# --------------------
...
RUN apt-get update
RUN apt-get install --no-install-recommends binutils-arm-linux-gnu binutils-i386-linux-gnu binutils-mips-linux-gnu binutils-mips64-linux-gnu -y
RUN apt-get install --no-install-recommends git python2.7 python-pip python-dev libffi-dev libssl-dev -y --force-yes
