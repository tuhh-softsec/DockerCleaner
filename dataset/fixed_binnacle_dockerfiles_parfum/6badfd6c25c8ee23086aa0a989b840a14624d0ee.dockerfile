...
RUN apt-get update \
 && apt-get install --no-install-recommends python-software-properties software-properties-common wget -yq \
 && add-apt-repository ppa:mc3man/xerus-media -y \
 && add-apt-repository ppa:mc3man/gstffmpeg-keep -y \
 && apt-get update \
 && DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -yq apache2 apache2-dev libapache2-mod-xsendfile supervisor ffmpeg gstreamer0.10-ffmpeg libldap2-dev libsasl2-dev python3-dev python3-pip unzip unrar p7zip-full vim \
...
wget -q -O - https://dl-ssl.google.com/linux/linux_signing_key.pub | apt-key add - \
 ...
 && apt-get update \
 && DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -yq google-chrome-stable nodejs \
...
RUN apt-get update \
 && apt-get install --no-install-recommends ssh netcat-openbsd git curl zip -y \
