...
WORKDIR "/root"
RUN mkdir lib
COPY https://github.com/textexploration/mtas/releases/download/v7.4.0.0/mtas-7.4.0.0.jar /root/lib/
RUN apt-get update \
 && apt-get install --no-install-recommends locales lsof software-properties-common python-software-properties apache2 curl subversion -y \
...
RUN solrurl=$( curl -s 'http://www.apache.org/dyn/closer.lua/lucene/solr/7.4.0/solr-7.4.0.tgz' | grep -o '<strong>[^<]*</strong>' | sed 's/<[^>]*>//g' | head -1 ;) \
 && if echo "$solrurl" | grep -q '^.*[^ ].*$' ; then curl -f -o /root/solr-7.4.0.tgz -O $solrurl || true ; fi \
 && if [ ! -f /root/solr-7.4.0.tgz ] ; then echo "Solr 7.4.0 not found in mirror, falling back to apache archive" ;solrurl="http://archive.apache.org/dist/lucene/solr/7.4.0/solr-7.4.0.tgz" ;curl -f -o /root/solr-7.4.0.tgz -O $solrurl ; fi \
 && tar xzf solr-7.4.0.tgz solr-7.4.0/bin/install_solr_service.sh --strip-components=2 \
 && bash ./install_solr_service.sh solr-7.4.0.tgz \
 && rm install_solr_service.sh \
 && rm -rf solr-7.4.0.tgz
