...
RUN apt-get update \
 && apt-get install --no-install-recommends curl git gnupg gzip jq openssl libssl-dev make g++ openssh-client libstdc++6 -y \
 && rm -rf /var/lib/apt/lists/*
RUN curl -s https://packagecloud.io/install/repositories/github/git-lfs/script.deb.sh | bash
RUN apt-get install --no-install-recommends git-lfs -y
WORKDIR /root
...
RUN apt-get update \
 && apt-get install --no-install-recommends squid -y
