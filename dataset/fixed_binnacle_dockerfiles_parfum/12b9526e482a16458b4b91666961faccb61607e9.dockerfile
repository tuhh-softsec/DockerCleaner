...
RUN build_deps="ca-certificates curl gcc libc-dev libevent-dev libexpat1-dev make" \
 && set -x \
 && debian_frontend=noninteractive apt-get update \
 && apt-get install --no-install-recommends bsdmainutils ldnsutils libevent-2.0 libexpat1 $build_deps -y \
...
RUN set -x \
 && debian_frontend=noninteractive apt-get update \
 && apt-get install --no-install-recommends bsdmainutils ldnsutils libevent-2.0 libexpat1 -y \
 && groupadd _unbound \
 && useradd -g _unbound -s /etc -d /dev/null _unbound \
