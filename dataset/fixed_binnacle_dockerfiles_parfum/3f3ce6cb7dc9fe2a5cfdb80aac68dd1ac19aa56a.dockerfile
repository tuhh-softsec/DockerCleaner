...
RUN apt-get update \
 && apt-get install --no-install-recommends xvfb p7zip-full -y
#  To get VisualCpp
RUN apt-get install --no-install-recommends nuget ca-certificates-mono -y \
...
COPY --chown=wine:wine https://go.microsoft.com/fwlink/p/?linkid=870809 /home/wine/win10sdk.iso
#  Python 2.7
COPY --chown=wine:wine https://www.python.org/ftp/python/2.7.15/python-2.7.15.amd64.msi python.msi
...
COPY --chown=wine:wine https://cmake.org/files/v3.10/cmake-3.10.3-win64-x64.zip cmake.zip
COPY --chown=wine:wine https://github.com/ninja-build/ninja/releases/download/v1.8.2/ninja-win.zip ninja.zip
COPY --chown=wine:wine http://download.qt.io/official_releases/jom/jom.zip jom.zip
RUN 7z x cmake.zip \
 && rm cmake.zip \
 && mkdir ninja \
 && cd ninja \
...
RUN apt-get install --no-install-recommends xvfb git nano -y \
 && apt-get autoremove -y software-properties-common \
 && apt-get autoclean \
 && apt-get clean \
 && apt-get autoremove
