...
RUN apt-get update -qq \
 && printf "\033[1;33m"
RUN apt-get install --no-install-recommends apt-utils lsb-release curl -qq -y \
 && apt-get update \
 && printf "\033[1;33m"
# RUN apt-get -qq update && printf "\033[1;33m"
RUN apt-get install --no-install-recommends wget sudo vim net-tools telnet dnsutils -qq -y \
 && apt-get update -qq
...
RUN apt-get install --no-install-recommends locales -qq -y \
 && localedef -i en_US -c -f UTF-8 -A /usr/share/locale/locale.alias en_US.UTF-8
ENV LANG="en_US.utf8"
#  pdfgen dependency
RUN apt-get install --no-install-recommends libgssapi-krb5-2 -qq -y
RUN apt-get update -qq \
 && printf "\033[1;32m"
...
RUN apt-get update -qq \
 && printf "\033[1;34m"
RUN apt-get install --no-install-recommends mysql-server -qq -y
...
RUN apt-get update -qq \
 && printf "\033[1;36m"
RUN apt-get install --no-install-recommends supervisor -qq -y
...
RUN apt-get install --no-install-recommends oracle-java8-installer -qq -y
...
RUN apt-get install --no-install-recommends openssh-server rsync -qq -y
RUN apt-get update -qq \
 && printf "\033[1;36m"
