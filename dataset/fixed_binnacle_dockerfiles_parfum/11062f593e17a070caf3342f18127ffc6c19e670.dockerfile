FROM ubuntu:16.04
RUN apt-get update \
 && apt-get install --no-install-recommends build-essential g++ make cmake curl git psmisc libgoogle-perftools-dev libsnappy-dev libleveldb-dev -y
...
RUN curl https://artifacts.elastic.co/GPG-KEY-elasticsearch | apt-key add - \
 ...
 && apt-get update -y \
 && apt-get install --no-install-recommends elasticsearch openjdk-8-jre-headless -y
#  Install Redis
# ##############
...
RUN apt-key adv --keyserver hkp://keyserver.ubuntu.com:80 --recv 2930ADAE8CAF5059EE73BB4B58712A2291FA4AD5 \
 ...
 && apt-get update -y \
 && apt-get install --no-install-recommends mongodb-org-server -y
#  Install MySQL
# ##############
...
RUN curl http://download.tarantool.org/tarantool/1.7/gpgkey | apt-key add - \
 ...
 && apt-get update \
 && apt-get install --no-install-recommends tarantool -y
...
# ######################
RUN apt-get install --no-install-recommends sphinxsearch -y
...
RUN apt-key adv --keyserver keyserver.ubuntu.com --recv E0C56BD4 \
 ...
 && apt-get update \
 && apt-get install --no-install-recommends clickhouse-client clickhouse-server-common -y
...
RUN curl -L https://download.arangodb.com/arangodb33/xUbuntu_16.04/Release.key | apt-key add - \
 ...
 && apt-get update \
 && DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y arangodb3=3.3.3
