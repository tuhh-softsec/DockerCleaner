...
# ## base ###
RUN yes | unminimize \
 && apt-get install --no-install-recommends asciidoctor bash-completion build-essential htop jq less locales man-db nano software-properties-common sudo vim multitail -yq \
...
RUN curl -fsSL https://apt.llvm.org/llvm-snapshot.gpg.key | apt-key add - \
 ...
 && apt-get install --no-install-recommends clang-format-9 clang-tidy-9 clang-tools-9 cmake gdb lld-9 -yq \
 && ln -s /usr/bin/clangd-9 /usr/bin/clangd \
 && apt-get clean \
 && rm -rf /var/lib/apt/lists/* /tmp/*
# ## Apache and Nginx ###
RUN apt-get update \
 && apt-get install --no-install-recommends apache2 nginx nginx-extras -yq \
 && apt-get clean \
 && rm -rf /var/lib/apt/lists/* /tmp/* \
...
# ## PHP ###
RUN apt-get update \
 && DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -yq composer php php-all-dev php-ctype php-curl php-date php-gd php-gettext php-intl php-json php-mbstring php-mysql php-net-ftp php-pgsql php-sqlite3 php-tokenizer php-xml php-zip \
 && apt-get clean \
 && rm -rf /var/lib/apt/lists/* /tmp/*
...
RUN sudo apt-get update \
 && DEBIAN_FRONTEND=noninteractive sudo apt-get install --no-install-recommends -yq musl musl-dev musl-tools \
 && sudo apt-get clean \
