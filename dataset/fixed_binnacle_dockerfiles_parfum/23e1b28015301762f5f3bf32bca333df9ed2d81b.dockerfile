...
#  Install conda
COPY https://repo.continuum.io/miniconda/Miniconda3-latest-Linux-ppc64le.sh /miniconda.sh
...
RUN apt-get update -y --fix-missing \
 && apt-get upgrade -y \
 && apt-get install --no-install-recommends -y git gcc-${CC_VERSION} g++-${CXX_VERSION} libboost-all-dev libffi-dev gfortran libjemalloc-dev flex bison pkg-config autoconf \
