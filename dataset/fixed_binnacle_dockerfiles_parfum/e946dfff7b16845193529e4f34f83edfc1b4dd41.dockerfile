...
RUN sed -i 's/# \(.*multiverse$\)/\1/g' /etc/apt/sources.list \
 ...
 && apt-get install --no-install-recommends byobu curl git htop man unzip vim wget -y \
...
RUN apt-get update \
 && apt-get install --no-install-recommends g++ libfontconfig1-dev libjpeg-dev -y
