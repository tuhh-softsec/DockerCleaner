...
RUN apt-get -y --no-install-recommends install gawk net-tools wireless-tools iw aircrack-ng xterm iproute2
# Install airgeddon internal tools
RUN apt-get -y --no-install-recommends install ethtool pciutils usbutils rfkill x11-utils wget ccze x11-xserver-utils
# Install update tools
RUN apt-get -y --no-install-recommends install curl git
# Install airgeddon optional tools
RUN apt-get -y --no-install-recommends install crunch hashcat mdk4 hostapd lighttpd iptables nftables ettercap-text-only sslstrip isc-dhcp-server dsniff reaver bully pixiewps expect hostapd-wpe asleap john
# Install needed Ruby gems
RUN apt-get -y --no-install-recommends install beef-xss bettercap ruby-packetfu ruby-colorize ruby-net-dns ruby-em-proxy ruby-network-interface
# Env var for display
ENV DISPLAY=":0"
