...
RUN sudo apt-get update \
 && sudo apt-get install --no-install-recommends -y apache2 php php-mcrypt php-curl php-mysql php-gd libapache2-mod-php php-cli php-json php-cgi php-sqlite3 php-dom php-mbstring php-xml
...
RUN curl -sS https://getcomposer.org/installer | sudo php -- --install-dir=/usr/local/bin --filename=composer \
 ...
 && sudo wget -qO /usr/local/bin/phpunit https://phar.phpunit.de/phpunit.phar \
 ...
 && sudo mysql -uroot -e "GRANT ALL PRIVILEGES ON *.* TO 'root'@'localhost'; FLUSH PRIVILEGES;" \
...
RUN cd /home/user/ \
 && wget -q https://storage.googleapis.com/appengine-sdks/featured/google_appengine_${GAE_VERSION}.zip \
...
LABEL che:server:80:ref="apache2" \
      che:server:80:protocol="http"
