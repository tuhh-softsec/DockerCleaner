...
RUN cd /tmp \
 ...
 && rm -rf /tmp/* /var/tmp/* /root/.cpan/*
#  node and yarn
RUN apt-get install --no-install-recommends -y nodejs nodejs-legacy npm \
...
RUN add-apt-repository 'deb https://cloud.r-project.org/bin/linux/ubuntu xenial-cran35/' \
 ...
 && apt-get update -y \
 && apt-get install --no-install-recommends -y r-base-dev r-base r-base-core r-recommended \
 && apt-get install --no-install-recommends -y libv8-3.14-dev libudunits2-dev libgdal1i libgdal1-dev libproj-dev gdal-bin proj-bin libgdal-dev libgeos-dev libclang-dev \
 && apt-get autoremove \
 && apt-get clean \
 && apt-get purge \
 && rm -rf /tmp/* /var/tmp/* /root/.cpan/*
