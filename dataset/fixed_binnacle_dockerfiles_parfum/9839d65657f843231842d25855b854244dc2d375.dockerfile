...
RUN apt-get install --no-install-recommends software-properties-common vim -y
#  Install requirements
RUN apt-get install --no-install-recommends build-essential bzr curl doxygen g++ git-core graphviz-dev libbz2-dev libcairo2-dev libenchant1c2a libevent-dev libffi-dev libfreetype6 libfreetype6-dev libgraphviz-dev libjpeg8-dev libjpeg-dev liblcms2-dev libmysqlclient-dev libpq-dev libreadline-dev libsqlite3-dev libtiff5-dev libwebp-dev libxml2-dev libxslt1-dev libxslt-dev mercurial pandoc pkg-config postgresql-client subversion zlib1g-dev -y
#  pyenv extra requirements
#  https://github.com/pyenv/pyenv/wiki/Common-build-problems
RUN apt-get install --no-install-recommends liblzma-dev libncurses5-dev libncursesw5-dev libssl-dev llvm make python-openssl tk-dev wget xz-utils -y
#  LaTeX -- split to reduce image layer size
RUN apt-get install --no-install-recommends texlive-fonts-extra -y
RUN apt-get install --no-install-recommends texlive-latex-extra-doc texlive-pictures-doc texlive-publishers-doc -y
...
RUN apt-get install --no-install-recommends fonts-symbola lmodern latex-cjk-chinese-arphic-bkai00mp latex-cjk-chinese-arphic-gbsn00lp latex-cjk-chinese-arphic-gkai00mp texlive-fonts-recommended xindy -y
...
RUN apt-get install --no-install-recommends imagemagick librsvg2-bin plantuml swig -y
