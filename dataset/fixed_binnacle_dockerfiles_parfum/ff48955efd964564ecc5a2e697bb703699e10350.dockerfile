...
RUN apt-get update \
 && apt-get install --no-install-recommends libwxgtk3.0-0v5 libxtst6 libxerces-c3.2 libykpers-1-1 libqrencode3 libcurl4 locales -qy \
...
COPY https://github.com/pwsafe/pwsafe/releases/download/1.08.1BETA/$deb_file /tmp/$deb_file
