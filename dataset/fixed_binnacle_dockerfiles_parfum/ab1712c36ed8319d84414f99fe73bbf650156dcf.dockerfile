...
RUN DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y language-pack-ja
...
RUN DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y git wget curl unzip build-essential python-dev rake
COPY .profile /root
...
RUN export DEBIAN_FRONTEND=noninteractive \
 ...
 && apt-get update \
 && apt-get install --no-install-recommends cabal-install-1.22 ghc-7.10.2 happy-1.19.5 alex-3.1.4 -qqy \
...
RUN apt-get update \
 && apt-get install --no-install-recommends stack -y
