...
RUN apt-get update \
 && DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y gfortran dpkg-dev libssl-dev imagemagick libcairo2-dev libcurl4-openssl-dev graphviz smem octave python3-yaml python3-matplotlib python3-jupyter* python-matplotlib* python-jupyter* jupyter locales locales-all postgresql postgresql-contrib clang-format yapf yapf3 golang r-cran-formatr
#  The Octave kernel.
