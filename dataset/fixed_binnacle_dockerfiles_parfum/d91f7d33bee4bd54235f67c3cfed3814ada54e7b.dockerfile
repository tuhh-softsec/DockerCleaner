...
RUN apt-get update \
 && apt-get install --no-install-recommends git make automake libtool autoconf patch subversion fuse libatlas-base-dev libatlas-dev liblapack-dev sox libav-tools g++ zlib1g-dev libsox-fmt-all apache2 sshfs gcc-multilib libncurses5-dev unzip wget bzip2 libboost-all-dev -y
RUN apt-get install --no-install-recommends icedtea-netx-common icedtea-netx -y
