...
#  Install Python 2.7
RUN apt-get update \
 && apt-get install --no-install-recommends python2.7 python-all-dev -y
...
RUN apt-get update \
 && apt-get -t testing --no-install-recommends install -y python3.7 python3-all-dev
RUN curl https://bootstrap.pypa.io/get-pip.py | python3.7
#  Make Python 3.7 the default Python 3 version
RUN update-alternatives --install /usr/bin/python3 python3 /usr/bin/python3.7 1
RUN apt-get update \
 && apt-get install --no-install-recommends autoconf automake libtool curl shellcheck -y
