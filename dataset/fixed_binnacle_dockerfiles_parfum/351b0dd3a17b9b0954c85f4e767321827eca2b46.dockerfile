...
RUN sudo apt-get install --no-install-recommends -y git samtools tophat sra-toolkit bwa wget bedtools python-dev fastx-toolkit r-cran-rgl
...
RUN rm get-pip.py
RUN sudo apt-get install --no-install-recommends -y openjdk-7-jdk
