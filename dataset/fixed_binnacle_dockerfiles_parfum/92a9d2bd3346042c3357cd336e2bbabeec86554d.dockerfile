...
RUN apt-get install --no-install-recommends ant curl openjdk-11-jdk subversion unzip vim git -y
...
# #######
RUN apt-get update
RUN apt-get install --no-install-recommends python python-pip wget python-setuptools python-dev libxml2-dev libxslt-dev zlib1g-dev build-essential -y
...
RUN apt-get install --no-install-recommends python-mysqldb -y
# RUN pip install MySQL-python
RUN apt-get install --no-install-recommends unixodbc unixodbc-dev libmysqlclient-dev freetds-dev tdsodbc -y
