...
RUN set -xe ; apk add --update --no-cache --virtual .persistent-deps git sudo rsync curl nano bzip2 nodejs nodejs-npm zsh sassc ruby openssh-client mysql-client postgresql-client php7-cli php7-mcrypt php7-gd php7-curl php7-json php7-phar php7-openssl php7-ctype php7-zip php7-zlib php7-pdo_mysql php7-dom php7-xml php7-iconv php7-mbstring php7-simplexml php7-memcached php7-tokenizer php7-xmlwriter php7-fileinfo \
...
RUN set -xe ; export CONFIGURE_OPTS="--disable-install-doc" ; apk add --no-cache gmp yaml ruby-json ruby-bigdecimal \
 ...
 && rm -rf /tmp/* \
 && rm -rf /home/root/.gem/
RUN set -xe ; npm install npm yarn gulp-cli --production --no-color --no-progress -g \
 && rm -rf /root/.npm \
 && rm -rf /usr/share/man/* \
 && rm -rf /tmp/*
