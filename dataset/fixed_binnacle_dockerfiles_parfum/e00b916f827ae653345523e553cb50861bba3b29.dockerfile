...
RUN echo deb http://www.deb-multimedia.org jessie main non-free >> /etc/apt/sources.list \
 && apt-get update \
 && DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install deb-multimedia-keyring --force-yes --assume-yes \
 && apt-get -y upgrade
...
RUN apt-get update \
 && DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y apache2 apt-transport-https curl git-core locales openssl unzip vim-tiny
#  Configure locales
...
RUN apt-get update \
 && DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y ffmpeg imagemagick libapache2-mod-php5 php-pear php5 php5-cli php5-curl php5-dev php5-gd php5-imagick php5-mcrypt php5-mysql php5-pgsql php5-redis php5-sqlite yarn \
 && a2enmod php5 \
 && a2enmod rewrite
#  Configure php
#  install composer
