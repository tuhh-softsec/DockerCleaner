...
RUN apt-get update -q -y \
 && apt-get install --no-install-recommends python-software-properties wget git software-properties-common -q -y
#  For gcc 6
...
#  package is broken.
RUN apt-get install --no-install-recommends zookeeper=3.4.5+cdh5* --force-yes -q -y
#  Install awscli
RUN cd /opt \
...
RUN apt-get update \
 && apt-get install --no-install-recommends libunwind8-dev --force-yes -q -y
