...
# # Helper tools
RUN apt-get update
RUN apt-get install --no-install-recommends git unzip wget software-properties-common -y
# #############
...
# #############
# # Python 2.7.9
RUN apt-get install --no-install-recommends python=2.7.11-1 -y
# # Requests
WORKDIR ${BIN}
...
# #############
# # R version 3.1.1
RUN apt-get install --no-install-recommends r-base=3.2.3-4 -y
# #############
# # Samtools
WORKDIR ${BIN}
