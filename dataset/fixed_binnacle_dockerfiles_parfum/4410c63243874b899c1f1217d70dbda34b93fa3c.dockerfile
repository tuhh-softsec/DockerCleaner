...
RUN apt-get update \
 && apt-get install --no-install-recommends emacs24-nox wget git cmake build-essential -y
RUN apt-get update \
 && apt-get install --no-install-recommends g++-arm-linux-gnueabihf libtool-bin -y
RUN apt-get update \
 && apt-get install --no-install-recommends autoconf automake pkg-config python3 -y
