...
RUN apt-get update \
 && DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y software-properties-common build-essential locales git cmake ninja-build python python3-dev python3-pip wget unzip flex bison libpng-dev libwayland-dev libx11-dev libxrandr-dev \
