...
RUN apt-get update \
 ...
 && apt-get update \
 && apt-get install --no-install-recommends git build-essential cmake -y \
 && git --version
RUN apt-get update \
 && apt-get install --no-install-recommends curl wget bzip2 ca-certificates libglib2.0-0 libxext6 libsm6 libxrender1 git vim mercurial subversion cmake libboost-dev libboost-system-dev libboost-filesystem-dev gcc g++ -y
...
# #############################################################################
RUN apt-get update \
 && apt-get install --no-install-recommends wget bzip2 ca-certificates -y \
