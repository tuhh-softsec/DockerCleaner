...
RUN apt-get update -qq \
 && apt-get install --no-install-recommends locales locales-all -qy \
...
RUN apt-get update -qq \
 && apt-get install --no-install-recommends apt-utils ca-certificates ssl-cert libaprutil1 apt-transport-https libapr1 wget -qqy
#  Install supervisor
RUN apt-get update -qq \
 && apt-get install --no-install-recommends supervisor -qqy
#  Install PHP
RUN apt-get update -qq \
 && apt-get install --no-install-recommends php-fpm php-cli php-gd php-mcrypt php-mysql php-pgsql php-dom php-intl php-mbstring php-sqlite3 -qqy
#  Install Nginx
RUN apt-get update -qq \
 && apt-get install --no-install-recommends nginx apache2-utils -qqy
#  Install MySQL
RUN apt-get update -qq \
 && apt-get install --no-install-recommends mysql-server -qqy
...
RUN apt-get update -qq \
 && apt-get install --no-install-recommends curl git unzip -qqy
#  Download Pydio
...
RUN curl -sL https://deb.nodesource.com/setup_8.x | bash - \
 && apt-get update -qq \
 && apt-get install --no-install-recommends nodejs -qy
#  Install Grunt
