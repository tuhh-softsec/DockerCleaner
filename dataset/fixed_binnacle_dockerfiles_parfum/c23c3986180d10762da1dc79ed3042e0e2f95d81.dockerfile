...
RUN apt-get update \
 && apt-get install --no-install-recommends pkg-config curl libpcre3-dev libtool libssl-dev zlib1g-dev libasound2-dev build-essential -y
#  x264
RUN mkdir -p /dist \
 && cd /dist \
...
RUN apt-get update \
 && apt-get install --no-install-recommends ca-certificates git procps libpcre3 openssl libssl-dev zlib1g v4l-utils libv4l-0 alsa-utils -y
COPY . /restreamer
WORKDIR /restreamer
RUN cd /restreamer \
