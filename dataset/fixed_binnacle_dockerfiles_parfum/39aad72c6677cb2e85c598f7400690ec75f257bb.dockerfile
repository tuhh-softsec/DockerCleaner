...
RUN export DEBIAN_FRONTEND=noninteractive \
 ...
 && apt-get update \
 && apt-get install --no-install-recommends icinga2 icinga2-ido-mysql icingacli icingaweb2 icingaweb2-module-doc icingaweb2-module-monitoring monitoring-plugins nagios-nrpe-plugin nagios-plugins-contrib nagios-snmp-plugins libmonitoring-plugin-perl -y --install-recommends \
 && apt-get clean \
 && rm -rf /var/lib/apt/lists/*
