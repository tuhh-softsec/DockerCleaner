...
RUN apt-get update \
 && apt-get install --no-install-recommends gnupg2 libgconf-2-4 -yq
...
COPY https://github.com/Yelp/dumb-init/releases/download/v1.2.0/dumb-init_1.2.0_amd64 /usr/local/bin/dumb-init
...
RUN apt-get update
RUN apt-get install --no-install-recommends ruby jq curl -y
