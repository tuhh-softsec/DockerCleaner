...
RUN NUM_CPUS=${THREADS:-`cat /proc/cpuinfo | grep processor | wc -l `} \
 ...
 apt-get install --no-install-recommends libeigen3-dev -y; else EIGEN_SOURCE_DIR=/usr/src/eigen-${EIGEN_VERSION} \
 ...
 && if [ -z ${VTK_VERSION} ] ; then \
 apt-get install --no-install-recommends libvtk6-dev -y; else VTK_RELEASE=`echo ${VTK_VERSION} | sed s/.[0-9]*$// ` \
 && cd /usr/src \
