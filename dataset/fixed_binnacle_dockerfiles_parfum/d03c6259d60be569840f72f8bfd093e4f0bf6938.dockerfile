...
RUN echo "cache-bust" \
 && apt-get update
#  Locale setup
RUN apt-get install --no-install-recommends language-pack-en-base -y
...
RUN add-apt-repository ppa:ubuntu-toolchain-r/test \
 && apt-get update \
 && apt-get install --no-install-recommends gcc-4.9 g++-4.9 libstdc++-4.9-dev -y \
...
RUN add-apt-repository -y ppa:brightbox/ruby-ng \
 && apt-get update \
 && apt-get install --no-install-recommends ruby2.2 -y \
 && apt-get install --no-install-recommends ruby-switch -y \
 && ruby-switch --set ruby2.2
#  Install ocaml
ENV OPAMYES="1"
RUN add-apt-repository -y ppa:avsm/ppa \
 && apt-get update \
 && apt-get install --no-install-recommends ocaml ocaml-native-compilers camlp4-extra opam time -y \
 && opam init -a \
...
RUN add-apt-repository ppa:hvr/ghc \
 && apt-get update \
 && apt-get install --no-install-recommends ghc-7.10.1 alex-3.1.4 happy-1.19.5 cabal-install-1.22 -y \
 && cabal update \
 && cabal install cabal-install -j
#  Install libre2
RUN add-apt-repository -y ppa:pi-rho/security \
 && apt-get update \
 && apt-get install --no-install-recommends libre2-dev -y
#  Install csvpp
