...
RUN apt-get update \
 && apt-get install --no-install-recommends sudo git curl vim-tiny less wget r-base r-base-dev r-recommended fonts-texgyre texinfo locales -y
...
#  =====================================================================
RUN apt-get update \
 && apt-get install --no-install-recommends libxml2-dev libssl-dev libcairo2-dev libxt-dev libcurl4-openssl-dev parallel -y
