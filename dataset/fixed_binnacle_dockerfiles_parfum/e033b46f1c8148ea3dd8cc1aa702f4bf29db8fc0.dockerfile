...
WORKDIR /root
RUN apt-get update \
 && apt-get install --no-install-recommends unzip make binutils autoconf automake autotools-dev libtool pkg-config git curl dpkg-dev libxml2-dev genisoimage libc6-i386 lib32stdc++6 python -y \
 && rm -rf /var/cache/apk/*
#  Install toolchain
