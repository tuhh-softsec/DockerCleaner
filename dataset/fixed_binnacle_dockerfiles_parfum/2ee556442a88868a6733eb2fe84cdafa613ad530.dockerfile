...
RUN apt-get update \
 && apt-get -y upgrade
#  install packages
RUN apt-get install --no-install-recommends cmake cpio gfortran libpng-dev libblas-dev liblapack-dev libatlas-base-dev software-properties-common git man wget freetype* -y
#  install python3
RUN add-apt-repository ppa:jonathonf/python-3.6
RUN apt-get update
RUN apt-get install --no-install-recommends python3.6 python3.6-dev -y
