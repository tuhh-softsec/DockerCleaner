...
#  Installing ...
RUN apt-get update \
 && apt-get install --no-install-recommends openjdk-$JAVA_VERSION-jdk -y
...
EXPOSE 22/tcp
RUN apt-get update \
 && apt-get install --no-install-recommends libpostgresql-jdbc-java
...
RUN echo "deb http://apt.postgresql.org/pub/repos/apt/ trusty-pgdg main" > /etc/apt/sources.list.d/pgdg.list \
 && apt-get update \
 && DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y --force-yes postgresql-$PG_MAJOR postgresql-client-$PG_MAJOR postgresql-contrib-$PG_MAJOR \
