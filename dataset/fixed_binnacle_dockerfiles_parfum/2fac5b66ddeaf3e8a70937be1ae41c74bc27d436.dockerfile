...
RUN apt-get install --no-install-recommends screen telnet netcat-openbsd byobu -y
RUN apt-get install --no-install-recommends wget vim unzip -y
#  GCC and basic build tools
RUN apt-get install --no-install-recommends gcc g++ make -y
#  Java
...
RUN apt-get install --no-install-recommends libcommons-logging-java -y
#  Wordnet
RUN apt-get install --no-install-recommends wordnet -y
RUN apt-get install --no-install-recommends wordnet-dev -y
...
#  Relex -- changes often
COPY http://github.com/opencog/relex/archive/master.zip /home/Downloads/relex-master.zip
