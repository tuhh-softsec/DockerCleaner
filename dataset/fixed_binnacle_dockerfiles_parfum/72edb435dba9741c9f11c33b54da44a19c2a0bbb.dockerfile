...
RUN apt-get update \
 && apt-get install --no-install-recommends build-essential cmake git wget vim python-dev python-numpy python-pip python-yaml locales curl apt-transport-https -y
RUN apt-get update -y \
...
RUN apt-get update -y \
 && apt-get install --no-install-recommends python3-dev python3-pip python3-yaml python3-pycurl locales ssh-askpass -y
...
RUN apt-get update -y \
 && apt-get install --no-install-recommends libxrender1 -y
