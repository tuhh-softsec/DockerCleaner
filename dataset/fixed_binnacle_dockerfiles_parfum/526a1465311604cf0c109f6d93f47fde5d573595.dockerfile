...
RUN apt-get update -y \
 && apt-get install --no-install-recommends gcc-6 g++-6 gfortran-6 gcc-7 g++-7 gfortran-7 gcc-8 g++-8 gfortran-8 gdb git cmake libopenblas-dev liblapack-dev libhdf5-dev hdf5-tools libgsl0-dev clang-5.0 clang-format-5.0 clang-tidy-5.0 libclang-5.0-dev wget libncurses-dev lcov cppcheck libboost-all-dev libssl-dev -y
#  Update is needed to get libc++ correctly
#  Install jemalloc
RUN apt-get update -y \
 && apt-get install --no-install-recommends libc++-dev libc++1 libc++abi-dev -y \
 && apt-get update -y \
 && apt-get install --no-install-recommends libjemalloc1 libjemalloc-dev -y
#  Install ccache to cache compilations for reduced compile time, and Doxygen
RUN apt-get install --no-install-recommends ccache doxygen -y
...
RUN apt-get update -y \
 && apt-get install --no-install-recommends curl lmod -y \
...
RUN apt-get update \
 && apt-get install --no-install-recommends locales language-pack-fi language-pack-en -y \
