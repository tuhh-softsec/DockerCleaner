...
RUN set -x ; apt-get update -qq \
 && apt-get -yqq upgrade \
 && apt-get install --no-install-recommends python3 ruby-compass python3-libsass fontconfig libfreetype6 libxml2 libxslt1.1 libjpeg62-turbo zlib1g libfreetype6 liblcms2-2 libtiff5 tk tcl libpq5 libldap-2.4-2 libsasl2-2 libx11-6 libxext6 libxrender1 locales-all zlibc bzip2 ca-certificates curl gettext-base git gnupg2 nano openssh-client postgresql-client telnet xz-utils -yqq \
 && curl https://bootstrap.pypa.io/get-pip.py | python3 /dev/stdin --no-cache-dir \
 && curl -sL https://deb.nodesource.com/setup_6.x | bash - \
 && apt-get install --no-install-recommends nodejs -yqq \
...
RUN gem install bootstrap-sass --no-rdoc --no-ri --no-update-sources \
...
RUN curl -SLo wkhtmltox.tar.xz https://github.com/wkhtmltopdf/wkhtmltopdf/releases/download/${WKHTMLTOPDF_VERSION}/wkhtmltox-${WKHTMLTOPDF_VERSION}_linux-generic-amd64.tar.xz \
 ...
 && wkhtmltopdf --version
RUN apt-get update \
 && apt-get install --no-install-recommends build-essential libevent-dev libjpeg-dev libldap2-dev libsasl2-dev libssl-dev libxml2-dev libxslt1-dev python3-dev zlib1g-dev -y \
