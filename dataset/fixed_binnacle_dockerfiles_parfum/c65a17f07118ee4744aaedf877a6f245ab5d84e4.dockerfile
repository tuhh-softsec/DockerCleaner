...
RUN apt-get update -y \
 && apt-get install --no-install-recommends automake libtool curl gzip wget unzip libxml2 libxml2-dev libgd2-xpm-dev libgeoip-dev libperl-dev libxslt1-dev libxslt1.1 libssl-dev make g++ php5-fpm subversion pkg-config libcppunit-dev libcurl4-openssl-dev libncurses-dev php5-common php5-cli screen -y
#  Gets PCRE
WORKDIR /install
...
RUN (sh autogen.sh ) \
 ...
 && (make install )
#  Installs xmlrpc
RUN apt-get install --no-install-recommends libxmlrpc-core-c3 libxmlrpc-core-c3-dev -y
#  Installs rtorrent
WORKDIR /rtorrent
