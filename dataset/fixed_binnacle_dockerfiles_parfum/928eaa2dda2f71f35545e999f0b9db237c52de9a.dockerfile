...
RUN dpkg --add-architecture i386 \
 ...
 && apt-get -q -y upgrade
#  Install basic tools
RUN apt-get install --no-install-recommends ssh sudo lynx links curl wget sysstat logtail vim build-essential zip unzip moreutils apt-transport-https locales -q -y
...
RUN apt-get install --no-install-recommends openjdk-6-jdk openjdk-7-jdk git mercurial subversion ant ant-contrib xvfb x11vnc fluxbox chromium-browser -q -y
...
RUN apt-get install --no-install-recommends linkchecker s3cmd python-dev python-setuptools python-lxml python-webunit python-docutils python-reportlab python-pypdf gnuplot python-pip tcpwatch-httpproxy dpkg-dev devscripts debhelper cdbs gnupg redis-server -q -y
...
#  Install converters
RUN apt-get install --no-install-recommends imagemagick ffmpeg2theora ufraw poppler-utils libreoffice libwpd-tools gimp -q -y
RUN git clone https://github.com/nuxeo/ffmpeg-nuxeo.git
ENV BUILD_YASM="true"
RUN cd ffmpeg-nuxeo \
 && ./build-all.sh true
#  Install Android SDK
RUN apt-get install --no-install-recommends libstdc++6:i386 lib32z1 expect -q -y
...
RUN cd /usr/local/src/node-v0.10.32 \
 ...
 && make \
 && make install
RUN npm install yo grunt-cli gulp bower -g
#  Install Java 6
