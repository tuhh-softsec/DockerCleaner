...
RUN apt-get install --no-install-recommends software-properties-common curl build-essential dos2unix gcc git libmcrypt4 libpcre3-dev memcached make python2.7-dev python-pip re2c unattended-upgrades whois vim libnotify-bin nano wget debconf-utils -y
...
#  install nginx
RUN apt-get install --no-install-recommends nginx -y --force-yes
...
#  install php
RUN apt-get install --no-install-recommends php7.0-fpm php7.0-cli php7.0-dev php7.0-pgsql php7.0-sqlite3 php7.0-gd php-apcu php7.0-curl php7.0-mcrypt php7.0-imap php7.0-mysql php7.0-readline php-xdebug php-common php7.0-mbstring php7.0-xml php7.0-zip -y --force-yes
...
#  install sqlite
RUN apt-get install --no-install-recommends sqlite3 libsqlite3-dev -y
#  install mysql
RUN echo mysql-server mysql-server/root_password password $DB_PASS | debconf-set-selections ; echo mysql-server mysql-server/root_password_again password $DB_PASS | debconf-set-selections ; apt-get install --no-install-recommends mysql-server -y \
...
#  install blackfire
RUN apt-get install --no-install-recommends blackfire-agent blackfire-php -y
#  install beanstalkd
RUN apt-get install --no-install-recommends beanstalkd -y --force-yes \
