...
RUN apt-get clean \
 && apt-get update \
 && apt-get install --no-install-recommends ruby-dev rubygems imagemagick graphviz sudo git vim libmemcached-tools libpng-dev libjpeg62-turbo-dev libmcrypt-dev libxml2-dev libxslt1-dev mysql-client zip wget linux-libc-dev libyaml-dev apt-transport-https zlib1g-dev libicu-dev libpq-dev libldap2-dev gnupg libpcre3-dev libssl-dev -y --fix-missing
...
RUN docker-php-pecl-install uploadprogress-1.0.3.1 memcache-3.0.8 yaml-1.2.0 mongodb-1.5.3
COPY docker-php-ext-install /usr/local/bin/
RUN chmod +x /usr/local/bin/docker-php-ext-install
RUN docker-php-ext-configure gd --with-jpeg-dir=/usr/include/
RUN docker-php-ext-configure ldap --with-libdir=lib/x86_64-linux-gnu/
RUN docker-php-ext-install gd mbstring mcrypt zip soap mysql pdo_mysql mysqli xsl opcache calendar intl exif pgsql pdo_pgsql ftp ldap
COPY core/memcached.conf /etc/memcached.conf
#  install xdebug and codesniffer
RUN apt-get install --no-install-recommends php5-xdebug --fix-missing -y; pecl install xdebug-2.5.5 ; echo ';zend_extension = /usr/local/lib/php/extensions/no-debug-non-zts-20131226/xdebug.so' >> /usr/local/etc/php/php.ini; touch /usr/local/etc/php/conf.d/xdebug.ini \
 && echo xdebug.remote_enable=1 >> /usr/local/etc/php/conf.d/xdebug.ini \
 && echo xdebug.remote_autostart=0 >> /usr/local/etc/php/conf.d/xdebug.ini \
 && echo xdebug.remote_connect_back=1 >> /usr/local/etc/php/conf.d/xdebug.ini \
 && echo xdebug.remote_port=9000 >> /usr/local/etc/php/conf.d/xdebug.ini \
 && echo xdebug.remote_log=/tmp/php5-xdebug.log >> /usr/local/etc/php/conf.d/xdebug.ini; gem install sass --version 3.4.22 ; gem install compass
