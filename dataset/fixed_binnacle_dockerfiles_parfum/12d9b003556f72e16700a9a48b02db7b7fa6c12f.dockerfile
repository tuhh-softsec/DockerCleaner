...
RUN apt-get update -y \
 && apt-get install --no-install-recommends sudo build-essential git python python-dev python-setuptools make g++ cmake gfortran ipython swig ant python-numpy python-scipy python-matplotlib cython python-lxml python-nose python-jpype libboost-dev jcc git subversion wget zlib1g-dev pkg-config clang -y
#  ========== Install pip for managing python packages ==================================
RUN apt-get install --no-install-recommends python-pip python-lxml -y \
 && pip install cython
...
RUN apt-get update -y \
 ...
 && apt-get install --no-install-recommends libblas-dev liblapack-dev -y
#  ======== Install numpy, scipy, Matplotlib ============================================
RUN apt-get install --no-install-recommends pkgconf libpng-dev libfreetype6-dev -y \
 && pip install numpy \
 && apt-get install --no-install-recommends python-matplotlib -y \
 && pip install scipy
...
RUN apt-get install --no-install-recommends ipython ipython-notebook -y
