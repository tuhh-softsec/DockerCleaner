...
RUN apt-get update \
 && apt-get install --no-install-recommends -y curl wget pkg-config parallel
ARG GO_VERSION
ARG TARGET_ARCH
...
ARG TARGET_ARCH
RUN apt-get update \
 && apt-get install --no-install-recommends -y curl host iproute2 netcat netcat.openbsd realpath vim
RUN apt-get install --no-install-recommends -y openjdk-8-jre-headless \
 && mkdir -p /opt/root \
...
ARG TARGET_ARCH
RUN apt-get update \
 && apt-get install --no-install-recommends -y curl git inotify-tools iproute2 jq net-tools python3-pip socat sudo vim wget
...
RUN apt-get update \
 && apt-get install --no-install-recommends -y dh-make devscripts dh-systemd apt-utils awscli
ARG agent_version="latest"
