...
# build ospray
RUN DEBIAN_FRONTEND=noninteractive apt-get update \
 && apt-get install --no-install-recommends libglfw3-dev libgl1-mesa-dri libxrandr-dev libxinerama-dev libxcursor-dev libmpich-dev mpich openssh-server openssh-client -y -q
