...
RUN apt-get update
RUN apt-get install --no-install-recommends curl wget -y
RUN apt-get install --no-install-recommends git subversion -y
RUN apt-get install --no-install-recommends unzip -y
RUN apt-get install --no-install-recommends sed gawk -y
RUN mkdir /code
#  NOTE(namin): I disabled some installations from source, because they get killed.
# ## Scheme ###
# ## Vicare ###
#
RUN apt-get install --no-install-recommends build-essential libtool autoconf libgmp-dev texinfo -y
#  --- killed ---
#  RUN cd /code;\
...
# ## Chez ###
RUN apt-get install --no-install-recommends libncurses-dev ncurses-dev libx11-dev -y
#  RUN cd /code;\
...
RUN apt-get install --no-install-recommends chezscheme -y
# ## ML ###
RUN apt-get install --no-install-recommends mlton-compiler -y
# ## Twelf ###
#  --- killed ---
#  RUN cd /code;\
...
RUN apt-get install --no-install-recommends openjdk-11-jdk -y
...
# # LaTeX ##
RUN apt-get install --no-install-recommends texlive-latex-base texlive-latex-extra -y
# # Racket ##
RUN add-apt-repository ppa:plt/racket
RUN apt-get update
RUN apt-get install --no-install-recommends racket -y
# # SMT ##
RUN apt-get install --no-install-recommends z3 -y
RUN apt-get install --no-install-recommends cvc4 -y
# # user runner ##
RUN apt-get install --no-install-recommends sudo -y
RUN apt-get install --no-install-recommends dos2unix -y
