...
RUN apt-add-repository ppa:brightbox/ruby-ng \
 && apt-get update \
 && apt-get upgrade -y \
 && apt-get install --no-install-recommends ruby2.1 ruby2.1-dev ruby ruby-switch unzip iptables lxc fontconfig libffi-dev build-essential git python-dev libssl-dev python-pip -y \
 && rm -rf /var/lib/apt/lists/*
