...
RUN apt-get update \
 && apt-get install --no-install-recommends subversion ant libicu-dev openjdk-7-jdk python-numpy gobject-introspection python-gobject-dev pkg-config ragel gtk-doc-tools libgirepository1.0-dev libpdf-api2-perl libintl-perl fntsample texlive-xetex wget xzdec latexmk packaging-dev pkg-config python-dev giflib-dbg libjpeg-dev libtiff-dev uthash-dev automake flex bison -y \
