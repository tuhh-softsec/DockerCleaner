...
USER root
RUN apt-get update \
 && apt-get install --no-install-recommends git gcc python-dev python-pip curl libtool autoconf flex python-socksipychain python-m2crypto python-levenshtein swig libssl-dev pff-tools libimage-exiftool-perl -y \
