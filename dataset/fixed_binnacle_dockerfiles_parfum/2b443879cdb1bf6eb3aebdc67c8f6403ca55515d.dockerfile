...
#  Java
RUN apt-get install --no-install-recommends maven screen telnet netcat-openbsd byobu wget vim git unzip sudo apt-utils -y
RUN apt-get install --no-install-recommends openjdk-11-jdk -y
#  GCC and basic build tools
RUN apt-get install --no-install-recommends gcc g++ make swig ant -y
#  Wordnet
RUN apt-get install --no-install-recommends wordnet wordnet-dev wordnet-sense-index -y
...
#  build tools
RUN apt-get install --no-install-recommends -y libboost-python-dev libblas-dev libboost-thread-dev libboost-filesystem-dev libboost-system-dev
RUN apt-get install --no-install-recommends -y cmake libboost-program-options-dev libboost-regex-dev libiberty-dev guile-2.2-dev protobuf-compiler uuid-dev
#  Conda
#  install conda
RUN apt-get install --no-install-recommends wget bzip2 ca-certificates libglib2.0-0 libxext6 libsm6 libxrender1 -y
