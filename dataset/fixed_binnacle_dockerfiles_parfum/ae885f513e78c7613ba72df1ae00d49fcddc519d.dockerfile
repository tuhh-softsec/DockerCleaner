...
RUN apt-get clean \
 && apt-get update \
 && apt-get install --no-install-recommends libtool bash-completion pkg-config build-essential autoconf automake cmake cmake-curses-gui -y
RUN apt-get clean \
 && apt-get update \
 && apt-get install --no-install-recommends git vim sudo unzip curl ctags git tmux -y
# LLVM
RUN apt-get clean \
 && apt-get update \
 && apt-get install --no-install-recommends clang lldb -y
...
RUN apt-get clean \
 && apt-get update \
 && apt-get install --no-install-recommends libboost-all-dev libeigen3-dev gfortran -y
# OpenCV
RUN apt-get clean \
 && add-apt-repository --yes ppa:xqms/opencv-nonfree \
 && apt-get update \
 && apt-get install --no-install-recommends libopencv-nonfree-dev libopencv-dev -y
RUN cd ~ \
