...
RUN set -eux \
 ...
 && apk update \
 && apk add ${APK_FLAGS_PERSISTENT} tini bash fping iputils libcurl libevent libldap libssh2 libxml2 net-snmp-agent-libs openipmi-libs pcre sqlite-libs unixodbc \
 && rm -rf /var/cache/apk/*
...
RUN set -eux \
 && apk add ${APK_FLAGS_DEV} --virtual build-dependencies alpine-sdk autoconf automake coreutils curl-dev libevent-dev libssh2-dev libxml2-dev net-snmp-dev openipmi-dev openldap-dev pcre-dev sqlite-dev git unixodbc-dev \
 && cd /tmp/ \
