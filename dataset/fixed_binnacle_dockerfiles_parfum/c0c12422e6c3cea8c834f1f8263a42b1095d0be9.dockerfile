...
RUN apt-get update -qq \
 && apt-get install --no-install-recommends iptables ca-certificates aufs-tools -qqy
#  Build dependencies
# RUN	dpkg-divert --local --rename --add /sbin/initctl
# RUN	ln -s /bin/true /sbin/initctl
RUN apt-get update -qq \
 && apt-get install --no-install-recommends wget curl git unzip -y -q
#  Install Go
...
#  build dependencies
RUN apt-get update -qq \
 && apt-get install --no-install-recommends make gcc libpam0g-dev ruby ruby-dev -y -q
RUN gem install fpm
RUN apt-get update -qq \
 && apt-get install --no-install-recommends rpm -y -q
RUN apt-get update -qq \
 && apt-get install --no-install-recommends mercurial bzr -y -q
RUN apt-get update -qq \
 && apt-get install --no-install-recommends libdevmapper-dev libsqlite3-dev -y -q
#  Install the xvfb for firefox and chrome so they can run on a headless system
RUN apt-get update -qq \
 && apt-get install --no-install-recommends xvfb -y -q
...
RUN apt-get update -qq \
 && apt-get install --no-install-recommends libxss1 libappindicator1 libindicator7 -y -q
...
RUN apt-get update -qq \
 && apt-get install --no-install-recommends -y nodejs=6.12.3-1nodesource1
