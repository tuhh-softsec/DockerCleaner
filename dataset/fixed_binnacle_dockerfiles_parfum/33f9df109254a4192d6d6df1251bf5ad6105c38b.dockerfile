...
#
RUN apt-get update \
 && apt-get install --no-install-recommends autoconf automake autopoint build-essential cmake gettext git libgcrypt-dev libidn11-dev libssl-dev libtool libunistring-dev pkg-config python-twisted rsync texinfo wget zlib1g-dev -y
...
RUN apt-get update \
 && apt-get install --no-install-recommends libc6 libgcc1 libssl1.1 libstdc++6 zlib1g ca-certificates lsb-release wget $( echo $OUINET_DEBUG | sed -n 's/^yes$/gdb/p' ;) -y \
...
RUN wget -q -P /tmp "https://github.com/PurpleI2P/i2pd/releases/download/${I2PD_VERSION}/i2pd_${I2PD_VERSION}-1$( lsb_release -sc ;)1_$( dpkg --print-architecture ;).deb" \
 && apt-get update \
 && apt-get install --no-install-recommends cron logrotate $( dpkg --info /tmp/i2pd_*.deb | sed -nE 's/^.*Depends: (.*)/\1/p' | sed -E 's/( \([^)]+\))?,//g' ;) -y \
