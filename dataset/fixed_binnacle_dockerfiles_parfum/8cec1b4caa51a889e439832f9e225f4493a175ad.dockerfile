...
RUN set -xe \
 && apt-get update \
 && apt-get install --no-install-recommends gfortran libblas-dev liblapack-dev libpcre3-dev -y \
...
RUN set -xe \
 ...
 && apt-get update \
 && apt-get install --no-install-recommends openjdk-8-jdk="$JAVA_8_DEBIAN_VERSION" openjdk-7-jdk="$JAVA_7_DEBIAN_VERSION" ca-certificates-java="$CA_CERTIFICATES_JAVA_VERSION" -y \
...
RUN set -xe \
 && apt-get update \
 && apt-get install --no-install-recommends nodejs-legacy npm -y \
...
RUN set -xe \
 && apt-get update \
 && apt-get install --no-install-recommends libcap-dev -y \
