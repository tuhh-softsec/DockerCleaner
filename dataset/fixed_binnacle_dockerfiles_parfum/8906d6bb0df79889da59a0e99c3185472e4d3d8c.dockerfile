...
RUN apt-get update \
 && apt-get install --no-install-recommends libcurl3 iptables python3.6 python-setuptools python-pytest python-pycurl python-kazoo python-zope.interface ruby1.9.1 ruby1.9.1-dev rubygems1.9.1 libxml2 libxml2-dev libxslt-dev libssl-dev build-essential zlib1g-dev -y
