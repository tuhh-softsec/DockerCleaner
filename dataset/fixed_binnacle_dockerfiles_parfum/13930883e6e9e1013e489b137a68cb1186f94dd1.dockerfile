...
RUN apt-get update -y \
 && apt-get install --no-install-recommends -y build-essential libssl-dev libffi-dev libxml2-dev libxslt1-dev locales
# set locales
...
RUN apt-get install --no-install-recommends -y software-properties-common \
 ...
 && apt-get install --no-install-recommends -y oracle-java12-installer
# Install Python 3
RUN apt-get install --no-install-recommends -y python3.6 python3-dev python3-setuptools \
...
RUN apt-get install --no-install-recommends -y sqlite3 fontconfig-config libjpeg-turbo8 fontconfig xorg xfonts-75dpi git
