...
FROM adtools
RUN apt-get update && apt-get install --no-install-recommends binutils patch gcc subversion build-essential wget debhelper devscripts texinfo lhasa libgmp-dev libmpfr-dev libmpc-dev fakeroot flex -y
#
