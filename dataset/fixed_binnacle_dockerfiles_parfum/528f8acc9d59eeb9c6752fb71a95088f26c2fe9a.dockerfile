...
#  Install packages.
RUN apt-get update
RUN apt-get install --no-install-recommends vim git apache2 php-cli php-mysql php-gd php-curl php-xdebug php-bcmath php7.0-sqlite3 libapache2-mod-php curl mysql-server mysql-client openssh-server phpmyadmin wget unzip cron gnupg supervisor -y
RUN apt-get clean
#  Setup PHP.
