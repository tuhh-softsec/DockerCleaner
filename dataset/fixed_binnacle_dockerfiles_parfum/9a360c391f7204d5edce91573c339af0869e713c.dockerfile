...
#  fskit
# #############################################
RUN apt-get install --no-install-recommends libfuse-dev libattr1-dev -y
USER syndicate
WORKDIR $HOME
...
#  syndicate
# #############################################
RUN apt-get install --no-install-recommends protobuf-compiler libprotobuf-dev libcurl4-gnutls-dev libmicrohttpd-dev libjson0-dev valgrind cython python-protobuf libssl-dev python-crypto python-requests -y
USER syndicate
WORKDIR $HOME
