...
RUN apt-get update \
 && apt-get install --no-install-recommends autoconf bedtools build-essential bwa cpanminus gawk git imagemagick libbz2-dev liblzma-dev python python-dev python-pip r-base vim-tiny wget zlib1g-dev -y \
 && apt-get clean
#   Various R packgages
...
RUN pip install pyvcf pysam numpy
#  Perl packags
RUN cpanm XML::XPath
#  Installing HTSLIB
RUN cd /usr/local \
