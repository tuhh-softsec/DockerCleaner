...
RUN apt-get install --no-install-recommends make python -y
#  Some typical implementation and test requirements
RUN apt-get install --no-install-recommends curl libreadline-dev libedit-dev -y
RUN mkdir -p /mal
WORKDIR /mal
# #########################################################
...
RUN apt-get install --no-install-recommends gnupg -y
...
RUN dpkg --add-architecture i386 \
 && apt-get update -y \
 && apt-get install --no-install-recommends lib32gcc-8-dev libsdl2-dev:i386 libsdl2-image-dev:i386 libedit-dev:i386 freeglut3-dev:i386 lib32gcc-7-dev libreadline-dev:i386 -y
#
#  binaryen
#
RUN apt-get install --no-install-recommends git-core cmake -y
...
#
RUN apt-get install --no-install-recommends llvm-3.9-dev libclang-3.9-dev clang-3.9 -y
...
RUN apt-get install --no-install-recommends libffi-dev pkg-config libz-dev libbz2-dev libsqlite3-dev libncurses-dev libexpat1-dev libssl-dev libgdbm-dev tcl-dev -y
