...
RUN DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y --fix-missing apt-utils autoconf automake bind9-host build-essential dh-autoreconf cpanminus curl devscripts exuberant-ctags git-core jq llvm libgeoip1 libgeoip-dev libpcre3 libpcre3-dbg libpcre3-dev libperl-dev libmagic-dev libtool lsof make mercurial ngrep procps python telnet tcpflow valgrind vim wget zlib1g zlib1g-dev
