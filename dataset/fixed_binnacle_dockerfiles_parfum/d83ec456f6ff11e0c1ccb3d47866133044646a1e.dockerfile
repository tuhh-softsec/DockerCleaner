...
RUN apt-get update -q \
 && apt-get install --no-install-recommends wget apt-transport-https vim git postgresql postgresql-plpython-9.5 libfontconfig python3-pip libssl-dev libpq-dev -qy \
 && pip3 install -U pip
