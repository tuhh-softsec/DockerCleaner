...
#  Install Other tools
RUN apt-get install --no-install-recommends virtualenv -y
...
RUN apt-get install --no-install-recommends picocom -y
...
RUN echo "===> Adding Ansible's PPA..." \
 ...
 && apt-get install --no-install-recommends sshpass openssh-client -y \
...
RUN apt-get update -y
RUN apt-get install --no-install-recommends curl -y
RUN apt-get install --no-install-recommends build-essential libssl-dev libffi-dev python-dev -y
