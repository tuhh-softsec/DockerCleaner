...
RUN apt-get update \
 && apt-get install --no-install-recommends libmcrypt-dev libmcrypt4 libcurl3-dev libfreetype6 libjpeg62-turbo libfreetype6-dev libjpeg62-turbo-dev apt-utils mariadb-server-core-10.1 mariadb-server-10.1 mariadb-client-10.1 libxml2-dev git wget zip vim gcc make autoconf libc-dev pkg-config -y
