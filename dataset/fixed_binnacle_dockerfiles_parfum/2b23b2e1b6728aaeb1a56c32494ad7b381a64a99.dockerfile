...
RUN set -x ; apt-get clean \
 && apt-get update ; apt-get install --no-install-recommends software-properties-common -y; apt-get install --no-install-recommends tzdata -y; echo "Europe/Amsterdam" > /etc/timezone \
...
#  PostgreSQL
RUN set -x \
 && apt-get install --no-install-recommends postgresql -y
#  GIS packages
RUN set -x \
