...
RUN apt-get update
RUN apt-get install --no-install-recommends build-essential wget curl git git-core -y
#  ffmpeg
...
RUN apt-get install --no-install-recommends libcurl4-openssl-dev libreadline-dev libssl-dev libxml2-dev libxslt1-dev libyaml-dev zlib1g-dev -y \
...
RUN apt-get install --no-install-recommends libmagickcore-dev libmagickwand-dev -y
