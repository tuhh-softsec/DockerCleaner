...
#
RUN apt-get update
RUN apt-get install --no-install-recommends git wget -y
RUN apt-get install --no-install-recommends libpcap-dev uuid-dev libelf-dev cmake -y
RUN apt-get install --no-install-recommends python3-setuptools python3-pyqt4 python3-ws4py -y
RUN apt-get install --no-install-recommends python3-netifaces python3-zmq python3-tornado python3-dev -y
RUN apt-get install --no-install-recommends bison flex -y
# 
#  for iou install 32 bit libraries, python is needed to generate the license file
#
RUN apt-get install --no-install-recommends lib32z1 lib32ncurses5 lib32bz2-1.0 -y
RUN apt-get install --no-install-recommends lxterminal telnet -y
RUN apt-get install --no-install-recommends python -y
RUN apt-get install --no-install-recommends wireshark cpulimit -y
#
#  -----------------------------------------------------------------
...
RUN apt-get update
RUN apt-get install --no-install-recommends libssl-dev:i386 -y
#
#  ---------------------------------------------------------------------------
#  install QEMU
#
...
#  and NAT
#
RUN apt-get install --no-install-recommends uml-utilities iptables -y
#
#  ---------------------------------------------------------------------------
...
RUN apt-get install --no-install-recommends expect python-vte python-glade2 -y
