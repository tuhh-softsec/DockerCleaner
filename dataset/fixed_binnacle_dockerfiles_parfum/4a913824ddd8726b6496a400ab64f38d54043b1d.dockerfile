...
WORKDIR /usr/picard
COPY https://github.com/broadinstitute/picard/releases/download/${picard_version}/picard.jar ./picard.jar
#  Install Python3
...
RUN apt-get update \
 && apt-get install --no-install-recommends -y build-essential cmake automake curl gcc-multilib git libcurl4-openssl-dev libssl-dev libboost-all-dev libncurses5-dev libxml2-dev libncurses5-dev libboost-all-dev libbz2-dev liblzma-dev lsb-release samtools sudo wget zlib1g-dev gfortran
...
RUN apt-get update \
 && apt-get install --no-install-recommends unstable cabal-install -t -y
RUN cabal update \
