...
RUN rustup update \
 && set -ex \
 && apt-get update \
 && apt-get install --no-install-recommends clang libclang-dev llvm-dev libncurses5 libncursesw5 cmake git libssl-dev --yes
...
RUN apt-get update \
 && DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y locales procps libssl-dev vim telnet curl python3 \
