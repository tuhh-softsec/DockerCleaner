...
ENV ALLOWED_HOSTS="*"
RUN apt-get update \
 && apt-get install --no-install-recommends libsm6 libboost-all-dev libglib2.0-0 libxrender-dev wget curl nginx -y
...
RUN apt-get update \
 && apt-get install --no-install-recommends cmake git build-essential -y \
