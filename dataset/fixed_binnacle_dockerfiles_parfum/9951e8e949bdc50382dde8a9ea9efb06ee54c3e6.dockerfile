...
RUN apt-get update \
 && apt-get install --no-install-recommends build-essential git libpng-dev perl perl-base unzip wget -y \
...
#  a lot of them are required for Bio::DB::BigFile
RUN apt-get update \
 && apt-get install --no-install-recommends build-essential cpanminus curl libmysqlclient-dev libpng-dev libssl-dev locales openssl perl perl-base unzip vim -y \
...
RUN apt-get update \
 && apt-get install --no-install-recommends apache2 apt-utils build-essential cpanminus curl git libmysqlclient-dev libpng-dev libssl-dev manpages mysql-client openssl perl perl-base unzip vim wget sudo -y
