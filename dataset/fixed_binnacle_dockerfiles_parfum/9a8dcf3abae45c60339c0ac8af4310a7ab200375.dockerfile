...
RUN apt-get update \
 && apt-get install --no-install-recommends wget bzip2 -y
...
RUN apt-get update -q \
 && apt-get -q -y upgrade \
 && apt-get install --no-install-recommends build-essential software-properties-common -q -y \
 && apt-get install --no-install-recommends curl wget git python python-dev -q -y \
...
RUN apt-get install --no-install-recommends pkg-config libpthread-stubs0-dev libomxil-bellagio-dev -y \
 && mkdir -p /root/build \
 && cd /root/build \
