...
RUN DEBIAN_FRONTEND=noninteractive apt-get update \
 && DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y --force-yes apt-utils \
 && DEBIAN_FRONTEND=noninteractive apt-get upgrade -y --force-yes
#  Create and configure locales
RUN DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y --force-yes locales
...
RUN DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y --force-yes wget curl net-tools vim git man-db inotify-tools locate
#  Install MySQL database packages
RUN DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y --force-yes mysql-server mysql-client
#  Install MySQL or MariaDB
RUN { echo "mysql-server-5.5 mysql-server/root_password password {{password}}" ;echo "mysql-server-5.5 mysql-server/root_password_again password {{password}}" ; } | debconf-set-selections \
 && DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y --force-yes mysql-server mysql-client \
 && service mysql start \
 && { echo "GRANT ALL PRIVILEGES ON *.* to 'admin'@'localhost' IDENTIFIED BY '{{password}}' WITH GRANT OPTION;" ;echo "FLUSH PRIVILEGES;" ; } | mysql -p{{password}}
#  Install Apache webserver packages
RUN DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y --force-yes apache2 apache2-utils
#  Install PHP packages
RUN DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y --force-yes php5 php5-cli libapache2-mod-php5 php5-mysqlnd php5-curl php5-gd php5-apcu
RUN DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y --force-yes graphicsmagick
