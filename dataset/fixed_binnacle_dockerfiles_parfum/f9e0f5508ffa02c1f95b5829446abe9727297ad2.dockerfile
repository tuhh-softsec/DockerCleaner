...
RUN apt-get update \
 ...
 && apt-get install --no-install-recommends apt apt-utils -y \
...
RUN apt-get update \
 && apt-get install --no-install-recommends libc6-i386 -y \
 && rm -rf /var/lib/apt/lists/*
...
RUN apt-get update \
 && apt-get install --no-install-recommends ruby redis-tools -y \
