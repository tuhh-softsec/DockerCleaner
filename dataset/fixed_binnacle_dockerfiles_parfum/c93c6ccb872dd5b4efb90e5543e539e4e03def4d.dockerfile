...
RUN sudo apt-get update \
 && sudo apt-get install --no-install-recommends --fix-missing -y build-essential ncbi-blast+ cmake autoconf autogen pigz xvfb default-jre wget zlib1g-dev libboost-all-dev \
