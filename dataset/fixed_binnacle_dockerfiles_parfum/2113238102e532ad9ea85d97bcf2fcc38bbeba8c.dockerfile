...
RUN apt-get update \
 && apt-get install --no-install-recommends -y libmp3lame0 libvorbis0a libass5 libsamplerate0 libtheora0 libvorbisenc2 libx264-148 libjansson4 libopus0 libnotify4 libdbus-glib-1-2 libgstreamer-plugins-base1.0-0 libwebkitgtk-3.0-0 libdvd-pkg \
 && apt-get clean
#  Build image 
FROM base AS build
#  Build packages 
