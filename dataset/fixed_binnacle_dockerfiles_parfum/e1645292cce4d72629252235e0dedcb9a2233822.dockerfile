...
RUN git clone https://github.com/glfw/glfw.git \
 ...
 && make -j4 \
 && make install
RUN apt-get install --no-install-recommends libopenblas-dev libfftw3-dev liblapacke-dev
WORKDIR /root
...
RUN apt-get update \
 && apt-get install --no-install-recommends clang-3.7 libclang-common-3.7-dev libclang-3.7-dev libclang1-3.7 libllvm-3.7-ocaml-dev libllvm3.7 lldb-3.7 llvm-3.7 llvm-3.7-dev llvm-3.7-runtime clang-modernize-3.7 clang-format-3.7 lldb-3.7-dev -y
RUN apt-get clean
...
RUN apt-get update
RUN apt-get install --no-install-recommends libglfw3 libglfw3-dev -qqy --force-yes
...
ENV LD_LIBRARY_PATH="/usr/lib/llvm-3.7/lib/"
ENV LLVM_CONFIG="/usr/lib/llvm-3.7/bin/llvm-config"
RUN apt-get install --no-install-recommends build-essential scons pkg-config libx11-dev libxcursor-dev libxinerama-dev libgl1-mesa-dev libglu-dev libasound2-dev libpulse-dev libfreetype6-dev libssl-dev libudev-dev libxrandr-dev -qyy
...
RUN apt-get install --no-install-recommends python2.7 python-pip python-dev ipython ipython-notebook -qyy
...
RUN apt-get install --no-install-recommends libsasl2-dev libldap2-dev libssl-dev libpq-dev postgresql-9.3 postgresql-client-9.3 postgresql-contrib-9.3 -qqy
...
RUN apt-get install --no-install-recommends postgresql postgresql-contrib -qqy
...
RUN apt-get install --no-install-recommends mesa-common-dev freeglut3-dev libglfw-dev libglm-dev libglew1.6-dev xorg-dev libglu1-mesa-dev libsdl2-dev -qqy
USER postgres
