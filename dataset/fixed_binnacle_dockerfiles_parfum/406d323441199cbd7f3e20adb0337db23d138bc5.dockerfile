...
RUN apt-get update \
 && apt-get install --no-install-recommends curl git graphicsmagick python-dev python-pip python-numpy python-nose python-scipy python-pandas python-protobuf wget zip -y \
 && apt-get clean \
 && rm -rf /var/lib/apt/lists/* /tmp/* /var/tmp/*
# ADD . /root/openface
