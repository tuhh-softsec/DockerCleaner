...
ARG PIP="pip"
RUN apt-get update \
 && apt-get install --no-install-recommends build-essential curl git libcurl3-dev libfreetype6-dev libhdf5-serial-dev libpng12-dev libzmq3-dev libssl-dev pkg-config rsync software-properties-common unzip zip zlib1g-dev openjdk-8-jdk openjdk-8-jre-headless -y
# install Python 3
RUN if [ ${PYTHON} = "python3.6" ] ; then curl https://www.python.org/ftp/python/3.6.5/Python-3.6.5.tar.xz -o /opt/python.tar.xz \
 ...
 && ./configure \
 && make \
 && make install ; else apt-get install --no-install-recommends python-dev ${PYTHON3_DEV} -y ; fi
RUN apt-get clean \
