...
RUN apt-get update \
 && apt-get install --no-install-recommends nodejs nodejs-dev nodejs-legacy `` -y
RUN apt-get update \
 && apt-get install --no-install-recommends libmono-system-web2.0-cil mono-devel `` -y
RUN apt-get update \
 && apt-get install --no-install-recommends xdg-utils dart lua5.2 lua5.2-dev mingw32 mingw32-binutils mingw32-runtime nsis `` `` `` `` `` -y \
 && rm -rf /var/cache/apt/* \
