FROM ubuntu:16.04
RUN apt-get update \
 && apt-get install --no-install-recommends git make software-properties-common wget -y
#  Install C
RUN apt-get update \
 && apt-get install --no-install-recommends gcc-4.7 gcc-4.8 gcc-4.9 gcc-5 gcc -y
#  Install Isolate
WORKDIR /tmp
...
#  Install C++
RUN apt-get install --no-install-recommends g++-4.7 g++-4.8 g++-4.9 g++-5 g++ -y
#  Install Java
...
#  Install Ruby
RUN apt-get install --no-install-recommends autoconf bison build-essential libssl-dev libyaml-dev libreadline6-dev zlib1g-dev libncurses5-dev libffi-dev libgdbm3 libgdbm-dev -y
WORKDIR /tmp
...
#  Install Python
RUN apt-get install --no-install-recommends python python3 -y
#  Install Pascal
RUN apt-get install --no-install-recommends fp-compiler-3.0.0 -y
