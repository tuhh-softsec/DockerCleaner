...
RUN apt-get update -y \
 && apt-get install --no-install-recommends apt-transport-https build-essential curl g++ gcc git-core libbz2-dev libmariadb-client-lgpl-dev libmysqlclient-dev libreadline-dev libsqlite3-dev libssl-dev libudev-dev libyaml-dev make nano sudo zlib1g-dev pkg-config -y
