...
RUN apt-get install --no-install-recommends software-properties-common apt-transport-https -y
#  Add a recent version of git
RUN add-apt-repository -y ppa:git-core/ppa
#  32-bit support
RUN dpkg --add-architecture i386 \
 && apt-get update --fix-missing \
 && apt-get install --no-install-recommends libc6:i386 libncurses5:i386 libstdc++6:i386 -y
RUN apt-get install --no-install-recommends bash console-setup sudo wget curl git python python-pip python3 python3-pip man-db -y \
...
RUN apt-get install --no-install-recommends ant maven2 -y
#  editors
RUN apt-get install --no-install-recommends nano vim vim-addon-manager vim-vimoutliner vim-doc ctags indent -y
#  servers
RUN apt-get install --no-install-recommends redis-server redis-tools nginx mysql-server rabbitmq-server couchdb -y
#  Postgres
RUN apt-get install --no-install-recommends postgresql-9.3 -y \
 && /etc/init.d/postgresql start \
 && sudo -u postgres createuser -srd ubuntu \
 && sudo -u postgres psql -c "create database ubuntu owner=ubuntu"
#  misc tools
RUN apt-get install --no-install-recommends dnsutils bash-completion xsltproc build-essential fakeroot tmux duplicity lftp htop apt-file parallel strace ltrace flex jq ack-grep gdb valgrind locate tree time zip unp cmake -y
RUN apt-file update
#  Version control
RUN apt-get install --no-install-recommends git-all tig subversion subversion-tools cvs mercurial bzr bzrtools git-svn -y
#  libraries and headers
RUN apt-get install --no-install-recommends libcairo2-dev libjpeg62-dev libgif-dev libpq-dev libboost-all-dev libzmq-dev libwww-curl-perl libbz2-dev dpkg-dev ruby1.9.1-dev dialog python-gtk2 libpango1.0 libpango1.0-dev libmysqlclient-dev -y
...
RUN apt-get install --no-install-recommends gawk libxml2-dev libxslt-dev libgdbm-dev libgmp-dev sqlite3 -y
...
RUN echo "Configure php" \
 ...
 && /etc/init.d/postgresql start \
 && apt-get install --no-install-recommends php5 php5-cli php5-fpm php5-pgsql php5-mysql phpmyadmin phppgadmin -y \
 && php5enmod c9 \
