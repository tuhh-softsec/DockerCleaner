...
RUN apt-get update \
 && apt-get install --no-install-recommends -y build-essential curl git gcc make openssl libssl-dev libbz2-dev apt-transport-https ca-certificates g++ gnupg graphviz lsb-release openssh-client zip libreadline-dev libsqlite3-dev cmake libxml2-dev wget bzip2 sudo vim unzip locales \
 && apt-get clean \
