...
RUN apt-get update \
 && apt-get install --no-install-recommends apt-utils cron curl dialog git lighttpd netcat net-tools python2.7 wget -y
#  Update to the latest software packages:
RUN apt-get update \
 && apt-get upgrade -y
#  Install required packages for building dump1090:
RUN apt-get update \
 && apt-get install --no-install-recommends debhelper dpkg-dev librtlsdr-dev librtlsdr0 libusb-1.0-0-dev pkg-config rtl-sdr -y
#  Prepare for install
