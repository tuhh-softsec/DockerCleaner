...
RUN export DEBIAN_FRONTEND=noninteractive DEBCONF_NONINTERACTIVE_SEEN=true \
 && apt-get update -q -y \
 && apt-get install --no-install-recommends python3 python3-pip libssl-dev libmpdec2 proj-bin libproj-dev libgeos-dev libgeos++-dev libatomic1 libbsd0 libcurl4 libxml2 libedit2 libsqlite3-0 libc6-dev binutils libgcc-5-dev libstdc++-5-dev libpython2.7 tzdata git pkg-config -q -y \
