...
RUN dpkg --add-architecture i386 \
 ...
 && apt-get install --no-install-recommends jessie-backports ca-certificates-java openjdk-8-jdk-headless openjdk-8-jre-headless ant -y -t
...
RUN git clone https://github.com/contribute-torque/torquegui.git \
 && cd torquegui \
