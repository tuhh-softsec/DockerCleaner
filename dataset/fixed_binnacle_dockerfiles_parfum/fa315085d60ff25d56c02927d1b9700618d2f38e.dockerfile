...
RUN DEBIAN_FRONTEND=noninteractive apt-get update -y \
 && DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends autoconf automake ca-certificates coreutils curl ethtool gcc gcc-multilib kmod libc6-dev-i386 libpcap0.8-dev libtool-bin make net-tools patch perl python-dev sed tar valgrind wget \
 && cd /tmp \
