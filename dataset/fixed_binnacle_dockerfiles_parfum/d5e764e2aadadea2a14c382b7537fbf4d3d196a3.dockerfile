...
RUN add-apt-repository -y ppa:iconnor/zoneminder \
 ...
 && apt-get install --no-install-recommends ssmtp mailutils php-curl net-tools -y \
 && apt-get install --no-install-recommends php-gd zoneminder=1.30.4* -y \
 && apt-get install --no-install-recommends libcrypt-mysql-perl libyaml-perl make libjson-perl -y
