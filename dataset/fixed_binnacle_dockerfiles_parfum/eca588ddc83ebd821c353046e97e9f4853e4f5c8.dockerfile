...
RUN apt-get install --no-install-recommends jackd -y
#  Install pretty much everything we need here
RUN DEBIAN_FRONTEND='noninteractive' apt-get --no-install-recommends -y install build-essential xvfb git yasm supervisor libsndfile1-dev libsamplerate0-dev liblo-dev libasound2-dev wget ghc emacs-nox haskell-mode zlib1g-dev xz-utils htop screen openssh-server cabal-install curl sudo
#  Install jack libs last
RUN apt-get install --no-install-recommends libjack-jackd2-dev -y
#  Build Dirt synth
WORKDIR /repos
...
WORKDIR /work
RUN mkdir /root/.ssh
COPY https://raw.githubusercontent.com/DoubleDensity/scratchpool/master/id_rsa-scratchpool /root/.ssh/id_rsa
