...
RUN apt-get update \
 && apt-get install --no-install-recommends sudo gdebi-core pandoc pandoc-citeproc libcurl4-gnutls-dev curl libcairo2-dev libxt-dev unp emacs supervisor libfuse-dev gnupg -y
#    aufs-tools \
#    cgroupfs-mount
...
# # Install Docker
RUN apt-get update \
 && apt-get install --no-install-recommends apt-transport-https ca-certificates gnupg2 software-properties-common -y \
