...
RUN apt-get update \
 && apt-get install --no-install-recommends sudo git vim-tiny nano wget r-base r-base-dev r-recommended fonts-texgyre texinfo locales libcurl4-gnutls-dev libcairo2-dev libxt-dev libssl-dev libxml2-dev -y
...
RUN apt-get install --no-install-recommends libudunits2-dev libproj-dev libgeos-dev libgdal-dev -y
...
RUN apt-get install --no-install-recommends libgtk2.0-0 libgconf-2-4 xvfb fuse desktop-file-utils -y
#  google chrome
