...
RUN set -eux \
 ...
 && apk update \
 && apk add ${APK_FLAGS_PERSISTENT} bash curl mariadb-client mariadb-connector-c nginx php7-bcmath php7-ctype php7-fpm php7-gd php7-gettext php7-json php7-ldap php7-mbstring php7-mysqli php7-session php7-simplexml php7-sockets php7-xmlreader php7-xmlwriter supervisor \
 && rm -rf /var/cache/apk/*
...
RUN set -eux \
 && apk add ${APK_FLAGS_DEV} --virtual build-dependencies coreutils gettext git \
 && cd /usr/share/ \
