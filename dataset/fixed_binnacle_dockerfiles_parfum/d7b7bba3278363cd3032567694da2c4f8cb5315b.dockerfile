...
apt-get install --no-install-recommends php${PHP_VERSION}-gmp -y; fi
# ##########################################################################
#  SOAP:
# ##########################################################################
USER root
...
apt-get install --no-install-recommends sudo wget -y \
 ...
 && echo 'aerospike.udf.lua_user_path=/usr/local/aerospike/usr-lua' >> /etc/php/${LARADOCK_PHP_VERSION}/cli/conf.d/aerospike.ini; else (cd aerospike-client-php/src \
 && phpize \
 && ./build.sh \
 && make install ) ; fi \
...
add-apt-repository -y ppa:pinepain/libv8-archived \
 && apt-get update -yqq \
 && apt-get install --no-install-recommends php-pear libv8-5.4 php${LARADOCK_PHP_VERSION}-xml php${LARADOCK_PHP_VERSION}-dev -y \
 && pecl install v8js \
...
apt-get install --no-install-recommends php5.6-sybase freetds-bin freetds-common libsybdb5 -y \
 ...
 && php -m | grep -q 'sqlsrv' \
 && php -m | grep -q 'pdo_sqlsrv'; fi; else curl https://packages.microsoft.com/keys/microsoft.asc | apt-key add - \
 && curl https://packages.microsoft.com/config/ubuntu/16.04/prod.list > /etc/apt/sources.list.d/mssql-release.list \
 && apt-get update -yqq \
 && ACCEPT_EULA=Y apt-get --no-install-recommends install -y msodbcsql17 mssql-tools unixodbc unixodbc-dev libgss3 odbcinst locales \
 && ln -sfn /opt/mssql-tools/bin/sqlcmd /usr/bin/sqlcmd \
...
USER root
RUN apt-get update -yqq \
 && apt-get install --no-install-recommends nasm -yqq
# ##########################################################################
#  Dusk Dependencies:
# ##########################################################################
USER root
