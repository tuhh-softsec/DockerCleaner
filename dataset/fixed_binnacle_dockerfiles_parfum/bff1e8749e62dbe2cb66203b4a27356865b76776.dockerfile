...
FROM ubuntu:18.04
RUN apt-get update \
 && apt-get install --no-install-recommends ca-certificates wget curl git-core python3 python3-pip libssl-dev libmpdec2 proj-bin libproj-dev libgeos-dev libgeos++-dev mime-support gnupg dirmngr gcc g++ -y \
 && apt-get clean \
