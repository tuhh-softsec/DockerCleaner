...
RUN apt-get update \
 && apt-get install --no-install-recommends locales apt-transport-https python-pip wget curl unzip dnsutils vim git lolcat toilet jshon -y \
 && apt-get clean \
...
#
RUN apt-get update \
 && apt-get install --no-install-recommends dnsmasq dnsutils -y \
 && apt-get clean \
 && rm -rf /var/lib/apt/lists/*
...
RUN echo "deb http://repos.mesosphere.com/ubuntu/ xenial main" > /etc/apt/sources.list.d/mesosphere.list \
 ...
 && apt-get update \
 && apt-get install --no-install-recommends openjdk-8-jre mesos=${MESOS_APP_VERSION} -y \
 && apt-get clean \
 && rm -rf /var/lib/apt/lists/* \
 && rm /etc/mesos/zk \
...
#  NETDATA
RUN apt-get update \
 && apt-get install --no-install-recommends zlib1g-dev gcc make autoconf autogen automake pkg-config uuid-dev libmnl-dev -y \
