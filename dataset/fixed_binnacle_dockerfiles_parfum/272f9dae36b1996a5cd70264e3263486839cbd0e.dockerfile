...
RUN apt-get update \
 && apt-get install --no-install-recommends cmake gcc-multilib g++-multilib libboost-all-dev p7zip-full screen texlive-latex-base texlive-fonts-recommended ghostscript openssh-server -y
