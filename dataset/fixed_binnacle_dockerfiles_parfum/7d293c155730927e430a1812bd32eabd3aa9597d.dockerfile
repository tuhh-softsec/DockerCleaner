...
COPY http://download.draios.com/apt-draios-priority /etc/apt/preferences.d/
RUN apt-get update \
 && apt-get upgrade -y \
 && apt-get install --no-install-recommends bash-completion bc clang-7 curl dkms gnupg2 ca-certificates gcc libc6-dev libelf-dev libelf1 less llvm-7 procps xz-utils -y \
...
RUN curl -s https://s3.amazonaws.com/download.draios.com/DRAIOS-GPG-KEY.public | apt-key add - \
 ...
 && apt-get update \
 && apt-get install --no-install-recommends sysdig -y \
 && apt-get clean \
 && rm -rf /var/lib/apt/lists/*
