...
RUN apt-get update \
 && apt-get install --no-install-recommends binutils-arm-none-eabi bsdmainutils cmake ctags dfu-util gcc-arm-none-eabi git libnewlib-arm-none-eabi libusb-1.0-0-dev lighttpd ninja-build nodejs npm php-cgi python3 python3-pil tmux -qy \
