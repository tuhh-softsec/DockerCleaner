...
USER root
RUN apt-get update
RUN DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends -y install python-software-properties
...
RUN apt-get update \
 && apt-get install --no-install-recommends build-essential git curl freeglut3-dev libcupti-dev libcurl3-dev libfreetype6-dev libpng12-dev libzmq3-dev pkg-config unzip rsync software-properties-common inkscape jed libsm6 libxext-dev libxrender1 lmodern pandoc vim libpng-dev g++ gfortran libffi-dev libhdf5-dev libjpeg-dev liblcms2-dev libopenblas-dev liblapack-dev libssl-dev libtiff5-dev libwebp-dev nano libopenslide-dev wget zlib1g-dev qt5-default libvtk6-dev libjasper-dev libopenexr-dev libgdal-dev libdc1394-22-dev libavcodec-dev libavformat-dev libswscale-dev libtheora-dev libvorbis-dev libxvidcore-dev libx264-dev yasm libopencore-amrnb-dev libopencore-amrwb-dev libv4l-dev libxine2-dev libtbb-dev libeigen3-dev ant default-jdk doxygen bc cmake python-dev python-tk python-setuptools python-numpy python-scipy python-nose python-h5py python-skimage python-matplotlib python-pandas python-sklearn python-sympy python3-dev python3-tk python3-setuptools python3-numpy python3-scipy python3-nose python3-h5py python3-skimage python3-matplotlib python3-pandas python3-sklearn python3-sympy libav-tools -y \
 && apt-get clean \
 && rm -rf /var/lib/apt/lists/*
#  Installing OpenSlide
RUN apt-get update \
 && apt-get install --no-install-recommends openslide-tools -y
RUN apt-get update \
 && apt-get install --no-install-recommends python3-openslide -y \
 && apt-get clean \
 && rm -rf /var/lib/apt/lists/*
#  Installing Libvips
RUN apt-get update \
 && apt-get install --no-install-recommends -y libvips libvips-dev libvips-tools libopenslide-dev
