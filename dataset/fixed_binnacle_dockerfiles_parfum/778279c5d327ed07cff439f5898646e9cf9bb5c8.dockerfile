...
RUN set -x \
 ...
 && apk add --no-cache --virtual .build-deps build-base findutils apr-dev apr-util-dev apache2-dev gnupg gperf icu-dev gettext-dev libjpeg-turbo-dev libpng-dev libtool ca-certificates automake autoconf git jemalloc-dev libtool binutils gnupg cmake go gcc build-base libc-dev make wget gzip libressl-dev musl-dev pcre-dev zlib-dev geoip-dev git linux-headers libxslt-dev nghttp2 gd-dev unzip \
 && apk add --no-cache --update curl monit bash bind-tools rsync geoip libressl tini tar \
 && cd /tmp \
