...
#  install tools
#  ======== 
RUN set -ex ; apt-get update ; apt-get install --no-install-recommends wget git tcpdump tcptraceroute net-tools -y ; cd /usr/bin ; wget http://www.vdberg.org/~richard/tcpping ; chmod 777 tcpping ; cd /var/www/html ; savedAptMark="$( apt-mark showmanual ;)" ; apt-get update ; apt-get install --no-install-recommends libjpeg-dev libpng-dev -y ; docker-php-ext-configure gd --with-png-dir=/usr --with-jpeg-dir=/usr ; docker-php-ext-install gd mysqli opcache zip ; apt-get install --no-install-recommends $PHPIZE_DEPS -y ; pecl install xdebug-beta ; apt-mark auto '.*' > /dev/null; apt-mark manual $savedAptMark ; ldd "$( php -r 'echo ini_get("extension_dir");' ;)"/*.so | awk '/=>/ { print $3 }' | sort -u | xargs -r dpkg-query -S | cut -d: -f1 | sort -u | xargs -rt apt-mark manual ; apt-get purge -y --auto-remove -o APT::AutoRemove::RecommendsImportant=false ; rm -rf /var/lib/apt/lists/*
