...
COPY https://api.github.com/repos/googlecartographer/cartographer/git/refs/heads/master cartographer_ros/cartographer_version.json
...
RUN sudo apt-get -y --no-install-recommends install openjdk-8-jdk python-pip
...
RUN pip install google-cloud-datastore --upgrade
RUN pip install google-cloud-bigquery --upgrade
