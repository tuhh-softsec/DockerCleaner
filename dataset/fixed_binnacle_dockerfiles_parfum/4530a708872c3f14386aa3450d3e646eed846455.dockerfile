...
RUN apt-get update \
 && apt-get install --no-install-recommends ca-certificates gnupg build-essential wget subversion unzip automake python cmake ninja-build curl git gcc-multilib g++-multilib libc6-dev bison flex libtool autoconf binutils-dev binutils-gold software-properties-common gnupg apt-transport-https sudo systemd sysvinit-utils systemd-sysv -y \
...
RUN apt-get update \
 && apt-get install --no-install-recommends bash-completion vim buildbot-slave -y \
 && rm -rf /var/lib/apt/lists/*
