...
RUN DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends -y install build-essential curl gccgo git pkg-config libxml-parser-perl flex bison cmake llvm clang
...
ENV CC="clang" \
    CXX="clang++" \
    PKG_CONFIG_PATH="/usr/repackaged/lib/pkgconfig:/usr/repackaged/share/pkgconfig" \
    STANDARD_CFLAGS="-I/usr/repackaged/include -fno-omit-frame-pointer -fPIE" \
    PIC_CFLAGS="-I/usr/repackaged/include -fno-omit-frame-pointer -fPIC" \
    MSAN_CFLAGS="-I/usr/repackaged/include -fno-omit-frame-pointer -fPIE -fsanitize=memory -fsanitize-memory-track-origins -fsanitize-recover=all" \
    LDFLAGS="-L/usr/repackaged/lib" \
    PATH="/usr/repackaged/bin:${PATH}" \
    LD_LIBRARY_PATH="/usr/repackaged/lib" \
    MSAN_OPTIONS="halt_on_error=0,exitcode=0" \
    MSAN_PIC_CFLAGS="-I/usr/repackaged/include -fno-omit-frame-pointer -fPIE -fsanitize=memory -fsanitize-memory-track-origins -fsanitize-recover=all" \
    MSAN_LDFLAGS="-L/usr/repackaged/lib -fno-omit-frame-pointer -fPIE -fsanitize=memory -fsanitize-memory-track-origins -fsanitize-recover=all"
COPY standard-download.sh standard-verified-download.sh standard-build.sh download-and-build.sh download-verified-and-build.sh clean-binaries.sh standard-rebuild.sh /root/installers/
...
COPY download-golang-1.9.sh /root/download-golang
COPY build-golang.sh /root/build-golang
RUN /root/download-golang \
 && /root/build-golang
COPY build-coyim.sh /root/build-coyim
VOLUME /src
