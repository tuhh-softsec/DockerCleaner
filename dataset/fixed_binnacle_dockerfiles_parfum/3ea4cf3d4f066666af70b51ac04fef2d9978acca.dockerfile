...
RUN apt-get update && apt-get install --no-install-recommends make binutils autoconf automake autotools-dev libtool pkg-config git curl dpkg-dev libxml2-dev genisoimage libc6-i386 lib32stdc++6 -y#  genisoimage, libc6-i386 and lib32stdc++6 are required to decompress ndk.
