...
RUN apt-get update \
 && apt-get install --no-install-recommends build-essential curl git libfreetype6-dev libpng12-dev libzmq3-dev pkg-config python python-dev python-numpy python-pip rsync software-properties-common swig unzip zip zlib1g-dev -y \
 && apt-get clean \
