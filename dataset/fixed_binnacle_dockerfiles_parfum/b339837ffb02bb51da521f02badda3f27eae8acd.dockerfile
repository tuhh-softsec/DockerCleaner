...
RUN DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y build-essential mingw-w64 gperf pkg-config cmake python python-lxml man wget unzip zip less \
 && apt-get -y clean
#  Enter versions!!!
