...
#  Basic Dependencies
RUN apt-get update \
 && apt-get install --no-install-recommends curl git zip unzip software-properties-common python-dev python-setuptools python-software-properties -y \
 && apt-get clean \
 && rm -rf /var/lib/apt/lists/* \
 && easy_install pip
#  Dependencies for vnc setup.
RUN apt-get update \
 && apt-get install --no-install-recommends xvfb fluxbox x11vnc -y \
 && apt-get clean \
 && rm -rf /var/lib/apt/lists/*
...
RUN apt-get update \
 && apt-get install --no-install-recommends lua5.1 liblua5.1-0-dev libffi-dev gettext freeglut3-dev libsdl2-dev libosmesa6-dev python-dev python-numpy realpath build-essential -y \
 && apt-get clean \
 && rm -rf /var/lib/apt/lists/*
