...
#  dependencies
RUN apt-key adv --keyserver hkp://keyserver.ubuntu.com:80 --recv-keys 913DFF12F86258E5
#  Install some dependencies which will be cached. This does not
#  need to be an exhaustive list because apt will install any
#  missing packages.
RUN echo "deb http://${LINOTP_MIRROR}/apt/debian jessie linotp" > /etc/apt/sources.list.d/linotp.list \
 && apt-get update \
 && extraPackages=' make python-psycopg2 ' \
 && apt-get install --no-install-recommends adduser debconf openssl pwgen python-beaker python-configobj python-decorator python-docutils python-formencode python-httplib2 python-ldap python-m2crypto python-mako python-mysqldb python-netaddr python-paste python-pastedeploy python-pastescript python-pygments python-pylons python-pyrad python-qrcode python-routes python-simplejson python-sqlalchemy python-tempita python-weberror python-webhelpers python-webob apache2 libapache2-mod-wsgi python-pycryptodomex python-pysodium python-requests $extraPackages
