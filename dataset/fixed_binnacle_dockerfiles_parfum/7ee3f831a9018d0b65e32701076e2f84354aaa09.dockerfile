...
RUN apt-get install --no-install-recommends python-software-properties -y \
 && apt-get install --no-install-recommends software-properties-common -y \
 && add-apt-repository ppa:fenics-packages/fenics \
 && apt-get update \
 && apt-get install --no-install-recommends fenics -y \
 && apt-get install --no-install-recommends cython python-h5py -y
...
RUN wget https://github.com/StochSS/stochss/archive/develop.tar.gz \
 ...
 && cd stochss-master \
 && apt-get update \
 && apt-get install --no-install-recommends curl -yy \
 && apt-get install --no-install-recommends libbz2-dev -yy \
 && apt-get install --no-install-recommends uuid-runtime libpng12-dev libffi-dev -yy \
 && pip install python-libsbml \
 && pip install boto \
 && pip install celery \
 && pip install setuptools --upgrade \
 && pip install http://cdn.mysql.com/Downloads/Connector-Python/mysql-connector-python-2.0.4.zip \
 && apt-get install --no-install-recommends gcc-multilib gcc g++ make libxml2-dev git r-base-core libgsl0-dev build-essential python-dev python-setuptools cython libbz2-dev vim docker libhdf5-mpi-dev -yy \
