...
RUN apt-get install --no-install-recommends openssh-server supervisor -y
...
RUN sudo apt-get install --no-install-recommends -y mongodb-10gen
...
RUN sudo apt-get install --no-install-recommends -y redis-server
#  update packages
# RUN apt-get update
#
...
RUN sudo apt-get install --no-install-recommends -y npm
COPY ./usr/src/app
WORKDIR /usr/src/app
