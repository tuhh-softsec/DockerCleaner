...
COPY bosh-cli/*.sh /usr/local/bin/
COPY bosh-cli/profile /tmp/profile
COPY bosh-cli/sshd.conf /etc/supervisor/conf.d/
RUN printf '\n=====================================================\n=> Install system packages\n=====================================================\n' \
 && apt-get update \
 && apt-get install --no-install-recommends ${INIT_PACKAGES} ${TOOLS_PACKAGES} ${NET_PACKAGES} ${DEV_PACKAGES} ${RUBY_PACKAGES} ${BDD_PACKAGES} -y \
 && apt-get upgrade -y \
 ...
 && useradd -m -g users -G sudo,rvm -s /bin/bash bosh \
 && echo "bosh ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/bosh \
 ...
 && wget "https://github.com/thomasmmitchell/bosh-complete/releases/download/v${BOSH_CLI_COMPLETION_VERSION}/bosh-complete-linux" -nv -O /home/bosh/bosh-complete-linux \
 && chmod 755 /home/bosh/bosh-complete-linux \
 && wget "https://cli.run.pivotal.io/stable?release=debian64&version=${CF_CLI_VERSION}&source=github-rel" -nv -O /tmp/cf.deb \
 && dpkg -i /tmp/cf.deb \
 ...
 && wget "https://github.com/cloudfoundry-incubator/bosh-backup-and-restore/releases/download/v${BBR_VERSION}/bbr-${BBR_VERSION}.tar" -nv -O - | tar -x -C /tmp releases/bbr \
 && mv /tmp/releases/bbr /usr/local/bin/bbr \
 && wget "https://dl.minio.io/client/mc/release/linux-amd64/mc" -nv -O /usr/local/bin/mc \
 && wget "https://github.com/rlmcpherson/s3gof3r/releases/download/v${S3GOFR_VERSION}/gof3r_${S3GOFR_VERSION}_linux_amd64.tar.gz" -nv -O - | tar -xz -C /tmp \
 && mv /tmp/gof3r_${S3GOFR_VERSION}_linux_amd64/gof3r /usr/local/bin/go3fr \
 ...
 && wget "https://storage.googleapis.com/kubernetes-helm/helm-v${HELM_VERSION}-linux-amd64.tar.gz" -nv -O - | tar -xz -C /tmp linux-amd64/helm \
 && mv /tmp/linux-amd64/helm /usr/local/bin/helm \
 && chmod 775 /usr/local/bin/helm \
 && /usr/local/bin/helm completion bash > /etc/bash_completion.d/kubectl \
 && wget "https://dev.mysql.com/get/Downloads/MySQL-Shell/mysql-shell_${MYSQL_SHELL_VERSION}ubuntu16.04_amd64.deb" -nv -O /tmp/mysql-shell.deb \
 && dpkg -i /tmp/mysql-shell.deb \
 && wget "https://raw.githubusercontent.com/rupa/z/master/z.sh" -nv -O /usr/local/bin/z.sh \
 && wget "https://github.com/Orange-OpenSource/db-dumper-cli-plugin/releases/download/v${DB_DUMPER_VERSION}/db-dumper_linux_amd64" -nv -O /tmp/db-dumper-plugin \
 && chmod 755 /tmp/db-dumper-plugin \
 && wget "https://releases.hashicorp.com/terraform/${TERRAFORM_VERSION}/terraform_${TERRAFORM_VERSION}_linux_amd64.zip" -nv -O /tmp/terraform.zip \
 && unzip -q /tmp/terraform.zip -d /usr/local/bin \
 && export PROVIDER_CLOUDFOUNDRY_VERSION="v${TERRAFORM_PLUGIN_CF_VERSION}" \
 && /bin/bash -c "$( wget https://raw.github.com/orange-cloudfoundry/terraform-provider-cloudfoundry/master/bin/install.sh -O - ;)" \
 && git clone --depth 1 https://github.com/junegunn/fzf.git /home/bosh/.fzf \
 && chown -R bosh:users /home/bosh/.fzf \
 && su -l bosh -s /bin/bash -c "/home/bosh/.fzf/install --all" \
 && printf '=====================================================\n=> Install CF plugins\n=====================================================\n' \
 && su -l bosh -s /bin/bash -c "export IFS=, ; for plug in $(echo ${CF_PLUGINS}) ; do cf install-plugin \"${plug}\" -r CF-Community -f ; done" \
 && su -l bosh -s /bin/bash -c "cf install-plugin /tmp/db-dumper-plugin -f" \
 && rm -f /tmp/db-dumper-plugin \
 && printf '\n=====================================================\n=> Setup system banner\n=====================================================\n' \
 ...
 && chmod 755 /usr/local/bin/* /etc/profile.d/* \
 && chmod 644 /etc/motd \
 && mv /tmp/profile /home/bosh/.profile \
 && chmod 664 /home/bosh/.profile \
 && touch /var/log/lastlog \
 && chgrp utmp /var/log/lastlog \
 && chmod 664 /var/log/lastlog \
 && mkdir -p /home/bosh/.ssh \
 && chmod 700 /home/bosh/.ssh \
 && chmod 700 /home/bosh \
