...
RUN apt-get update \
 && apt-get install --no-install-recommends openssh-server wget -y; apt-get upgrade -y
RUN mkdir /build
#  Prerequisites
#
#  polymake
RUN apt-get install --no-install-recommends g++ libboost-dev libgmp-dev libgmpxx4ldbl libmpfr-dev libperl-dev libsvn-perl libterm-readline-gnu-perl libxml-libxml-perl libxml-libxslt-perl libxml-perl libxml-writer-perl libxml2-dev xsltproc clang bliss libbliss-dev make -y
#  Singular
RUN apt-get update \
 && apt-get install --no-install-recommends git build-essential autoconf autogen libtool libreadline6-dev libglpk-dev libgmp-dev libmpfr-dev libcdd-dev libntl-dev -y
#  normaliz
...
RUN apt-get install --no-install-recommends surf-alggeo -y
#  Userland
WORKDIR /
