...
RUN apt-get update \
 && apt-get install --no-install-recommends bzr curl daemontools gcc git libc-ares-dev libev-dev libevent-dev libffi-dev libjansson-dev libjemalloc-dev libssl-dev libsystemd-dev magma-nghttpx=1.31.1-1 make net-tools pkg-config python-cffi python3-pip redis-server rsyslog sudo unzip vim virtualenv -y
#  Install Golang 1.12.
WORKDIR /usr/local
...
RUN apt-get update -y \
 && apt-get install --no-install-recommends daemontools -y
