...
RUN { echo "mysql-server-5.5 mysql-server/root_password password $MYSQL_ROOT_PASSWORD" ;echo "mysql-server-5.5 mysql-server/root_password_again password $MYSQL_ROOT_PASSWORD" ;echo "mysql-server-5.5 mysql-server/root_password seen true" ;echo "mysql-server-5.5 mysql-server/root_password_again seen true" ; } | debconf-set-selections \
 && apt-get update \
 && apt-get install --no-install-recommends mysql-server python-mysqldb -y
#  Util
RUN apt-get install --no-install-recommends curl -y
#  Ubuntu Cloud archive keyring and repository
RUN apt-get install --no-install-recommends ubuntu-cloud-keyring -y \
 && echo "deb http://ubuntu-cloud.archive.canonical.com/ubuntu" "trusty-updates/kilo main" > /etc/apt/sources.list.d/cloudarchive-kilo.list \
 && apt-get update \
 && apt-get -y dist-upgrade
#  RabbitMQ
RUN apt-get install --no-install-recommends rabbitmq-server -y
#  Keystone
RUN echo "manual" > /etc/init/keystone.override \
 && apt-get install --no-install-recommends keystone python-openstackclient apache2 libapache2-mod-wsgi memcached python-memcache -y \
...
#  Glance Setup
RUN apt-get install --no-install-recommends glance python-glanceclient -y \
 && rm -f /var/lib/glance/glance.sqlite
#  Nova Management
RUN apt-get install --no-install-recommends nova-api nova-cert nova-conductor nova-consoleauth nova-novncproxy nova-scheduler python-novaclient -y \
 && rm -f /var/lib/nova/nova.sqlite
#  Neutron
RUN apt-get install --no-install-recommends neutron-server neutron-plugin-ml2 python-neutronclient -y
#  Horizone
RUN apt-get install --no-install-recommends openstack-dashboard -y \
 && dpkg --purge openstack-dashboard-ubuntu-theme
#  Heat
RUN apt-get update \
 && apt-get install --no-install-recommends heat-api heat-api-cfn heat-engine python-heatclient -y
#  Cinder
RUN apt-get install --no-install-recommends cinder-api cinder-scheduler python-cinderclient -y
#  MySQL Data Volume
VOLUME ["/data"]
#  MySQL
