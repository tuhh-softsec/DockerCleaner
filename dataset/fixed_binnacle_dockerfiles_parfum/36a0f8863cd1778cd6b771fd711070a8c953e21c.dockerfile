...
ENV WORKSPACE="/workspace" \
    TERM="xterm"
RUN apt-get update \
 && apt-get install --no-install-recommends build-essential ca-certificates curl git jq dnsutils openssh-client python wget apt-transport-https gnupg2 shellcheck software-properties-common lsb-release gettext-base -y \
 && apt-get clean
#  Install gcloud
...
RUN apt-get install --no-install-recommends dirmngr
