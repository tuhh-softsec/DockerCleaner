...
RUN DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y cron curl build-essential unzip vim git
#  Install Python
RUN DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y python python-setuptools python-dev python-pip
#  Install Node.js
RUN curl -sL https://deb.nodesource.com/setup_6.x | sudo -E bash -
RUN DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y nodejs
#  Install Ruby
RUN DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y zlib1g-dev build-essential libssl-dev libreadline-dev libyaml-dev libsqlite3-dev sqlite3 libxml2-dev libxslt1-dev libcurl4-openssl-dev python-software-properties libffi-dev
...
RUN DEBIAN_FRONTEND=noninteractive apt-get --no-install-recommends install -y openssl libcurl4-openssl-dev libxml2 libssl-dev libxml2-dev pinentry-curses xclip cmake build-essential pkg-config
