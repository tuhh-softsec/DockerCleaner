# ###########################################################
#  Dockerfile to build biotoolbox with ensembl78 api container images
...
#  Install Samtools
RUN apt-get install --no-install-recommends libbam-dev samtools --yes
RUN ln -s /usr/lib/libbam.a /usr/include/samtools/libbam.a
ENV SAMTOOLS="/usr/include/samtools"
RUN cpanm LDS/Bio-SamTools-1.41.tar.gz
#  Install JKENT lib from UCSC
RUN apt-get install --no-install-recommends wget unzip sed --yes
#  Install JKENT lib dependancies
RUN apt-get install --no-install-recommends libmysqlclient15-dev libpng12-dev mysql-client-5.5 mysql-client-core-5.5 --yes
WORKDIR /usr/local
...
RUN apt-get install --no-install-recommends libssl1.0.0 libssl-dev -y
WORKDIR /tmp
...
RUN cpanm -v TJPARNELL/Bio-ToolBox-1.24001.tar.gz
