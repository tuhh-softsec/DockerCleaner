...
RUN apt-get install --no-install-recommends apt-transport-https apt-utils curl gnupg lsb-release -y --allow-downgrades
...
RUN apt-get install --no-install-recommends autoconf automake build-essential cmake devscripts expect gcc-6 g++ git golang-go haxe inotify-tools lib32ncurses5 lib32z1 libbz2-1.0:i386 libgconf-2-4 libsodium-dev libstdc++6:i386 libtool mono-complete nano nodejs openjdk-8-jdk perl pinentry-curses procps python python-pip ruby ruby-dev shellcheck sudo tightvncserver wget yarn zopfli -y --allow-downgrades
RUN apt-get update -y --allow-downgrades
RUN apt-get -y --allow-downgrades upgrade
