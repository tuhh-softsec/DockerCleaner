...
RUN DEBIAN_FRONTEND=noninteractive apt-get update \
 && apt-get install --no-install-recommends gcc g++ perl python automake make cmake less vim wget git curl libdb-dev zlib1g-dev bzip2 libncurses5-dev texlive-latex-base default-jre python-pip python-dev gfortran build-essential libghc-zlib-dev libncurses-dev libbz2-dev liblzma-dev libpcre3-dev libxml2-dev libblas-dev gfortran git unzip ftp libzmq3-dev nano ftp fort77 libreadline-dev libcurl4-openssl-dev libx11-dev libxt-dev x11-common libcairo2-dev libpng-dev libreadline6-dev libjpeg8-dev pkg-config build-essential cmake gsl-bin libgsl0-dev libeigen3-dev libboost-all-dev libssl-dev libcairo2-dev libxt-dev libgtk2.0-dev libcairo2-dev xvfb xauth xfonts-base apt-utils apt-transport-https gdebi-core locales -y
...
RUN apt-get install --no-install-recommends openssh-server libncurses5-dev apache2 supervisor -y
# ########
