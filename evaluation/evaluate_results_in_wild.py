...
def process_metadatas(metadata_path, metadata_files, generated_dockerfile_path):
    ...
    for file_name in metadata_files:
        try:
            ...
            print("Error while parsing metadata: " + file_name)
...
def combine_results(oracle_df, dockercleaner_df):
    ...
    if not pd.isnull(evaluation_df.loc[0, 'injected_smells']):
        for index, row in evaluation_df.iterrows():
            ...
            injected_smells = set(row['injected_smells'].split(","))
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Add argument 'detection' or 'fixing' to evaluate the results of DockerCleaner")
        exit(1)

    if sys.argv[1] == "detection":
        collect_oracle_results(SMELLY_DOCKERFILE_PATH)
